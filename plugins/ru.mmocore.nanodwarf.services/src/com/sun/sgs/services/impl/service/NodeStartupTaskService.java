...
/**
 * A simple utility {@code Service} that runs a single task on startup of a
 * node. Because this may be run on any given node at startup, the task is
 * node-local, and so the application will not learn if running the task has
 * failed for any reason.
 * <p>
 * The task to run must implement the {@code AsyncRunnable} interface, and must
 * have a no-arg constructor. The task is run through the
 * {@code AsyncTaskService}, so while there may be a delay before execution,
 * once started the task is not bound by the transaction timeout limit. Because
 * the task implements the {@code AsyncRunnable} interface it will be able to
 ...
 * class should be available in the classpath. If no class is specified, then no
 * startup task is run.
 */
public class NodeStartupTaskService implements Service {

	/** The property used to define the startup task class. */
	public static final String RUNNABLE_PROPERTY = NodeStartupTaskService.class
			.getName() + ".task.class";

	// the service's logger
	private static final Logger logger = Logger
			.getLogger(NodeStartupTaskService.class.getName());

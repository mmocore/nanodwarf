...
/**
 * A {@code Service} implementation of {@code StopwatchManager}. Note that the
 * instances of {@code Stopwatch} returned by this class are
 * {@code Serializable} but do not implement {@code ManagedObject}. This is done
 * for a few reasons, but mainly becuse it's assumed that most {@code Stopwatch}
 * instances will be tied to a specific object's state and will be consulted
 * regularly. A side-effect is that the {@code Stopwatch} will not mark itself
 * when updating its state (on calls to {@code start} or {@code split}.
 */
public class StopwatchService implements StopwatchManager, Service {
	 ...
	 * @param registry
	 *            the system component registry
	 * @param proxy
	 *            the system transaction proxy
	 */
	public StopwatchService(Properties p, ComponentRegistry registry,
			TransactionProxy proxy) {
		StopwatchService.proxy = proxy;

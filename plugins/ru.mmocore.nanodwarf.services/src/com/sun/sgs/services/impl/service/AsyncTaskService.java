...
public class AsyncTaskService implements Service, AsyncTaskManager {
	...
	private static final String CALLBACK_NS_ROOT = AsyncTaskService.class
			.getName() + ".";
	...
	public AsyncTaskService(Properties p, ComponentRegistry cr,
		...
		AsyncTaskService.transactionProxy = tp;
	...
	public <T> void startTask(AsyncCallable<T> c, AsyncTaskCallback<T> callback)
		...
		dataService.setServiceBinding(name, new CallbackWrapper<T>(callback));
		ctxFactory.joinTransaction().addRunner(
				new CallableKernelRunner<T>(c, name));
	}

	/** Local implementation of TransactionContext to manage state. */
	private class TxnState extends TransactionContext {
		private final HashSet<TaskReservation> reservations = new HashSet<TaskReservation>();
	...
	private static class CallbackWrapper<T> implements ManagedObject,
			Serializable {
		...
		CallbackWrapper(AsyncTaskCallback<T> callback) {
			if (callback instanceof ManagedObject) {
				managedRef = AsyncTaskService.transactionProxy.getService(
						DataService.class).createReference(callback);
				ref = null;
			} else {
				managedRef = null;
				ref = callback;
	...
	private final class CallableKernelRunner<T> implements KernelRunnable {
		...
		public void run() {
			...
			try {
				...
				r = new CallbackKernelRunner<Throwable>(throwable, name, false);
			}
			try {
				transactionScheduler.scheduleTask(r,
	...
	private class CallbackKernelRunner<T> implements KernelRunnable {
		...
		public void run() throws Exception {
			...
			CallbackWrapper<T> wrapper = (CallbackWrapper<T>) (dataService
					.getServiceBinding(name));
			if (succeeded)
	...
	private class RecoveryListenerImpl implements RecoveryListener {
		public void recover(Node node, SimpleCompletionHandler handler) {
			...
			final HashSet<CallbackKernelRunner<Throwable>> taskSet = new HashSet<CallbackKernelRunner<Throwable>>();
			...
			try {
				transactionScheduler.runTask(new KernelRunnable() {
					...
					public void run() throws Exception {
						taskSet.clear();
						String name = dataService.nextServiceBoundName(nsRoot);
						while (name.startsWith(nsRoot)) {
							String newName = namespace
									+ idGenerator.getAndIncrement();
							CallbackWrapper<?> obj = (CallbackWrapper<?>) (dataService
									.getServiceBinding(name));
							dataService.setServiceBinding(newName, obj);
							dataService.removeServiceBinding(name);

...
/**
 * A scalable implementation of {@code Set} backed by a {@link ScalableHashMap}.
 * The internal structure of the set is separated into distributed pieces, which
 * reduces the amount of data any one operation needs to access. The distributed
 * structure increases the concurrency and allows for parallel write operations
 * to successfully complete.
 * 
 * <p>
 * 
 * Developers may use this class as a drop-in replacement for the
 * {@link java.util.HashSet} class for use in a {@link ManagedObject}. A
 * {@code HashSet} will typically perform better than this class when the number
 ...
 * modify the set at the same time, so this class may perform better in
 * situations where multiple tasks need to modify the set concurrently, even if
 * the total number of elements is small. Also note that, unlike {@code HashSet}
 ...
 * implement {@code Serializable} will result in an
 * {@link IllegalArgumentException} being thrown. If an element is an instance
 * of {@code Serializable} but does not implement {@code ManagedObject}, this
 * class will persist the element as necessary; when such an element is removed
 * from the set, it is also removed from the {@code DataManager}. If an element
 * is an instance of {@code ManagedObject}, the developer will be responsible
 * for removing these objects from the {@code DataManager} when done with them.
 * Developers should not remove these object from the {@code DataManager} prior
 * to removing them from the set.
 * 
 * <p>
 * 
 ...
 * particular, elements that use {@link Object#equals Object.equals} and
 * {@link Object#hashCode Object.hashCode} will typically not be equal, and will
 * have different hash codes, each time they are deserialized, and so are
 * probably not suitable for use with this class.
 * 
 * <p>
 * 
 ...
 * collections, the {@code size} and {@code isEmpty} methods for this class are
 * <em>not</em> constant-time operations. Because of the asynchronous nature of
 * the set, these operations may require accessing all of the entries in the
 * set.
 * 
 * <p>
 * 
 ...
 * @see ManagedObject
 */
public class ScalableHashSet<E> extends AbstractSet<E> implements Serializable,
	...
	/**
	 * Creates an empty set; the backing {@code ScalableHashMap} has the default
	 * minimum concurrency ({@code 32}).
	 * 
	 * @see ScalableHashMap#ScalableHashMap()
	 */
	public ScalableHashSet() {
		map = AppContext.getDataManager().createReference(
				new ScalableHashMap<E, Marker>());
	}
	...
	public ScalableHashSet(int minConcurrency) {
		map = AppContext.getDataManager().createReference(
				new ScalableHashMap<E, Marker>(minConcurrency));
	}
	 ...
	 * 
	 * @throws IllegalArgumentException
	 *             if any elements contained in the argument are not
	 *             {@code null} and do not implement {@code Serializable}
	 */
	 ...
	 * 
	 * @throws IllegalArgumentException
	 *             if the argument is not {@code null} and does not implement
	 *             {@code Serializable}
	 */
	 ...
	 * this operation is <i>not</i> constant time. Clearing a set takes
	 * {@code O(n*log(n))} time.
	 */
	public void clear() {
		map.get().clear();
	 ...
	 * 
	 * @param o
	 *            the element that should be removed from the set, if present
	 * 
	 ...
	 * Determining the size of a set takes {@code O(n*log(n))}.
	 * 
	...
	public void removingObject() {
		AppContext.getDataManager().removeObject(map.get());
	}

	/**
	 * An internal marker class for storing as the value in the backing map. All
	 * marker objects are equivalent to ensure equality after deserialization
	 * without the added cost of write replacement.
	 */

...
/**
 * A utility class for wrapping a {@link Serializable} object within a
 * {@code ManagedObject} instance. This class is primarily intended to allow
 * class that does not implement {@code ManagedObject} to be persistently stored
 * and accessed through a {@code ManagedReference}.
 * 
 * <p>
 * 
 * The serialization costs for a class largely depend on the size of the objects
 * that it references. The {@code ManagedReference} class allows developers the
 * ability to create breaks in the serialization graphs where not all of the
 * fields are deserialized when a class is deserialized. The
 ...
 * </pre>
 * 
 * Application developers are responsible for removing
 * {@code ManagedSerializable} instances by calling
 * {@link DataManager#removeObject DataManager.removeObject}. Developers should
 * call {@link DataManager#markForUpdate DataManager.markForUpdate} or
 * {@link ManagedReference#getForUpdate DataManager.getForUpdate} if the
 * application modifies objects wrapped by instances of this class.
 * 
 * @param <T>
...
public class ManagedSerializable<T> implements ManagedObject, Serializable {
	 ...
	 * which must not implement {@link ManagedObject}, but must either implement
	 * {@link Serializable} or be {@code null}.
	 * 
	 * @param object
	 *            the object to wrap
	 * 
	 * @throws IllegalArgumentException
	 *             if {@code object} is an instance of {@code ManagedObject}, or
	 *             if {@code object} is not {@code null} and does not implement
	 *             {@code Serializable}
	 */
	...
	public boolean equals(Object o) {
		if (o instanceof ManagedSerializable) {
			...
			return (object == null) ? object == m.object : object
					.equals(m.object);
		}
		return false;
	}

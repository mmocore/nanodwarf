 ...
 * insertions and removals, arbitrary searches for values, and a number of other
 * methods which are also defined in the {@code java.util.List} interface. This
 * data structure builds upon the {@code AbstractList} class by implementing
 * methods specific to concurrent and scalable operations. As an implementation
 * decision, this data structure does not support {@code null} insertions but
 * does implement all optional operations defined in the {@code java.util.List}
 * interface.
 * <p>
 ...
 * <p>
 * This class actually stores a collection of {@code ManagedReference}s, which
 * in turn point to {@code ManagedObject}s. Therefore, each element stored is
 * converted into a {@code ManagedObject} in the form of an {@code Element}
 * wrapper if it is not one already, and a {@code ManagedReference} is created
 * to reference it. The wrapper is detected during an element removal: if the
 * element is an instance of the wrapper {@code Element} class, then it is
 * removed from the data manager. Conversely, if the element had no wrapper (but
 * by definition is still a {@code ManagedObject}), then it remains within the
 * data manager until the user explicitly removes it.
 * <p>
 ...
 * once. In fact, each {@code ListNode} only holds onto the size of its
 * {@code SubList} (its children) and not a cumulative total of all its previous
 * siblings. This enables intermediate changes to have no effect on neighbouring
 * nodes, such as re-indexing.
 * <p>
 ...
 * {@code bucketSize}. This is because changes that occur at the leaf level need
 * to propagate to the parents; for a deep tree, this can affect a number of
 * different nodes. However, it is seen that the benefits provided by the
 * partitioning of the list that enable concurrency outweigh the performance hit
 ...
 * {@code ListNode} being examined, they will be incorporated in the iteration,
 * suggesting that the target element may not be the same one as when the call
 * was initially made. However, if a modification in the form of an addition or
 * removal happens on the same {@code ListNode}, then the iterator will throw a
 ...
 * the {@code ScalableList} type are not {@code ManagedObject}s. Therefore, they
 * are not stored within the data manager and do not need to be removed using
 * the {@code DataManager}.
 * <p>
 ...
 * it is highly recommended (and considered good practice) that potentially long
 * iterations be broken up into smaller tasks. This strategy improves the
 * concurrency of the {@code ScalableList} as it reduces the locked elements
 * owned by any one task.
 * <p>
 * 
 * @param <E>
...
public class ScalableList<E> extends AbstractList<E> implements Serializable,
	...
	/**
	 * The maximum number of children contained in a TreeNode<E>; this parameter
	 * is passed to the TreeNode<E> during instantiation. If it is not
	 * explicitly set, it defaults to a value of 5.
	 */
	 ...
	 *            {@code ScalableList}
	 * @throws IllegalArgumentException
	 *             if the collection is invalid (contains {@code null}
	 *             elements), or if the {@code branchingFactor} or
	 ...
	 * @return a {@code ManagedReference} of the object, or {@code null} if the
	 *         argument is null.
	 */
	...
	/**
	 * Returns the value stored within the {@code ManagedReference}, depending
	 * on whether it is an {@code Element} or {@code ManagedObject}
	 * 
	 * @param <E>
	 *            the type of element
	 * @param ref
	 *            the {@code ManagedReference} storing the value
	 * @param delete
	 *            {@code true} if the {@code Element} object should be deleted,
	...
	/**
	 * Removes all of the elements referred to in the list, but retains a basic
	 * structure that consists of a {@code SubList}, {@code ListNode}, and a
	 * {@code TreeNode}, along with {@code DummyConnector}s representing the
	 * head and tail of the list. The call to {@code removingObject()} is
	 * asynchronous, meaning the objects are deleted from the data manager in a
	 * scheduled task.
	 */
	public void clear() {
	 ...
	 * @return whether the object is contained in the list; {@code true} if so,
	...
	/**
	 * Determines if the provided index is positive (not negative). If the index
	 * is negative, then an {@code IndexOutOfBoundsException} is thrown.
	 * 
	 * @param index
	 ...
	 * locate the {@code ListNode} using a recursive search process and return a
	 * {@code SearchResult} which contains both the {@code ListNode} and the
	...
	public void removingObject() {
		...
		AsynchronousClearTask<E> clearTask = new AsynchronousClearTask<E>(this);
	...
	static class TreeNode<E> implements ManagedObject, Serializable, Node<E> {
		 ...
		 *            {@code TreeNode}, based on the elements that already exist
		 *            among the {@code child} and its siblings.
		 */
		 ...
		 * @param parent
		 *            the intended parent {@code ListNode}
		 * @param isSplit
		 *            {@code true} if the {@code TreeNode} is to be created due
		 *            to a {@code split} operation, and {@code false} otherwise.
		 */
		 ...
		 * This method is guaranteed to delete itself. This method re-links
		 * references that are dangling as a result of this node's removal.
		 ...
		 * <p>
		 * The second condition is if the {@code TreeNode} is the tail node. If
		 * so, then the previous element's next reference is set to null before
		 * the node is pruned.
		 * <p>
		 * The third condition is if the {@code TreeNode} is the head node. If
		 * so, then the next element's previous reference is set to null before
		 * the node is pruned.
		 * <p>
		 * The last condition is if the {@code TreeNode} is an only- child. If
		 * so, then the parent's child reference is set to null before the node
		 * is pruned.
		 */
		void prune() {
			...
			if (prevRef != null) {
				...
				parentRef.getForUpdate()
						.setChild(nextRef.get(), parentRef.get().size(),
		 ...
		 * @return the {@TreeNode<E>} that not represents a
		 *         sibling which is connected to the tree
		 */
		 ...
		 * is the root; only these nodes are orphaned while siblings have
		...
		void decrementChildrenAndSize() {
			 ...
			 * the parent has only one child (this), then link the parent to the
			 * children and prune this node. For both these cases, we propagate
			 * the decrement of size and children to the parent. Otherwise, we
			 * only propagate the decrement the size.
			 */
	...
	private static class AsynchronousClearTask<E> implements Serializable,
		...
		/**
		 * Removes some number of elements from the ScalableList and returns
		 * {@code true} if there is more work to be done. If there are no more
		 * elements to remove, then it will return {@code false}, signifying to
		 * the {@code AsynchronousClearTask} to start taking values from the
		 * queue
		 * 
		 * @return {@code true} if more work needs to be done, and {@code false}
		 *         if there are no more elements to remove.
		 */
	...
	static class ScalableListNodeIterator<E> implements Serializable,
		 ...
		 * @throws NoSuchElementException
		 *             if there exists no previous sibling
		 */
		 ...
		 *             {@code ScalableList} only removes elements explicitly,
		 *             not {@code ListNodes}.
	...
	static class ScalableIterator<E> implements Serializable, Iterator<E> {
		 ...
		 * @param offset
		 *            the offset
		 * @param currentListNode
		 *            the current {@code ListNode} being examined
		 ...
		 * 
		 * @param list
		 *            the {@code ScalableList} over which to iterate
		 */
		...
		ScalableIterator(ScalableList<E> list, ListNode<E> startingNode) {
			owner = AppContext.getDataManager().createReference(list);
			currentNode = AppContext.getDataManager().createReference(
					startingNode);

			cursor = 0;
			listNodeReferenceValue = currentNode.get().getDataIntegrityValue();
			wasNextCalled = false;
		}

		/**
		 * Retrieves the index of the current element by walking up the tree to
		 * the root and aggregating the counts of each {@code ListNode} and
		 * {@code TreeNode}. This operation is slightly expensive because of the
		 * required percolation up the tree.
		 * 
		...
		public E next() {
			...
			List<ManagedReference<ManagedObject>> elements = currentNode.get()
		 ...
		 * 
		 * @param next
		 *            whether we intend on travelling in the {@code next}
		...
		private boolean loadNextListNode() {
			...
			if (next != null) {
				currentNode = AppContext.getDataManager().createReference(next);
				cursor = 0;
				return true;
			}
			return false;
		 ...
		 * 
		 * @throws ConcurrentModificationException
		 *             if the data integrity value has changed or if it cannot
		 *             be verified
		 */
		void checkDataIntegrity() {
			try {
				if (currentNode.get().getDataIntegrityValue() == listNodeReferenceValue) {
					return;
				}
		 ...
		 *         otherwise
		 * @throws ConcurrentModificationException
		 *             if the {@code ListNode} has been modified or removed
		 */
		public boolean hasNext() {
	 ...
	 * {@code ScalableList} data structure. This iterator allows bi-directional
	 * traversal and other operations native to the {@code ListIterator}
	 * interface.
	 * 
	 * @param <E>
	 *            the type of element
	 */
	static class ScalableListIterator<E> extends ScalableIterator<E> implements
			ListIterator<E> {
		 ...
		 * list, a {@code startingNode} and a {@code startingIndex} denoting the
		 * starting point. This constructor is used primarily for when the user
		 * specifies an index that is one larger than the highest index value.
		 * 
		 * @param list
		 *            a reference to the {@code ScalableList}
		 * @param startingIndex
		 *            the starting index (relative)
		 * @param startingNode
		 *            the starting node
		 */
		 ...
		 * @param searchResult
		 */
		ScalableListIterator(ScalableList<E> list, SearchResult<E> searchResult) {
		...
		private boolean loadPrevListNode() {
			ListNode<E> prev = currentNode.get().prev();
			if (prev != null) {
				currentNode = AppContext.getDataManager().createReference(prev);
				return true;
			}
			return false;
		}
		 ...
		 * 
		 * @return the index of the element that would be returned by a
		 *         subsequent call to <tt>previous</tt>, or -1 if list iterator
		 ...
		 * This process will automatically remove the old element from the data
		 * manager if it was not a {@code ManagedObject}.
		 * 
		 * @param o
		 *            the element with which to replace the last element
		 *            returned by <tt>next</tt> or <tt>previous</tt>.
		 * @throws IllegalStateException
		 *             if the operation is called without {@code next} or
		 *             {@code previous} being called
		 */
		public void set(E o) {
		 ...
		 * 
		 * @throws IllegalStateException
		 *             if the method is not preceded by {@code next()} or
		 *             {@code prev()}
		 */
		public void remove() {
			if (cannotRemoveOrSet) {
		 ...
		 * <tt>next</tt>, if any, and after the next element that would be
		 * returned by <tt>previous</tt>, if any. (If the list contains no
		 ...
		 * to <tt>next</tt> would be unaffected, and a subsequent call to
		 ...
		 * @throws IndexOutOfBoundsException
		 *             if the index which the element is to be added is out of
		 *             bounds
		 */
		public void add(E o) {
	...
	/**
	 * Node which parents a {@code SubList<E>}. These nodes can be considered as
	 * the leaf nodes of the tree and contain references to a portion of the
	 * list. A {@code ListNode}'s parent is always a {@code TreeNode} since they
	 * are the deepest organizational element of the {@code ScalableList}.
	 * {@code ListNode}s are arranged in a doubly-linked list, each having a
	...
	static class ListNode<E> implements ManagedObject, Serializable, Node<E> {
		 ...
		 * 
		 * @param list
		 *            a reference to the {@code ScalableList}; this argument
		 *            should only ever be null during the
		 ...
		 * 
		 * @param list
		 *            a reference to the {@code ScalableList}; since this method
		 *            is not called by the {@code AsynchronousClearTask}, this
		 ...
		 * @param obj
		 *            the value to replace the existing one
		 ...
		 * after a removal has taken place. This will occur as long as there is
		 * a next sibling, the next sibling has the same parent as {@code this},
		 * the parent has sufficient children, and if the parent was not yet
		 * removed. Otherwise, if the parent has no children, then there is no
		 * child to update the child reference to, so we set it to null and
		 * prune the node during the propagation.
		 */
		 ...
		 */
		private void split() {
			List<ManagedReference<ManagedObject>> contents = getSubList()
					.getElements();
			List<ManagedReference<ManagedObject>> spawned = new ArrayList<ManagedReference<ManagedObject>>();
			...
			ListNode<E> spawnedNode = new ListNode<E>(getParent(),
	 ...
	 * sizes do not need to change since no elements are being added or removed.
	 * 
	 * @param <E>
	 *            the type of element stored in the {@code ScalableList}
	 */
	static class SubList<E> implements ManagedObject, Serializable {
		 ...
		 * @throws IndexOutOfBoundsException
		 *             if the index is out of bounds (less than 0 or larger than
		 *             the {@code SubList} size)
		 */
		@SuppressWarnings("unchecked")
		E get(int index) {
	...
	static interface Node<E> {
		...
		/**
		 * Walks up the tree and removes the object and any of its parents. This
		 * method is intended to be called during the
		 * {@code AsynchronousClearTask} operation.
		 */
		void clear();
		 ...
		 * contains the index provided. If no ListNode<E> can be found, then
		 * null is returned.
		 * 
		 * @param currentValue
	...
	private static class SearchResult<E> {
		...
		/**
		 * Constructor which creates a new SearchResult based on the target node
		 * and the calculated offset. The offset is not an absolute index, but
		 * rather the index of the element with respect to the {@code SubList}
		 * of the {@code ListNode}.
		 * 
		 * @param node
		 *            the target node
		 * @param offset

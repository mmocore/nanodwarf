 ...
 * <p>
 * 
 * The iterator will only make a best-effort to reflect any concurrent change to
 * the deque. If the next element that the iterator is to return was been
 * removed during a separate task, the iterator <i>will</i> throw a
 * {@code ConcurrentModificationException}. In this behavior, not supporting
 * concurrent updates incurs no performance overhead for mutating operations.
 ...
 * implementations. The operations {@link ScalableDeque#remove(Object) remove},
 * {@link ScalableDeque#removeAll(Collection) removeAll},
 * {@link ScalableDeque#removeFirstOccurrence(Object) removeFirstOccurrence},
 * and {@link ScalableDeque#removeLastOccurrence(Object) removeLastOccurrence}
 * run in time linear to the number of instances of that object in the deque,
 * not the number of elements in the deque. In addition, the
 * {@link ScalableDeque#contains(Object) contains} operation is constant time.
 * Due to the distributed nature of the deque, the {@link ScalableDeque#size()
 * size} operation takes time linear to the size of the deque. For large deques,
 * this method should not be called, as it would take longer than the time
 * available to a {@code Task}. Similarly, the {@link ScalableDeque#toArray()
 * toArray} and {@link ScalableDeque#toArray(Object[]) toArray(T[])} operations
 * should not be used for anything but small deques. The
 * {@link ScalableDeque#isEmpty() isEmpty} and {@link ScalableDeque#clear()
 * clear} operations however are still constant time.
 * 
 * <p>
 * 
 * All elements stored by this deque must be instances of {@link Serializable}.
 * This class additionally supports elements that are instances of
 ...
 * <p>
 * 
 * This class requires that all elements have a meaningful {@code hashCode} and
 * {@code equals} operation. Failure to do so will result in the
 * {@code contains}, {@code remove}, {@code removeFirstOccurrrence},
 * {@code removeLastOccurrence} and {@code removeAll} operations not working.
 * The remaining operations will still operate as normal, however.
 * 
 * <p>
 * 
 * This class will mark itself for update as necessary; no additional calls to
 * the {@link DataManager} are necessary when modifying the deque. Developers
 * should not need to call {@code markForUpdate} or {@code getForUpdate} on a
 * deque instance, as this will eliminate all the concurrency benefits of this
 * class. However, calling {@code getForUpdate} or {@code markForUpdate} can be
 * used if an operation needs to prevent all access to the map.
 * 
 * <p>
 * 
 ...
 * improves the concurrency of the deque as it reduces the locked elements owned
 * by any one task. Iterators belonging to the deque implement the
 * {@code java.io.Serializable} interface, but not
 * {@code com.sun.sgs.app.ManagedObject}. Therefore, to persist them in the data
 * manager, it is necessary to wrap them within another {@code ManagedObject}.
 * If the iterators are persisted, the {@code DataManager.markForUpdate()} (with
 * the wrapper as the argument) should be called when performing iterator
 * operations like {@code next()}, {@code remove()}, etc, so that the iterator's
 * state is flushed to the data manager.
 * 
 * <p>
 * 
 ...
 * 
 * @param <E>
 *            the type of elements held by this deque
 * 
 * @see Deque
 * @see ManagedObject
 * @see Serializable
 */
public class ScalableDeque<E> extends AbstractCollection<E> implements
		Deque<E>, Serializable, ManagedObjectRemoval {

	/*
	 * IMPLEMENTATION NOTES:
	 * 
	 * The deque is implemented as a doubly-linked list of Element objects,
	 * which are stored in a ScalableHashMap. We use a ScalableHashMap instead
	 * of a ScalableHashSet, as the Set implementations is wrapper around Map
	 * (just like this class), and therefore would cause an extra data access
	 * penalty. Currently, the value stored in the Map is not meaningful.
	 * 
	 * A deque may have multiple elements with the same value. Since a Map is
	 * used to store the Element instances, we have to impose an additional
	 * factor to ensure each Element's uniqueness. Thefefore each Element has an
	 * associated Id based on when it was inserted into the deque.
	 * 
	 ...
	 * defined in the Deque interface. For example, removeFirst(Object) would
	 * look for all elements that have that same value as Object and choose the
	 * one with the lowest Id.
	 * 
	 * In general, a deque need not support random access to the elements other
	 * than the head and tail. However, to properly support the removal
	 * operations, we cannot traverse the entire map's keyset to find all the
	 * elements with the provided value. Therefore, we use an indexing technique
	 * to locate the Elements based on using a substitute class, ElementMatcher
	 * that is equivalent to an Element that is being searched for.
	 * 
	 * Random access to the elements occurs as follows. Each Element the of map
	 * uses the hashCode of the value that it stores. Multiple Element instances
	 * with the same value will still be unique by way of their Ids. The
	 * Map.Entry instances that contain the Element can be obtained from the
	 ...
	 * backing map. When the map is comparing the ElementMatcher instance to the
	 * Element instances (by doing its key-comparisons), the ElementMatcher will
	 * return true if the Element has the same value as the one stored in the
	 * ElementMatcher. This basic behavior is all that is necessary to support
	 ...
	 * already seen. This behavior lets us find *all* the Element instances with
	 * the provided Id. Since the Ids are monotonically ordered, we can then do
	 * Id comparisons to determine the first and last occurrences for the
	 * respective remove operations.
	 */
	/**
	...
	/**
	 * A counter for the unique Id assigned to elements at the head of the
	 * deque. When a new head element is added, this counter is decremented for
	 * the next element. In doing so, this ensures that a monotonic ordering
	 * exists for all elements, where the element at the head of the deque has a
	 * numerically smaller id than all other elements.
	 * 
	...
	/**
	 * A counter for the unique Id assigned to elements at the tail end of the
	 * deque. When a new tail element is added, this counter is incremented for
	 * the next element. In doing so, this ensures that a monotonic ordering
	 * exists for all elements, where the element at the tail end of the list
	 * has a numerically larger id than all other elements.
	 * 
	...
	/**
	 * A reference to the {@code ScalableHashMap} that will store all the
	 * mappings
	 */
	private final ManagedReference<ScalableHashMap<Element<E>, Long>> backingMapRef;
	/**
	 * A transient Java reference to the {@code ScalableHashMap} refered to by
	 * {@link #backingMapRef}. This field is set by a task's first access to the
	 * backing map.
	 * 
	...
	/**
	 * Creates a new empty {@code ScalableDeque}. Users of this constructor
	 * should refer to the class javadoc regarding the structure's performance.
	 */
	public ScalableDeque() {
		...
		// always be non-null
		ManagedSerializable<ManagedReference<Element<E>>> head = new ManagedSerializable<ManagedReference<Element<E>>>(
				null);
		ManagedSerializable<Long> headCount = new ManagedSerializable<Long>(-1L);

		ManagedSerializable<ManagedReference<Element<E>>> tail = new ManagedSerializable<ManagedReference<Element<E>>>(
				null);
		ManagedSerializable<Long> tailCount = new ManagedSerializable<Long>(0L);
	...
	/**
	 * Creates a {@code ScalableDeque} and adds all the elements in the provided
	 * collection according to their traversal ordering.
	 * 
	 * @param c
	 *            the collection of elements the deque will initially contain
	 */
	...
	/**
	 * Adds the provided {@code Element} to the head of the deque and updates
	 * the head reference.
	 * 
	 * @param e
	 *            the new head {@code Element}.
	 */
	...
	/**
	 * Adds the provided {@code Element} to the tail end of the deque and
	 * updates the tail reference.
	 * 
	 * @param e
	 *            the new tail {@code Element}.
	 */
	...
	public void clear() {
		...
		AppContext.getTaskManager().scheduleTask(
				new AsynchronousClearTask<E>(headRef));
	 ...
	 * {@inheritDoc}
	 * <p>
	 * The iterator implements the {@code java.io.Serializable} interface but
	 * does not implement the {@code com.sun.sgs.app.ManagedObject} interface,
	 * so it is not stored in the data manager by default.
	 * <p>
	 * The iterator throws a {@code ConcurrentModificationException} if either
	 * the next element was removed and {@code hasNext()} was not called before
	 * {@code next()}, or if the iterator was serialized and the next element
	 * was removed before a call to retrieve the next element is made.
	 */
	...
	public boolean equals(Object o) {
		/*
		 * IMPLEMENTATION NOTE: The Java general contract for hashCode requires
		 * that if two objects are equal, that they have the same hash code.
		 * However, there is not standard hashCode algorithm for Deque classes.
		 * Therefore, we make a best-effort to at least check for
		 * identity-equality within members of this class.
		 */
	...
	/**
	 * Returns the next unique identifier for an {@code Element} that is being
	 * added to the head of the deque.
	 * 
	...
	/**
	 * Returns the next unique identifier for an {@code Element} that is being
	 * added to the tail of the deque.
	 * 
	...
	/**
	 * Returns the {@code Element} at the front of this deque, or {@code null}
	 * if this deque is empty.
	 * 
	 * @return the element at the front of this deque or {@code null} if the
	 *         deque is empty
	 */
	...
	/**
	 * {@inheritDoc} Note that this implementation takes time proportinal to the
	 * number of instances of {@code o} in the deque, <i>not</i> the number of
	 * elements.
	 * 
	 * @param o
	 *            {@inheritDoc}
	 ...
	 * 
	 * @return {@code true} if the deque was modified as a result of this
	 *         operation
	 */
	public boolean removeAllOccurrences(Object o) {
		...
		ScalableHashMap.PrefixEntry<Element<E>, Long> entry = map().getEntry(
				matcher);
	...
	/**
	 * Removes the provided {@code Element} from the deque and updates all the
	 * deque-internal structures as necesary. This method will remove the
	 * element from the backing map, patch the element doubly-linked list, and
	 * then if necessary, update head and tail references.
	 * 
	 * @param e
	 *            the {@code Element} to be removed from the deque
	 * 
	...
	/**
	 * Clears the backing map, then removes all the {@code Element} instances
	 * created by this deque.
	 */
	...
	/**
	 * {@inheritDoc}. This operation runs in time proportinal to the length of
	 * this deque.
	 * 
	...
	/**
	 * Returns the {@code Element} at the end of this deque, or {@code null} if
	 * this deque is empty.
	 * 
	 * @return the element at the end of this deque or {@code null} if the deque
	 *         is empty
	 */
	...
	/**
	 * A deque-internal class for wrapping elements within the deque. This class
	 * maintains a doubly-linked list of all the elements in the deque. This
	 * class relies on {@code ScalableDeque} to maintain correct references to
	 * to the head and tail of this list, and also to update it accordingly on
	 * additions and removals.
	 * 
	 * <p>
	 * 
	 * The value of the elements themselves are refered to by
	 * {@code ManagedReference} instances if they implement
	 * {@code ManagedObject} or are refered to using a standard Java reference.
	 * 
	 * <p>
	 * 
	 * {@code Element} instances use their value's hash code, but can be
	 * distinguished by their {@code id}, which corresponds to the id assigned
	 * to them at the time of their addition to the deque. This method of
	 * hashing has the effect that when an {@code Element} is added to the
	 * deque, and subsequently added to the backing map, the deque can locate
	 * all instances of the element by using the element's hash code. This
	 * behavior is required to correctly support the random access behavior of
	 * {@link ScalableDeque#remove(Object) remove},
	 * {@link ScalableDeque#removeFirstOccurrence(Object) removeFirstOccurrence}, {@link ScalableDeque#removeLastOccurrence(Object) removeLastOccurrence}
	 * , and {@link ScalableDeque#removeAll(Collection) removeAll}.
	 * 
	 * @see ScalableDeque$ElementMatcher
	 * 
	 * @param <E>
	 *            The type of element held by the deque
	 */
	static class Element<E> implements Serializable, ManagedObject {
		...
		/**
		 * A {@code ManagedReference} to value of this element if the elemented
		 * implements {@code ManagedObject}, or {@code null} otherwise. The
		 * value of this variable is conditionally set during object
		 * deserialization based on {@code useRef}
		 * 
		 * @serial
		 */
		private transient ManagedReference<E> valueRef;
		/**
		 * A Java reference to the value of this element if the element does not
		 * implement {@code ManagedObject}, , or {@code null} otherwise. The
		 * value of this variable is conditionally set during object
		 * deserialization based on {@code sueRef}
		 * 
		 * @serial
		 */
		private transient E value;
		/**
		 * The flag that determines whether the value contained by this element
		 * should be accessed by a {@code ManagedReference} or a Java reference.
		 */
		private final boolean useRef;
		/**
		 * The Id assigned to this {@code Element} at the time of its creation.
		 * {@code Element} instances are numerically ordered where the head
		 * element will have the numerically smallest Id, and the tail element
		 * will have the numerically largest Id.
		 * 
		...
		/**
		 * A reference to the previous {@code Element} in the deque, or
		 * {@code null} if this element is the head of the deque.
		 */
		private ManagedReference<Element<E>> prevElement;
		/**
		 * A reference to the next {@code Element} in the deque, or {@code null}
		 * if this element is the tail of the deque.
		 */
		private ManagedReference<Element<E>> nextElement;

		/**
		 * Constructs a new {@code Element} with the provided {@code id} to
		 * contain the value.
		 * 
		 * @param value
		 *            the value held by this {@code Element}
		 * @param id
		...
		/**
		 * Returns {@code true} if {@code o} is an instance of {@code Element},
		 * contains the same value as this instance and has the same id.
		 * 
		 * @param o
		 *            {@inheritDoc}
		 * 
		 * @return {@code true} if {@code o} is an instance of {@code Element},
		 *         contains the same value as this instance and has the same id.
		 * 
		 * @see ScalableDeque$ElementMatcher
		 */
		...
		/**
		 * Returns the hash code provided by the value contained by this
		 * element.
		 * 
		 * @return the hash code of the value contained by this element
		 */
		public int hashCode() {
			E e = getValue();
			return e.hashCode();
		...
		/**
		 * Returns the {@code Element} after this instance in the deque, or
		 * {@code null} if this element is the tail of the deque.
		 * 
		 * @return the {@code Element} after this instance in the deque, or
		 *         {@code null} if this element is the tail of the deque.
		 */
		Element<E> next() {
		...
		/**
		 * Returns the {@code Element} before this instance in the deque, or
		 * {@code null} if this element is the head of the deque.
		 * 
		 * @return the {@code Element} before this instance in the deque, or
		 *         {@code null} if this element is the head of the deque.
		 */
		Element<E> prev() {
		...
		/**
		 * Sets the link from this {@code Element} to the next {@code Element}
		 * in the deque to {@code next}.
		 * 
		 * @code prev the {@code Element} after this {@code Element} in the
		 *       deque
		 */
		void setNext(Element<E> next) {
			DataManager dm = AppContext.getDataManager();
			ManagedReference<Element<E>> ref = (next == null) ? null : dm
		 ...
		 * 
		 * @code prev the {@code Element} before this {@code Element} in the
		 *       deque
		 */
		void setPrev(Element<E> prev) {
			DataManager dm = AppContext.getDataManager();
			ManagedReference<Element<E>> ref = (prev == null) ? null : dm
		...
		/**
		 * Writes out all non-transient state and then conditionally writes
		 * either {@code valueRef} or {@code value} depending on whether this
		 * element is supposed to use a {@code ManagedReference} to access its
		 * value.
		 * 
		 * @param s
		 *            {@inheritDoc}
		...
		/**
		 * Reconstructs the {@code Element} and initializes {@code valueRef} or
		 * {@code value} depending on whether this element is supposed to use a
		 * {@code ManagedReference} to access its value.
		 * 
		 * @param s
		 *            {@inheritDoc}
		 */
	...
	/**
	 * A utility class for locating {@code Element} instances in the backing
	 * map. This class hashes to the same value as the {@code toFind} object
	 * provided to the constructor. This class uses an implementation of
	 * {@code hashCode} and {@code equals} that will return true if an
	 * {@code Element} is found that has the same {@code hashCode},
	 * {@code toFind} is equal to the value of the {@code Element}, and where
	 * the Id of the {@code Element} has not already been seen. This allows us
	 * to use a {@code ElementMatcher} that wraps an object with
	 * {@code ScalableHashMap#getEntry(Object)} to return the actual
	 * {@code Element} contained within the map. This functionality is required
	 * to support the random access required for
	 * {@link ScalableDeque#remove(Object) remove},
	 * {@link ScalableDeque#removeFirstOccurrence(Object) removeFirstOccurrence}
	 * , {@link ScalableDeque#removeLastOccurrence(Object) removeLastOccurrence}
	 * , and {@link ScalableDeque#removeAll(Collection) removeAll}.
	 * 
	 * <p>
	 * 
	 * This class also supports repeated use in searching for elements by way of
	 * its {@code alreadySeen} field. A single {@code ElementMatcher} instance
	 * maybe be updated with the {@code Element} Ids already seen from past
	 * searches. For example, this allows us to search for all {@code Element}
	 * instances with a certain value.
	 * 
	 * @see ScalableDeque$Element
	 * 
	 * @param <T>
	 *            the type of element to be matched
	 */
	/*
	 * IMPLEMENTATION NOTE: this class does not implement Serializable to ensure
	 * that it can never be accidentally stored in the backing map. (Or that if
	 * it was, an error would occur such that the programmer would know)
	 */
	private static class ElementMatcher<T> {

		/**
		 * The set of {@code Element} Id values already seen. This set is empty
		 * on construction.
		 */
		private Set<Long> alreadySeen;
		/**
		 * The value of the {@code Element} that this matcher should find.
		 */
		 ...
		 * 
		 * @param toFind
		 *            the value of the {@code Element} that this instance should
		 *            match
		 */
		 ...
		 * 
		 * @param id
		 *            an {@code Element} id that has already been seen
		 */
		 ...
		 * this instance, and the Id of the {@code Element} is not in the
		 * {@code alreadySeen} set.
		 * 
		 * @param o
		 *            {@code inheritDoc}
		 * 
		...
		public boolean equals(Object o) {
			if (o == null) {
				...
				T value = e.getValue();
				return (value == toFind || (value != null && value
						.equals(toFind))) && !alreadySeen.contains(e.getId());
			}
			return false;
		}
	...
	/**
	 * A implementation of the iterator for the {@code ScalableDeque} that
	 * allows element traverse from head-to-tail or tail-to-head as required.
	 * 
	 * <p>
	 * 
	 ...
	 */
	static class BidirectionalDequeIterator<E> implements Iterator<E>,
			Serializable {
		 ...
		 * recheck whether its next element is still valid.
		 * 
		 ...
		 * 
		 * @param deque
		 *            the deque that will be iterated over
		 * @param isReverse
		 *            whether this iterator should traverse the provided deque
		 *            from tail to head
		 */
		 ...
		 * to the next element is still valid.
		 * 
		 * <p>
		 * 
		 * If this iterator was created based on an empty deque, and has never
		 * iterated over the first element, the iterator must check whether any
		 * new elements exist in the deque. Once an element exists, the iterator
		 * updates its nextElement reference and is no longer in the "empty map"
		 * state.
		 * 
		 * <p>
		 * 
		 * Otherwise, if the iterator was serialized and the next element was
		 * removed, the deque will throw a
		 * {@code ConcurrentModificationException} during a call to retrieve the
		 * next element.
		 * 
		...
		private void checkForNextElementUpdates() {
			...
			if (nextElementWasNullOnCreation) {
				...
				if (isReverse) {
					...
					nextElement = (tail == null) ? null : AppContext
							.getDataManager().createReference(tail);
				} else {
					// see if the first element in the deque is now
					// non-null
					Element<E> head = dequeRef.get().headElement();
					nextElement = (head == null) ? null : AppContext
							.getDataManager().createReference(head);
				}
		...
		/**
		 * Returns the next element in the {@code ScalableDeque}. This method
		 * will throw a {@link java.util.ConcurrentModificationException} if the
		 * next element that the iterator was set to return has been removed
		 * from the deque, and {@code hasNext()} as not been called first.
		 * 
		 ...
		 *             from the deque and {@code hasNext()} had not been called
		...
		public E next() {
			...
			nextElement = (isReverse) ? element.prevElement
					: element.nextElement;
		...
		/**
		 * Reconstructs the {@code BidirectionalDequeIterator} from the provided
		 * stream and marks that this iterator should check that its next
	...
	/**
	 * A helper task that will remove the {@code ManagedObject} node instances
	 * of the dequeue after a {@code clear} operation has been performed.
	 * 
	 * @see ScalableDeque#clear()
	 * @see ScalableDeque#removingObject()
	 * 
	 * @param <E>
	 *            the type of elements being removed.
	 */
	private static class AsynchronousClearTask<E> implements ManagedObject,
			Serializable, Task {
		...
		/**
		 * Constructs the task with the first node in the list of entries to be
		 * removed
		 * 
		 * @param headElement
		 *            the element at the head of the deque at the time of the
		...
		/**
		 * Clears some entries and re-enqueues this task if more entries remain.
		 */
		public void run() {

 ...
 * {@link java.util.HashMap} class for use in a {@link ManagedObject}. A
 * {@code HashMap} will typically perform better than this class when the number
 * of mappings is small, the objects being stored are small, and minimal
 ...
 * provide any concurrency for {@code Task}s running in parallel that attempt to
 * modify the map at the same time, so this class may perform better in
 * situations where multiple tasks need to modify the map concurrently, even if
 * the total number of mappings is small. Also note that, unlike {@code HashMap}
 * , this class can be used to store {@code ManagedObject} instances directly.
 * 
 * <p>
 * 
 ...
 * that do not implement {@code Serializable} will result in an
 * {@link IllegalArgumentException} being thrown. If a key or value is an
 * instance of {@code Serializable} but does not implement {@code ManagedObject}
 * , this class will persist the object as necessary; when such an object is
 * removed from the map, it is also removed from the {@code DataManager}. If a
 * key or value is an instance of {@code ManagedObject}, the developer will be
 * responsible for removing these objects from the {@code DataManager} when done
 * with them. Developers should not remove these object from the
 * {@code DataManager} prior to removing them from the map.
 * 
 * <p>
 * 
 ...
 * use with this class. The same requirements apply to objects used as values if
 * the application intends to use {@link #containsValue containsValue} or to
 * compare map entries.
 * 
 * <p>
 * 
 ...
 * parallel operations will also grow beyond the specified minimum. Setting the
 * minimum concurrency too high will waste space and time, while setting it too
 * low will cause conflicts until the map grows sufficiently to support more
 ...
 * {@code Serializable}. A single iterator may be saved by different
 * {@code ManagedObject} instances, which will create distinct copies of the
 * original iterator. A copy starts its iteration from where the state of the
 * original was at the time of the copy. However, each copy maintains a
 * separate, independent state from the original and will therefore not reflect
 * any changes to the original iterator. To share a single {@code Iterator}
 ...
 * collection, but may ignore additions and removals made to the collection
 * during iteration, and may also visit more than once a key value that is
 * removed and re-added. Attempting to use an iterator when the associated map
 * has been removed from the {@code DataManager} will result in an
 * {@code ObjectNotFoundException} being thrown, although the
 * {@link Iterator#remove remove} method may throw {@code IllegalStateException}
 ...
 * @see ManagedObject
 */
public class ScalableHashMap<K, V> extends AbstractMap<K, V> implements
		Serializable, ManagedObjectRemoval {

	/*
	 * This class's implementation is based on the paper "Extendible hashing - a
	 * fast access method for dynamic files" by Fagin, Nievergelt, Pippenger,
	 * and Strong, ACM Transactions on Database Systems, Volume 4, Issue 3
	 * (September 1979), pp 315 - 344.
	 * 
	 * The basic structure is a tree whose leaves are standard hash tables. Each
	 * non-leaf, or directory, node contains a fixed-size array, the node
	 * directory, of references to its children. Keys are looked up in the tree
	 * based on their hash code. Each directory node records its depth in the
	 * tree, which represents the number of the high order bits of the hash code
	 * to ignore when selecting the child node. The necessary number of bits
	 * following that position are used as an index into the node directory to
	 * select the child node. Although the full node directory array is
	 * allocated at the start, the children nodes are allocated as needed, and
	 * appear multiple times in the directory in order to select the proper node
	 * for the multiple indices that are associated with it. For example, if a
	 * node's directory can support 32 children but has only 2, then the first
	 * 16 entries in the node directory will point to the first child, and the
	 * remainder to the second child.
	 * 
	 * Leaf nodes contain an array of hash buckets, each of which stores a
	 * linked list of the entries that hash to the that bucket. The hash bucket
	 * chosen is based on the bits immediately below those used to choose the
	 * node in the parent's node directory.
	 * 
	 ...
	 * are ordered the same way as children nodes are. Using the same order for
	 * nodes and buckets insures that the order of entries will be maintained
	 * when a node is split. Keys with the same hash code are further ordered by
	 * the object ID of managed object that stores the key. Storing the hash
	 * code and object ID in the iterator provides a way to uniquely identify
	 * the location of the iteration within the table.
	 */
	 ...
	 */
	private static final int DEFAULT_MINIMUM_CONCURRENCY = DEFAULT_DIRECTORY_SIZE;
	 ...
	 * nodes is done -- used for testing. Note that this method is called during
	 * the transaction that completes the removal.
	 */
	 ...
	 * change to the data structure. Incompatible changes should be marked by a
	 * change to the serialVersionUID.
	 * 
	 * @serial
	 */
	...
	/**
	 * The parent node directly above this. For the root node, this should
	 * always be null.
	 * 
	 * @serial
	 */
	 ...
	 * provided prefix. If this instance is a leaf node, the
	 * {@code nodeDirectory} for that instance will be {@code null}. Note that
	 * this directory contains both leaf nodes as well as other directory nodes.
	 * 
	 * @serial
	 */
	...
	/**
	 * The monotonic counter that reflects the number of times this instance has
	 * been modified. The counter is used by the {@code ConcurrentIterator}
	...
	/**
	 * The number of entries in this node's table. Note that this is <i>not</i>
	 * the total number of entries in the entire tree. For a directory node,
	...
	/**
	 * The maximum number of bits used in the node directory. This is calculated
	 * based on the {@code directorySize} provided in the constructor.
	 * 
	 ...
	 *            the depth of this node in the tree
	 * @param minDepth
	 *            the minimum depth of leaf nodes requested to support a minimum
	 *            number of concurrent write operations
	 * @param splitThreshold
	 *            the number of entries in a leaf node that will cause the leaf
	 *            to split
	 * @param directorySize
	 *            the maximum number of children nodes of a directory node
	 * 
	 * @throws IllegalArgumentException
	 *             if:
	 *             <ul>
	 *             <li> {@code depth} is negative or greater than
	 *             {@link #MAX_DEPTH}
	 *             <li> {@code minDepth} is negative or greater than
	 *             {@code MAX_DEPTH}
	...
	ScalableHashMap(int depth, int minDepth, int splitThreshold,
		...
		if (depth < 0 || depth > MAX_DEPTH) {
			throw new IllegalArgumentException("Illegal tree depth: " + depth);
		}
	 ...
	 * 
	 * @param minConcurrency
	 *            the minimum number of concurrent write operations to support
	 * 
	 * @throws IllegalArgumentException
	 *             if {@code minConcurrency} is not greater than zero
	 */
	...
	/**
	 * Constructs an empty map with the default minimum concurrency ({@code 32}
	 * ).
	 */
	 ...
	 * and the default minimum concurrency ({@code 32}).
	 * 
	 * @param map
	 *            the mappings to include
	 * 
	 * @throws IllegalArgumentException
	 *             if any of the keys or values contained in the argument are
	 *             not {@code null} and do not implement {@code Serializable}
	 */
	 ...
	 * 
	 * @param minConcurrency
	 *            the minimum number of concurrent write operations to perform
	 * 
	 * @return the necessary minimum depth to the tree
	 * 
	 * @throws IllegalArgumentException
	 *             if minConcurrency is not greater than zero
	 */
	 ...
	 * 
	 * @param minDepth
	 *            the minimum depth of the leaf nodes under this node
	 * 
	 * @see #split split
	 */
	...
	private void initDepth(int minDepth) {
		...
		ManagedReference<ScalableHashMap<K, V>> thisRef = dm
				.createReference(this);

		ScalableHashMap[] leaves = new ScalableHashMap[numLeaves];
		for (int i = 0; i < numLeaves; ++i) {
			ScalableHashMap<K, V> leaf = new ScalableHashMap<K, V>(depth
					+ leafBits, minDepth, splitThreshold, 1 << maxDirBits);
			leaves[i] = leaf;
		...
		lastLeaf.leftLeafRef = uncheckedCast(dm
				.createReference(leaves[numLeaves - 2]));
	...
	/**
	 * Mark this node as a directory node by setting its entry table to
	 * {@code null}.
	 */
	...
	/**
	 * Clears the map of all entries. When clearing, all non-
	 * {@code ManagedObject} keys and values persisted by this map will be
	 * removed from the {@code DataManager}.
	 */
	public void clear() {
		assert isRootNode() : "The clear method should only be called on the root";
	 ...
	 * 
	 * @param key
	 *            the key that is associated with an {@code Entry}
	 * 
	...
	PrefixEntry<K, V> getEntry(Object key) {
		...
		for (PrefixEntry<K, V> e = leaf.getBucket(leaf.indexFor(hash)); e != null; e = e.next) {
			if (e.hash == hash) {
				K k;
				try {
					k = e.getKey();
	...
	/**
	 * Returns the first entry in this leaf for the specified index, or
	 ...
	 *            the hash code
	 * @param keyRef
	 *            the reference for the entry key
	 * 
	 ...
	 *            second value
	 * 
	 * @return {@code true} if {@code x} is less than {@code y} when viewed as
	 *         an unsigned integer, else {@code false}
	 */
	...
	/**
	 * Divides the entries in this leaf node into two leaf nodes on the basis of
	 * prefix, and then either adds the new nodes to the parent or converts this
	 * node to a directory node. This method should only be called when the
	 * entries contained within this node have valid prefix bits remaining (i.e.
	 * they have not already been shifted to the maximum possible precision).
	 * 
	...
	private void split() {
		...
		ScalableHashMap<K, V> leftChild = new ScalableHashMap<K, V>(depth + 1,
				minDepth, splitThreshold, 1 << maxDirBits);
		ScalableHashMap<K, V> rightChild = new ScalableHashMap<K, V>(depth + 1,
				minDepth, splitThreshold, 1 << maxDirBits);
		...
		for (int i = 0; i < table.length; i++) {
			ScalableHashMap<K, V> child = (i < firstRight) ? leftChild
					: rightChild;
		...
		ManagedReference<ScalableHashMap<K, V>> leftChildRef = dataManager
				.createReference(leftChild);
		ManagedReference<ScalableHashMap<K, V>> rightChildRef = dataManager
				.createReference(rightChild);
		...
		if (isRootNode() || depth % maxDirBits == 0 || depth == minDepth) {

			// this leaf node will become a directory node
			ManagedReference<ScalableHashMap<K, V>> thisRef = dataManager
					.createReference(this);
			...
			// children
			parentRef.get().addLeavesToDirectory(prefix, leftChildRef,
					rightChildRef);
		}
	}
	 ...
	 * 
	 * @param prefix
	 *            the initial prefix for which to search
	 * 
	 ...
	 *            the value
	 * @param numBits
	 *            the number of highest order bits
	 * 
	 ...
	 * 
	 * @param prefix
	 *            the prefix that leads to the node that will be replaced
	 * @param leftChildRef
	 *            the left child of the node that will be replaced
	 * @param rightChildRef
	 *            the right child of the node that will be replaced
	 */
	 ...
	 * contains no mapping for this key. Note that the return value of
	 * {@code null} does not necessarily imply that no mapping for this key
	 * existed since this implementation supports {@code null} values. The
	 * {@link #containsKey containsKey} method can be used to determine whether
	 * a mapping exists.
	 * 
	 * @param key
	 ...
	 *             if the value associated with the key has been removed from
	 ...
	 * 
	 * @throws IllegalArgumentException
	 *             if either {@code key} or {@code value} is not {@code null}
	 *             and does not implement {@code Serializable}
	 * @throws ObjectNotFoundException
	 *             if the previous value associated with the key has been
	 *             removed from the {@link DataManager}
	 */
	 ...
	 * @throws ObjectNotFoundException
	 *             if the previous value associated with the key has been
	 *             removed from the {@link DataManager} and is being returned
	 */
	private V putInternal(K key, V value, boolean returnOldValue) {
		...
		for (PrefixEntry<K, V> e = leaf.getBucket(i); e != null; e = e.next) {
			/*
			 * Keep bucket chain sorted by hash code, treating the hash codes as
			 * unsigned integers so that they sort the same way as directory
			 * lookups.
			 */
			if (unsignedLessThan(e.hash, hash)) {
				prev = e;
	 ...
	 * 
	 * @throws IllegalArgumentException
	 *             if the object is not {@code null} and does not implement
	 *             {@code Serializable}
	 */
	 ...
	 *             if any of the keys or values contained in the argument are
	 *             not {@code null} and do not implement {@code Serializable}.
	 *             If this exception is thrown, some of the entries from the
	 *             argument may have already been added to this map.
	 */
	 ...
	 * {@link #split split} operation is necessary.
	 * 
	 * @param entry
	 *            the entry to add
	 * @param prev
	 *            the entry immediately prior to the entry to add, or
	 *            {@code null} if the entry should be the first in the bucket
	 */
	 ...
	 * @param prev
	 *            the entry immediately prior to the entry to add, or
	 *            {@code null} if the entry should be the first in the list
	 */
	private void addEntry(PrefixEntry<K, V> entry, PrefixEntry<K, V> prev) {
		...
		if (prev == null) {
			...
			assert indexFor(entry.hash) == indexFor(prev.hash) : "Previous node was in a different bucket";
	...
	/**
	 * Returns the bucket index for this hash value given the provided number of
	 * buckets.
	 * 
	 * @param h
	 *            the hash value
	...
	private int indexFor(int h) {
		/*
		 * Return the bits immediately to the right of those used to choose this
		 * node from the parent, but using the full complement of bits if this
		 * is a very deep node. Using the bits immediately after the directory
		 * bits insures that the buckets are ordered the same way as the nodes
		 * would be after a split.
		 */
	 ...
	 * <p>
	 * 
	 * This implementation removes from the {@code DataManager} all non-
	 * {@code ManagedObject} keys and values persisted by this map, as well as
	 * objects that make up the internal structure of the map itself.
	 */
	...
	/**
	 * Recursively removes the children and entries of this node, performing the
	 * actual removal in a separate task to avoid scaling problems.
	 */
	...
	/**
	 * Removes some entries from this leaf node, returning true if they were all
	 * removed.
	 */
	...
	/**
	 * Removes some entries from the specified array of entries, returning true
	 ...
	 */
	private static final class RemoveNodeEntriesTask implements ManagedObject,
			Serializable, Task {
		/** The version of the serialized form. */
		private static final long serialVersionUID = 1;
	 ...
	 * node by depth-first recursion. Does not remove the node itself, and makes
	 * a copy of the node's children so that the node can continue to be used.
	 * 
	 * Each time the task is run it moves to the next non-empty leaf node,
	 * removes a limited number of entries and just emptied nodes, and if needed
	 * walks up the node graph to the next non-empty directory node. If there is
	 * more work to do, it reschedules the task, otherwise it removes it. This
	 * scheme insures that the number of nodes visited per run is the same as
	 * the number needed to perform a lookup, and limits the number of entries
	 * visited, all in an attempt to keep the work small enough to fit within
	 * the transaction timeout.
	 */
	private static final class RemoveNodesTask<K, V> implements ManagedObject,
			Serializable, Task {
		/** The version of the serialized form. */
		private static final long serialVersionUID = 1;
		...
		private final Stack<ManagedReference<ScalableHashMap<K, V>>> nodeRefs = new Stack<ManagedReference<ScalableHashMap<K, V>>>();
		...
		private RemoveNodesTask(ScalableHashMap<K, V> node) {
			...
			for (int i = 0; i < node.nodeDirectory.length; i++) {
				ManagedReference<ScalableHashMap<K, V>> ref = uncheckedCast(node.nodeDirectory[i]);
		...
		private boolean doWork() {
			...
			if (!node.isLeafNode()) {
				while (true) {
					currentNodeRef = uncheckedCast(node.nodeDirectory[offsets
							.peek()]);
	 ...
	 * the prefix at which it is stored, as well as whether the
	 * {@link ScalableHashMap} is responsible for the persistent lifetime of the
	 * key and value.
	 * 
	 * <p>
	 * 
	 * If a key or value that does not implement {@link ManagedObject} is stored
	 * in the map, then it is wrapped using the {@link ManagedSerializable}
	 * utility class so that the entry may have a {@code ManagedReference} to
	 * the value, rather than a standard reference.
	 * 
	 * <p>
	 * 
	 ...
	 * <p>
	 * 
	 * Note that the ordering of entries depends on the object ID of the managed
	 * object that represents the key. To insure the proper behavior of the
	 * iterator, that object ID must not change after the entry is created. The
	 * object ID used is either that of the key itself, if the key is a managed
	 * object, or that of the ManagedSerializable wrapper created to wrap either
	 * the just key or both the key and value. In either case, the managed
	 * object associated with the key is determined in the constructor
	 * 
	...
	static class PrefixEntry<K, V> implements Entry<K, V>, Serializable {
		...
		/**
		 * The hash value for this entry. This value is also used to compute the
		 * prefix.
		 * 
		 * @serial
		 */
		...
		PrefixEntry(int h, K k, V v) {
			...
			if (!(k instanceof ManagedObject) && !(v instanceof ManagedObject)) {
				setKeyValuePair();
				keyOrPairRef = dm
						.createReference(new ManagedSerializable<Object>(
				...
				keyOrPairRef = dm
						.createReference(isKeyWrapped() ? new ManagedSerializable<Object>(
								k) : k);
				setValueWrapped(!(v instanceof ManagedObject));
				valueRef = dm
						.createReference(isValueWrapped() ? new ManagedSerializable<V>(
								v) : v);
			}
		}
		...
		/**
		 * Returns the key stored by this entry. If the mapping has been removed
		 * from the backing map before this call is made, an
		 * {@code ObjectNotFoundException} will be thrown.
		 * 
		 * @return the key stored in this entry
		 * @throws ObjectNotFoundException
		 *             if the key in the backing map was removed prior to this
		 *             call
		 */
		public final K getKey() {
			if (isKeyValuePair()) {
				ManagedSerializable<KeyValuePair<K, V>> msPair = uncheckedCast(keyOrPairRef
						.get());
				return msPair.get().getKey();
			} else if (isKeyWrapped()) {
				ManagedSerializable<K> msKey = uncheckedCast(keyOrPairRef.get());
				return msKey.get();
			} else {
		 ...
		 * These references are not guaranteed to stay valid across transactions
		 * and should therefore not be used except for comparisons.
		 * 
		 ...
		 * removed from the backing map before this call is made, an
		 * {@code ObjectNotFoundException} will be thrown.
		 * 
		 * @return the value stored in this entry
		 * @throws ObjectNotFoundException
		 *             if the value in the backing map was removed prior to this
		 *             call
		 */
		...
		void setValueInternal(V newValue) {
			...
			if (newValue instanceof ManagedObject) {
				if (isKeyValuePair()) {
					...
					ManagedSerializable<KeyValuePair<K, V>> msPair = uncheckedCast(keyOrPairRef
							.get());
					ManagedSerializable<K> msKey = uncheckedCast(keyOrPairRef
							.get());
				...
				valueRef = dm.createReference(new ManagedSerializable<V>(
						newValue));
		...
		/**
		 * Returns the string form of this entry as {@code entry}={@code value}.
		 */
		 ...
		 * manager. This should only be called from
		 * {@link ScalableHashMap#clear ScalableHashMap.clear},
		 * {@link ScalableHashMap#remove ScalableHashMap.remove}, or
		 * {@link #remove remove} under the condition that this entry's
		 * map-managed object will never be referenced again by the map.
		 */
		final void unmanage() {
			DataManager dm = AppContext.getDataManager();
	...
	/**
	 * A concurrent, persistable {@code Iterator} implementation for the
	 * {@code ScalableHashMap}. This implementation is stable with respect to
	 * concurrent changes to the associated collection, but may ignore additions
	 * and removals made to the collection during iteration, and may also visit
	 * more than once a key value that is removed and re-added.
	 */
	abstract static class ConcurrentIterator<E, K, V> implements Iterator<E>,
			Serializable {
		 ...
		 * case the map was cleared or removed, since then the leaf is no longer
		 * part of the map.
		 */
		...
		/**
		 * The next entry, or null if there is no next entry or if not computed.
		 */
		private transient PrefixEntry<K, V> nextEntry = null;

		/**
		 * The value of the modification count when the nextLeaf and nextEntry
		 ...
		 * 
		 * @param root
		 *            the root node of the {@code ScalableHashMap}
		 */
		 ...
		 * <p>
		 * 
		 * This method will never throw a
		...
		Entry<K, V> nextEntry() {
			...
			currentLeafRef = AppContext.getDataManager().createReference(
					nextLeaf);
	 ...
	 */
	private static final class EntryIterator<K, V> extends
			ConcurrentIterator<Entry<K, V>, K, V> {
		 ...
		 * 
		 * @param root
		 *            the root node of the backing trie
		 */
	 ...
	 */
	private static final class KeyIterator<K, V> extends
			ConcurrentIterator<K, K, V> {
		private static final long serialVersionUID = 0x1L;
	 ...
	 */
	private static final class ValueIterator<K, V> extends
			ConcurrentIterator<V, K, V> {
	 ...
	 * mappings contained in this map. The returned {@code Set} is backed by the
	 * map, so changes to the map will be reflected by this view. Note that the
	 ...
	 */
	private static final class KeySet<K, V> extends AbstractSet<K> implements
			Serializable {
	 ...
	 *             chained. The deserialization should count each chained entry
	 ...
	 * The root node has a depth of {@code 1}. This method is used for testing.
	 * 
	...
	void checkLeafRefs() {
		ManagedReference<ScalableHashMap<K, V>> lastRef = null;
		ManagedReference<ScalableHashMap<K, V>> nodeRef = AppContext
				.getDataManager().createReference(leftMost());
		while (nodeRef != null) {
			ScalableHashMap<K, V> node = nodeRef.get();
			if (!node.isLeafNode()) {
				throw new AssertionError("Node " + nodeRef
						+ " is not a leaf node");
			}
			...
			if (lastRef == null) {
				if (leftRef != null) {
					throw new AssertionError("Node " + nodeRef
							+ " has left leaf " + leftRef
							+ " but should be null");
				}
			} else if (!lastRef.equals(leftRef)) {
				throw new AssertionError("Node " + nodeRef + " has left leaf "
						+ leftRef + " but should be " + lastRef);
			}
			lastRef = nodeRef;
		...
		if (lastRef != null) {
			...
			if (node.rightLeafRef != null) {
				throw new AssertionError("Node " + lastRef + " has right leaf "

 ...
 * scheduler. It is used by implementations of {@code SchedulerQueue} to manage
...
public interface ScheduledTask {
	...
	/**
	 * Returns the transaction timeout. A Scheduler may or may not choose to use
	 * this timeout value.
	 * 
	...
	/**
	 * Returns the {@code Throwable} that was the last failure of this task or
	 * {@code null} if it has never been run or never failed.
	 * 
	...
	/**
	 * Returns whether this is a recurring task. If this is not a recurring task
	 * then {@code getPeriod} should always return {@code NON_RECURRING}.
	 * 
	 * @return {@code true} if this task is a recurring task, {@code false}
	 *         otherwise.
	 */
	boolean isRecurring();

	/**
	 * Returns the {@code RecurringTaskHandle} associated with this task if this
	 * task is recurring, or {@code null} if this is not recurring.
	 * 
	 ...
	 * running then calling this method may not have any affect. Also note that
	 * this method should never be called with a value of {@code true} for the
	 * {@code block} parameter if the calling thread is also currently running
	 * this {@code ScheduledTask}.
	 * 
	 * @param block
	 *            if {@code true}, this call will block until the task is

...
public interface SchedulerRetryPolicy {
	 ...
	 * the task should be retried by the scheduler at some point in the future.</li>
	 * <li>A return value of {@link SchedulerRetryAction#RETRY_NOW} means that
	 * the task should be retried immediately.</li>
	 * </ul>
	 * This method may modify the given {@code task} in order to affect how the
	 * scheduler behaves when re-executing.
	 * <p>
	 ...
	 * {@link TaskManager} with regard to {@code Throwable}s that implement this
	 * interface.
	 * <p>
	 * Note: The proper way to drop a task with a custom policy is to return
	 * {@code DROP} with this method. Implementations <em>should not</em> use a
	 * call to {@link ScheduledTask#cancel(boolean) cancel} on the given task.
	 * 
	 * @param task
	 *            the task that has been aborted
	 * @return the {@code SchedulerRetryAction} that the scheduler should take
	 *         with respect to retrying the given task
	 * @throws IllegalArgumentException
	 *             if {@code task} is {@code null}
	 * @throws IllegalStateException
	 *             if the given {@code task}'s most recent failure is
	 *             {@code null}
	 */

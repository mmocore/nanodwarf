...
public interface DataService extends DataManager, Service {
	...
	/**
	 * Obtains the object associated with the service binding of a name. Callers
	 * need to notify the system before modifying the object or any of the
	 * non-managed objects it refers to by calling {@link #markForUpdate
	 ...
	 * both the object and any objects it refers to must implement
	 * {@link Serializable}. Note that this method will throw
	 ...
	 * name and the object is removed. To remove the object, use the
	 ...
	 *            beginning
	 * @return the next name with a service binding following <code>name</code>,
	 *         or <code>null</code> if there are no more bound names
	 * @throws TransactionException
	 *             if the operation failed because of a problem with the current
	 *             transaction
	 */
	...
	/**
	 * Creates a managed reference for the object with the specified identifier,
	 * which should have been obtained from a call to
	 * {@link ManagedReference#getId ManagedReference.getId}. Callers should
	 * make sure that the associated object is reachable from an existing name
	 * binding. This method does not check to see whether the associated object
	 * has been removed.
	 * 
	 * @param id
	 *            the identifier
	 ...
	 * @throws IllegalArgumentException
	 *             if the implementation is able to determine that {@code id}
	 *             was not returned by a call to {@code ManagedReference.getId}
	 * @throws TransactionException
	 *             if the operation failed because of a problem with the current
	 *             transaction
	 */
	...
	/**
	 * Returns a unique identifier for the next object after the object with the
	 * specified identifier, or {@code null} if there are no more objects. If
	 * {@code objectId} is {@code null}, then returns the identifier of the
	 ...
	 * {@code objectId} to have already been removed.
	 * <p>
	 * 
	 ...
	 * will continue to be retained by the data service.
	 * 
	 * @param objectId
	 *            the identifier of the object to search after, or {@code null}
	 *            to request the first object
	 * @return the identifier of the next object following the object with
	 *         identifier {@code objectId}, or {@code null} if there are no more
	 *         objects
	 * @throws IllegalArgumentException
	 *             if the implementation can determine that {@code objectId} is

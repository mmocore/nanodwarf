...
/**
 * Defines the interface to the underlying persistence mechanism to store byte
 * data.
 * <p>
 * 
 * Objects are identified by object IDs, which are positive <code>long</code>s.
 * Names are mapped to object IDs.
 */
public interface DataStore {
	 ...
	 * {@link #setObject setObject} or {@link #setObjects setObjects} is called.
	 * Aborting a transaction is also not required to unassign the ID so long as
	 * other operations treat it as a non-existent object.
	 * 
	 * @param txn
	 ...
	 * exclusive lock on the object in order to avoid contention when the object
	 ...
	 * that information to obtain an exclusive lock on the object in order avoid
	 ...
	 * @throws IllegalArgumentException
	 *             if <code>oids</code> and <code>data</code> are not the same
	 *             length, or if <code>oids</code> contains a value that is
	 *             negative
	 * @throws TransactionAbortedException
	 *             if the transaction was aborted due to a lock conflict or
	 *             timeout
	...
	/**
	 * Removes the object with the specified object ID. The implementation will
	 * make an effort to flag subsequent references to the removed object by
	 * throwing {@link ObjectNotFoundException}, although this behavior is not
	 * guaranteed. The implementation is not required to check that the object
	 * is an externally visible object rather than one used internally by the
	 * implementation.
	 * 
	 * @param txn
	 *            the transaction under which the operation should take place
	 ...
	 *             if no object ID is bound to the name
	 * @throws TransactionAbortedException
	 *             if the transaction was aborted due to a lock conflict or
	 *             timeout
	 * @throws TransactionNotActiveException
	...
	/**
	 * Shuts down this data store. This method will block until the shutdown is
	 * complete.
	 * <p>
	 */
	void shutdown();
	 ...
	 * information. Obtains an existing ID for the class information if present;
	 * otherwise, stores the information and returns the new ID associated with
	 * it. Class IDs are always greater than {@code 0}. The class information is
	 * the serialized form of the {@link ObjectStreamClass} instance that
	 * serialization uses to represent the class.
	 * 
	 * @param txn
	 *            the transaction under which the operation should take place
	 * @param classInfo
	...
	/**
	 * Returns the class information associated with the specified class ID. The
	 * class information is the serialized form of the {@link ObjectStreamClass}
	 ...
	 * @throws IllegalArgumentException
	 *             if {@code classId} is not greater than {@code 0}
	 ...
	 * specified ID, or {@code -1} if there are no more objects. If
	 * {@code objectId} is {@code -1}, then returns the ID of the first object.
	 * The IDs returned by this method will not include ones for objects that
	 ...
	 * returned by this method, but which cannot be reached by traversing object
	 * field references starting with an object associated with a name binding,
	 * will continue to be retained by the data store.
	 * 
	 * @param txn
	 *            the transaction under which the operation should take place
	 * @param oid
	 ...
	 *         objects
	 * @throws IllegalArgumentException
	 *             if the argument is less than {@code -1}
	 * @throws TransactionAbortedException
	 *             if the transaction was aborted due to a lock conflict or
	 *             timeout
	 * @throws IllegalStateException
	 ...
	 * accesses. The {@code description} should provide a meaningful

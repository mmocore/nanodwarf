...
/**
 * The {@code WatchdogService} monitors the health of server nodes and notifies
 * registered listeners of node status change events. It also provides
 * information and notification services about node backup and recovery.
 */
public interface WatchdogService extends Service {

	/**
	 * Returns the health of the local node. This method should only be called
	...
	/**
	 * Returns the health of the local node. This method returns the most recent
	 * information known to this service and may not be definitive. For
	 * definitive information, use the {@link #getLocalNodeHealth
	 * getLocalNodeHealth} method.
	 * <p>
	 * 
	 * This method may be invoked any time after this service is initialized,
	 * whether or not the calling context is inside or outside of a transaction.
	 * 
	 * @return the health of the local node
	 */
	...
	/**
	 * Returns an iterator for the set of nodes that this service monitors. The
	 * {@code remove} operation of the returned iterator is not supported and
	 * will throw {@code UnsupportedOperationException} if invoked. This method
	 * must be called within a transaction, and the returned iterator must only
	 ...
	 * it should throw an exception in the case where there is no existing node
	 * corresponding to {@code nodeId}.</b>
	 * 
	 * @param nodeId
	 *            a node ID
	 ...
	 *             if this method is invoked from a transactional context
	 */
	void addNodeListener(NodeListener listener);

	/**
	 * Adds the specified recovery {@code listener} for the local node. If the
	 * local node is designated as a backup for a node that fails, the specified
	 * {@code listener} will be notified (outside of a transaction) by having
	 * its {@link RecoveryListener#recover recover} method invoked, passing the
	 * failed node and a {@link SimpleCompletionHandler} whose
	 * {@link SimpleCompletionHandler#completed completed} method must be
	 * invoked when the recovery operations initiated by the {@code listener}
	 * are complete.
	 * <p>
	 * 
	 * This method must be invoked outside of a transaction.
	...
	/**
	 * Informs the watchdog of this node's health. Multiple components may
	 * report on a node's health. The watchdog will use these reports to
	 * determine the overall node's health. The {@code component} parameter may
	 * be any identifying string, but is typically the class name of the
	 * component.
	 * <p>
	 * Once {@code reportLocalHealth} has been called with a health that returns
	 * {@code false} from its {@code Health.isAlive} method, the health of the
	 * local node can not be changed.
	 * <p>
	 * 
	 * This method must be invoked outside of a transaction.
	 * 
	 ...
	 *            the health
	 * @param component
	 *            the name of the component reporting health
	 * 
	 * @throws IllegalStateException
	 *             if this method is invoked from a transactional context
	 */
	 ...
	 * proceed to shutting down the node. The node specified as the
	 ...
	 * <p>
	 * 
	 * Once {@code reportFailure} has been called, the health of the specified
	...
	/**
	 * Returns the current global application time in milliseconds. This method
	 * returns the amount of time in milliseconds that the current application
	 * has been running since the
	 * {@link AppListener#initialize(java.util.Properties) initialize} method
	 * was called on the application's {@code AppListener} object. Any time that
	 * passes while the application is not running does not affect this value.
	 * This means that this method can effectively be used to measure the global
	 * wall clock time of the current application's running state.
	 * <p>
	 ...
	 * skew. Additionally, the <em>global</em> accuracy of the clock time may
	 * drift as much as a few seconds in either direction due to inherent

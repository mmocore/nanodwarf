...
/**
 * This is a proxy that provides access to the current transaction and its
 * owner. Note that there is only ever one instance of
...
public interface TransactionProxy {
	 ...
	 *             if there is no current, active transaction, or if the current
	 *             transaction has already started preparing or aborting
	 * @throws TransactionTimeoutException
	 *             if the current transaction has timed out
	 */
	 ...
	 * available in the context of the current <code>Transaction</code>. If the
	 * type is unknown, or if there is more than one <code>Service</code> of the
	 * given type, <code>MissingResourceException</code> is thrown.
	 * 
	 * @param <T>
	 *            the type of the <code>Service</code>
	 * @param type

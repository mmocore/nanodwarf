...
/**
 * A channel that can read and write bytes asynchronously. Some channels may not
 * allow more than one read to be outstanding at any given time. If a thread
 * invokes a read method before a previous read operation has completed then a
 * {@link ReadPendingException} will be thrown. Similarly, if a write method is
 * invoked before a previous write has completed then
 * {@link WritePendingException} is thrown. Whether or not other kinds of I/O
...
public interface AsynchronousByteChannel extends Channel {
	 ...
	 * <p>
	 * This method initiates an operation to read a sequence of bytes from this
	 * channel into the given buffer. The method returns an {@link IoFuture}
	 * representing the pending result of the operation. The result of the
	 * operation, obtained by invoking the {@code IoFuture}'s
	 * {@link IoFuture#get() get} method, is the number of bytes read, possibly
	 * zero, or {@code -1} if all bytes have been read and the channel has
	 * reached end-of-stream.
	 * <p>
	 * This method initiates a read operation to read up to <i>r</i> bytes from
	 * the channel, where <i>r</i> is the number of bytes remaining in the
	 ...
	 * transferred into the buffer so that the first byte in the sequence is at
	 ...
	 * buffer's position will be equal to <code><i>p</i> + <i>n</i></code>; its
	 ...
	 *            {@link IoFuture} object; can be {@code null}
	 * @param handler
	 ...
	 * <p>
	 * This method initiates an operation to write a sequence of bytes to this
	 * channel from the given buffer. The method returns an {@link IoFuture}
	 * representing the pending result of the operation. The result of the
	 * operation, obtained by invoking the {@code IoFuture}'s
	 * {@link IoFuture#get() get} method, is the number of bytes written,
	 * possibly zero.
	 * <p>
	 ...
	 * buffer, that is, {@code src.remaining()} at the time that the write is
	 * attempted.
	 * <p>
	 * Suppose that a byte sequence of length <i>n</i> is written, where
	 * <code>0 &lt;= <i>n</i> &lt;= <i>r</i></code>. This byte sequence will be
	 * transferred from the buffer starting at index <i>p</i>, where <i>p</i> is
	 * the buffer's position at the moment the write is performed; the index of
	 * the last byte written will be <code><i>p</i> + <i>n</i> - 1</code>. Upon
	 * completion the buffer's position will be equal to

 ...
 * methods defined by this class. It is not possible to create a channel for an
 * arbitrary, pre-existing datagram socket. A newly-created asynchronous
 * datagram channel is open but not connected. It need not be connected in order
 * for the send and receive methods to be used. A datagram channel may be
 * connected, by invoking its connect method, in order to avoid the overhead of
 * the security checks that are otherwise performed as part of every send and
 * receive operation when a security manager is set. The channel must be
 * connected in order to use the read and write methods, since those methods do
 * not accept or return socket addresses. Once connected, an asynchronous
 * datagram channel remains connected until it is disconnected or closed.
 * <p>
 * Socket options are configured using the setOption method. Channels of this
 * type support the following options: <blockquote>
 * <table>
 * <tr>
 * <th>Option Name</th>
 * <th>Description</th>
...
public abstract class AsynchronousDatagramChannel extends AsynchronousChannel
	 ...
	 * openAsynchronousDatagramChannel method on the AsynchronousChannelProvider
	 * object that created the given group. If the group parameter is null then
	 * the resulting channel is created by the system-wide default provider, and
	 * bound to the default group.
	 * <p>
	 * The pf parameter is used to specify the ProtocolFamily. If the datagram
	 * channel is to be used for Internet Protocol multicasting then this
	 * parameter should correspond to the address type of the multicast groups
	 ...
	 * Opens an asynchronous datagram channel.
	 * <p>
	 * This method returns an asynchronous datagram channel that is bound to the
	...
	/**
	 * Returns the remote address to which this channel is connected, or null if
	 ...
	 * 
	 * @return true, if and only if, a read is pending for this channel but has
	 *         not yet completed.
	 ...
	 * for purposes of coordination.
	 * 
	 * @return true, if and only if, a write is pending for this channel but has
	 *         not yet completed.
	 ...
	 * <p>
	 * This method initiates an operation to configure the channel so that it
	 * only receives datagrams from, and sends datagrams to, the given remote
	 * <em>peer</em> address. Once connected, datagrams may not be received from
	 * or sent to any other address. The channel remains connected until it is
	 * {@link #disconnect(Object, CompletionHandler) disconnected} or until it
	 * is closed. This method returns an {@code IoFuture} representing the
	 * pending result of the operation. The {@link IoFuture#get() get} method
	 * returns {@code null} upon successful completion.
	 * <p>
	 * This method performs exactly the same security checks as the
	 * {@link DatagramSocket#connect(java.net.InetAddress, int) connect} method
	 * of the {@link DatagramSocket} class. That is, if a security manager has
	 * been installed then this method verifies that its
	 * {@link SecurityManager#checkAccept checkAccept} and
	 * {@link SecurityManager#checkConnect(String, int) checkConnect} methods
	 * permit datagrams to be received from and sent to, respectively, the given
	 * remote address.
	 * <p>
	 * This method may be invoked at any time and may thus affect read or write
	 * operations that are already in progress at the moment that the socket is
	 * connected.
	 * 
	 * @param <A>
	 *            the attachment type
	 ...
	 * @param attachment
	 *            the object to {@link IoFuture#attach attach} to the returned
	 *            {@code IoFuture} object; can be {@code null}
	 * @param handler
	 ...
	 * <p>
	 * This method initiates an operation to configure the channel so that it
	 * can receive datagrams from, and sends datagrams to, any remote address so
	 * long as the security manager, if installed, permits it. This method
	 * returns an {@code IoFuture} representing the pending result of the
	 * operation. The {@link IoFuture#get() get} method returns {@code null}
	 ...
	 *             if this channel is closed
	 */
	public final <A> IoFuture<Void, A> disconnect(
			CompletionHandler<Void, ? super A> handler) {
	 ...
	 * {@code IoFuture} representing the pending result of the operation. The
	 * {@code IoFuture}'s {@link IoFuture#get() get} method returns the source
	 * address of the datagram upon successful completion.
	 * <p>
	 * The datagram is transferred into the given byte buffer starting at its
	 * current position, as if by a regular
	 * {@link AsynchronousByteChannel#read(ByteBuffer, Object, CompletionHandler)
	 * read} operation. If there are fewer bytes remaining in the buffer than
	 * are required to hold the datagram then the remainder of the datagram is
	 * silently discarded.
	 * <p>
	 ...
	 * {@link ExecutionException} with cause {@link SecurityException}. The
	 * overhead of this security check can be avoided by first connecting the
	 * socket via the {@link #connect(SocketAddress, Object, CompletionHandler)
	 * connect} method.
	 * 
	 * @param <A>
	 *            the attachment type
	...
	public final <A> IoFuture<SocketAddress, A> receive(ByteBuffer dst,
			A attachment, CompletionHandler<SocketAddress, ? super A> handler) {
	 ...
	 * {@code IoFuture} representing the pending result of the operation. The
	 * operation sends the remaining bytes in the given buffer as a single
	 * datagram to the given target address. The result of the operation,
	 * obtained by invoking the {@code IoFuture}'s {@link IoFuture#get() get}
	 * method, is the number of bytes sent. The number of bytes sent is either
	 * the number of bytes that were remaining in the buffer or zero if there
	 * was insufficient room for the datagram in the underlying output buffer.
	 * <p>
	 ...
	 * {@link SecurityManager#checkConnect(String, int) checkConnect} method.
	 ...
	 * @throws SecurityException
	 *             if a security manager has been installed and it does not
	 *             permit datagrams to be sent to the given address
	 */
	 ...
	 */
	public final <A> IoFuture<Integer, A> send(ByteBuffer src,
			SocketAddress target, CompletionHandler<Integer, ? super A> handler) {
	 ...
	 * This method initiates the receiving of a datagram, returning an
	 * {@code IoFuture} representing the pending result of the operation. The
	 * {@code IoFuture}'s {@link IoFuture#get() get} method returns the the
	 * number of bytes transferred upon successful completion.
	 * <p>
	 * This method may only be invoked if this channel is connected, and it only
	 * accepts datagrams from the peer that the channel is connected too. The
	 * datagram is transferred into the given byte buffer starting at its
	 * current position and exactly as specified in the
	 ...
	 * <p>
	 * This method is equivalent to invoking
	 * {@link #read(ByteBuffer, long, TimeUnit, Object, CompletionHandler)} with
	 * a timeout of {@code 0L}.
	 * 
	 * @param <A>
	 *            the attachment type
	 * @param dst
	 ...
	 * This method is equivalent to invoking
	 * {@link #read(ByteBuffer, long, TimeUnit, Object, CompletionHandler)} with
	 * a timeout of {@code 0L}, and an attachment of {@code null}.
	 * 
	 * @param <A>
	 *            the attachment type
	 * @param dst
	 ...
	 * {@code IoFuture}'s {@link IoFuture#get() get} method, is the number of
	 * bytes sent. The number of bytes sent is either the number of bytes that
	 * were remaining in the buffer or zero if there was insufficient room for
	 ...
	 * write} operation.
	 * <p>
	 * This method may only be invoked if this channel is connected, in which
	 * case it sends datagrams directly to the socket's peer. Otherwise it
	 * behaves exactly as specified in the {@link AsynchronousByteChannel}
	 * interface.
	 * 
	 * @param <A>
	 *            the attachment type

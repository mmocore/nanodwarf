 ...
 * initiating the operation. The attachment method is used to retrieve it. The
 * {@link IoFuture#attachment attachment} may later be discarded by
...
public interface IoFuture<R, A> extends Future<R> {
	 ...
	 * <p>
	 * The method is intended to be invoked from a {@link CompletionHandler} to
	 * retrieve the result of a completed operation. It is equivalent to
	 ...
	 *             if the operation was {@link #cancel(boolean) cancelled}
	 ...
	 * <p>
	 * If the value of {@code mayInterruptIfRunning} is {@code true} then this
	 * method may cancel the operation <i>forcefully</i> by
	 * {@link AsynchronousChannel#close() closing} the channel. Whether it does
	 * close the channel is implementation and operation specific. If the
	 * channel is closed then all outstanding operations on the channel complete
	 * by throwing {@link ExecutionException} with cause
	 * {@link AsynchronousCloseException}. Where an implementation does not
	 * close the channel then the channel may be put into an error state that
	 * prevents further operations on the channel. For example, if a read
	 * operation is cancelled and the implementation cannot guarantee that no
	 * bytes have been read from the channel then it puts the channel into an
	 * implementation specific error state. Any subsequent attempt to initiate a
	 * read operation on the channel throws an unspecified runtime exception.
	 * <p>
	 * This method otherwise behaves exactly as specified by the {@link Future}
	 * interface.
	 * 
	 * @param mayInterruptIfRunning
	 *            {@code true} if the operation can be cancelled forcefully
	 *            (possibily by closing the channel), {@code false} otherwise

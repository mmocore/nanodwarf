 ...
 * method of this class. A newly-created asynchronous server-socket channel is
 * open but not yet bound. It can be bound to a local address and configured to
 * listen for connections by invoking the
 * {@link AsynchronousServerSocketChannel#bind(SocketAddress, int) bind} method.
 * Once bound, the
 * {@link AsynchronousServerSocketChannel#accept(Object, CompletionHandler)
 * accept} method is used to initiate the accepting of connections to the
 * channel's socket. An attempt to invoke the {@code accept} method on an
 * unbound channel will cause a {@link NotYetBoundException} to be thrown.
 * <p>
 ...
 * completed then an {@link AcceptPendingException} will be thrown. Whether or
 * not an operation is pending may be determined by invoking the
 * {@link AsynchronousServerSocketChannel#isAcceptPending isAcceptPending}
 * method.
 * <p>
 * Socket options are configured using the
 * {@link AsynchronousServerSocketChannel#setOption setOption} method. Channels
 * of this type support the following options: <blockquote>
 * <table>
 * <tr>
 * <th>Option Name</th>
 * <th>Description</th>
 ...
 * The list of options supported is obtained by invoking the
 ...
 * </pre>
 */
public abstract class AsynchronousServerSocketChannel extends
		AsynchronousChannel implements NetworkChannel {
	/**
	 ...
	 * {@link AsynchronousChannelProvider} object that created the given group.
	 * If the group parameter is {@code null} then the resulting channel is
	 * created by the system-wide default provider, and bound to the
	 * <em>default group</em>.
	 * 
	 * @param group
	 ...
	 * bound until the associated channel is closed. An attempt to bind a socket
	 * that is already bound throws AlreadyBoundException. If the local
	 * parameter has the value null then the socket address is assigned
	 * automatically.
	 * <p>
	 * The backlog parameter is the maximum number of pending connections on the
	 * socket. Its exact semantics are implementation specific. An
	 ...
	 * value 0, or a negative value, then an implementation specific default is
	 * used.
	 * 
	 * @param local
	 *            the local address to bind the socket, or {@code null} to bind
	 ...
	 * socket, returning an {@link IoFuture} representing the pending result of
	 * the operation. The {@code IoFuture}'s {@link IoFuture#get() get} method
	 * will return the {@link AsynchronousSocketChannel} for the new connection
	 ...
	 * method. The permission check is performed with privileges that are
	 * restricted by the calling context of this method. If the permission check
	 * fails then the operation completes by throwing {@link ExecutionException}
	 * with cause {@link SecurityException}.
	 * 
	 * @param <A>
	 *            the attachment type
	 * @param attachment
	 ...
	 * This method initiates accepting a connection made to this channel's
	 * socket, returning an {@link IoFuture} representing the pending result of
	 * the operation. Its {@link IoFuture#get() get} method will return the
	 * {@link AsynchronousSocketChannel} for the new connection on successful
	 * completion.
	 * <p>
	 * This method works as if invoking it were equivalent to evaluating the
	 * expression:
	 ...
	 * <p>
	 * The result of this method is a <em>snapshot</em> of the channel state. It
	 * may be invalid when the caller goes to examine the result and should not
	 * be used for the purpose of coordination.
	 * 
	 * @return {@code true} if, and only if, an accept is pending for this this

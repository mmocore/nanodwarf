 ...
 * {@link #provider} method. The first invocation of that method will locate the
 * default provider as specified below.
 * <p>
 * If the system-wide default provider implements {@link ManagedChannelFactory}
 * then the management interfaces for the pools of channels created by the
 * provider will be included in the list of {@link ChannelPoolMXBean} objects
 * returned by the {@link Channels#getChannelPoolMXBeans} method [[NOT
 * IMPLEMENTED: and also registered with the platform {@link MBeanServer} when
 * the {@code MBeanServer} is obtained by invoking the
 * {@link ManagementFactory#getPlatformMBeanServer} method]]. To ensure that the
 * {@link ObjectName} for uniquely identifying the {@code ChannelPoolMXBean}
 * objects is unique it is recommended that the <i>pool name</i> is
 * "asynchronous.<i>name</i>" where <i>name</i> identifies the provider's
 * channel pool.
 * <p>
...
public abstract class AsynchronousChannelProvider {
	 ...
	 *             {@code ("asynchronousChannelProvider")}
	 */
	...
	private static void loadProviderFromProperty() {
		...
		try {
			//FIXME OSGI: Class<?> c = Class.forName(cn, true, ClassLoader.getSystemClassLoader());
	 ...
	 * <li>
	 * [[NOT IMPLEMENTED: If a provider class has been installed in a jar file
	 * that is visible to the system class loader, and that jar file contains a
	 ...
	 *             if the provider that created the group differs from this
	 *             provider
	 * @throws ShutdownChannelGroupException
	 *             if the group is shutdown
	 * @throws IOException
	 *             if an I/O error occurs
	 */

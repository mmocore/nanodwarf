 ...
 * the group, or it may be <em>source-specific</em>, meaning that it represents
 * a membership that receives only datagrams from a specific source address.
 * Whether or not a membership key is source-specific may be determined by
 * invoking its {@link #getSourceAddress} method.
 * <p>
 * A membership key is valid upon creation and remains valid until the
 * membership is dropped by invoking the {@link #drop} method, or the channel is
 * closed. The validity of the membership key may be tested by invoking its
 * {@link #isValid} method.
 * <p>
 * Where a membership key is not source specific and the underlying operation
 * system supports source filtering, then the {@link #block} and
 * {@link #unblock} methods can be used to block or unblock multicast packets
...
public abstract class MembershipKey {
	 ...
	 * until the membership is dropped by invoking the {@link #drop} method, or
	 ...
	 * that source address.
	 * <p>
	 * After membership is dropped it may still be possible to receive datagams
	 * sent to the group. This can arise when datagrams are waiting to be
	 * received in the socket's receive buffer.
	 * <p>
	 * Upon return, this membership object will be {@link #isValid invalid}. If
	 * the multicast group membership is already invalid then invoking this
	 * method has no effect. Once a multicast group membership is invalid, it
	 ...
	 * address is already blocked then this method has no effect. After a source
	 * address is blocked it may still be possible to receive datagams from that
	 * source. This can arise when datagrams are waiting to be received in the
	 ...
	 *             i this membership key is source specific or is no longer
	 *             valid
	 ...
	 * 
	 * @param source
	 *            a list of source addresses to unblock
	 * @return this membership key
	 * @throws IllegalStateException
	 ...
	 * method will continue to return the channel even after the membership is
	 * dropped.
	 * 
	 * @return the channel
	 */
	 ...
	 * membership is dropped or the channel is closed.
	 * 
	 ...
	 * {@code null} if this membership is not source specific.
	 * 

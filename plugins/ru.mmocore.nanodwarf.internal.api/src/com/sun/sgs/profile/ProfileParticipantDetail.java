...
public interface ProfileParticipantDetail {
	 ...
	 * <code>wasPrepared</code> is <code>false</code>, then this method's return
	 * value has no meaning and will always return <code>false</code>.
	 * 
	...
	/**
	 * Returns whether this participant was called to prepare and commit in one
	 * step (via <code>TransactionParticipant.prepareAndCommit</code>). If this
	 * returns <code>true</code> then both <code>getPrepareTime</code> and
	 * <code>getCommitTime</code> will return the same value, as they represent
	 * the same work. If the return from <code>wasCommitted</code> is
	 * <code>false</code> then the call to <code>prepareAndCommit</code> failed.
	 * 
	 ...
	 * called to commit directly, then this is the time it took both to prepare
	 ...
	 * aborted and therefore this participant never committed, then this method
	 * will always return 0. If this participant was called to commit directly,
	 * then this is the time it took both to prepare to commit and to commit.
	 * 
	 * @return the time in milliseconds this participant spent committing, or
	 *         the time spent preparing and committing if called directly to
	 *         commit
	 */
	long getCommitTime();

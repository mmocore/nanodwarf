 ...
 * logging in and communicating with the server. A {@code SimpleClient} is used
 * to establish (or re-establish) a login session with the server, send messages
 * to the server, and log out.
 * <p>
 * A {@code SimpleClient} is constructed with a {@link SimpleClientListener}
 * which receives connection-related events as well as messages from the server
 * application.
 * <p>
 * If the server session associated with a simple client becomes disconnected,
 * then its {@link #send send} method will throw {@code IllegalStateException}.
 * A disconnected client can use the {@link #login login} method to log in
 * again.
 */
public class SimpleClient implements ServerSession {
	...
	/**
	 * The current {@code ClientConnection}, if connected, or {@code null} if
	 * disconnected.
	 */
	...
	/**
	 * Indicates that either a connection or disconnection attempt is in
	 * progress.
	 */
	...
	private final ConcurrentHashMap<BigInteger, SimpleClientChannel> channels = new ConcurrentHashMap<BigInteger, SimpleClientChannel>();

	/**
	 * Creates an instance of this class with the specified listener. Once this
	 * client is logged in (by using the {@link #login login} method), the
	 ...
	 * it may use the {@code login} method to log in again.
	 * 
	 * @param listener
	 ...
	 * 
	 * <p>
	 * First, this client attempts to establish a connection with the server. If
	 * the client fails to establish a connection, then the client listener's
	 * {@link SimpleClientListener#disconnected disconnected} method is invoked
	 * with a {@code String} indicating the reason for the failure.
	 * 
	 * <p>
	 * If a connection with the server is successfully established, this
	 * client's {@link PasswordAuthentication login credential} is obtained by
	 * invoking its {@link SimpleClientListener listener}'s
	 * {@link SimpleClientListener#getPasswordAuthentication
	 * getPasswordAuthentication} method with a login prompt.
	 * 
	 * <p>
	 * Next, this client sends a login request to the server. If the login
	 * request is malformed, the client listener's
	 * {@link SimpleClientListener#disconnected disconnected} method is invoked
	 * with a {@code String} indicating the reason for the failure or
	 * {@code null} if no reason can be determined.
	 * 
	 * <p>
	 * If the client's login credential (as obtained above) is verified, then
	 * the client listener's {@link SimpleClientListener#loggedIn loggedIn}
	 * method is invoked. If, however, the login fails due to a login
	 * authentication failure or some other failure on the server while
	 * processing the login request, the client listener's
	 * {@link SimpleClientListener#loginFailed loginFailed} method is invoked
	 * with a {@code String} indicating the reason for the failure.
	 * 
	 * <p>
	 * If this client is disconnected for any reason (including login failure),
	 * this method may be used again to log in.
	 * <p>
	 ...
	 * 
	 * @param props
	 *            the connection properties to use in creating the client's
	 *            session
	 * 
	 * @throws IOException
	 ...
	 *             if the caller does not have permission to connect to the
	 *             remote endpoint
	 */
		...
		if (force) {
			...
			try {
				ByteBuffer msg = ByteBuffer
						.wrap(new byte[] { SimpleSgsProtocol.LOGOUT_REQUEST });
	 ...
	 * 
	 * <p>
	 * Note: The server does not guarantee delivery of any session message (sent
	 * via this method) that is received by the server before the sending client
	 * is logged in. Therefore messages sent before this client is logged in,
	 * that is, before the {@link SimpleClientListener#loggedIn
	 * SimpleClientListener.loggedIn} method is invoked, may be dropped by the
	 * server.
	 */
	public void send(ByteBuffer message) throws IOException {
		synchronized (lock) {
			...
			msg.put(SimpleSgsProtocol.SESSION_MESSAGE).put(message).flip();
			sendRaw(msg);
		}
	}

	/**
	 * Sends raw data to the underlying connection without checking whether the
	 * client is logged in or suspended.
	 * 
	 * @param buf
	 *            the data to send
	 * @throws IOException
	...
	/**
	 * Marks this client as no longer suspended, and invokes the
	 * {@code reconnected} method on the associated {@code SimpleClientListener}
	 * .
	 */
	 ...
	 */
	final class SimpleClientConnectionListener implements
		...
		/**
		 * Indicates whether the disconnected callback should not be invoked.
		 */
		private volatile boolean loginFailed = false;

		/**
		 * Indicates whether this listener has been disabled because of an
		 * automatic login redirect to another host and port. We need to
		 * disconnect our previous connection, but we don't want to tell the
		 * client listener. We'll accept no messages when we're in this state.
		 */
		private volatile boolean redirectedOrRelocated = false;

		/**
		 * Store the login failure message sent by the server. This will be held
		 * until the disconnection from the server is confirmed by a call to
		 * disconnected, at which point the application's loginFailure callback
		...
		/**
		 * Constructs an instance with the specified password
		 * {@code authentication}. This is used in the redirect case, so the
		 * password authentication doesn't need to be reobtained from the user.
		 */
		...
		/**
		 * Constructs an instance with the specified {@code relocateKey}. This
		 * constructor is used when the client is relocating its connection to a
		 * new node.
		 * 
		 */
		...
		/**
		 * Sends a login request, obtaining a password authentication if one was
		 * not provided upon construction.
		 */
		private void sendLoginRequest() {
			...
			MessageBuffer msg = new MessageBuffer(2
					+ MessageBuffer.getSize(user) + MessageBuffer.getSize(pass));
			msg.putByte(SimpleSgsProtocol.LOGIN_REQUEST)
					.putByte(SimpleSgsProtocol.VERSION).putString(user)
					.putString(pass);
			try {
		...
		private void sendRelocateRequest() {
			...
			buf.put(SimpleSgsProtocol.RELOCATE_REQUEST)
					.put(SimpleSgsProtocol.VERSION).put(relocateKey).flip();
			try {
		...
		public void receivedMessage(byte[] message) {
			try {
				...
				if (logger.isLoggable(Level.FINER)) {
					String logMessage = String.format("Message length:%d",
							message.length);
		...
		private void handleApplicationMessage(MessageBuffer msg)
			...
			default:
				throw new IOException(String.format(
						"Unknown session opcode: 0x%02X", command));
			}
		}
		 ...
		 *             if an IO problem occurs
		 */
		private void handleLoginRedirect(MessageBuffer msg) throws IOException {
			String host = msg.getString();
			int port = msg.getInt();
			logger.log(Level.FINER, "Login redirect: {0}:{1}", host, port);

			redirectOrRelocateToNewNode(host, port,
		...
		private void handleSuspendMessages(MessageBuffer msg)
			...
			ack.put(SimpleSgsProtocol.SUSPEND_MESSAGES_COMPLETE).flip();
			sendRaw(ack);
		}
		...
		private void handleRelocateNotification(MessageBuffer msg)
			...
			logger.log(Level.FINER, "Relocate notification: {0}:{1}", host,
					port);
			checkLoggedIn();
			synchronized (lock) {
				if (!isSuspended) {
			...
			redirectOrRelocateToNewNode(host, port,
					new SimpleClientConnectionListener(relocateKey));
		}

		/**
		 * Disconnects the existing connection and creates a new connection with
		 * the specified connection {@code listener} to the node with the
		 * specified {@code host} and {@code port}.
		 * 
		 * @param host
		 ...
		 * @param listener
		 *            the connection listener for the new connection
		 * @throws IOException
		 *             if an I/O problem occurs
		 */
		private void redirectOrRelocateToNewNode(String host, int port,
				ClientConnectionListener listener) throws IOException {
		...
		private void handleChannelJoin(MessageBuffer msg) {
			...
			byte[] channelIdBytes = msg.getBytes(msg.limit() - msg.position());
			BigInteger channelId = new BigInteger(1, channelIdBytes);
			SimpleClientChannel channel = new SimpleClientChannel(channelName,
					channelId);
		...
		private void handleChannelLeave(MessageBuffer msg) {
			...
			if (channel != null) {
				...
				logger.log(Level.WARNING,
						"Cannot leave channel {0}: not a member", channelId);
			}
		}
		...
		private void handleChannelMessage(MessageBuffer msg) {
			...
			checkLoggedIn();
			BigInteger channelId = new BigInteger(1, msg.getBytes(msg
					.getShort()));
		...
		public void reconnected(byte[] message) {
			RuntimeException re = new UnsupportedOperationException(
	...
	final class SimpleClientChannel implements ClientChannel {
		...
		/**
		 * The listener for this channel if the client is a member, or null if
		 * the client is no longer a member of this channel.
		 */
		...
		public void send(ByteBuffer message) throws IOException {
			synchronized (lock) {
				...
				ByteBuffer msg = ByteBuffer.allocate(3 + idBytes.length
						+ message.remaining());
		...
		void joined() {
			if (!isJoined.compareAndSet(false, true)) {
				throw new IllegalStateException("Already joined to channel "
						+ channelName);
			}

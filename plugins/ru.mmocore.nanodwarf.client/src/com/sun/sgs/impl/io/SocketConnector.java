...
/**
 * This is a socket-based implementation of an {@code Connector} using the
 * Apache MINA framework for the underlying transport. It uses an
 * {@link org.apache.mina.common.IoConnector} to initiate connections on remote
 * hosts.
 * <p>
...
class SocketConnector implements Connector<SocketAddress> {
	...
	private static final LoggerWrapper logger = new LoggerWrapper(
			Logger.getLogger(SocketConnector.class.getName()));
	 ...
	 * @param connector
	 *            the {@link IoConnector MINA IoConnector} to use for
	 ...
	 * the connection that it connects.
	 */
	 ...
	 * {@inheritDoc}
	 */
	public boolean waitForConnect(long timeout) throws IOException,
	...
	static final class ConnectorConnListner extends SocketConnectionListener {
		 ...
		 * 
		 * @param listener
		 *            the ConnectionListener for the completed connection
		 */
		...
		/**
		 * If a connection is in progress, but not yet connected, cancel the
		 * pending connection.
		 * 
		...
		public void sessionCreated(IoSession session) throws Exception {
			synchronized (this) {
				if (cancelled) {
					logger.log(Level.FINE,
							"cancelled; ignore created session {0}", session);
					session.close();
					return;
				}
				connected = true;
			...
			SocketConnection connection = new SocketConnection(listener,
					filter, session);

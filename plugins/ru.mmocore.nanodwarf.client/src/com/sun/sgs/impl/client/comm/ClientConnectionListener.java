...
public interface ClientConnectionListener {
	 ...
	 * 
	 * @param message
	 *            protocol-specific session-start data
	 * 
	...
	/**
	 * Notifies this listener that its associated server connection is in the
	 * process of reconnecting with the server.
	 * <p>
	 * If a connection can be re-established with the server in a timely manner,
	 * this listener's {@link #reconnected reconnected} method will be invoked.
	 * Otherwise, if a connection cannot be re-established, this listener's
	 * {@link #disconnected disconnected} method will be invoked with
	 * {@code false} indicating that the associated session has been
	 * disconnected from the server and the client must log in again.
	 * 
	 * @param message
	 *            protocol-specific reconnection data
	 */
	 ...
	 * was due to other circumstances, such as forced disconnection or network
	 * failure.
	 * 
	 * @param graceful
	 *            {@code true} if disconnection was part of graceful logout,

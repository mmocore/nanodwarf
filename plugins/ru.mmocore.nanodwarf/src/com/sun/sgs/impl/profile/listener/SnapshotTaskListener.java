 ...
 * intervals. It provides a very simple view of what tasks have run over the
 * last interval. By default the time interval is 5 seconds.
 * <p>
 ...
 * 43010.
 * <p>
 * The <code>com.sun.sgs.impl.profile.listener.SnapshotTaskListener.</code> root
 * is used for all properties in this class. The <code>report.port</code> key is
 * used to specify an alternate port on which to report profiling data. The
 * <code>report.period</code> key is used to specify the length of time, in
...
public class SnapshotTaskListener implements ProfileListener {
	...
	private static final String PROP_BASE = SnapshotTaskListener.class
			.getName();
	 ...
	 * @throws IOException
	 *             if the socket where data will be published cannot be created
	 */
	 ...
	 * 
	 * @param profileReport
	 *            the summary for the finished {@code Task}
	 */
		...
		public void run() throws Exception {
			...
			synchronized (map) {
				for (Entry<String, TaskDetail> entry : map.entrySet()) {
					reportStr
							.format("%s%s%n", entry.getKey(), entry.getValue());
				}
				map.clear();
			}
			reportStr.format("%n");

...
/**
 * A text-output listener that generates task-periodic summary reports about the
 * performance of tasks being executed by a server. Each report summaries a
 * fixed number of tasks. The number of tasks summarized can be modified by
 * setting the {@code com.sun.sgs.profile.listener.window.size} property in the
 * system properties. The default number of tasks is {@code 5000}.
 * 
 * <p>
 * 
...
public class ProfileSummaryListener implements ProfileListener {
	...
	/**
	 * The window of tasks that are aggregated before the next text output when
	 * none is provided
	 */
	private static final int DEFAULT_WINDOW_SIZE = 5000;

	/**
	 * The property for specifying the number of windows to combine for printing
	 * averages.
	 */
	...
	/**
	 * How many tasks are aggregated between status updates. Note that the
	 * update might not occur exactly on window crossing due to concurrent
	 * updates.
	 */
	...
	public ProfileSummaryListener(Properties properties, Identity owner,
		...
		average = wrappedProps
				.getIntProperty(AVERAGE_PROPERTY, DEFAULT_AVERAGE);
	...
	public void report(ProfileReport profileReport) {
		...
		if (profileReport.wasTaskSuccessful()) {
			...
			lagTimeSum += (profileReport.getActualStartTime() - profileReport
		...
		if (taskCount % windowSize == 0) {
			...
			System.out
					.printf("past %d tasks:%n" + "  mean runtime: %4.2fms,"
							+ "  max: %6dms," + "  failed: %d (%2.2f%%)"
							...
							meanReadyCount,
							meanLagTime,
							((double) runTime / (double) (windowEndTime - lastWindowStart)),
							meanThroughput, meanLatency);
			System.out.printf("past %d tasks:%n" + "  mean runtime: %4.2fms,"
					+ "  failed: %2.2f%%" + "%n  mean ready count: %.2f,"
					+ "  mean lag time: %.2fms"
					+ "%n  mean throughput: %.2f txn/sec,"
					+ "  mean latency: %.2f ms/txn%n", maRunTime.count()
					* taskCount, maRunTime.average(), maFailed.average(),
					maReadyCount.average(), maLagTime.average(),
					maThroughput.average(), maLatency.average());

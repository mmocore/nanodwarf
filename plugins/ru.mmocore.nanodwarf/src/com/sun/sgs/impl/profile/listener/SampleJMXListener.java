...
/**
 * This implementation of {@code ProfileListener} shows how to use JMX MBeans
 * from within a listener.
 * <p>
 * Every 5 seconds, this listener reports the total number of successful tasks,
 * the average lag time for successful tasks, and the number of successful tasks
 * which were owned by the kernel.
 * <p>
...
public class SampleJMXListener implements ProfileListener {
	...
	public SampleJMXListener(Properties properties, Identity owner,
		...
		ProfileCollector collector = registry
				.getComponent(ProfileCollector.class);
		taskBean = (TaskAggregateMXBean) collector
				.getRegisteredMBean(TaskAggregateMXBean.MXBEAN_NAME);
		if (taskBean == null) {
			// The bean hasn't been registerd yet. This is unexpected,
			// so throw an exception to indicate we're in a bad state.
			throw new IllegalStateException(
					"Could not find task aggregate mbean");
		}
	...
	/**
	 * Update statistics about particular tasks. In this case, we are reporting
	 * the number of successful tasks of a particular type.
	 * 
	 * @param profileReport
	...
	private class TaskRunnable implements KernelRunnable {
		...
		public void run() throws Exception {
			System.out
					.println("Time:  " + new Date(System.currentTimeMillis()));
			System.out.printf("  Total tasks run: %d%n"
					+ "  Successful tasks: %d%n"
					+ "  Successful non-kernel tasks: %d%n"
					+ "  Average lag time: %.2fms%n", taskBean.getTaskCount(),
					(taskBean.getTaskCount() - taskBean.getTaskFailureCount()),
					numSuccessfulNonKernelTasks,

 ...
 * over a set number of operations, reports the aggregated data when that number
 * of operations have been reported, and then clears its state and starts
 * aggregating again. By default the interval is 100000 tasks.
 * <p>
 * This listener logs its findings at level <code>FINE</code> to the logger
 * named <code>
			...
			if (logger.isLoggable(Level.FINE)) {
				...
				if (!localCounters.isEmpty()) {
					...
					for (Entry<String, Long> entry : localCounters.entrySet()) {
						counterTally.format("  %s: %d%n", entry.getKey(),
								entry.getValue());
					}
				}
				...
				logger.log(
						Level.FINE,
						"Operations [logOps=" + logOps + "]:\n"
								+ "  succeeded: " + commitCount + "  failed: "
								+ abortCount + "\n" + "  elapsed time: "
								+ (now - lastReport) + " ms\n"
								+ "  running time: " + totalRunningTime
								+ " ms " + "[threads=" + threadCount + "]\n"

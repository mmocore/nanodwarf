...
/**
 * An implementation of {@code ProfileListener} that prints access detail. For
 * any transaction that fails due to conflict this will display the accesses for
 * that transaction and the accesses, if known, for the transaction that caused
 * the conflict.
 * <p>
 * Note that in the current default implementation, conflict detail will only be
 * provided if the {@code AccessCoordinator} is using a backlog to track
 * finished transactions. See {@code TrackingAccessCoordinator} for more detail.
 * <p>
 * By default, the set of accesses displayed for a given transaction will be
 * limited to the first 20. This can be changed by setting the
...
public class AccessedObjectsListener implements ProfileListener {
	...
	public static final String ACCESS_COUNT_PROPERTY = AccessedObjectsListener.class
			.getName() + ".access.count";
	 ...
	 * 
	 * @param properties
	 *            the {@code Properties} for this listener
	 * @param owner
	 *            the {@code Identity} to use for all tasks run by this listener
	 * @param registry
	 *            the {@code ComponentRegistry} containing the available system
	 *            components
	 * 
	 * @throws IllegalArgumentException
	 *             if either of the backlog or count properties is provided but
	 *             invalid
	 */
	public AccessedObjectsListener(Properties properties, Identity owner,
		...
		String backlogProp = properties.getProperty("com.sun.sgs.impl.kernel."
				+ "TrackingAccessCoordinator.queue.size");
		if (backlogProp != null) {
			try {
				backlogMap = new BoundedLinkedHashMap<TransactionId, AccessedObjectsDetail>(
	...
	public void report(ProfileReport profileReport) {
		...
		if (detail.getConflictType() != ConflictType.NONE) {
			...
			System.out.printf("Task type %s failed due to conflict.  Details:"
					...
					+ "conflict type: %s%n", profileReport.getTask()
					.getBaseTaskType(), txnId, profileReport.getRetryCount(),
					formatAccesses(detail.getAccessedObjects()), detail
							.getConflictType());
			...
			if (backlogMap != null) {
				...
				AccessedObjectsDetail conflictingDetail = backlogMap
						.get(conflictingId);
				...
				if (conflictingDetail != null) {
					System.out.printf("Conflicting transaction id: %s, objects"
							+ " accessed, ordered by first access:" + "%n%s%n",
							conflictingId, formatAccesses(conflictingDetail
									.getAccessedObjects()));
	 ...
	 *            a {@code List} of {@code AccessedObject}
	 * 
	 * @return a formatted representation of the accessed objects
	 */
	private String formatAccesses(List<? extends AccessedObject> accessedObjects) {
		...
		for (AccessedObject object : accessedObjects) {
			...
			try {
				formatted.append(String.format("[source: %s] %-5s %s, "
						+ "description: %s%n", object.getSource(),
						object.getAccessType(), object.getObjectId(),
				...
				formatted.append(String.format("[source %s] %-5s %s [%s."
						+ "toString() threw: %s]%n", object.getSource(), object
						.getAccessType(), object.getObjectId(), object
						.getDescription().getClass(), t));
			}
		}
		...
		if (--count == accessesToShow) {
			formatted.append(String.format("[%d further accesses truncated]%n",
	...
	/**
	 * A private implementation of {@code LinkedHashMap} that is bounded in
	 * size.
	 */

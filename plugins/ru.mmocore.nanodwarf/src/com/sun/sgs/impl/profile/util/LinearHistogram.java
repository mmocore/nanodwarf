...
/**
 * A histogram that bins elements based on provided bounds and a step size. This
 * class is useful for viewing a particular range of data and specifying how the
 * distribution is grouped. The lower and upper bounds should be used to filter
 * out all extraneous data points.
 */
public class LinearHistogram implements Histogram {
	...
	/**
	 * The maximum number of entries current in one bin of the histogram.
	 */
	...
	/**
	 * Creates a linear frequency histogram with the provided lower bound, upper
	 * bound and step size.
	 * 
	 * @param lBound
	 *            the lower bound for the histogram; values strictly less than
	 *            this will not be included
	 * @param uBound
	 *            the upper bound for the histogram; values strictly greater
	 *            than this will not be included
	 * @param stepSize
	 *            the maximum range for each bin between {@code lBound} and
	 *            {@code uBound}; if {@code stepSize} does not evenly divide the
	 *            difference, the last bin will be truncated in size
	 * 
	 * @throws IllegalArgumentException
	 *             if {@code lBound} is not less than {@code uBound}, or if
	 *             {@code stepSize} is not strictly positive
	 */
	 ...
	 * {@inheritDoc}
	 * 
	 * Values are binned in the largest bin that is <i>less than</i> the element
	 * is incremented. Values less than the lower bound or values that are
	 * greater than the upper bound are not counted.
	 */
	...
	/**
	 * Generates a text representation of this linear frequency histogram. The
	 * histogram is presented similar to the following example:
	 * 
	 * <pre>
	 *    [lower-bound] |***
	 *    [l.b. + step] |
	 ...
	 * </pre>
	 * 
	 * Bins that have at least one sample will have a bar displayed. All leading
	 * and trailing empty bins are truncated, which may result in the lower and
	 * upper bound bins not being displayed. The final line will have a newline
	 * at the end.
	 * 
	 ...
	 * similar to {@link #toString()} but with labels on bins. For example, a
	 * bin would appear as:
	 * 
	 * <pre>
	 * 200ms |***
	 ...
	 * 
	 * @param binLabel
	 *            the label to append to each of the bins
	 * 
	 * @return a string representation of this histogram
	 */
	public String toString(String binLabel) {
		...
		int maxLength = Long.toString(lBound + (maxIndex * stepSize)).length();

		StringBuilder b = new StringBuilder(128);

		for (int i = minIndex; i <= maxIndex; ++i) {

			String binName = Long.toString(Math.min(lBound + (i * stepSize),
					uBound));

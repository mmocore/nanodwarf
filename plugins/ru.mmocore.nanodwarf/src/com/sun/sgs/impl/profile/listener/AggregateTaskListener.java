...
/**
 * This implementation of <code>ProfileListener</code> aggregates task data over
 * the lifetime of the system, and reports at fixed intervals. By default the
...
public class AggregateTaskListener implements ProfileListener {
	...
	private static final String PROP_BASE = AggregateTaskListener.class
			.getName();
	...
	private static class TaskDetail {
		...
		public String toString() {
			...
			formatter.format(" avgRetries=%2.2f", (double) retries
					/ (double) count);
			if (opCount > 0) {
				formatter.format("%n  ");
			}
	...
	/**
	 * A runnable for periodically reporting task summaries to the network.
	 */

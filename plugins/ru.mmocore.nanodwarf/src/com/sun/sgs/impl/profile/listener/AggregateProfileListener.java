...
/**
 * This implementation of <code>ProfileListener</code> aggregates profiling data
 * over the lifetime of the system, and reports at fixed intervals. By default
...
public class AggregateProfileListener implements ProfileListener {
	...
	private Map<String, Long> sOpCounts = new HashMap<String, Long>();
	private Map<String, Long> fOpCounts = new HashMap<String, Long>();
	...
	private static final String PROP_BASE = AggregateProfileListener.class
			.getName();
	...
	public AggregateProfileListener(Properties properties, Identity owner,
		...
		handle = registry
				...
				.scheduleRecurringTask(new AggregatingRunnable(), owner,
						System.currentTimeMillis() + reportPeriod, reportPeriod);
		handle.start();
	}
	...
	public void report(ProfileReport profileReport) {
		...
		if (profileReport.wasTaskSuccessful()) {
			...
			delayTime += (profileReport.getActualStartTime() - profileReport
	...
	private class AggregatingRunnable implements KernelRunnable {
		...
		public void run() throws Exception {
			...
			double transactionalAvgLength = (double) tRunTime
					/ (double) tTaskCount;

			// averages just for successes
			double avgSuccessfulLength = totalTime / (double) sTaskCount;
			double avgSuccessfulDelay = (double) delayTime
					/ (double) sTaskCount;
			double avgSuccessfulOps = (double) sTaskOpCount
					/ (double) sTaskCount;

			// average for failures
			double avgRetries = (double) tryCount / (double) sTaskCount;
			double avgFailedOps = (fTaskCount == 0) ? 0 : (double) fTaskOpCount
					/ (double) fTaskCount;
			...
			if (!localCounters.isEmpty()) {
				...
				for (Entry<String, Long> entry : localCounters.entrySet()) {
					reportStr.format("  %s=%2.2f%n", entry.getKey(),

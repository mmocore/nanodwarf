...
class ProfileReportImpl implements ProfileReport {
	...
	/**
	 * An empty map for returning when no profile counters have been updated.
	 */
	private static final Map<String, Long> EMPTY_COUNTER_MAP = Collections
			.emptyMap();
	...
	/**
	 * An empty map for returning when no profile samples have been updated. We
	 * need this map as well because typing issues prevent us from using
	 * {@link Collections#emptyMap()}.
	 */
	private static final Map<String, List<Long>> EMPTY_SAMPLE_MAP = Collections
			.unmodifiableMap(new HashMap<String, List<Long>>());
	...
	/**
	 * Creates an instance of <code>ProfileReportImpl</code> with the actual
	 * starting time being set to the current time.
	 * 
	 * @param task
	 *            the <code>KernelRunnable</code> being reported on
	 * @param owner
	 *            the <code>Identity</code> that owns the task
	 * @param scheduledStartTime
	 *            the time the task was scheduled to run
	 * @param readyCount
	 *            the number of tasks in the scheduler, ready to run, that are
	 *            associated with the same context as the task
	 */
	...
	/**
	 * Package-private method used to increment task-local counters that were
	 * changed during this task. If this counter hasn't had a value reported yet
	 * for this task, then the provided value is set as the current value for
	 * the counter.
	 * 
	 * @param counter
	...
	/**
	 * Package-private method used to add to a task-local sample. If this sample
	 * hasn't had a value reported yet for this task, then a new list is made
	 * and the the provided value is added to it.
	 * 
	 * @param sampleName
	 *            the name of the sample
	 * @param value
	 *            the latest value for the sample
	 */
	 ...
	 * another. This is typically used when a nested, profiled task completes
	 * and needs to share its data with its parent.
	 */
	void merge(ProfileReportImpl report) {
		...
		if (report.taskCounters != null) {
			if (taskCounters == null) {
				...
				for (Map.Entry<String, Long> e : report.taskCounters.entrySet()) {
					Long curCount = taskCounters.get(e.getKey());
					taskCounters.put(
							e.getKey(),
							(curCount == null) ? e.getValue() : curCount
									+ e.getValue());
				}
			}
		}
		...
		if (report.taskSamples != null) {
			if (taskSamples == null) {
				taskSamples = new HashMap<String, List<Long>>();
				for (Map.Entry<String, List<Long>> e : report.taskSamples
						.entrySet()) {
	...
	/**
	 * Package-private method to note that a task has finished running. All
	 * collections are modified to be immutable.
	 */
	void finish() {
		if (ops != null) {

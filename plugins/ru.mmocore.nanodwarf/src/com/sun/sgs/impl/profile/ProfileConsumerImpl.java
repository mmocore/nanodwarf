...
/**
 * This simple implementation of <code>ProfileConsumer</code> reports all data
 * to a backing <code>ProfileCollectorImpl</code>.
 */
class ProfileConsumerImpl implements ProfileConsumer {
	...
	/**
	 * The default aggregate sample smoothing capacity, for calculating the
	 * exponential weighted average of the samples.
	 */
	 ...
	 * 
	 * @param profileCollector
	 *            the backing <code>ProfileCollectorImpl</code>
	 * @param name
	...
	public synchronized ProfileOperation createOperation(String name,
		...
		if (op == null) {
			...
			case TASK:
				...
				break;
			case AGGREGATE:
				op = new AggregateProfileOperationImpl(fullName, type, minLevel);
				break;
			...
			if (op instanceof AbstractProfileData) {
				...
				if (oldLevel != minLevel) {
					throw new IllegalArgumentException("Operation with name "
							+ name + " already created, but with level "
							+ oldLevel);
				}
				...
				if (oldType != type) {
					throw new IllegalArgumentException("Operation with name "
							+ name + " already created, but with type "
							+ oldType);
				}
			} else {
				// Can't happen in this implementation
				throw new IllegalArgumentException("Operation with name "
						+ name + " already created with an unknown type");
			}
		}
	...
	public synchronized ProfileCounter createCounter(String name,
		...
		if (counters.containsKey(fullName)) {
			...
			if (oldCounter instanceof AbstractProfileData) {
				...
				ProfileLevel oldLevel = oldData.getMinLevel();
				if (oldLevel != minLevel) {
					throw new IllegalArgumentException("Counter with name "
							+ name + " already created, but with level "
							+ oldLevel);
				}
				...
				// Can't happen in this implementation
				throw new IllegalArgumentException("Counter with name " + name
			...
			switch (type) {
			case TASK:
				counter = new TaskProfileCounterImpl(fullName, type, minLevel);
				break;
			case AGGREGATE:
			...
			case TASK_AND_AGGREGATE:
			default:
				counter = new TaskAggregateProfileCounterImpl(fullName, type,
						minLevel);
				break;
			}
	 ...
	 * {@value #DEFAULT_SAMPLE_AGGREGATE_CAPACITY} and can be modified by
	 * calling {@link AggregateProfileSample#setCapacity}.
	 * <p>
	 * These samples use an exponential weighted average to calculate their
	 * averages. The default smoothing factor for the aggregate sample is
	 * {@value #DEFAULT_SAMPLE_AGGREGATE_SMOOTHING} and can be modified by
	 * calling {@link AggregateProfileSample#setSmoothingFactor}.
	 */
	public synchronized ProfileSample createSample(String name,
		...
		if (samples.containsKey(fullName)) {
			...
			if (oldSample instanceof AbstractProfileData) {
				...
				ProfileLevel oldLevel = oldData.getMinLevel();
				if (oldLevel != minLevel) {
					throw new IllegalArgumentException("Sample with name "
							+ name + " already created, but with level "
							+ oldLevel);
				}
				...
				// Can't happen in this implementation
				throw new IllegalArgumentException("Sample with name " + name
						+ " already created with an unknown type");
			}
			...
			switch (type) {
			case TASK:
				sample = new TaskProfileSampleImpl(fullName, type, minLevel);
				break;
			case AGGREGATE:
			...
			case TASK_AND_AGGREGATE:
			default:
				sample = new TaskAggregateProfileSampleImpl(fullName, type,
						minLevel);
				break;
			}
	...
	/**
	 * Abstract base class for all profile data implementations, to hold common
	 * information.
	 */
	 ...
	 */
	private class AggregateProfileOperationImpl extends AbstractProfileData
	 ...
	 */
	private class TaskProfileOperationImpl extends AbstractProfileData
		...
		public void report() {
			...
			try {
				ProfileReportImpl profileReport = profileCollector
	 ...
	 */
	private class TaskAggregateProfileOperationImpl extends
			AggregateProfileOperationImpl implements TaskProfileOperation {
		private final TaskProfileOperationImpl taskOperation;

		TaskAggregateProfileOperationImpl(String opName, ProfileDataType type,
				ProfileLevel minLevel) {
			super(opName, type, minLevel);
			taskOperation = new TaskProfileOperationImpl(opName, type, minLevel);
		}
	 ...
	 */
	private class AggregateProfileCounterImpl extends AbstractProfileData
	 ...
	 */
	private class TaskProfileCounterImpl extends AbstractProfileData implements
			TaskProfileCounter {
	...
	/**
	 * Profile counter which both aggregates globally and reports task-local
	 * data into profile reports.
	 */
	private class TaskAggregateProfileCounterImpl extends
			AggregateProfileCounterImpl implements TaskProfileCounter {
	 ...
	 */
	private class AggregateProfileSampleImpl extends AbstractProfileData
		...
		private volatile double smoothingFactor = DEFAULT_SAMPLE_AGGREGATE_SMOOTHING;
		private long minSampleValue = Long.MAX_VALUE;
		private long maxSampleValue = Long.MIN_VALUE;
		private final ExponentialAverage avgSampleValue = new ExponentialAverage();
	 ...
	 */
	private class TaskProfileSampleImpl extends AbstractProfileData implements
			TaskProfileSample {
	 ...
	 */
	private class TaskAggregateProfileSampleImpl extends
			AggregateProfileSampleImpl implements TaskProfileSample {

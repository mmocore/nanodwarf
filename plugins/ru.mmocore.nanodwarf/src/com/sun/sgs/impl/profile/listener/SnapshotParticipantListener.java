 ...
 * intervals. It provides a very simple view of transactional participants that
 * were active over the last interval. By default the time interval is 5
 * seconds.
 * <p>
...
public class SnapshotParticipantListener implements ProfileListener {
	...
	private static final String PROP_BASE = SnapshotParticipantListener.class
			.getName();
	...
	public void report(ProfileReport profileReport) {
		if (profileReport.wasTaskTransactional()) {
			synchronized (participantMap) {
				taskCount++;
				for (ProfileParticipantDetail detail : profileReport
						.getParticipantDetails()) {
					ParticipantCounts counts = participantMap.get(detail
							.getParticipantName());
					if (counts == null) {
						counts = new ParticipantCounts();
						participantMap.put(detail.getParticipantName(), counts);
					}
		...
		public void run() throws Exception {
			...
			synchronized (participantMap) {
				reportStr.format(
						...
						taskCount);
				for (Entry<String, ParticipantCounts> entry : participantMap
						.entrySet()) {
					reportStr

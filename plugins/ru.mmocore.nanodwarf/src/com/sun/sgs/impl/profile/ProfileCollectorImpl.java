...
/**
 * This is the implementation of {@code ProfileCollector} used by the kernel to
 * collect and report profiling data. It uses a single thread to consume and
 * report profiling data to {@code ProfileListeners}.
 * <p>
 ...
 * Specifies whether a new {@code MBeanServer} should be created, rather than
 * using the existing platform {@code MBeanServer}.
 * <p>
 * 
 * </dl>
 * <p>
...
public final class ProfileCollectorImpl implements ProfileCollector {
	...
	private ThreadLocal<Stack<ProfileReportImpl>> profileReports = new ThreadLocal<Stack<ProfileReportImpl>>() {
	 ...
	 * 
	 * @param level
	 *            the default system profiling level
	 * @param appProperties
	 *            the application properties, used for instantiating
	 *            {@code ProfileListener}s
	 * @param systemRegistry
	 *            the system registry, used for instantiating
	 *            {@code ProfileListener}s
	 */
	public ProfileCollectorImpl(ProfileLevel level, Properties appProperties,
		...
		boolean createServer = wrappedProps.getBooleanProperty(
				CREATE_MBEAN_SERVER_PROPERTY, false);
		if (createServer) {
		...
		taskStats = new TaskAggregateStats(this, CORE_CONSUMER_PREFIX
				+ "TaskAggregateStats");
		try {
	...
	public ProfileConsumer getConsumer(String name) {
		...
		if (oldpc != null) {
			logger.log(Level.FINE, "Found consumer {0} already created", name);
			return oldpc;
		} else {
	...
	public Map<String, ProfileConsumer> getConsumers() {
		...
		ConcurrentHashMap<String, ProfileConsumer> retMap = new ConcurrentHashMap<String, ProfileConsumer>(
				consumers);
	...
	public void addListener(String listenerClassName) throws Exception {
		...
		Constructor<?> listenerConstructor = listenerClass.getConstructor(
				Properties.class, Identity.class, ComponentRegistry.class);
		...
		Object obj = listenerConstructor.newInstance(appProperties, owner,
				systemRegistry);
	...
	public List<ProfileListener> getListeners() {
		...
		ArrayList<ProfileListener> list = new ArrayList<ProfileListener>(
				listeners.keySet());
	...
	public void registerMBean(Object mBean, String mBeanName)
		...
		try {
			...
			logger.logThrow(Level.CONFIG, ex, "Could not register MBean {0}",
					mBeanName);
			throw ex;
		}
	}
	...
	/**
	 * Tells the collector that a new task is starting in the context of the
	 * calling thread.
	 * 
	 * @param task
	 *            the <code>KernelRunnable</code> that is starting
	 * @param owner
	 *            the <code>Identity</code> of the task owner
	 * @param scheduledStartTime
	 *            the requested starting time for the task
	 * @param readyCount
	 *            the number of ready tasks at the scheduler
	 */
	void startTask(KernelRunnable task, Identity owner,
		...
		profileReports.get().push(
				new ProfileReportImpl(task, owner, scheduledStartTime,
						readyCount));
	}

	/**
	 * Tells the collector that the current task associated with the calling
	 * thread (as associated by a call to {@code startTask}) is transactional.
	 * 
	 * @param txnId
	 ...
	 * participant has finished participating (i.e., has committed, has prepared
	 * read-only, or has aborted).
	 * 
	 * @param participantDetail
	 *            the detail associated with the participant
	 */
	...
	/**
	 * Tells the collector about a listener of a transaction when that listener
	 * is finished with its work (i.e., after its afterCompletion method has
	 * been called).
	 * 
	 * @param listenerDetail
	 *            the detail associated with the listener
	 */
	...
	/**
	 * Sets the detail for all objects accessed during the task as reported to
	 * the {@code AccessCoordinator}.
	 * 
	 * @param detail
	 *            all detail of the accessed objects
	 */
	...
	/**
	 * Tells the collector that the current task associated with the calling
	 * thread (as associated by a call to {@code startTask}) has now
	 * successfully finished.
	 * 
	 * @param tryCount
	 *            the number of times that the task has tried to run
	 */
	...
	/**
	 * Tells the collector that the current task associated with the calling
	 * thread (as associated by a call to {@code startTask}) has now finished.
	 * 
	 * @param tryCount
	 *            the number of times that the task has tried to run
	 * @param t
	 *            the {@code Throwable} thrown during task execution
	 */
	 ...
	 * 
	 * @param event
	 *            the change the listeners will be notified about
	 */
	...
	/**
	 * Package private method to get the current profile report, used by the
	 * operations, counters, and samples.
	 * 
	...
	/**
	 * Private class that implements the long-running collector and reporter of
	 * task data. The task blocks on the queue, and whenever there is a report
	 * ready notifies all installed listeners.
	 * <p>
	 * NOTE: The commented-out code here may still be useful to make sure that
	 * the single-threaded queue keeps up. It was originally used just to to
	 * observe performance, and it's unclear whether it's worth reporting
	 * anywhere, and where to report it.
	 */
	private class CollectorThread extends Thread {
		/*
		 * private volatile long queueSize = 0; private volatile long
		 * queueSamples = 0;
		 */
		...
		public void run() {
			try {
				while (true) {
					 ...
					 * else { queueSize += queue.size(); queueSamples++; }
					 * 
					 * double avgQueueSize = (queueSamples == 0) ? 0 :
					 * (double)queueSize / (double)queueSamples; double
					 * percentQueueNotEmpty = (double)queueSamples /
					 * (double)totalTasks; reportStr += " [  AvgStatQueueSize="
					 * + avgQueueSize + "  StatQueueNonEmpty=" +
					 * percentQueueNotEmpty + "%  ]\n\n";
					 */
		 ...
		 * interrupted. That way we can have the thread exit if an interrupt has
		 * occurred, even if something (say logging) catches an interrupt and
		 * forgets to reset the interrupt status.
		 */
		@Override

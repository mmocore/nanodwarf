 ...
 * system. Only bounded transactions are aggregated. Successful transactions
 * that take longer than the standard transaction timeout are typically used for
 * application startup tasks, and are not included.
 */
public class TaskAggregateStats extends NotificationBroadcasterSupport
	...
	/**
	 * Smoothing factor for exponential smoothing, between 0 and 1. A value
	 * closer to one provides less smoothing of the data, and more weight to
	 * recent data; a value closer to zero provides more smoothing but is less
	...
	/**
	 * The last time {@link #clear} was called, or when this object was created
	 * if {@code clear} has not been called.
	 */
	private volatile long lastClear = System.currentTimeMillis();

	/**
	 * The standard transaction timeout, used to determine if a successful task
	 * was unbounded or not. We don't count the unbounded tasks in our
	 * statistics.
	 */
	...
	/**
	 * A latch for the first task, so we can lazily retrieve the standard
	 * transaction timeout.
	 */
	...
	private static MBeanNotificationInfo[] notificationInfo = new MBeanNotificationInfo[] { new MBeanNotificationInfo(
			...
			Notification.class.getName(), "Task queue is not keeping up") };
	 ...
	 * 
	 * @param collector
	 *            the system profile collector
	 * @param name
	 *            the name of the profile consumer created to support this
	 *            object
	 */
	TaskAggregateStats(ProfileCollector collector, String name) {
		...
		numTasks = (AggregateProfileCounter) consumer.createCounter("numTasks",
				type, level);
		numTransactionalTasks = (AggregateProfileCounter) consumer
				.createCounter("numTransactionalTasks", type, level);
		numFailedTasks = (AggregateProfileCounter) consumer.createCounter(
				"numFailedTasks", type, level);
		readyCount = (AggregateProfileSample) consumer.createSample(
				"readyCount", type, level);
		runtime = (AggregateProfileSample) consumer.createSample("runtime",
				type, level);
		lagtime = (AggregateProfileSample) consumer.createSample("lagTime",
				type, level);
		latency = (AggregateProfileSample) consumer.createSample("latency",
				type, level);
	...
	/**
	 * Send a notification that the task queue is falling behind. This method is
	 * not used yet.
	 */
	void notifyTaskQueue() {
		sendNotification(new Notification("com.sun.sgs.task.queue.behind",
				MXBEAN_NAME, seqNumber.incrementAndGet(),
				System.currentTimeMillis(), "Task queue is behind"));
	}
	 ...
	 * 
	 * @param smooth
	 *            the new smoothing factor
	 * @throws IllegalArgumentException
	 *             if {@code smooth} is not between 0.0 and 1.0, inclusive
	 */

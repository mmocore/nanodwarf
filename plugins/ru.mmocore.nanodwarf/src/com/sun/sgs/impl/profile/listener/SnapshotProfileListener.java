...
/**
 * This implementation of <code>ProfileListener</code> takes snapshots at fixed
 * intervals. It provides a very simple view of what the system has done over
 * the last interval. By default the time interval is 10 seconds.
 * <p>
 * Note that this is still a work in progress. It doesn't report anything except
 * the number of threads and the succeeded/total number of tasks. To get a
 * better view of the system, we probably need things like the time threads are
 * waiting for tasks, the size of the queues, etc., but those haven't been added
 * to the interfaces yet.
 * <p>
 * This listener reports its findings on a server socket. Any number of users
 * may connect to that socket to watch the reports. The default port used is
 * 43007.
 * <p>
 ...
 * key is used to specify an alternate port on which to report profiling data.
 * The <code>report.period</code> key is used to specify the length of time, in
...
public class SnapshotProfileListener implements ProfileListener {
	...
	private static final String PROP_BASE = SnapshotProfileListener.class
			.getName();
	 ...
	 * 
	 * @param properties
	 *            the <code>Properties</code> for this listener
	 * @param owner
	 *            the <code>Identity</code> to use for all tasks run by this
	 *            listener
	 * @param registry
	...
	public SnapshotProfileListener(Properties properties, Identity owner,
			ComponentRegistry registry) throws IOException {
		PropertiesWrapper wrappedProps = new PropertiesWrapper(properties);

		int port = wrappedProps.getIntProperty(PORT_PROPERTY, DEFAULT_PORT);
		networkReporter = new NetworkReporter(port);

		long reportPeriod = wrappedProps.getLongProperty(PERIOD_PROPERTY,
				DEFAULT_PERIOD);
		handle = registry.getComponent(TaskScheduler.class)
				.scheduleRecurringTask(new SnapshotRunnable(reportPeriod),
						owner, System.currentTimeMillis() + reportPeriod,
						reportPeriod);
		handle.start();
	}
	...
	/**
	 * Private internal class that is used to run a recurring task that reports
		...
		public void run() throws Exception {
			...
			synchronized (lock) {
				try {
					...
					reportStr.format("  Tasks=%d/%d%n", successCount,
							totalCount);

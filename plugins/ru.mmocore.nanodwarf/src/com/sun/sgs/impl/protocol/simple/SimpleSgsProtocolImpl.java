...
public class SimpleSgsProtocolImpl implements SessionProtocol {
	...
	/**
	 * The default length of the reconnect key, in bytes. TBD: the reconnection
	 ...
	 *            a protocol listener
	 * @param acceptor
	 *            the {@code SimpleSgsProtocol} acceptor
	 * @param byteChannel
	 *            a byte channel for the underlying connection
	 ...
	 * {@code scheduleRead} after constructing the instance to commence reading.
	 * 
	 * @param listener
	 *            a protocol listener
	 * @param acceptor
	...
	/**
	 * Returns the associated identity, or {@code null} if the client has not
	 * yet authenticated.
	 * 
	...
	public int getMaxMessageLength() {
		...
		return SimpleSgsProtocol.MAX_MESSAGE_LENGTH - 1 - // Opcode
		2 - // channel ID size
	...
	public void channelJoin(String name, BigInteger channelId, Delivery delivery) {
		...
		MessageBuffer buf = new MessageBuffer(1 + MessageBuffer.getSize(name)
				+ channelIdBytes.length);
		buf.putByte(SimpleSgsProtocol.CHANNEL_JOIN).putString(name)
	...
	public void channelLeave(BigInteger channelId) {
		byte[] channelIdBytes = channelId.toByteArray();
		ByteBuffer buf = ByteBuffer.allocate(1 + channelIdBytes.length);
		buf.put(SimpleSgsProtocol.CHANNEL_LEAVE).put(channelIdBytes).flip();
		write(buf);
	}
	 ...
	 * 
	 * <p>
	 * This implementation invokes the protected method {@link #writeBuffer
	 * writeBuffer} with the channel protocol message (a {@code ByteBuffer}) and
	 * the specified delivery requirement. A subclass can override the
	 * {@code writeBuffer} method if it supports other delivery guarantees and
	 * can make use of alternate transports for those other delivery
	 * requirements.
	 */
	public void channelMessage(BigInteger channelId, ByteBuffer message,
		...
		buf.put(SimpleSgsProtocol.CHANNEL_MESSAGE)
				.putShort((short) channelIdBytes.length).put(channelIdBytes)
				.put(message).flip();
		writeBuffer(buf, delivery);
	}
	...
	protected void loginSuccess() {
		...
		buf.putByte(SimpleSgsProtocol.LOGIN_SUCCESS).putBytes(reconnectKey);
	...
	private void loginRedirect(long nodeId, Set<ProtocolDescriptor> descriptors) {
		for (ProtocolDescriptor descriptor : descriptors) {
			if (acceptor.getDescriptor().supportsProtocol(descriptor)) {
				byte[] redirectionData = ((SimpleSgsProtocolDescriptor) descriptor)
						.getConnectionData();
				MessageBuffer buf = new MessageBuffer(
						1 + redirectionData.length);
				buf.putByte(SimpleSgsProtocol.LOGIN_REDIRECT).putBytes(
						redirectionData);
				writeNow(ByteBuffer.wrap(buf.getBuffer()), true);
	...
	/**
	 * Notifies the associated client that the previous login attempt was
	 * unsuccessful for the specified {@code reason}. The specified
	 * {@code throwable}, if non-{@code null} is an exception that occurred
	 * while processing the login request. The message channel should be careful
	 * not to reveal to the associated client sensitive data that may be present
	 * in the specified {@code throwable}.
	 * 
	 * @param reason
	...
	private void loginFailure(String reason, Throwable ignore) {
		...
		MessageBuffer buf = new MessageBuffer(1 + MessageBuffer.getSize(reason));
		buf.putByte(SimpleSgsProtocol.LOGIN_FAILURE).putString(reason);
		writeNow(ByteBuffer.wrap(buf.getBuffer()), true);
		monitorDisconnection();
	}
	...
	private void logoutSuccess() {
		...
		buf.put(SimpleSgsProtocol.LOGOUT_SUCCESS).flip();
		writeNow(buf, false);
		monitorDisconnection();
	}
	...
	private class ConnectedWriteHandler extends WriteHandler {
		 ...
		 * {@code isWriting}. The locks {@code lock} and {@code writeLock} should
		 * only be acquired in that specified order.
		 */
	...
	/**
	 * Processes the received message. This implementation processes opcodes for
	 * {@code SimpleSgsProtocol} version {@code 0x04}. A subclass can override
	 * this implementation to process additional opcodes, and then delegate to
	 * this implementation to process the version {@code 0x04} opcodes.
	 * 
	 * @param opcode
	 *            the message opcode
	 * @param msg
	 *            a message buffer containing the entire message, but with the
	 *            position advanced to the payload (just after the opcode)
	 */
	protected void handleMessageReceived(byte opcode, MessageBuffer msg) {
		...
		case SimpleSgsProtocol.LOGIN_REQUEST:
			...
			if (version != getProtocolVersion()) {
				if (logger.isLoggable(Level.SEVERE)) {
					logger.log(Level.SEVERE, "got protocol version:{0}, "
			...
			listener.newLogin(identity, SimpleSgsProtocolImpl.this,
					new LoginHandler());
		...
		case SimpleSgsProtocol.SESSION_MESSAGE:
			ByteBuffer clientMessage = ByteBuffer.wrap(msg.getBytes(msg.limit()
					- msg.position()));
			if (protocolHandler == null) {
			...
			protocolHandler.sessionMessage(clientMessage, new RequestHandler());
			break;

		case SimpleSgsProtocol.CHANNEL_MESSAGE:
			BigInteger channelRefId = new BigInteger(1, msg.getBytes(msg
					.getShort()));
			ByteBuffer channelMessage = ByteBuffer.wrap(msg.getBytes(msg
					.limit() - msg.position()));
			if (protocolHandler == null) {
				// ignore message before authentication
				if (logger.isLoggable(Level.FINE)) {
		...
		default:
			if (logger.isLoggable(Level.SEVERE)) {
				logger.log(Level.SEVERE, "unknown opcode 0x{0}",
						Integer.toHexString(opcode));
			}
			close();
			break;
	...
	/**
	 * Monitors the client's disconnection and closes this instance's underlying
	 * connection if the client hasn't closed the connection in a timely
	 * fashion.
	 */
	...
	/**
	 * A completion handler that is notified when the associated login request
	...
	private class LoginHandler implements
		 ...
		 * 
		 * <p>
		 * Otherwise, if the {@code get} invocation throws an
		 * {@code ExecutionException} and the exception's cause is a
		 * {@link LoginRedirectException}, it sends a login redirect message to
		 * the client with the redirection information obtained from the
		 * exception. If the {@code ExecutionException}'s cause is a
		 * {@link LoginFailureException}, it sends a login failure message to
		 * the client.
		 * 
		 * <p>
		 * If the {@code get} method throws an exception other than
		 * {@code ExecutionException}, or the {@code ExecutionException}'s cause
		 * is not either a {@code LoginFailureException} or a
		 * {@code LoginRedirectException}, then a login failed message is sent to
		 * the client.
		 */

 ...
 */
public class SimpleSgsRelocationProtocolImpl extends SimpleSgsProtocolImpl
	...
	/**
	 * A lock for {@suspendCompletionFuture}, and {@code relocationInfo} fields.
	 */
	private final Object lock = new Object();

	/**
	 * The completion future if suspending messages is in progress, or null.
	 */
	 ...
	 * implementation.
	 * 
	 * This implementation returns the latest {@code SimpleSgsProtocol} version.
	 * 
	...
	public void suspend(RequestCompletionHandler<Void> completionHandler) {
		synchronized (lock) {
			...
			suspendCompletionFuture = new SuspendMessagesCompletionFuture(
					completionHandler);
		}
		ByteBuffer buf = ByteBuffer.allocate(1);
		buf.put(SimpleSgsProtocol.SUSPEND_MESSAGES).flip();
		writeNow(buf, true);
	}
	...
	public void resume() {
		...
		ByteBuffer buf = ByteBuffer.allocate(1);
		buf.put(SimpleSgsProtocol.RESUME_MESSAGES).flip();
		writeNow(buf, true);
	}
	...
	public void relocate(Set<ProtocolDescriptor> descriptors,
		...
		synchronized (lock) {
			...
			if (logger.isLoggable(Level.FINE)) {
				logger.log(Level.FINE, "relocating, identity:{0} key:{1}",
						getIdentity(),
	...
	/**
	 * Notifies the associated client that the previous relocation attempt was
	 * successful.
	 */
	private void relocateSuccess() {
		MessageBuffer buf = new MessageBuffer(1 + reconnectKey.length);
		buf.putByte(SimpleSgsProtocol.RELOCATE_SUCCESS).putBytes(reconnectKey);
	 ...
	 * while processing the relocation request. The message channel should be
	 * careful not to reveal to the associated client sensitive data that may be
	 * present in the specified {@code throwable}.
	 * 
	 * @param reason
	 *            a reason why the relocation was unsuccessful
	 * @param throwable
	 *            an exception that occurred while processing the relocation
	...
	private void relocateFailure(String reason, Throwable ignore) {
		...
		buf.putByte(SimpleSgsProtocol.RELOCATE_FAILURE).putString(reason);
		writeNow((ByteBuffer.wrap(buf.getBuffer())), true);
		monitorDisconnection();
	}

	/**
	 * Throws {@link IllegalStateException} if the client session is relocating
	 * or has suspended messages.
	 * 
	 * @throws IllegalStateException
	 *             if the client session is relocating
	 */
	...
	/**
	 * Handles v5 protocol messages (relocate and suspend), and delegates to the
	 * super class to handle the v4 protocol messages.
	 * 
	 * @param opcode
	 *            the message opcode
	 * @param msg
	...
	protected void handleMessageReceived(byte opcode, MessageBuffer msg) {
		...
		case SimpleSgsProtocol.RELOCATE_REQUEST:
			...
			listener.relocatedSession(relocationKey,
					SimpleSgsRelocationProtocolImpl.this, new RelocateHandler());
			...
			synchronized (lock) {
				if (suspendCompletionFuture != null) {
					...
					if (logger.isLoggable(Level.WARNING)) {
						logger.log(Level.WARNING, "{0} received unexpected "
	 ...
	 */
	private class RelocateHandler implements
		 ...
		 * 
		 * <p>
		 * If the relocate request completed successfully (without throwing an
		 * exception), it sends a relocate success message to the client.
		 * 
		 * <p>
		 * Otherwise, if the {@code get} invocation throws an
		 * {@code ExecutionException} and the exception's cause is a
		 * {@link LoginRedirectException} or {@link RelocateFailureException},
		 * it sends a relocate failure message to the client.
		 * 
		 * <p>
		 * If the {@code get} method throws an exception other than
		 * {@code ExecutionException}, or the {@code ExecutionException}'s cause
		 * is not either a {@code RelocateFailureException} or a
	...
	private class RelocationInfo {
		...
		/**
		 * Sends a relocate notification to the underlying session, and monitors
		 * the underlying connection for timely disconnection.
		 */
	 ...
	 */
	private static class SuspendMessagesCompletionFuture extends
			AbstractCompletionFuture<Void> {
		SuspendMessagesCompletionFuture(
				RequestCompletionHandler<Void> completionFuture) {

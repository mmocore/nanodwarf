...
public class AsynchronousMessageChannel implements Channel {
	 ...
	 * based on the data read into the buffer between position 0 and the current
	 * position, or {@code -1} if the length cannot be determined.
	 * 
	 * @return the length, or {@code -1}
	 */
	int getMessageLength() {
		return (readBuffer.position() >= PREFIX_LENGTH) ? (readBuffer
				.getShort(0) & 0xffff) + PREFIX_LENGTH : -1;
	}
	...
	private final class Reader extends
		...
		protected IoFuture<Integer, Void> implCompleted(
				IoFuture<Integer, Void> result) throws ExecutionException,
				EOFException {

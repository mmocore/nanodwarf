 ...
 * relocation, and <b><code>0x04</code></b>, which does not support client
 * session relocation but is compatible with clients using the older protocol
 * version. Protocol version <b><code>0x05</code></b> is incompatible with
 * clients using protocol version <b><code>0x04</code></b>.
 * <p>
 * 
 * <dt><i>Property:</i> <code><b>
...
public class SimpleSgsProtocolAcceptor extends AbstractService implements
	...
	private static final LoggerWrapper staticLogger = new LoggerWrapper(
			Logger.getLogger(PKG_NAME + ".acceptor"));
	...
	public static final String READ_BUFFER_SIZE_PROPERTY = PKG_NAME
			+ ".read.buffer.size";
	...
	public static final String DISCONNECT_DELAY_PROPERTY = PKG_NAME
			+ ".disconnect.delay";
	...
	protected SimpleSgsProtocolAcceptor(Properties properties,
		...
		try {
			...
			identityManager = systemRegistry
					.getComponent(IdentityCoordinator.class);

			transport = wrappedProps.getClassInstanceProperty(
					TRANSPORT_PROPERTY, DEFAULT_TRANSPORT, Transport.class,
					new Class[] { Properties.class }, properties);

			protocolVersion = wrappedProps.getIntProperty(
			 ...
			 */
			monitorDisconnectingSessionsTaskHandle = taskScheduler
					.scheduleRecurringTask(
							new MonitorDisconnectingSessionsTask(), taskOwner,
							System.currentTimeMillis(), disconnectDelay);
	...
	public synchronized ProtocolDescriptor getDescriptor() {
		if (protocolDesc == null) {
			protocolDesc = new SimpleSgsProtocolDescriptor(
		...
		public void newConnection(AsynchronousByteChannel byteChannel)
			...
			if (protocolVersion == PROTOCOL4) {
				new SimpleSgsProtocolImpl(protocolListener,
						SimpleSgsProtocolAcceptor.this, byteChannel,
						readBufferSize);
			} else /* latest SimpleSgsProtocol version */{
				new SimpleSgsRelocationProtocolImpl(protocolListener,
						SimpleSgsProtocolAcceptor.this, byteChannel,
						readBufferSize);
			}
		}
	...
	public Identity authenticate(String name, String password)
		...
		return identityManager
				.authenticateIdentity(new NamePasswordCredentials(name,
						password.toCharArray()));
	}

	/**
	 * Adds the specified {@code protocol} to the map containing
	 * {@code SessionProtocol}s that are disconnecting. The map is keyed by
	 * connection expiration time. The connection will expire after a fixed
	 * delay and will be forcibly terminated if the client hasn't already closed
	 * the connection.
	 * 
	 * @param protocol
	...
	public void monitorDisconnection(SessionProtocol protocol) {
		disconnectingHandlersMap.put(System.currentTimeMillis()
				+ disconnectDelay, protocol);
	}
	 ...
	 * closed.
	 */
	private class MonitorDisconnectingSessionsTask extends
		...
		public void run() {
			...
			if (!disconnectingHandlersMap.isEmpty()
				...
				Map<Long, SessionProtocol> expiredSessions = disconnectingHandlersMap
						.headMap(now);

 ...
 */
abstract class AsyncGroupImpl extends AsynchronousChannelGroup {
	 ...
	 * {@link AsyncKey} that can be used to invoke asynchronous IO operations.
	 * If this group is shutdown, this method will throw
	 * {@link ShutdownChannelGroupException} and close the channel.
	 * 
	 * @param ch
	 *            the underlying channel for IO operations
	 * @return an {@code AsyncKey} that supports asynchronous operations on the
	 *         underlying channel
	 * 
	 * @throws IOException
	 *             if an I/O error occurs
	 * @throws ShutdownChannelGroupException
	 *             if this group is shutdown
	 */
	abstract AsyncKey register(SelectableChannel ch) throws IOException;

	/**
	 * Returns a runnable that will invoke the given completion handler. Passes
	 * the handler an {@code IoFuture} constructed from the given future and
	 * attachment object.
	 * 
	 * @param <R>
	 *            the result type
	...
	/**
	 * Invokes the uncaught exception handler of the current thread, if one is
	 * present. Does not re-throw the exception.
	 * 
	 * @param <T>
	...
	private class CompletionRunner<R, A> implements Runnable {
		 ...
		 *            the completion handler to invoke
		 * @param attachment
		 *            the attachment for the future
		 * @param future
		 *            the future to pass to the completion handler
		 */
		CompletionRunner(CompletionHandler<R, A> handler, A attachment,
				Future<R> future) {
	...
	/**
	 * Invokes {@link AsynchronousChannelGroup#shutdown()} when this group is no
	 * longer referenced.
	 */
	@Override

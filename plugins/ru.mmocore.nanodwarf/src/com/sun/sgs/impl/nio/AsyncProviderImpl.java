...
abstract class AsyncProviderImpl extends AsynchronousChannelProvider {
	...
	/**
	 * The default uncaught exception handler (or {@code null} if no handler is
	 * set), until the default group is created. After the default group has
	 * been created, this field is set to {@code null} to avoid pinning the
	 * handler in memory.
	 */
	 ...
	 * {@link SelectorProvider}. If the parameter is {code null}, the system
	 * default {@code SelectorProvider} will be used.
	 * 
	 * @param selProvider
	 *            the {@code SelectorProvider}, or {@code null} to use the
	 *            system default {@code SelectorProvider}
	 * 
	 * @see SelectorProvider#provider()
	 */
	protected AsyncProviderImpl(SelectorProvider selProvider) {
		selectorProvider = selProvider != null ? selProvider : SelectorProvider
				.provider();
	...
	/**
	 * Returns the default thread pool factory as specified by the class
	 * documentation for {@link AsynchronousChannelGroup}. Delegates to
	 * {@link #createDefaultThreadPoolFactory()} if the other factory creation
	 * mechanisms fail.
	 * 
	...
	private ThreadPoolFactory getThreadPoolFactory() {
		return AccessController
				.doPrivileged(new PrivilegedAction<ThreadPoolFactory>() {
					public ThreadPoolFactory run() {
						String cn = System
								.getProperty("com.sun.sgs.nio.channels.DefaultThreadPoolFactory");
						if (cn != null) {
							try {
	...
	/**
	 * Creates a default {@link ThreadPoolFactory} when none has been specified
	 * by other mechanisms.
	 * 
	...
	/**
	 * Returns the default {@link AsynchronousChannelGroup} for this provider,
	 ...
	 * 
	 * @param group
	 *            a channel group, or {@code null} to return the default group
	 *            for this provider
	 * @return the given group, if it was created by this provider, or the
	 *         default group if {@code null} was given
	 * 
	 * @throws IllegalArgumentException
	 *             if the group was not created by this provider
	 * @throws IOException
	 *             if an IO error occurs while constructing the default group
	 */
	...
	/** {@inheritDoc} */
	@Override
	public abstract AsyncGroupImpl openAsynchronousChannelGroup(
			ExecutorService executor) throws IOException;

	/** {@inheritDoc} */
	@Override
	public AsyncDatagramChannelImpl openAsynchronousDatagramChannel(
			ProtocolFamily pf, AsynchronousChannelGroup group)
			throws IOException {

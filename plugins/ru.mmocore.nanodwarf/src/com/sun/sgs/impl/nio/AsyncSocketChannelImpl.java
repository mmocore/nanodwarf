...
/**
 * An implementation of {@link AsynchronousSocketChannel}. Most interesting
 * methods are delegated to the {@link AsyncKey} returned by this channel's
 * channel group.
 */
class AsyncSocketChannelImpl extends AsynchronousSocketChannel {
	...
	/**
	 * Creates a new instance registered with the given channel group, with the
	 * given underlying {@link SocketChannel}. Used by an
	 * {@link AsyncServerSocketChannelImpl} when a new connection is accepted.
	 * 
	 * @param group
	 *            the channel group
	 * @param channel
	 *            the {@link SocketChannel} for this async channel
	 * @throws IOException
	 *             if an I/O error occurs
	 */
	 ...
	 */
	@Override
	public AsyncSocketChannelImpl bind(SocketAddress local) throws IOException {
	...
	public <A> IoFuture<Void, A> connect(SocketAddress remote,
		...
		return key.execute(OP_CONNECT, attachment, handler, 0,
				TimeUnit.MILLISECONDS, new Callable<Void>() {
	 ...
	 */
	@Override
	public <A> IoFuture<Long, A> read(final ByteBuffer[] dsts,
			final int offset, final int length, long timeout, TimeUnit unit,
	 ...
	 */
	@Override
	public <A> IoFuture<Long, A> write(final ByteBuffer[] srcs,
			final int offset, final int length, long timeout, TimeUnit unit,
			A attachment, CompletionHandler<Long, ? super A> handler) {

...
final class Util {
	...
	/**
	 * Returns an {@link IllegalStateException} indicating that the given
	 * exception was not expected, and setting the cause to that exception.
	 * 
	 * @param exception
	 ...
	 * @param result
	 *            the result for the returned {@code Future}
	...
	/**
	 * Returns a new, completed {@link Future} that always throws the given
	 * exception when its {@code get} methods are called.
	 * 
	 * @param <V>
	 *            the type of the result
	 * @param exception
	 *            the exception for the returned {@code Future} to throw
	...
	private static final class ResultFuture<V> extends DoneFuture<V> {
		 ...
		 * 
		 * @param result
		 *            the result of this {@code Future}
		 */
	...
	/**
	 * A future that has already completed by throwing an execution exception.
	 * 
	 * @param <V>
	 *            the result type
	 */
	private static final class FailedFuture<V> extends DoneFuture<V> {
		 ...
		 * 
		 * @param exception
		 *            the exception to wrap with an {@link ExecutionException}
		 *            and throw from this {@code Future}'s {@link Future#get()
		 *            get} methods.
		 */
	...
	private static final String[] opsTable = new String[] { "", "C", "R", "CR",
			"W", "CW", "RW", "CRW", "A", "AC", "AR", "ACR", "AW", "ACW", "ARW",
			"ACRW" };

	/**
	 * Returns a concise string representation of the active
	 * {@link SelectionKey} operations set in the parameter {@literal ops}. For
	 * example, if {@literal ops} is {@code OP_READ | OP_WRITE}, this method
	 * returns the string "RW".
	 * 
	 * @param ops
	 *            the {@code SelectionKey} operations to format
	 ...
	 * 
	 * @param op
	 *            a {@link SelectionKey} operation
	 * @return a human-readable string, such as "OP_READ"
	 * @throws IllegalArgumentException
	 *             if the op is not one of the operation constants defined by
	 *             {@link SelectionKey}
	 * @see SelectionKey
	 */

...
/**
 * An implementation of {@link AsynchronousDatagramChannel}. Most interesting
 * methods are delegated to the {@link AsyncKey} returned by this channel's
 * channel group.
 * <p>
 * Also implements the creation and management of multicast
 * {@link MembershipKey}s for this channel.
 */
class AsyncDatagramChannelImpl extends AsynchronousDatagramChannel {
	/** The valid socket options for this channel. */
	private static final Set<SocketOption> socketOptions;
	static {
		Set<? extends SocketOption> es = EnumSet.of(
				StandardSocketOption.SO_SNDBUF, StandardSocketOption.SO_RCVBUF,
				StandardSocketOption.SO_REUSEADDR,
				StandardSocketOption.SO_BROADCAST, StandardSocketOption.IP_TOS,
	...
	final ConcurrentHashMap<MembershipKeyImpl, MembershipKeyImpl> mcastKeys = new ConcurrentHashMap<MembershipKeyImpl, MembershipKeyImpl>();

	/** Indicates whether a connect operation is pending on this channel. */
	final AtomicBoolean connectionPending = new AtomicBoolean();

	/**
	 * Creates a new instance registered with the given channel group. If this
	 * channel is used for multicast, the protocol family should be specified to
	 * match that of the multicast group. If {@code null} is specified as the
	 * protocol family, the default family is used.
	 * 
	 * @param group
	 *            the channel group
	 * @param pf
	 ...
	 * {@inheritDoc}
	 * <p>
	 * This method performs exactly the same security checks as the bind method
	 * of the DatagramSocket class. That is, if a security manager has been
	 * installed then this method verifies that its checkListen method permits
	 * waiting for a connection request on the specified local port number.
	 * 
	...
	public AsyncDatagramChannelImpl setOption(SocketOption name, Object value)
		...
		try {
			...
			case IP_MULTICAST_TTL:
				((MulticastSocket) socket).setTimeToLive(((Integer) value)
						.intValue());
				break;

			case IP_MULTICAST_LOOP:
				// TODO should we reverse the sense of setLoopbackMode? -JM
				((MulticastSocket) socket).setLoopbackMode(((Boolean) value)
						.booleanValue());
				break;
	...
	public MembershipKey join(InetAddress group, NetworkInterface interf)
		...
		if (protocolFamily == StandardProtocolFamily.INET) {
			if (!((group instanceof Inet4Address) || ((group instanceof Inet6Address) && ((Inet6Address) group)
		...
		MembershipKeyImpl existingKey = mcastKeys.putIfAbsent(newKey, newKey);
	 ...
	 */
	@Override
	public <A> IoFuture<Void, A> connect(final SocketAddress remote,
			final A attachment, final CompletionHandler<Void, ? super A> handler) {
	 ...
	 */
	@Override
	public <A> IoFuture<SocketAddress, A> receive(final ByteBuffer dst,
	 ...
	 */
	@Override
	public <A> IoFuture<Integer, A> send(final ByteBuffer src,
			final SocketAddress target, long timeout, TimeUnit unit,
	 ...
	 */
	@Override
	public <A> IoFuture<Integer, A> read(final ByteBuffer dst, long timeout,
	 ...
	 */
	@Override
	public <A> IoFuture<Integer, A> write(final ByteBuffer src, long timeout,
			TimeUnit unit, A attachment,
			CompletionHandler<Integer, ? super A> handler) {
		return key.execute(OP_WRITE, attachment, handler, timeout, unit,
				new Callable<Integer>() {
	...
	/**
	 * A simple multicast membership key for datagram channels. Does
	 * <strong>not</strong> support source-specific filtering.
	 */
	final class MembershipKeyImpl extends MembershipKey {
		 ...
		 * @param netIf
		 *            the network interface of the group
		 */
		MembershipKeyImpl(InetSocketAddress mcastaddr, NetworkInterface netIf) {
		 ...
		 * {@inheritDoc}
		 * <p>
		 * This implementation is not source-specific, so always returns
		 * {@code null}.
		 */
		@Override
		...
		@Override
		public int hashCode() {
			return getChannel().hashCode() ^ getGroup().hashCode()

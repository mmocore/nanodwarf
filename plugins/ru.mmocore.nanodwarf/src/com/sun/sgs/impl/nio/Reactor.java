 ...
 * enables interest in that operation with a {@link Selector}, returning a
 * future that will be completed when the operation becomes ready and the IO is
 * performed.
 * <p>
...
class Reactor {
	...
	/**
	 * Selector guard. Any code that accesses selector data structures, (e.g.,
	 * selection keys and their interest sets), must obtain this lock before
	 * waking the selector. Doing so prevents the selector from blocking on
	 * {@code select()} again until the code that awakened it has released this
	 * guard.
	 * <p>
	 * The selector must obtain this lock <strong>and release it</strong> before
	 * blocking on {@code select()}.
	 * <p>
	 * If both the {@code selectorLock} and {@link AsyncKey} need to be locked,
	 * the {@code selectorLock} must be locked <em>first</em>.
	 */
	final Object selectorLock = new Object();

	/**
	 * The lifecycle state of this reactor. Increases monotonically. It may only
	 * be accessed with selectorLock held.
	 */
	...
	/**
	 * The channel group for this reactor, used to obtain completion handler
	 * runners.
	 */
	final ReactiveChannelGroup group;

	/**
	 * The {@code Selector} that waits for available IO operations on registered
	 * channels.
	 */
	final Selector selector;

	/**
	 * The executor for this {@code Reactor}. Typically an executor is shared by
	 * all reactors in a group, but each reactor may have its own logical
	 * executor.
	 */
	final Executor executor;

	/** Operations that having pending timeouts. */
	final DelayQueue<TimeoutHandler> timeouts = new DelayQueue<TimeoutHandler>();
	 ...
	 * @param executor
	 *            the executor for tasks in this reactor
	 * 
	 * @throws IOException
	 *             if an I/O error occurs, e.g. while opening the
	 *             {@code Selector} for this reactor
	 */
	 ...
	 * channels. If this reactor is already marked for shutdown, this method has
	 * no effect.
	 * 
	...
	/**
	 * Notifies this reactor that it should shutdown immediately, closing any
	 * open channels registered with it. If this reactor is already marked for
	 * immediate shutdown, this method has no effect.
	 * 
	 * @throws IOException
	...
	/**
	 * Performs a single iteration of the reactor's event loop, and returns a
	 * flag indicating whether the reactor is still running. Only one call may
	 * be active on a Reactor instance at a time.
	 * 
	 * @return {@code false} if this reactor is stopped, otherwise {@code true}
	 * @throws IOException
	 *             if an I/O error occurs
	 */
	boolean performWork() throws IOException {
		...
		synchronized (selectorLock) {
			...
			if (log.isLoggable(Level.FINER)) {
				...
				log.log(Level.FINER, "{0} select on {1} keys", new Object[] {
						this, numKeys });
				if (numKeys <= 5) {
					for (SelectionKey key : selector.keys()) {
						try {
							...
							log.log(Level.FINER,
									...
									new Object[] { this, key.attachment() });
						}
					}
				}
			}
		...
		if (nextExpiringTask == null) {
			...
			long nextTimeoutMillis = nextExpiringTask
		...
		if (log.isLoggable(Level.FINER)) {
			log.log(Level.FINER, "{0} selected {1} / {2}", new Object[] { this,
					readyCount, selector.keys().size() });
		}
		...
		while (keys.hasNext()) {
			...
			ReactiveAsyncKey asyncKey = (ReactiveAsyncKey) key.attachment();
		...
		final List<TimeoutHandler> expiredHandlers = new ArrayList<TimeoutHandler>();
	 ...
	 * 
	 * @param ch
	 *            the {@code SelectableChannel} to register
	 * @return an {@link AsyncKey} for the given channel
	 * 
	 * @throws ShutdownChannelGroupException
	 *             if the reactor is shutdown
	 * @throws IOException
	 *             if an IO error occurs
	 */
	ReactiveAsyncKey register(SelectableChannel ch) throws IOException {
		synchronized (selectorLock) {
			if (lifecycleState != RUNNING) {
	...
	/**
	 * Registers interest in an IO operation on the channel associated with the
	 * given {@link AsyncKey}, returning a future representing the result of the
	 * operation.
	 * <p>
	 * When the requested operation becomes ready, the given {@code task} is
	 * invoked so that it may perform the IO operation. The selector's interest
	 * in all ready operations is cleared before dispatching to the task.
	 * <p>
	 * Several checks are performed on the channel at this point to avoid race
	 * conditions where the check succeeds but the condition immediately becomes
	 * false. We lock both the {@code selectorLock} and the {@code asyncKey} to
	 * ensure that we get a proper view of the state when registering the
	 * operation, and so that if the state later changes the operation will be
	 * terminated properly.
	 * <p>
	 * If the channel is closed, {@link ClosedAsynchronousChannelException} is
	 * thrown.
	 * <p>
	 * Additional checks are performed on {@code SocketChannel}s:
	 * <ul>
	 * <li>
	 * If the requested operation is {@code OP_READ} or {@code OP_WRITE} and the
	 * channel is not connected, {@link NotYetConnectedException} is thrown.
	 * <li>
	 ...
	 *            the result type
	 * @param asyncKey
	 *            the key for async operations on the channel
	 * @param op
	 *            the {@link SelectionKey} operation requested
	 * @param task
	 *            the task to invoke when the operation becomes ready
	 * 
	 * @throws ClosedAsynchronousChannelException
	 *             if the channel is closed
	 * @throws NotYetConnectedException
	 *             if a read or write operation is requested on an unconnected
	 *             {@code SocketChannel}
	 * @throws AlreadyConnectedException
	 *             if a connect operation is requested on a connected
	 *             {@code SocketChannel}
	 */
	...
	static class AsyncOp<R> extends FutureTask<R> {
		 ...
		 * 
		 * @param callable
		 *            the work to perform when this task is run
		 */
		...
		/**
		 * Completes this future as if its {@code run()} method threw an
		 * {@code AbortedByTimeoutException}, unless this future has already
		 * completed.
		 */
		void timeoutExpired() {
	...
	/**
	 * Manages a single asynchronous IO operation for a {@link ReactiveAsyncKey}
	 * . Behaves appropriately when no operation is pending, or when a race
	 * occurs between, e.g., timeout and user cancellation.
	 */
	abstract class PendingOperation {

		/**
		 * The continuation of the IO task to perform, if one is pending, or a
		 * reference to {@code null} if an operation is not pending.
		 */
		protected final AtomicReference<AsyncOp<?>> task = new AtomicReference<AsyncOp<?>>();

		/**
		 * The timeout action for the pending task, or {@code null} if there is
		 * no pending timeout.
		 */
		...
		/**
		 * Creates a new instance to manage the given operation for the given
		 * key.
		 * 
		 * @param asyncKey
		 *            the async key
		 * @param op
		 *            the operation to manage
		 */
		 ...
		 * attempt is made to invoke this operation while it is already pending.
		 * <p>
		...
		void selected() {
			...
			if (selectedTask == null) {
				log.log(Level.FINEST, "selected but nothing to do {0}", this);
				return;
			} else {
		...
		/**
		 * Attempts to initiate an asynchronous operation. If an operation is
		 * already pending, an appropriate exception is thrown by a subclass via
		 * its {@link PendingOperation#pendingPolicy() pendingPolicy()}.
		 * 
		 * @param <R>
		 *            the result type
		 * @param <A>
		 *            the attachment type
		 * @param attachment
		 *            the attachment for the completion handler; may be
		 *            {@code null}
		 * @param handler
		 *            the completion handler; may be {@code null}
		 * @param timeout
		 *            the timeout, or {@code 0} indicating no timeout
		 * @param unit
		 *            the unit of the timeout
		 * @param callable
		 *            the IO action to perform when this operation is ready
		 * @return an {@code IoFuture} representing the pending operation
		 * 
		 * @see AsyncKey#execute(int, Object, CompletionHandler, long, TimeUnit,
		 *      Callable)
		 */
		<R, A> IoFuture<R, A> execute(final A attachment,
				final CompletionHandler<R, ? super A> handler, long timeout,
		...
		public String toString() {
			return String.format("PendingOp[key=%s,op=%s]", asyncKey,
					Util.opName(op));
		}
	}

	/**
	 * Provides support for initiating asynchronous IO operations on an
	 * underlying reactive channel registered with this {@link Reactor}.
	 * 
	 * @see AsyncKey
	 */
	class ReactiveAsyncKey implements AsyncKey {
		...
		private final PendingOperation pendingAccept = new PendingOperation(
				this, OP_ACCEPT) {
		...
		private final PendingOperation pendingConnect = new PendingOperation(
				this, OP_CONNECT) {
			protected void pendingPolicy() {
		...
		private final PendingOperation pendingRead = new PendingOperation(this,
				OP_READ) {
			protected void pendingPolicy() {
		...
		private final PendingOperation pendingWrite = new PendingOperation(
				this, OP_WRITE) {
			protected void pendingPolicy() {
		 ...
		 * 
		 * @param key
		 *            the selection key that represents the underlying channel's
		 ...
		 * Closes the channel
		 * <li>
		 * Awakens any pending asynchronous operations so they can complete with
		 * {@link AsynchronousCloseException} when they notice the channel is
		 * closed.
		 * </ul>
		 */
		 ...
		 * {@inheritDoc}
		 */
		public <R, A> IoFuture<R, A> execute(int op, A attachment,
				CompletionHandler<R, ? super A> handler, long timeout,
				TimeUnit unit, Callable<R> callable) {
			switch (op) {
			case OP_WRITE:
				return pendingWrite.execute(attachment, handler, timeout, unit,
						callable);
			case OP_READ:
				return pendingRead.execute(attachment, handler, timeout, unit,
						callable);
			case OP_CONNECT:
				return pendingConnect.execute(attachment, handler, timeout,
						unit, callable);
			case OP_ACCEPT:
				return pendingAccept.execute(attachment, handler, timeout,
						unit, callable);
			default:
				throw new IllegalArgumentException("bad op " + op);
			}
	...
	/**
	 * Represents a timeout action for a {@code PendingOperation}. Instances are
	 * placed in the {@code timeouts} queue and run when they have expired.
	 * <p>
	 * Tasks that complete before their timeout expires should remove their
	 * {@code TimeoutHandler} from the queue, since timeouts may be much longer
	 * than the typical operation and the timeouts queue could become filled
	 * with obsolete handlers.
	 */
	private static final class TimeoutHandler implements Delayed, Runnable {
		...
		/**
		 * Creates a new instance that will notify the given operation when the
		 * (relative) timeout expires.
		 * 
		 * @param task
		 *            the task to notify
		 * @param timeout
		...
		TimeoutHandler(AsyncOp<?> task, long timeout, TimeUnit unit) {
			this.task = task;
			this.deadlineMillis = unit.toMillis(timeout)
		...
		public long getDelay(TimeUnit unit) {
			return unit.convert(deadlineMillis - System.currentTimeMillis(),
		...
		public int compareTo(Delayed o) {
			...
			if (o instanceof TimeoutHandler) {
				return Long.signum(deadlineMillis
						- ((TimeoutHandler) o).deadlineMillis);
			} else {

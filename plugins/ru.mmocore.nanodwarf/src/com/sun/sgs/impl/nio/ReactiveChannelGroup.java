 ...
 * <p>
 * This class is a container for a set of {@link Reactor}s which do the actual
 * work of registering and dispatching asynchronous operations on channels. It
 * provides:
 * <ul>
 ...
 * {@linkplain AsynchronousChannelGroup#shutdownNow() immediate shutdown}, and
 * {@linkplain AsynchronousChannelGroup#awaitTermination awaiting termination}.
 * <li>Channel registration and load balancing across multiple {@code Reactor}s:
 * channels are assigned to one of the reactors in the set. Since reactors are
 * single-threaded, the reactor set allows multiple CPUs to be utilized by
 * having multiple separate reactors.
 * </ul>
 * The default number of {@code Reactor}s is set as the number of
 * {@linkplain Runtime#availableProcessors() available processors}, but it can
 * be changed by setting the requested number in the system property
 * {@value #REACTORS_PROPERTY}.
 */
class ReactiveChannelGroup extends AsyncGroupImpl {
	/** The logger for this class. */
	static final Logger log = Logger.getLogger(ReactiveChannelGroup.class
			.getName());

	/**
	 * Lock held on updates to lifecycleState and reactors list, and the
	 * condition variable for awaiting group termination.
	 */
	final Object stateLock = new Object();

	/**
	 * The lifecycle state of this group. Increases monotonically. It may only
	 * be accessed with stateLock held.
	 */
	...
	/**
	 * The active {@linkplain Reactor reactors} in this group. It may only be
	 * accessed with stateLock held.
	 */
	final List<Reactor> reactors;

	/**
	 * The property to specify the number of reactors to be used by channel
	 * groups: {@value}
	 */
	 ...
	 */
	public static final int DEFAULT_REACTORS = Runtime.getRuntime()
			.availableProcessors();
	 ...
	 * 
	 * @param provider
	 *            the provider that created this group
	 * @param executor
	 *            the executor for this group
	 * 
	 * @throws IOException
	...
	/**
	 * Creates a new group with the requested number of reactors. If {code 0}
	 * reactors are requested, a default is chosen as the number of
	 ...
	 *            the number of reactors to create in this group, or {@code 0}
	 *            to use the default
	 * 
	 * @throws IllegalArgumentException
	 *             if a negative number of reactors is requested
	 * @throws IOException
	 *             if an I/O error occurs
	 */
	ReactiveChannelGroup(ReactiveAsyncChannelProvider provider,
			ExecutorService executor, int requestedReactors) throws IOException {
	...
	interface ReactorAssignmentStrategy {
		 ...
		 * 
		 * @param channel
		 *            a channel to assign to a {@code Reactor}
	...
	/**
	 * A {@code Reactor} load balancing strategy that chooses a reactor based on
	 * the {@linkplain Object#hashCode() hash} of the channel.
	 */
	final class HashingReactorAssignmentStrategy implements
		...
		public Reactor getReactorFor(SelectableChannel channel) {
			return reactors.get(Math.abs(channel.hashCode() % reactors.size()));
		}
	}
	...
	/**
	 * If the group is trying to shutdown, check that all the reactors have
	 * shutdown. If they have, mark this group as done and wake anyone blocked
	 * on awaitTermination.
	 * 

 ...
 * implementing methods that return an {@code IoFuture}, have a
 * {@code CompletionHandler} parameter, and are implemented by making calls to
 * similar methods. This class is intended to support the interaction between
 * the internal (involving to the delegating function) and external (involving
 * the caller) futures and completion handlers, allowing subclasses supply the
 * desired behavior by customizing the three protected methods:
 * {@link #implStart}, {@link #implCompleted}, and {@link #done}.
 * <p>
 * 
 * For example, suppose you wanted to implement a method like
 * {@code AsynchronousByteChannel.read} that delegated to an existing channel,
 * but printed a message before and after reading. A simple implementation might
 * look like:
 * 
 * <pre>
 * public class PrintReader&lt;A&gt; extends
 * 		DelegatingCompletionHandler&lt;Integer, A, Integer, A&gt; {
 * 	private final AsynchronousByteChannel channel;
 * 	private final ByteBuffer dst;
 * 
 * 	public static &lt;A&gt; IoFuture&lt;Integer, A&gt; read(
 * 			AsynchronousByteChannel channel, ByteBuffer dst, A attachment,
 ...
 * 
 * @param <OR>
 *            the result type for the outer handler
 * @param <OA>
 *            the attachment type for the outer handler
 * @param <IR>
 *            the result type for this handler
 * @param <IA>
 *            the attachment type for this handler
 */
public abstract class DelegatingCompletionHandler<OR, OA, IR, IA> extends
		IoFutureTask<OR, OA> implements CompletionHandler<IR, IA> {
	 ...
	 * exception thrown will terminate the computation. If an
	 * {@link ExecutionException} is thrown, then its cause will be used.
	 * 
	 * @param innerResult
	...
	protected abstract IoFuture<IR, IA> implCompleted(
			IoFuture<IR, IA> innerResult) throws Exception;

	/**
	 * Called when the computation is completed, which occurs when
	 * {@link #implCompleted} returns {@code null} or throws an exception, or
	 * when the outer future is cancelled.
	 * <p>
	 * 

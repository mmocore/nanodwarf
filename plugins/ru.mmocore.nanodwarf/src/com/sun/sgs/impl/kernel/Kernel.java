...
/**
 * This is the core class for the server. It is the first class that is created,
 * and represents the kernel of the runtime. It is responsible for creating and
 * initializing all components of the system and the applications configured to
 * run in this system.
 * <p>
 * The kernel must be configured with certain <a href=
 * "../../impl/kernel/doc-files/config-properties.html#RequiredProperties">
 ...
 * specified in the {@link StandardProperties} class, and supports the following
 ...
 * which is used internally by the system is enabled. To enable more profiling,
 * this property must be set to a valid level for
 * {@link ProfileCollector#setDefaultProfileLevel(ProfileLevel)}.
 * <p>
 * 
 ...
 * To enable a set of listeners, set this property to a colon-separated list of
 * fully-qualified class names, each of which implements {@link ProfileListener}
 * . A number of listeners are provided with the system in the
 * {@link com.sun.sgs.impl.profile.listener} package.
 * <p>
 * 
 ...
 * 
 * <dd style="padding-top: .5em">The implementation class used to track access
 * to shared objects. The value of this property should be the name of a public,
 * non-abstract class that implements the {@link AccessCoordinatorHandle}
 * interface, and that provides a public constructor with the three parameters
 * {@link Properties}, {@link TransactionProxy}, and
 * {@link ProfileCollectorHandle}.
 * <p>
 * 
 * 
 * </dl>
...
public class Kernel {
	...
	private static final LoggerWrapper logger = new LoggerWrapper(
			Logger.getLogger(Kernel.class.getName()));
	...
	private static final String DEFAULT_ACCESS_COORDINATOR_JE = "com.sun.sgs.impl.kernel.LockingAccessCoordinator";
		// the default access coordinator to use if the db type is unknown
	private static final String DEFAULT_ACCESS_COORDINATOR = "com.sun.sgs.impl.kernel.LockingAccessCoordinator";
	...
	/**
	 * Creates an instance of <code>Kernel</code>. Once this is created the code
	 * components of the system are running and ready. Creating a
	 ...
	 * 
	 * @throws Exception
	 *             if for any reason the kernel cannot be started
	 */
	protected Kernel(Properties appProperties) throws Exception {
		...
		try {
			...
			IdentityCoordinator identityCoordinator = createIdentityCoordinator();
			...
			BindingKeyedCollections collectionsFactory = new BindingKeyedCollectionsImpl(
					proxy);
	...
	/**
	 * Construct the AccessCoordinatorHandle. Our default access coordinator is
	 * different depending on whether we are using bdb or je. This of course
	 * assumes we are using the default BerkeleyDB backed data store, if we
	 * aren't, or we don't know, use the global default.
	 */
	private AccessCoordinatorHandle getAccessCoordinator(Properties properties,
			TransactionProxy proxy, ProfileCollectorHandle profileCollectorHandle)
	{
		String defaultCoordinatorClass = DEFAULT_ACCESS_COORDINATOR;
		String dbType = properties.getProperty("com.sun.sgs.impl.service.data.store.db.environment.class");
		if (dbType == null) {
			defaultCoordinatorClass = DEFAULT_ACCESS_COORDINATOR;
		} else if (dbType.equals("com.sun.sgs.impl.service.data.store.db.je.JeEnvironment")) {
			defaultCoordinatorClass = DEFAULT_ACCESS_COORDINATOR_JE;
		}

		return wrappedProperties.getClassInstanceProperty(
				ACCESS_COORDINATOR_PROPERTY, defaultCoordinatorClass,
				AccessCoordinatorHandle.class, new Class[] { Properties.class,
						TransactionProxy.class, ProfileCollectorHandle.class },
	...
	private void loadProfileListeners(ProfileCollector profileCollector) {
		List<String> listenerList = wrappedProperties.getListProperty(
				PROFILE_LISTENERS, String.class, "");
		List<String> extListenerList = wrappedProperties.getListProperty(
	 ...
	 * <code>Service</code>s associated with the application and then starts the
	 * application.
	 * 
	 * @throws Exception
	 *             if there is any error in startup
	 */
	private void createAndStartApplication() throws Exception {
		String appName = wrappedProperties
	 ...
	 * <code>Manager</code>s (if any) in order, in preparation for starting up
	 * an application.
	 */
	...
	private void fetchServices(StartupKernelContext startupContext)
		...
		List<ServiceNodeTypes> externalNodeTypes = wrappedProperties
				.getEnumListProperty(StandardProperties.SERVICE_NODE_TYPES,
						ServiceNodeTypes.class, ServiceNodeTypes.ALL);
		List<ServiceNodeTypes> extensionNodeTypes = wrappedProperties
				.getEnumListProperty(
		...
		NodeType type = NodeType.valueOf(wrappedProperties
				.getProperty(StandardProperties.NODE_TYPE));

		loadCoreServices(type, startupContext);
		loadExternalServices(allServices, allManagers, allNodeTypes, type,
				startupContext);
	}
	...
	private void loadCoreServices(NodeType type,
		...
		long nodeId = startupContext.getService(DataService.class)
				.getLocalNodeId();
	...
	private void loadExternalServices(List<String> externalServices,
			...
			List<ServiceNodeTypes> externalNodeTypes, NodeType type,
			StartupKernelContext startupContext) throws Exception {
		if (externalServices.size() != externalManagers.size()
			...
			if (logger.isLoggable(Level.SEVERE)) {
				logger.log(Level.SEVERE, "External service count "
						...
						+ "({2}) do not match.", externalServices.size(),
	...
	/**
	 * Private helper that creates an instance of a <code>service</code> with no
	 * manager, based on fully qualified class names.
	 */
		...
		if (!type.equals(NodeType.coreServerNode)) {
			try {
				...
				transactionScheduler.runUnboundedTask(new AppStartupRunner(
						wrappedProperties.getProperties()), owner);

				logger.log(Level.INFO, "{0}: application is ready", application);
			} catch (Exception e) {
				if (logger.isLoggable(Level.CONFIG)) {
	 ...
	 * too long. The timer is started as a daemon so the task won't be run if a
	 ...
	 * {@link BootProperties#EXTENSION_AUTHENTICATORS_PROPERTY} properties. The
	 * returned {@code IdentityCoordinator} is then generated with this as a
	 * backing collection of authenticators.
	 */
	private IdentityCoordinator createIdentityCoordinator() throws Exception {
		List<String> authClassNameList = wrappedProperties.getListProperty(
				StandardProperties.AUTHENTICATORS, String.class, "");
		List<String> extAuthClassNameList = wrappedProperties.getListProperty(
				BootProperties.EXTENSION_AUTHENTICATORS_PROPERTY, String.class,
				"");
		...
		for (String authClassName : allAuthClassNames) {
			authenticators.add(getAuthenticator(authClassName,
	...
	/**
	 * Helper method that filters properties, loading appropriate defaults if
	 * necessary.
	 */
	...
	/**
	 * Check for obvious errors in the properties file, logging and throwing an
	...
	private static void checkProperties(Properties appProperties) {
		...
		NodeType type = NodeType.valueOf(appProperties
				.getProperty(StandardProperties.NODE_TYPE));
		if (!type.equals(NodeType.coreServerNode)) {
	...
	/**
	 * This runnable calls the application's <code>initialize</code> method, if
	 * it hasn't been called before, to start the application for the first
	...
	private static final class AppStartupRunner extends AbstractKernelRunnable {
		...
		public void run() throws Exception {
			DataService dataService = Kernel.proxy
			...
			try {
				...
				AppListener listener = (new PropertiesWrapper(properties))
	...
	private final class KernelShutdownControllerImpl implements
				...
				if (isReady) {
					...
					if (caller instanceof WatchdogService) {
						...
						if (nodeId != -1 && watchdogSvc != null) {
							watchdogSvc.reportFailure(nodeId, caller.getClass()
									.toString());
						} else {
		 ...
		 * possible deadlock due to a service or component's doShutdown() method
		 * waiting for the thread it was issued from to shutdown. For example,
		 * the watchdog service's shutdown method would block if a Kernel
		 * shutdown was called from RenewThread.
		 */
	...
	/**
	 * This method is used to automatically determine an application's set of
	...
	private static Properties findProperties(String propLoc) throws Exception {
		...
		String extPropFile = System
				.getProperty(BootProperties.EXTENSION_FILE_PROPERTY);
		if (extPropFile != null) {
	 ...
	 * <p>
	 * If a single argument is given, the value of the argument is assumed to be
	 * a filename. This file is used in combination with additional
	 * configuration settings to determine an application's configuration
	 * properties.
	 * <p>
	 * The order of precedence for properties is as follows (from highest to
	 * lowest):
	 * <ol>
	 * <li>System properties specified on the command line using a "-D" flag</li>
	 * <li>Properties specified in the file from the user's home directory with
	 * the name specified by {@link BootProperties#DEFAULT_HOME_CONFIG_FILE}.</li>
	 * <li>Properties specified in the file given as a command line argument.</li>
	 * <li>Properties specified in the resource with the name
	 * {@link BootProperties#DEFAULT_APP_PROPERTIES} This file is typically
	 * included as part of the application jar file.</li>
	 ...
	 * then a default is used or an <code>Exception</code> is thrown (depending
	 * on whether a default value is available). Certain properties are required
	 * to be specified somewhere in the application's configuration.
	 * <p>
	 * See {@link StandardProperties} for the required and optional properties.
	 * 
	 * @param args
	 *            optional filename for <code>Properties</code> file associated

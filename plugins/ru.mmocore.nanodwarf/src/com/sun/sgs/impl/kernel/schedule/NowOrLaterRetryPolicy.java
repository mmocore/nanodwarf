 ...
 * restrictions after a certain number of task retries in order to push through
 * tasks that are frequently timing out. This {@code SchedulerRetryPolicy}
 * causes a task that throws a retryable exception to retry immediately.
 * However, once a task has been retried more than the configurable threshold
 * (see below), then the transaction timeout for the task is doubled, and the
 * task is scheduled to be retried later (rather than immediately). This class
 ...
 * number of times, then the task's transaction restrictions are loosened the
 * next time it runs and the task is scheduled to be retried later. This value
...
public class NowOrLaterRetryPolicy implements SchedulerRetryPolicy {
	...
	private static final LoggerWrapper logger = new LoggerWrapper(
			Logger.getLogger(NowOrLaterRetryPolicy.class.getName()));
	 ...
	 * retried later.
	 */
	static final String RETRY_BACKOFF_THRESHOLD_PROPERTY = "com.sun.sgs.impl.kernel.schedule.retry.backoff.threshold";
	...
	public NowOrLaterRetryPolicy(Properties properties) {
		...
		this.retryBackoffThreshold = wrappedProps.getIntProperty(
				...
				DEFAULT_RETRY_BACKOFF_THRESHOLD, 1, Integer.MAX_VALUE);
		...
		if ((result instanceof ExceptionRetryStatus)
			...
			if (task.getTryCount() > retryBackoffThreshold) {
				if (result instanceof TransactionTimeoutException
					...
					logger.logThrow(Level.WARNING, task.getLastFailure(),

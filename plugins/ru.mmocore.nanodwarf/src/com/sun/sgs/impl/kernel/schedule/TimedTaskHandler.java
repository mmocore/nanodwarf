...
class TimedTaskHandler {
	...
	/**
	 * The milliseconds in the future that a task must be to be accepted by the
	 * handler.
	 */
	...
	/**
	 * Creates an instance of <code>TimedTaskHandler</code>. This has the effect
	 * of creating a new <code>Timer</code> which involves creating at least one
	 * new thread.
	 * 
	 * @param listener
	 *            the <code>TimedTaskListener</code> that will consume the task
	 *            when its time comes, causing it to be executed
	 */
	...
	/**
	 * Tries to pass off a task to be run at some point in the future. If the
	 * task should be run directly instead of delayed, then it is rejected and
	 * the method returns <code>false</code>.
	 * 
	 * @param task
	 *            the <code>ScheduledTask</code> to run in the future
	 * 
	...
	boolean runDelayed(ScheduledTask task) {
		...
		if (task.getStartTime() < (System.currentTimeMillis() + FUTURE_THRESHOLD)) {
			return false;
		}
		...
		if (task.isRecurring()) {
			RecurringTaskHandleImpl handle = (RecurringTaskHandleImpl) (task

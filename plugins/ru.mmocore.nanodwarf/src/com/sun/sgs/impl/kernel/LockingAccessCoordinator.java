 ...
 * <p>
 * 
 * This implementation checks for deadlock whenever an access request is blocked
 * due to a conflict. It selects the youngest transaction as the deadlock
 * victim, determining the age using the originally requested start time for the
 * task associated with the transaction. The implementation does not deny
 * requests that would not result in deadlock. When requests block, it services
 * the requests in the order that they arrive.
 * <p>
 * 
 ...
 * the {@code com.sun.sgs.txn.timeout} property, if specified, otherwise times
 * the value of the default transaction timeout.
 * 
 * <dd style="padding-top: .5em">The maximum number of milliseconds to wait for
 * obtaining a lock. The value must be greater than {@code 0}, and should be
 * less than the transaction timeout.
 * <p>
 * 
 ...
 * and maps. The number of maps controls the amount of concurrency, and should
 * typically be set to a value to support concurrent access by the number of
 * active threads. The value must be greater than {@code 0}.
 * <p>
 * 
 * </dl>
 * <p>
 ...
 * {@code com.sun.sgs.impl.kernel.LockingAccessCoordinator} to log information
...
public class LockingAccessCoordinator extends AbstractAccessCoordinator {
	 ...
	 */
	public static final String LOCK_TIMEOUT_PROPERTY = CLASS + ".lock.timeout";
	 ...
	 */
	public static final String NUM_KEY_MAPS_PROPERTY = CLASS + ".num.key.maps";
	...
	private final ConcurrentMap<Transaction, LockerImpl> txnMap = new ConcurrentHashMap<Transaction, LockerImpl>();
	 ...
	 *             if the values of the configuration properties are illegal
	 */
	public LockingAccessCoordinator(Properties properties, TransactionProxy txnProxy,
			ProfileCollectorHandle profileCollectorHandle)
	{
		...
		long defaultLockTimeout = Math.max(1L,
		...
		int numKeyMaps = wrappedProps.getIntProperty(NUM_KEY_MAPS_PROPERTY,
				NUM_KEY_MAPS_DEFAULT, 1, Integer.MAX_VALUE);
	...
	public void notifyNewTransaction(Transaction txn, long requestedStartTime,
		...
		LockerImpl locker = new LockerImpl(lockManager, txn, requestedStartTime);
	...
	LockerImpl getLocker(Transaction txn) {
		...
		if (locker == null) {
			throw new IllegalArgumentException("Transaction not active: " + txn);
		}
		return locker;
	}

	/**
	 * Releases the locks for the transaction and reports object accesses to the
	 * profiling system.
	 * 
	 * @param txn
	...
	/**
	 * Define a locker that records information about the transaction requesting
	 * locks, and descriptions.
	 */
	public static class LockerImpl extends TxnLocker<Key> implements
			AccessedObjectsDetail {
		/** The lock requests made by this transaction. */
		private final List<AccessedObjectImpl> requests = new ArrayList<AccessedObjectImpl>();
		 ...
		 *            the time milliseconds that the task associated with the
		 *            transaction was originally requested to start
		 * @throws IllegalArgumentException
		 *             if {@code requestedStartTime} is less than {@code 0}
		 */
		LockerImpl(TxnLockManager<Key> lockManager, Transaction txn,
		 ...
		 */
		@Override
		protected LockRequest<Key> newLockRequest(Key key, boolean forWrite,
				boolean upgrade) {
			assert !getEnded();
			AccessedObjectImpl request = new AccessedObjectImpl(this, key,
					forWrite, upgrade);
		...
		public byte[] getConflictingId() {
			LockConflict<Key> conflict = getConflict();
			if (conflict != null) {
				LockerImpl conflictingLocker = (LockerImpl) conflict
		 ...
		 * description should not be {@code null}. Does not replace an existing
		 * description.
		 * 
		 * @param key
		 *            the key
		...
		Object getDescription(Key key) {
			return (keyToDescriptionMap == null) ? null : keyToDescriptionMap
					.get(key);
		}
	...
	private static class AccessedObjectImpl extends LockRequest<Key> implements
			AccessedObject {
		/**
		 * Creates an instance of this class.
		 * 
		 ...
		 */
		AccessedObjectImpl(LockerImpl locker, Key key, boolean forWrite,
				boolean upgrade) {
		...
		/**
		 * Two instances are equal if they are instances of this class, and have
		 * the same source, object ID, and access type.
		 * 
		 * @param object
		...
		public String toString() {
			return "AccessedObjectImpl["
					...
					+ getKey()
					+ ", "
					+ (getForWrite() ? "WRITE" : getUpgrade() ? "UPGRADE"
							: "READ") + "]";
		}
	}
	...
	private class AccessReporterImpl<T> extends AbstractAccessReporter<T> {
		 ...
		 * 
		 * @param source
		 *            the source of the objects managed by this reporter
		 */
		...
		/** {@inheritDoc} */
		public void reportObjectAccess(Transaction txn, T objectId,
			...
			LockConflict<Key> conflict = lockManager.lock(locker, key,
			...
			if (conflict != null) {
				...
				String accessMsg = "Access txn:" + txn + ", type:" + type
						+ ", source:" + source + ", objectId:" + objectId
						+ descriptionMsg + " failed: ";
				LockerImpl conflictingLocker = (LockerImpl) conflict
						.getConflictingLocker();
				...
				case TIMEOUT:
					exception = new TransactionTimeoutException(accessMsg
							+ "Transaction timed out" + conflictMsg);
					break;
				case DENIED:
					exception = new TransactionConflictException(accessMsg
							+ "Access denied" + conflictMsg);
					break;
				case INTERRUPTED:
					exception = new TransactionInterruptedException(accessMsg
							+ "Transaction interrupted" + conflictMsg);
					break;
				case DEADLOCK:
					exception = new TransactionConflictException(accessMsg
							+ "Transaction deadlock" + conflictMsg);
					break;
				default:
					throw new AssertionError("Should not be "
							+ conflict.getType());
				}
				txn.abort(exception);
				throw exception;
		...
		/** {@inheritDoc} */
		public void setObjectDescription(Transaction txn, T objectId,
				Object description) {
			LockerImpl locker = getLocker(txn);
			if (description == null) {
				checkNull("objectId", objectId);
	 ...
	 * after the transaction completes. Use a listener instead of a transaction
	 * participant to make sure that locks are released only after all of the
	 * transaction participants have finished their work.
	 */
	private class TxnListener implements TransactionListener {
		 ...
		 * 
		 * @param txn
		 *            the transaction we're listening for
		 */

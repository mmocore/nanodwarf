 ...
 * <code>Transaction</code> and its owner. Because <code>Service</code>s are the
 * only components outside of the kernel that should have visibility into this
 * state, they are the only components in the system that are provided a
 * reference to the usable proxy.
 */
final class TransactionProxyImpl implements TransactionProxy {

	/**
	 * This package-private constructor creates a new instance of
	 * <code>TransactionProxyImpl</code>. It is package-private so that only the
	 * kernel components can create instances for access to the current

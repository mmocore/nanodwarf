...
/**
 * This implementation of <code>SchedulerQueue</code> tries to provide fairness
 * between users without taking too long to service any given user's tasks. This
 * is typified by the phrase "no one gets seconds until everyone who wants them
 * has had firsts."
 * <p>
 * The implementation uses a priority queue, keyed off what is called a window
 * value. Users submit tasks into increasing window values, and may never submit
 * tasks into windows that have already passed.
 */
public class WindowSchedulerQueue implements SchedulerQueue, TimedTaskListener {

	// logger for this class
	private static final LoggerWrapper logger = new LoggerWrapper(
			Logger.getLogger(WindowSchedulerQueue.class.getName()));
	 ...
	 * 
	 * @param properties
	 *            the application <code>Properties</code>
	 */
	...
	public void timedTaskReady(ScheduledTask task) {
		...
		synchronized (user) {
			...
			scheduledWindow = (currentWindow > user.nextWindow) ? currentWindow
					: user.nextWindow;
	...
	private static class QueueElement implements Comparable<QueueElement> {
		...
		public boolean equals(Object o) {
			...
			return ((window == other.window) && (timestamp == other.timestamp));
		}

...
/**
 * This class contains the standard property keys that the kernel looks for (and
 * may provide) on startup.
 * <p>
 ...
 * set the associated behavior for that application. If no value is provided for
 * a given key, then the default or system-provided value is used. Note that
 * some keys are required to have values, specifically <code>APP_NAME</code>,
 * <code>APP_ROOT</code> and <code>APP_LISTENER</code>.
 * <p>
...
public final class StandardProperties {
	 ...
	 */
	public static final String NODE_MAPPING_SERVICE = NS + "nodeMappingService";
	/**
	 * An optional key specifying a specific class to use for the
	 ...
	 */
	public static final String CLIENT_SESSION_SERVICE = NS
	 ...
	 * <code>SERVICES</code>. Each manager in this list will be paired with the
	 * corresponding <code>Service</code> from the <code>SERVICES</code> list.
	 * To specify a <code>Service</code> with no manager, leave the appropriate
	 * element in the list empty. E.g.:
	 * 
	 * <pre>
	 *    -DSERVICES=S1:S2:S3 -DMANAGERS=M1::M3
	 * </pre>
	 * 
	 * This specifies three <code>Service</code>s where S2 has no associated
	 * manager.
	 */
	 ...
	 * should be started on. Each item in this list must be set to a value in
	 * {@link ServiceNodeTypes} and is associated with the <code>Service</code>/
	 * <code>Manager</code> at the same respective index in the
	 * <code>SERVICES</code> and <code>MANAGERS</code> lists. If this property
	 * is omitted, all configured additional services will default to
	...
	public enum ServiceNodeTypes {
		...
		/**
		 * Returns {@code true} if this node types identifier indicates that the
		 * associated service should be started on the specified node type.
		 * 
		 * @param nodeType
		 *            the current {@code NodeType}
		...
		public boolean shouldStart(NodeType nodeType) {
			switch (nodeType) {
			case singleNode:
				return equals(SINGLE) || equals(SINGLE_OR_CORE)
						|| equals(SINGLE_OR_APP) || equals(ALL);
			case coreServerNode:
				return equals(CORE) || equals(SINGLE_OR_CORE)
						|| equals(CORE_OR_APP) || equals(ALL);
			case appNode:
				return equals(APP) || equals(SINGLE_OR_APP)
						|| equals(CORE_OR_APP) || equals(ALL);
			default:
				return false;
			}
	...
	/**
	 * An enumeration of the known, standard {@code Service}s. The ordering
	 * represents the order in which the services are started. Each
	 * {@link NodeType} will start up each service in the order specified up to
	 * and including the last service configured for that particular
	 * {@code NodeType}. The last service is respectively specified for each
	 * node with the {@code LAST_APP_SERVICE}, {@code LAST_SINGLE_SERVICE}, and
	 * {@code LAST_CORE_SERVICE} attributes. Any additional configured services
	 * will then be started after this last service. Care should therefore be
	 * taken for additional services to only depend on standard services that
	 * are started on a particular {@code NodeType}.
	 */
	public enum StandardService {
		...
		/**
		 * The last service that gets configured for a {@code coreServerNode}.
		 */
	 ...
	 * <code>IdentityAuthenticator</code>s to use for the application. The order
	 * here is used to define order of precedence when authenticating an
	 * identity.
	 */
	public static final String AUTHENTICATORS = NS + "app.authenticators";

	/**
	 * An optional property that specifies the type of node being started. It
	 * must be set to a value in {@link NodeType} and defaults to
	 * {@code singleNode}. The value of this property may cause other property
	 * settings to be overridden.
	 * <p>
	 * In particular, setting this property to {@code coreServerNode} causes the
	 * following property to be set:
	 * <ul>
	 * <li>
	 * {@code com.sun.sgs.impl.service.data.DataServiceImpl.data.store.class}
	 * set to {@code com.sun.sgs.impl.service.data.store.net.DataStoreClient} to
	 * indicate the multi-node data service should be used
	 * 
	 * </ul>
	 * <p>
	...
	/**
	 * The property for specifying the maximum length of time, in milliseconds,
	 * for a client session to relocate to a new node.
	 */

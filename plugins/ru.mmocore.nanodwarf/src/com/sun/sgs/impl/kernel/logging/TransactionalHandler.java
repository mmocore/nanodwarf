...
/**
 * A {@code Handler} implementation that given a backing {@code Handler},
 * provides transactional semantics for the {@code #publish(LogRecord)} method.
 * This class is used by the {@link TransactionAwareLogManager} to wrap existing
 * handlers as specificed by the application.
 * 
 * @see com.sun.sgs.impl.kernel.logging.TransactionAwareLogManager
 */
public class TransactionalHandler extends Handler implements
		NonDurableTransactionParticipant {

	/**
	 * A mapping from transaction to the list of records waiting to be published
	 * on transaction commit.
	 */
	private final ConcurrentMap<Transaction, Queue<LogRecord>> bufferedRecords;

	/**
	 * The proxy used to join transactions upon the first
	 * {@link #publish(LogRecord)} call for that transaction.
	 */
	private final TransactionProxy proxy;

	/**
	 * The backing handler for this instance. All modification calls are passed
	 ...
	 * {@code proxy} for joining transactions a the backing handler for
	 * performing the actual logging.
	 * 
	 * @param proxy
	 *            the proxy used to join transactions
	 * @param backingHandler
	 *            the handler used to perform the actual logging at commit time
	 * 
	 * @throws NullPointerException
	 *             if the {@code backingHandler} or {@code proxy} is
	 *             {@code null}.
	 */
	...
	/**
	 * Removes any buffered records for this transaction and performs no
	 * logging.
	 * 
	 * @param txn
	 *            the failed transaction that in the past performed some logging
	 *            operation
	 */
	...
	/**
	 * Removes any buffered records for this transaction and logs all buffered
	 * records using the backing {@code Handler}.
	 * 
	 * @param txn
	 *            the successful transaction that in the past performed some
	 *            logging operation
	 */
	...
	/**
	 * Returns the {@code Handler} used to publish all buffered
	 * {@code LogRecord}s when a transaction commmits.
	 * 
	...
	/**
	 * Returns the class name of the {@code Handler} that this instance wraps.
	 * 
	...
	/**
	 * Joins the current transaction and buffers the provided record for later
	 * publication until transaction commit time.
	 * 
	 * @param record
	 *            the record to be buffer and later publication upon transaction
	 *            success.
	 */

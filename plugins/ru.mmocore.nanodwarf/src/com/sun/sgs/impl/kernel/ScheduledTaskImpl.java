 ...
 * associated task can be cancelled or waited on to finish. The resulting value
 * provided by a call to {@code get} is {@code null} if the task completed
 * successfully, or the {@code Throwable} that caused the task to fail
 * permanently in the case of failure. See the documentation on the associated
 * methods for more details on how interruption is handled.
 * <p>
 * Note: This class is located in the {@code com.sun.sgs.impl.kernel} package
 * instead of the {@code com.sun.sgs.impl.kernel.schedule} package because it
 * needs to be a package-private implementation for use by the schedulers.
 */
final class ScheduledTaskImpl implements ScheduledTask {
	...
	/**
	 * We use the builder pattern here to avoid constructor explosion. This
	 * builder takes three required parameters, {@code task}, {@code owner}, and
	 * {@code priority} and then provides setter methods for each of the
	 * remaining optional parameters for building a {@code ScheduledTaskImpl}
	 * object.
	 * <p>
	 * Example usage:
	 * <p>
	 * 
	 * <pre>
	 * ScheduledTaskImpl task = new ScheduledTaskImpl.Builder(task, owner, priority)
	 * 		.period(1000).build();
	 * </pre>
	 */
	static class Builder {
		 ...
		 * 
		 * @param task
		 *            existing {@code ScheduledTaskImpl}
		 */
		...
		/**
		 * Set the default value of {@code timeout} for new instances of
		 * {@code ScheduledTaskImpl} built with a builder.
		 * 
		 * @param timeout
		...
		/**
		 * Builds a {@code ScheduledTaskImpl} object from the attributes in this
		 * builder.
		 * 

...
/**
 * This is the implementation of <code>KernelAppContext</code> used by the
 * kernel to manage the context of a single application. It knows the name of an
 * application, its available managers, and its backing services.
 * 
 * FIXME: the context should check that it isn't shutdown before handing out
...
class KernelContext {
	...
	private static final LoggerWrapper logger = new LoggerWrapper(
			Logger.getLogger(KernelContext.class.getName()));
	...
	/**
	 * Creates an instance of <code>KernelContext</code> based on the components
	 * that have already been collected in another instance.
	 * 
	 * @param context
	 *            the existing <code>KernelContext</code> to use as a source of
	 *            components
	 */
	...
	/**
	 * Creates an instance <code>KernelContext</code> with the given name and
	 * components. This is used by sub-classes to create an instance that can be
	 * provided components as needed by adding to the provided
	 ...
	 * <code>ManagerNotFoundException</code> is thrown. This may be used to find
	 * any available manager, including the three standard managers.
	 * 
	 * @param type
	 ...
	 * unknown, or if there is more than one <code>Service</code> of the given
	 * type, <code>MissingResourceException</code> is thrown. This is the only
	 * way to resolve service components directly, and should be used with care,
	 * as <code>Service</code>s should not be resolved and invoked directly
	...
	/**
	 * Shut down all the service components in the reverse order that they were
	 * added.
	 */
	...
	/**
	 * Returns a unique representation of this context, in this case the name of
	 * the application.
	 * 

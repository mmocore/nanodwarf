...
/**
 * Package-private implementation of {@code TransactionScheduler} that is used
 * by the system for scheduling and running all transactional tasks. This class
 ...
 * to define which queue implementation should back this scheduler. The value of
 * this property should be the name of a public, non-abstract class that
 * implements the {@link SchedulerQueue} interface, and that provides a public
 ...
 * 
 * <dd style="padding-top: .5em">The implementation class used to define which
 * retry policy implementation to use when tasks fail or abort. The value of
 * this property should be the name of a public, non-abstract class that
 * implements the {@link SchedulerRetryPolicy} interface, and that provides a
 * public constructor with the parameters {@link Properties}
 * <p>
 * 
 * </dl>
 */
final class TransactionSchedulerImpl implements TransactionScheduler,
		PriorityScheduler, ProfileListener {

	// logger for this class
	private static final LoggerWrapper logger = new LoggerWrapper(
			Logger.getLogger(TransactionSchedulerImpl.class.getName()));

	/**
	 * The property used to define which queue implementation should back this
	 * scheduler.
	 */
	...
	/**
	 * The property used to define which retry policy should be used in this
	 * scheduler
	 */
	...
	private static final Priority defaultPriority = Priority
			.getDefaultPriority();
	 ...
	 * 
	 * @param properties
	 *            the {@code Properties} for the system
	 * @param transactionCoordinator
	 *            the {@code TransactionCoordinator} used by the system to
	 *            manage transactions
	 * @param profileCollectorHandle
	 *            the {@code ProfileCollectorHandler} used to manage collection
	 *            of per-task profiling data
	 * @param accessCoordinator
	 *            the {@code AccessCoordinator} used by the system to managed
	 *            shared data
	 * 
	 * @throws InvocationTargetException
	 *             if there is a failure initializing the {@code SchedulerQueue}
	 * @throws Exception
	 *             if there is any failure creating the scheduler
	 */
	TransactionSchedulerImpl(Properties properties,
			TransactionCoordinator transactionCoordinator,
			ProfileCollectorHandle profileCollectorHandle,
			AccessCoordinatorHandle accessCoordinator) throws Exception {
		...
		this.requestedThreads = Integer.parseInt(properties.getProperty(
				CONSUMER_THREADS_PROPERTY, DEFAULT_CONSUMER_THREADS));
		...
		ScheduledTaskImpl.Builder.setDefaultTimeout(transactionCoordinator
				.getDefaultTimeout());
	 ...
	 * 
	 * @param kernelContext
	 *            the {@code KernelContext} for this scheduler
	 */
	...
	public TaskReservation reserveTask(KernelRunnable task, Identity owner) {
		ScheduledTaskImpl t = new ScheduledTaskImpl.Builder(task, owner,
				defaultPriority).build();
	...
	public TaskReservation reserveTask(KernelRunnable task, Identity owner,
			long startTime) {
		ScheduledTaskImpl t = new ScheduledTaskImpl.Builder(task, owner,
				defaultPriority).startTime(startTime).build();
		return backingQueue.reserveTask(t);
	}
	...
	public void scheduleTask(KernelRunnable task, Identity owner) {
		backingQueue.addTask(new ScheduledTaskImpl.Builder(task, owner,
				defaultPriority).build());
	}
	...
	public RecurringTaskHandle scheduleRecurringTask(KernelRunnable task,
			Identity owner, long startTime, long period) {
		ScheduledTaskImpl scheduledTask = new ScheduledTaskImpl.Builder(task,
				owner, defaultPriority).startTime(startTime).period(period)
				.build();
		RecurringTaskHandle handle = backingQueue
				.createRecurringTaskHandle(scheduledTask);
	...
	/**
	 * Private method that blocks until the task has completed, re-throwing any
	 * exception resulting from the task failing.
	 */
	private void waitForTask(ScheduledTaskImpl task) throws Exception {
		Throwable t = null;
	...
	/**
	 * Package-private method that runs the given task in a transaction that is
	 * not bound by any timeout value (i.e., is bound only by the
	 * {@code com.sun.sgs.txn.timeout.unbounded} property value).
	 * 
	 * @param task
	 *            the {@code KernelRunnable} to run transactionally
	 * @param owner
	 *            the {@code Identity} that owns the task
	 * 
	 * @throws IllegalStateException
	 *             if this method is called from an actively running transaction
	 * @throws Exception
	 *             if there is any failure that does not result in re-trying the
	 *             task
	 */
	void runUnboundedTask(KernelRunnable task, Identity owner) throws Exception {
		...
		ScheduledTaskImpl scheduledTask = new ScheduledTaskImpl.Builder(task,
				owner, defaultPriority).timeout(ScheduledTask.UNBOUNDED)
				.build();
		waitForTask(scheduledTask);
	}

	/**
	 * Notifies the scheduler that a thread has been started to consume tasks as
	 * they become ready.
	 */
	 ...
	 * from the {@code SchedulerQueue}. Once started, it will continue running
	 * until it catches an {@code InterruptedException}.
	 */
		...
		public void run() {
			...
			try {
				while (true) {
					...
					ScheduledTaskImpl task = (ScheduledTaskImpl) (backingQueue
							.getNextTask(true));
					...
					if (executeTask(task, true)) {
						...
						if (task.isRecurring()) {
							long nextStart = task.getStartTime()
									+ task.getPeriod();
						...
						TaskQueueImpl queue = (TaskQueueImpl) (task
								.getTaskQueue());
						if (queue != null) {
	 ...
	 * {@code retryOnInterruption} is {@code true} and always re-throws the
	 * associated {@code InterruptedException}.
	 * <p>
	...
	private boolean executeTask(ScheduledTaskImpl task,
			boolean retryOnInterruption) throws InterruptedException {
		...
		try {
			...
			while (true) {
				...
				int waitSize = backingQueue.getReadyCount()
						+ dependencyCount.get();
				profileCollectorHandle.startTask(task.getTask(),
						task.getOwner(), task.getStartTime(), waitSize);
				...
				try {
					...
					TransactionHandle handle = transactionCoordinator
							.createTransaction(task.getTimeout());
					...
					try {
						...
						profileCollectorHandle.noteTransactional(transaction
								.getId());
						accessCoordinator.notifyNewTransaction(transaction,
								task.getStartTime(), task.getTryCount());
	...
	private final class TaskQueueImpl implements TaskQueue {
		...
		public void addTask(KernelRunnable task, Identity owner) {
			ScheduledTaskImpl schedTask = new ScheduledTaskImpl.Builder(task,
					owner, defaultPriority).build();

...
/**
 * Package-private implementation of {@code TaskScheduler} that is used by the
 * system scheduling and running all non-transactional, arbitrary-length tasks.
 * This is an intentionally simple implementation that uses a backing
 * {@code Executor} instead of a {@code SchedulerQueue} until there is better
 * understanding of what (if any) custom scheduling behavior will help these
 * kinds of tasks.
 * <p>
 ...
 * <p>
 * </dl>
 * FIXME: the profiling code needs a way to learn about the thread count from
 * this scheduler separately from the transaction pool. When this gets added,
 * this class should start tracking thread counts.
 */
final class TaskSchedulerImpl implements TaskScheduler {

	// logger for this class
	private static final LoggerWrapper logger = new LoggerWrapper(
			Logger.getLogger(TaskSchedulerImpl.class.getName()));
	...
	TaskSchedulerImpl(Properties properties,
			ProfileCollectorHandle profileCollectorHandle) throws Exception {
		...
		int requestedThreads = Integer.parseInt(properties.getProperty(
		...
		this.executor = Executors.newScheduledThreadPool(requestedThreads,
				new NamedThreadFactory("TaskScheduler"));
	...
	private class TaskReservationImpl implements TaskReservation {
		 ...
		 *             if the system has become too overloaded to honor this
		 *             reservation
		 */
		public void use() {
			synchronized (this) {
			...
			try {
				...
				throw new TaskRejectedException("The system has run out of "
						+ "resources and cannot run " + "the requested task",
						ree);
			}
		}
	}
			...
			try {
				...
				if (logger.isLoggable(Level.WARNING)) {
					if (taskDetail.isRecurring()) {
						...
						logger.logThrow(Level.WARNING, e, "failed to run "
								+ "recurrence of task {0}", taskDetail.task);
					}
				}
			} finally {

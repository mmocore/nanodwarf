 ...
 * {@code Logger} instances in the application's namespace. This class will
 * either infer the application's name space from the package used by the main
 * {@code AppListener}, or it can be specified manually by the following
 * property:
 * 
 * <p>
 * <dl style="margin-left: 1em">
 * 
 * <dt><i>Property:</i> <b> {@code com.sun.sgs.logging.app.namespace} </b><br>
 * <i>Default:</i> the package named used in the
 * {@code com.sun.sgs.app.listener} property.
 * 
 ...
 * must set the {@code java.util.logging.manager} system property to
 * {@code com.sun.sgs.impl.kernel.logging.TransactionAwareLogManager} prior to
 * JVM start up.
 * 
 * <p>
 * 
...
public final class TransactionAwareLogManager extends LogManager {
	 ...
	 */
	private static final String APP_NAMESPACE_PROPERTY = PROPERTIES_PREFIX
			+ ".app.namespace";
	 ...
	 * configured. For this reason, we keep this list to later reconfigure them
	 * when the {@code TransactionProxy} is available.
	 */
	 ...
	 * the {@code TransactionProxy} to add transactional semantics to any of the
	 * specified application {@code Logger} instances.
	 * 
	 * <p>
	 * 
	 * Note that prior to this call, the application's namespace is not known so
	 * any {@code Logger} instances will be of type {@code TransactionalLogger},
	 * but will be configured as non-transactional. After this call, any of
	 * these instances that were in the application's namespace will be
	 * transactional.
	 * 
	 * @param properties
	 *            the properties for configuring this component
	 * @param txnProxy
	...
	public synchronized void configure(Properties properties,
		...
		String defaultAppNamespace = appListener.substring(0,
				lastDotBeforeClass);
	...
	public synchronized Logger getLogger(String name) {
		...
		if (result == null) {
			...
			result = (!configured || name.startsWith(appNamespace)) ? new TransactionalLogger(
					name, null, txnProxy) : new SimpleLogger(name, null);
	 ...
	 * {@code Logger} instances in the
	 * {@link TransactionAwareLogManager#getLogger(String)} method.
	 */
	 ...
	 * constructed with a valid, non-{@code null} instance of a
	 * {@link TransactionProxy}, or if they are configured after construction
	 * with a valid, non-{@code null} instance of a {@code TransactionProxy}.
	 * 
	 * <p>
	 * 
	 ...
	 * {@link Handler} at run-time will function as expected and will not change
	...
	private static final class TransactionalLogger extends Logger {
		 ...
		 * TransactionAwareLogManager#configure(Properties,TransactionProxy} has
		 * been called, this will be {@code null}.
		 */
		private TransactionProxy txnProxy;

		/**
		 * Constructs a {@code TransactionalLogger} that will have transactional
		 * semantics if {@code txnProxy} is valid and non-{@code null}.
		 * 
		 * @param name
		 *            the name of this logger
		 * @param resourceBundleName
		 *            the name of a {@link ResourceBundle} to be used for
		 *            localizing messages for this logger. May be {@code null}
		 *            if none of the messages require localization.
		 * @param txnProxy
		 *            the {@code TransactionProxy} used to join the current
		 *            transaction when a report is logged.
		 */
		public TransactionalLogger(String name, String resourceBundleName,
				TransactionProxy txnProxy) {
			super(name, resourceBundleName);
			this.txnProxy = txnProxy;
		}
		 ...
		 * {@inheritDoc}
		 * 
		 * If the provided {@code Handler} is an instance of
		 * {@link TransactionalHander} it will be added immediately. Otherwise,
		 * the provided {@code Handler} will be wrapped by a
		 * {@code TransactionalHandler} and that handler will be added instead.
		 */
		public void addHandler(Handler handler) {
			if (!(handler instanceof TransactionalHandler)) {
				...
				if (txnProxy == null) {
					...
					// semantics
					super.addHandler(new TransactionalHandler(txnProxy, handler));
				}
			} else {
		...
		/**
		 * Configures this {@code Logger} with the provided
		 * {@code TransactionProxy} so that all messages logged will have
		 * transaction semantics. This method is used for reconfiguring loggers
		 * that were created prior to the {@link TransactionAwareLogManager}
		 * being configured.
		 * 
		 * @param txnProxy
		 *            the {@code TransactionProxy} used to join the current
		...
		/**
		 * Sets the parent of this {@code Logger} and calls
		 * {@link #attachParentHandlers()} if this logger has been configured to
		 * be transactional and does not yet have any handlers.
		 * 
		 * @param {@inheritDoc}
		 */
		 ...
		 * transaction semantics, or otherwise returns the default
		 * {@link Logger#toString()}.
		 * 
		 * @return a description of this logger
		 */
		public String toString() {
			return (txnProxy == null) ? "Non-transactional Logger:" + getName()

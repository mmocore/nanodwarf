...
/**
 * Simple implementation of <code>RecurringTaskHandle</code> that lets the
 * handle be associated with a <code>TimerTask</code> so that cancelling the
 * handle also cancels the associated <code>TimerTask</code>.
 */
class RecurringTaskHandleImpl implements RecurringTaskHandle {
	 ...
	 * 
	 * @param queue
	 *            the <code>SchedulerQueue</code> that is using this handle
	 * @param task
	 *            the task for this handle
	 * 
	 * @throws IllegalArgumentException
	 *             if the task is not recurring
	 */
	public RecurringTaskHandleImpl(SchedulerQueue queue, ScheduledTask task) {
		if (queue == null) {
	...
	/**
	 * Sets the associated <code>TimerTask</code> for this handle. This method
	 * may be called any number of times on a handle. Typically a recurring task
	 * will re-set the associated <code>TimerTask</code> with each recurrence of
	 * execution.
	 * 
	 * @param timerTask
	 *            the associated <code>TimerTask</code>
	 */
	 ...
	 * <code>TimerTask</code> and notify the <code>SchedulerQueue</code> about
	 * the task being cancelled. If this handle has already been cancelled, then
	 * an exception is thrown.
	 * 
	 * @throws IllegalStateException
	 *             if this handle has already been cancelled
	 */
	public void cancel() {
		synchronized (this) {
			if (cancelled) {
	 ...
	 * <code>SchedulerQueue</code> via a call to <code>addTask</code>. If this
	 * handle has already been started or cancelled, then an exception is
	 * thrown.
	 * 
	 * @throws IllegalStateException
	 *             if the handle has already been started or cancelled
	 */
	public void start() {
		synchronized (this) {

...
final class ContextResolver {
	...
	private static ThreadLocal<KernelContext> context = new ThreadLocal<KernelContext>();

	// the current owner of any given thread
	private static ThreadLocal<Identity> currentOwner = new ThreadLocal<Identity>();
	...
	/**
	 * Private constructor used to ensure that no one constructs an instance of
	 * this class.
	 */
	 ...
	 *             if there is no available {@code ChannelManager} in this
	 *             context
	 */
	 ...
	 * 
	 * @throws IllegalStateException
	 *             if there is no available {@code DataManager} in this context
	 */
	 ...
	 * 
	 * @throws IllegalStateException
	 *             if there is no available {@code TaskManager} in this context
	 */
	 ...
	 * @param type
	 *            the {@code Class} of the requested manager
	 * 
	 * @return the matching manager
	 * 
	 * @throws ManagerNotFoundException
	 *             if no manager is found that matches the given type
	 * @throws IllegalStateException
	 *             if there are no available managers in this context
	 */
	...
	/**
	 * Package-private method used to set task state. This is called each time a
	 * task is run through a {@code Scheduler}
	 * 
	 * @param ctx
	 *            the {@code KernelContext} for the current task
	 * @param owner
	 *            the {@code Identity} that owns the current task
	 */
	...
	/**
	 * Returns the current <code>Transaction</code>, throwing an exception if
	 * there is no currently active transaction or if the currently active
	...
	/**
	 * Returns {@code true} if there is a current transaction, no matter the
	 * state of that transaction.
	 * 
	...
	/**
	 * Sets the current <code>Transaction</code>, throwing an exception if there
	 * is already an active transaction.
	 * 
	 * @param transaction
	 *            the <code>Transaction</code> to set as currently active
	 * 
	 * @throws IllegalStateException
	...
	/**
	 * Clears the currently active <code>Transaction</code>. This is typically
	 * done once a transaction has finished.
	 * 
	 * @param transaction
	 *            the <code>Transaction</code> to clear, which must be the
	 *            currently active transaction
	 * 
	 * @throws IllegalStateException
	 *             if there is no currently active transaction to clear
	 * @throws IllegalArgumentException
	 *             if the given <code>Transaction</code> is not the active
	 *             transaction
	 */

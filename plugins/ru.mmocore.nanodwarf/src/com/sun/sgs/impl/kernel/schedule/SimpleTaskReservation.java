...
/**
 * Package-private utility implementation of <code>TaskReservation</code>. These
 * reservations are fairly light-weight, and assume that the queue is unbounded
 * and therefore doesn't actually track these reservations or use them to
 * actually reserve any space.
 */
class SimpleTaskReservation implements TaskReservation {
	 ...
	 * 
	 * @param queue
	 *            the associated <code>SchedulerQueue</code>
	 * @param task
	 *            the <code>ScheduledTask</code> being reserved
	 */
	public SimpleTaskReservation(SchedulerQueue queue, ScheduledTask task) {
		if (queue == null) {

 ...
 * ordering is used to determine precidence when more than one authenticator can
 * consume the same credentials.
 * <p>
 * This implementation is kept private to this package so that the context can
 * be assigned, safely, after the coordinator is created. An instance of the
...
class IdentityCoordinatorImpl implements IdentityCoordinator {
	...
	private static final LoggerWrapper logger = new LoggerWrapper(
			Logger.getLogger(IdentityCoordinatorImpl.class.getName()));
	...
	/**
	 * Creates an instance of <code>IdentityCoordinatorImpl</code> that has the
	 * given <code>IdentityAuthenticators</code> available to authenticate
	 * identities. The order of the authenticators determines precidence when
	 * more than one authenticator supports a given credentials type.
	 * 
	 * @param authenticators
	 *            the available <code>IdentityAuthenticator</code>s
	 */
	public IdentityCoordinatorImpl(List<IdentityAuthenticator> authenticators) {
		...
		for (IdentityAuthenticator authenticator : authenticators) {
			String[] identifiers = authenticator.getSupportedCredentialTypes();
			for (String identifier : identifiers) {
				List<IdentityAuthenticator> list = authenticatorMap
						.get(identifier);
				if (list == null) {
	 ...
	 *             if the authenticator for the credentials is unknown
	 */
	public Identity authenticateIdentity(IdentityCredentials credentials)
		...
		List<IdentityAuthenticator> authenticators = authenticatorMap
				.get(credentials.getCredentialsType());
		if (authenticators == null) {
			...
			try {
				...
				if (logger.isLoggable(Level.FINEST)) {
					logger.logThrow(Level.FINEST, le, "Could not "
							...
							+ "authenticator {0}", authenticator.getClass()
							.getName());
				}
			}
		}

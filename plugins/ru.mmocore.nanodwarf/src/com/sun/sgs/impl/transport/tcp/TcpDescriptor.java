...
class TcpDescriptor implements TransportDescriptor, Serializable {
	 ...
	 *            host name
	 * @param listeningPort
	 *            port transport is listening on
	 */
	 ...
	 * {@inheritDoc}
	 * 
	 * This method will return a {@code byte} array that contains the following
	 * data:
	 * <ul>
	 * <li>(String) hostname
	 * <li>(int) port
	...
	public byte[] getConnectionData() {
		MessageBuffer buf = new MessageBuffer(
				MessageBuffer.getSize(hostName) + 4);
		buf.putString(hostName).putInt(listeningPort);

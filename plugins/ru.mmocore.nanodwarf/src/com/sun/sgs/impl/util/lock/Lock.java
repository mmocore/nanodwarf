...
final class Lock<K> {
	...
	/**
	 * The requests that currently own this lock. Use a small initial size,
	 * since the number of owners is typically small.
	 */
	private final List<LockRequest<K>> owners = new ArrayList<LockRequest<K>>(2);

	/**
	 * The requests that are waiting for this lock. Use a small initial size,
	 * since the number of owners is typically small.
	 */
	private final List<LockRequest<K>> waiters = new ArrayList<LockRequest<K>>(
			2);
	 ...
	 * Otherwise, returns a {@code LockAttemptResult} containing the
	 * {@link LockRequest}, with its {@code conflict} field set to {@code null}
	 * if the lock was acquired or a conflicting transaction if the lock could
	 * not be obtained. Adds the locker to the waiters list if the lock was not
	 ...
	 * @param waiting
	 *            whether the locker is known to be a waiter
	 * @return a {@code LockAttemptResult} or {@code null}
	 */
	LockAttemptResult<K> lock(Locker<K> locker, boolean forWrite,
			boolean waiting) {
		...
		if (!waiters.isEmpty()) {
			...
			for (Iterator<LockRequest<K>> i = waiters.iterator(); i.hasNext();) {
				...
				if (locker == waiterRequest.getLocker()) {
					if (forWrite != waiterRequest.getForWrite()) {
						 ...
						 * case probably only happens in practice due to network
						 * failures since the caching data store does not make
						 * such requests otherwise. This case is obscure enough
						 * that it is not worth supporting more complex waiters
						 * cases -- just reject the new request.
						 */
						conflict = locker;
	...
	/**
	 * Adds a lock request to the list of requests waiting for this lock. If
	 * this is an upgrade request, puts the request after any other upgrade
	 * requests, but before other requests. Otherwise, puts the request at the
	 * end of the list.
	 */
	private void addWaiter(LockRequest<K> request) {
		if (waiters.isEmpty() || !request.getUpgrade()) {
			...
			/*
			 * Add upgrade requests after any existing ones, but before other
			 * requests.
			 */
			boolean added = false;
	 ...
	 * 
	 * @param locker
	 *            the locker whose ownership will be released
	 * @param downgrade
	 *            whether to downgrade ownership from write to read rather than
	...
	List<Locker<K>> release(Locker<K> locker, boolean downgrade) {
		...
		if (logger.isLoggable(FINEST)) {
			logger.log(FINEST, "release {0}, downgrade:{1}, {2}", locker,
					downgrade, this);
		}
		boolean owned = false;
		for (Iterator<LockRequest<K>> i = owners.iterator(); i.hasNext();) {
			...
			if (locker == ownerRequest.getLocker()) {
				if (!downgrade || ownerRequest.getForWrite()) {
					...
					owned = true;
					if (downgrade) {
						owners.add(locker.newLockRequest(ownerRequest.getKey(),
								false, false));
					}
				}
				break;
		...
		if (owned && !waiters.isEmpty()) {
			...
			for (int i = 0; i < waiters.size(); i++) {
				...
				if (waiter.getLocker() == locker) {
					 ...
					 * for a multi-locker where waiting and releasing can happen
					 * concurrently.
					 */
					...
					LockAttemptResult<K> result = lock(waiter.getLocker(),
							waiter.getForWrite(), true);
					if (logger.isLoggable(FINEST)) {
						logger.log(FINEST,
	...
	/**
	 * Validates the state of this lock, given that it is known to be in use.
	 * <p>
	 * 
	 * Locks that are in use must satisfy the following consistency constraints:
	 * <ul>
	 * <li>at least one owner
	 * <li>if more than one owner, they are only read owners
	 * <li>all upgrade waiters precede other waiters
	 * <li>locker appears once in owners and waiters <em>except</em> that an
	 * upgrade request can be a waiter if a read request is an owner
	...
	private boolean validateInUse() {
		...
		for (int i = 0; i < numWaiters - 1; i++) {
			...
			if (!waiter.getUpgrade()) {
				...
				throw new AssertionError("Upgrade waiter follows non-upgrade: "
						+ this + ", waiters: " + waiters);
			}
			for (int j = i + 1; j < numWaiters; j++) {
				LockRequest<K> waiter2 = waiters.get(j);
				if (waiter.getLocker() == waiter2.getLocker()) {
					throw new AssertionError("Locker waits twice: " + this
							+ ", " + waiter + ", " + waiter2);
				}
			}
			...
			if (waiter.getUpgrade() && !foundOwner) {
				throw new AssertionError("Waiting for upgrade but not owner: "
						+ this + ", waiter: " + waiter);
			}
		}
		return true;
	...
	/**
	 * Returns whether this lock is currently in use, or whether it is not in
	 * use and can be removed from the lock table. Locks are in use if they have
	 * any owners or waiters.
	 */
	...
	void flushWaiter(Locker<K> locker) {
		assert locker.lockManager.checkKeySync(key);
		for (Iterator<LockRequest<K>> iter = waiters.iterator(); iter.hasNext();) {

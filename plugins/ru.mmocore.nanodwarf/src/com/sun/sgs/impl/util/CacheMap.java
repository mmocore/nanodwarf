 ...
 * that entry. A timeout is typically used to have the implementation remove
 * values that become stale after a certain period of time. The implementation
 * ensures that a caller will not have access to stale entries (i.e., entries
...
public class CacheMap<K, V> {
	...
	/**
	 * Creates an instance of this class with the specified {@code entryTimeout}
	 * .
	 * 
	 * @param entryTimeout
	 *            an entry timeout, in milliseconds
	 * @throws IllegalArgumentException
	 *             if {@code entryTimeout} is negative
	 */
	...
	public V put(K key, V value) {
		processQueue();
		Value<K, V> oldValue = map.put(key, new Value<K, V>(key, value, queue,
				entryTimeout));
	...
	/**
	 * Removes the association for the given key, returning the value previously
	 * associated with the key, or {@code null} if the key is not found.
	 * 
	 * @param key
	 *            the key
	...
	/**
	 * Removes all entries from the map whose keys have been determined to be no
	 * longer referenced.
	 */
	...
	private static final class Value<K, V> extends SoftReference<V> {
		...
		/**
		 * Creates an instance of this class and registers it with the specified
		 * reference queue.
		 * 
		 * @param key
		 *            the key
		 * @param value
		 *            the value (held softly)
		 * @param queue
		 *            the reference queue
		 * @param timeout
		 *            a timeout (0 = infinite)
		 */
		Value(K key, V value, ReferenceQueue<V> queue, long timeout) {
			super(value, queue);
			this.key = key;
			this.expirationTime = timeout > 0 ? System.currentTimeMillis()
					+ timeout : Long.MAX_VALUE;
		}

		/**
		 * Returns {@code true} if this value has expired, otherwise returns
		 * {@code false}.
		 */
		boolean isExpired() {

...
final class LockAttemptResult<K> {
	...
	/**
	 * A conflicting locker, if the request was not granted, or {@code null}.
	 */
	final Locker<K> conflict;

	/**
	 * The type of lock conflict, or {@code null} if and only if
	 * {@link #conflict} is {@code null}.
	 */
	final LockConflictType conflictType;

	/**
	 * Creates an instance of this class, with the specified conflict type. The
	 * {@code conflicct} and {@code conflictType} must either both be
	 * {@code null} or both not {@code null}.
	 * 
	 * @param request
	 *            the lock request
	 * @param conflict
	 *            a conflicting locker or {@code null}
	 * @param conflictType
	 *            the conflict type or {@code null}
	 */

...
/**
 * Records information about an entity that requests locks from a
 * {@link LockManager}.
 * 
 * @param <K>
 *            the type of key
...
public abstract class Locker<K> {
	 ...
	 * and the lock timeout supplied by the caller. Subclasses can override this
	 * method, for example to enforce a transaction timeout.
	 * 
	 * @param now
	 *            the current time in milliseconds
	 * @param lockTimeout
	 *            the amount of time in milliseconds to wait for a lock
	 ...
	 * <p>
	 * 
	 * The default implementation creates and returns an instance of
	 * {@link LockRequest}.
	 * 
	 * @param key
	 *            the key that identifies the lock
	 * @param forWrite
	 *            whether the request is for write
	 * @param upgrade
	 *            whether the request is for an upgrade
	...
	/**
	 * Checks if there is a conflict that should cause this locker's current
	 * request to be denied. Returns {@code null} if there was no conflict.
	 * <p>
	 * 
	...
	/**
	 * Clears the conflict that should cause this locker's current request to be
	 * denied. If there is no conflict, then this method has no effect. If the
	 * conflict is a deadlock, represented by a non-{@code null} return value
	 * from {@link #getConflict getConflict} with a {@code type} field equal to
	 * {@link LockConflictType#DEADLOCK DEADLOCK}, then the conflict cannot be
	 * cleared and {@code IllegalStateException} will be thrown.
	 * <p>
	 * 
	 * The default implementation of this method does nothing.
	 * 
	 * @throws IllegalStateException
	 *             if the conflict is a deadlock
	 */
	 ...
	 * 
	 * @return the result of the attempt to request a lock that this locker is
	 *         waiting for, or {@code null} if it is not waiting
	 */
	 ...
	 * {@code null}, then it should represent a conflict, and it's
	 * {@code conflict} field must not be {@code null}.
	 * 
	 * @param waitingFor
	 *            the lock or {@code null}
	 * @throws IllegalArgumentException
	 *             if {@code waitingFor} is not {@code null} and its
	 *             {@code conflict} field is {@code null}
	 */

 ...
 * or remotely in the data store, while still allowing common operations as well
 * as the ability to dynamically switch where the field's value is stored.
 * 
 * <p>
 * 
 ...
 * will not be used). An {@code AdaptiveField} allow the developer finer control
 * over whether a field needs to be included in the serialization graph by
 * providing a common interface for interacting with the field regardless of how
 * it is stored.
 * 
 * <p>
 * 
 ...
 * field will be included in its serialization graph. For this reason a
 * {@code ManagedObject} <i>cannot</i> be stored in an {@code AdaptiveField}. If
 * the developer wants to change the field so that it is stored in the data
 * store, the {@link AdaptiveField#makeManaged AdaptiveField.makeManaged} call
 * can be used. This causes the field's value to no longer be deserialized when
 * the containing {@code ManagedObject} is deserialized.
 * 
 * @param <T>
...
public final class AdaptiveField<T> implements Serializable {
	 ...
	 * calls to return immediately instead of having to go to the
	 ...
	 *             if the provided value is a {@code ManagedObject}
	 */
	...
	/**
	 * Constructs this {@code AdaptiveField} with the provided value, and stores
	 * it as specified.
	 * 
	 * @param value
	 *            the value of this field
	 * @param isLocal
	 *            whether this field should be kept with a local reference or
	...
	/**
	 * Returns the value of the field. Note that if the field is stored locally
	 * and changes, the caller should call
	 * {@link com.sun.sgs.app.DataManager#markForUpdate
	 * DataManager.markForUpdate} on the {@code ManagedObject} that contains
	 * this field since its state has been changed. The values of fields that
	 * are not local are cached after the initial call to the
	 * {@code DataManager}, so subsequent calls to {@code get} will act as if
	 * they are local.
	 * 
	 ...
	 * store to being kept as a local reference. This field will now be included
	 * in the serialization graph of the object that contains this field.
	 */
	...
	public void makeManaged() {
		if (isLocal) {
			ref = AppContext.getDataManager().createReference(
					new ManagedSerializable<T>(local));
			remoteCache = local;
			local = null;
			isLocal = false;
		}

...
/**
 * A simple implementation of a persistent queue. When an element is removed
 * from the queue, that element is also removed from the data store.
 * 
 * <p>
 * Note: The {@code size} and {@code iterator} methods are not supported.
 * 
 * <p>
 * Note: If a given instance of {@code ManagedQueue} contains a large number of
 * elements, invoking the {@code clear} method on the queue may be a lengthy
 * operation. Therefore, the queue should either contain a smaller number of
 * elements, or elements should be removed from the queue a few at at time.
 * 
 * @param <E>
 *            the type for elements in the queue
 * 
 *            TODO: The element type should not be required to be a managed
 *            object, but should just be serializable. Should it handle both?
 */
public class ManagedQueue<E> extends AbstractQueue<E> implements Serializable,
	...
	/**
	 * A queue entry consisting of a reference to the element object and a
	 * reference to the next entry in the queue.
	 */
	...
	public boolean offer(E o) {
		...
		ManagedReference<Entry<E>> entryRef = dataManager
	...
	@Override
	public E peek() {
		return (headRef != null) ? getHead().getElement() : null;
	}
	...
	public boolean equals(Object o) {
		...
		return dm.createReference(this).getId()
				.equals(dm.createReference(d).getId());
	}
	 ...
	 * 
	 * <p>
	 * This implementation clears all elements from the queue, thus removing all
	 * elements from the data store.
	 * 
	 * TODO: For queues with a large number of elements, removing the enqueued
	 * elements should be performed in a separate task (or tasks).
	 */

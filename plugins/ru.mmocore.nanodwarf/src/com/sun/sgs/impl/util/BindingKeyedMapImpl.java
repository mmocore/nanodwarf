...
/**
 * An implementation of a persistent {@code Map} that uses service bindings in
 * the data service to store key/value pairs. This map does not permit
 * {@code null} keys or values.
 * 
 * <p>
 * Note: This map is parameterized by value type only. A {@code String} is the
 * only valid key type for a {@code BindingKeyedMap}.
 * 
 * <p>
 * A value is stored in the data service using its associated key (a String) as
 * a suffix to the {@code keyPrefix} specified during construction. All values
 * must implement {@code Serializable}. If a value implements
 * {@code Serializable}, but does not implement {@link ManagedObject}, the value
 * will be wrapped in an instance of {@code ManagedSerializable} when storing it
 * in the data service. Note: users of this map must use this map's APIs to
 * avoid leaking wrappers for non-managed, serializable objects.
 * 
 * <p>
 * Instances of {@code BindingKeyedMap} as well as its associated iterators are
 * serializable, but not managed, objects.
 * 
 * @param <V>
 *            the type for the map's values
 */
public class BindingKeyedMapImpl<V> extends AbstractMap<String, V> implements
		BindingKeyedMap<V>, Serializable {
	 ...
	 */
	private static final class EntrySet<V> extends
			AbstractSet<Entry<String, V>> implements Serializable {
		/** The serialVersionUID for this class. */
		private static final long serialVersionUID = 1L;
	 ...
	 * A serializable {@code Set} for this map's entries.
	 */
	private static final class KeySet<V> extends AbstractSet<String> implements
			Serializable {
		/** The serialVersionUID for this class. */
		private static final long serialVersionUID = 1L;
	...
	/**
	 * An abstract iterator for obtaining this map's entries. values.
	 */
	private abstract static class AbstractIterator<E, V> implements
		...
		/**
		 * The name fetched in the {@code hasNext} method, which is only valid
		 * if {@code hasNext} returns {@code true}.
		 */
		...
		Entry<String, V> nextEntry() {
			try {
				...
				key = nextName;
				return new KeyValuePair<V>(prefix,
		...
		private V getValue(String bindingName) {
			ManagedObject v = dataService.getServiceBinding(bindingName);
			return v instanceof Wrapper ? (V) ((Wrapper) v).get() : (V) v;
		}
	}
	 ...
	 */
	private static final class EntryIterator<V> extends
			AbstractIterator<Entry<String, V>, V> {
		/** The serialVersionUID for this class. */
		private static final long serialVersionUID = 1L;
	 ...
	 */
	private static final class KeyIterator<V> extends
	 ...
	 */
	private static final class KeyValuePair<V> implements Entry<String, V>,
			Serializable {
		/** The serialVersionUID for this class. */
		private static final long serialVersionUID = 1L;
		...
		public boolean equals(Object o) {
			if (o instanceof Entry) {
				...
				return entryKey != null && getKey().equals(entryKey)
	...
	/**
	 * Returns the binding name for the specified key name (i.e., adds the key
	 ...
	 * @param keyPrefix
	 *            a key prefix
	 * @returns {@code true} if the {@code keyPrefix} corresponds to an empty
	 *          map
	 */
	 ...
	 * applicable) and returns {@code true}. Otherwise, returns {@code false}.
	 */
	private static boolean removeOverrideInternal(String bindingName) {
		boolean previouslyMapped = containsKeyInternal(bindingName);
		if (previouslyMapped) {
			DataService dataService = BindingKeyedCollectionsImpl
					.getDataService();
			try {
				removeValue(bindingName);
	...
	/**
	 * Removes the wrapper (if applicable) for the value associated with the
	 * specified {@code bindingName}.
	 * 
	 * @throws NameNotBoundException
	 *             if the service binding does not exist
	 * @throws ObjectNotFoundException
	 *             if the value associated with the specified
	 *             {@code bindingName} has been removed
	 */
	...
	/**
	 * Puts the specified {@code key}/{@code value} pair in this map, wrapping
	 * the value if the value does not implement {@code ManagedObject}. The
	 * caller is responsible for removing the wrapper for the old value, if
	 * applicable.
	 * 
	 * @param key
	 *            a key
	...
	private static void putKeyValue(String bindingName, Object value) {
		...
		ManagedObject v = value instanceof ManagedObject ? (ManagedObject) value
				: new Wrapper<Object>(value);
		BindingKeyedCollectionsImpl.getDataService().setServiceBinding(
				bindingName, v);
	}
	...
	/**
	 * Throws {@code ClassCastException} if (@code key) is not an instance of
	 * {@code String}.
	 */
	private static void checkKey(String keyName, Object key) {
		Objects.checkNull(keyName, key);
		if (!(key instanceof String)) {
			throw new ClassCastException("key is not an instance of String: "

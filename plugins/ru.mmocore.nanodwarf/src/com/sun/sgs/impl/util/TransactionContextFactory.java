...
/**
 * Utility class for participating in a transaction, parameterized by a type of
 * {@link TransactionContext} to be used with a participant.
 * 
 * @param <T>
 *            a type of transaction context
 */
public abstract class TransactionContextFactory<T extends TransactionContext> {
	/** The logger for this class. */
	private static final LoggerWrapper logger = new LoggerWrapper(
			Logger.getLogger(TransactionContextFactory.class.getName()));
	 ...
	 * {@code TransactionContextMap}.
	 * 
	 * @param contextMap
	 *            the transaction context map
	 * @param participantName
	 *            the type name of the transaction participants
	 */
	 ...
	 * {@code TransactionProxy} to create the {@link TransactionContextMap}. Use
	 * this constructor if access to the {@code TransactionContextMap} is not
	 * needed.
	 * 
	 * @param txnProxy
	 ...
	 * {@link TransactionContextMap#joinTransaction joinTransaction} on the
	 * {@link TransactionContextMap} supplied to, or created by, the
	 * constructor, passing this instance as the argument.
	 * 
	 * @return the context for the current transaction
	 * @throws TransactionNotActiveException
	 *             if no transaction is active
	 * @throws IllegalStateException
	 *             if there is a problem with the state of the transaction.
	 */
	...
	/**
	 * Returns a new {@code TransactionContext} to hold state for the specified
	 * transaction, {@code txn}.
	 * 
	 * @param txn
	 *            a transaction
	 * 
	...
	/**
	 * Returns a new transaction participant that operates on the currently
	 * active transaction context.
	 * <p>
	 * 
	 * The default implementation of this method returns a transaction
	 * participant that implements {@link NonDurableTransactionParticipant}.
	 * 
		...
		public boolean prepare(Transaction txn) throws Exception {
			try {
				...
				if (logger.isLoggable(Level.FINE)) {
					logger.log(Level.FINER, "prepare txn:{0} returns {1}", txn,
							readOnly);
				}
				...
				if (logger.isLoggable(Level.FINER)) {
					logger.logThrow(Level.FINER, e, "prepare txn:{0} throws",
							txn);
				}
				throw e;
			}
					...
					if (!context.isPrepared()) {
						...
						if (logger.isLoggable(Level.WARNING)) {
							logger.logThrow(Level.WARNING, e,
									"commit: not yet prepared txn:{0}", txn);
						}
						throw e;
					}
				} finally {
	...
	protected class NonDurableParticipant extends Participant implements
	...
	/**
	 * Returns {@code true} if the given {@code Throwable} is a "retryable"
	 * exception, meaning that it implements {@code ExceptionRetryStatus}, and
	 * invoking its {@link ExceptionRetryStatus#shouldRetry shouldRetry} method
	 * returns {@code true}.
	 * 
	 * @param t
	 *            a throwable
	 * 
	 * @return {@code true} if the given {@code Throwable} is retryable, and

 ...
 * 
 * @param <T>
 *            the type of transaction context
...
public class TransactionContextMap<T extends TransactionContext> {
	...
	/**
	 * Makes sure the participant obtained from {@code contextFactory} is joined
	 * to the current transaction and returns the associated context. If the
	 * participant has not yet joined the current transaction, creates a new
	 * context by invoking {@link TransactionContextFactory#createContext
	 * createContext} on {@code contextFactory}, passing the current
	 * transaction, sets that context as the current context for the current
	 * thread, and joins the transaction. Returns the context for the current
	 * transaction.
	 * 
	...
	/**
	 * Checks that the specified {@code context} is currently active, throwing
	 * {@code TransactionNotActiveException} if it isn't.
	 * 
	 * @param context
	 *            a context
	 * 
	 * @throws TransactionNotActiveException
	 *             if the specified {@code context} is not currently active
	 */
	...
	/**
	 * Checks the specified transaction, throwing {@code IllegalStateException}
	 * if the current context is {@code null} or if the specified transaction is
	 * not equal to the transaction in the current context.
	 * 
	 * @param txn
	 *            a transaction

 ...
 * 
 * <ul>
 * <li> {@link Level#FINER FINER} - Releasing locks; requesting, waiting for, and
 ...
 * <ul>
 * 
 * <li>Synchronization is only used on {@link Locker} objects and on the
 * {@code Map}s that hold {@link Lock} objects
 * 
 ...
 * <ul>
 * 
 * <li>The {@code Lock} class is not synchronized
 * <p>
 * 
 * Callers of non-{@code Object} methods on the {@code Lock} class should make
 * sure that they are synchronized on the associated key map.
 * 
 ...
 * <p>
 * 
 * The implementation enforces this requirement by having lock methods not make
 * calls to other classes, and by performing minimal work while synchronized on
 ...
 * {@code Lock} -- the {@link #waitForLockInternal waitForLockInternal} method.
 * That method also makes sure that the only synchronized {@code Locker} methods
 * that it calls are on the locker it has already synchronized on.
 * 
...
public class LockManager<K> {
	...
	/**
	 * The maximum number of milliseconds to spend attempting to acquire a lock.
	 */
	...
	/**
	 * An array of maps from key to lock. The map to use is chosen by using the
	 * key's hash code mod the number of key maps. Synchronization for locks is
	 * based on locking the associated key map. Non-{@code Object} methods on
	 * locks should not be used without synchronizing on the associated key map
	 * lock.
	 */
	private final Map<K, Lock<K>>[] keyMaps;

	/**
	 * When assertions are enabled, holds the {@code Locker} that the current
	 * thread is synchronized on, if any.
	 */
	private final ThreadLocal<Locker<K>> currentLockerSync = new ThreadLocal<Locker<K>>();
	 ...
	 * 
	 * @param lockTimeout
	 *            the maximum number of milliseconds to acquire a lock
	 * @param numKeyMaps
	 *            the number of separate maps to use for storing keys
	 * @throws IllegalArgumentException
	 *             if {@code lockTimeout} or {@code numKeyMaps} is less than
	 *             {@code 1}
	 */
	...
	/**
	 * Attempts to acquire a lock, waiting if needed. Returns information about
	 * conflicts that occurred while attempting to acquire the lock that
	 * prevented the lock from being acquired, or else {@code null} if the lock
	 * was acquired. If the {@code type} field of the return value is
	 * {@link LockConflictType#DEADLOCK DEADLOCK}, then the caller should abort
	 * the transaction, and any subsequent lock or wait requests will return the
	 * same deadlock conflict. Otherwise, the caller can repeat this call, and
	 * any conflicts from earlier calls will be ignored.
	 * 
	 * @param locker
	 *            the locker requesting the lock
	 * @param key
	 *            the key identifying the lock
	 * @param forWrite
	 *            whether to request a write lock
	 * @return lock conflict information, or {@code null} if there was no
	 *         conflict
	 * @throws IllegalArgumentException
	 *             if {@code locker} has a different lock manager
	 * @throws IllegalStateException
	 *             if still waiting for an earlier attempt to complete
	 */
	 ...
	 * about any conflict that occurred while attempting to acquire the lock, or
	 * else {@code null} if the lock was acquired. If the attempt to acquire the
	 * lock was blocked, returns a value with a {@code type} field of
	 * {@link LockConflictType#BLOCKED BLOCKED} rather than waiting. If the
	 * {@code type} field of the return value is
	 * {@link LockConflictType#DEADLOCK DEADLOCK}, then the caller should abort
	 * the transaction, and any subsequent lock or wait requests will return the
	 * same deadlock conflict. Otherwise, the caller can repeat this call, and
	 * any conflicts from earlier calls will be ignored.
	 ...
	 *             if still waiting for an earlier attempt to complete
	 */
	public LockConflict<K> lockNoWait(Locker<K> locker, K key, boolean forWrite) {
	...
	/**
	 * Releases a lock held by a locker. This method does nothing if the lock is
	 * not held.
	 * 
	 * @param locker
	 *            the locker holding the lock
	 * @param key
	 *            the key identifying the lock
	 * @throws IllegalArgumentException
	 *             if {@code locker} has a different lock manager
	...
	/**
	 * Returns a possibly read-only list that contains a snapshot of the current
	 * owners of a lock, as identified by lock requests.
	 * 
	 * @param key
	 *            the key identifying the lock
	...
	/**
	 * Returns a possibly read-only list that contains a snapshot of the current
	 * waiters for a lock, as identified by lock requests.
	 * 
	 * @param key
	 *            the key identifying the lock
	 * @return a list of the requests
	 ...
	 * which should be the one returned by calling {@link #getKeyMap getKeyMap}.
	 * The lock on {@code keyMap} should be held.
	 * 
	 * @param key
	 *            the key
	 * @param keyMap
	...
	/**
	 * Attempts to acquire a lock, returning immediately. Like
	 * {@link #lockNoWait}, but does not check that the correct lock manager was
	 * supplied.
	 * 
	 * @param locker
	 *            the locker requesting the lock
	 ...
	 *             if still waiting for an earlier attempt to complete
	 */
	LockConflict<K> lockNoWaitInternal(Locker<K> locker, K key, boolean forWrite) {
		if (logger.isLoggable(FINEST)) {
			logger.log(FINEST, "lock {0}, {1}, forWrite:{2}", locker, key,
					forWrite);
		}
		try {
			...
			if (result == null) {
				if (logger.isLoggable(FINER)) {
					logger.log(FINER, "lock {0}, {1}, forWrite:{2}"
							+ "\n  returns null (already granted)", locker,
							key, forWrite);
				}
				return null;
			}
			if (result.conflict == null) {
				if (logger.isLoggable(FINER)) {
					logger.log(FINER, "lock {0}, {1}, forWrite:{2}"
							+ "\n  returns null (granted)", locker, key,
							forWrite);
				}
				return null;
			}
			...
			conflict = new LockConflict<K>(result.conflictType, result.conflict);
			if (logger.isLoggable(FINER)) {
				logger.log(FINER, "lock {0}, {1}, forWrite:{2}\n  returns {3}",
						locker, key, forWrite, conflict);
			}
	...
	/**
	 * Waits for a previous attempt to obtain a lock that blocked. Like
	 * {@link #waitForLock}, but does not check that the correct lock manager
	 * was supplied.
	 * 
	 * @param locker
	 *            the locker requesting the lock
				...
				synchronized (locker) {
					...
					while (true) {
						...
						if (isOwner) {
							...
							if (logger.isLoggable(FINER)) {
								logger.log(FINER, "lock {0}, {1}, forWrite:{2}"
										+ "\n  returns null (granted)", locker,
										key, result.request.getForWrite());
							}
							return null;
						} else if (timedOut) {
						...
						if (logger.isLoggable(FINER)) {
							logger.log(FINER,
									"wait for lock {0}, {1}, forWrite:{2}"
											+ ", wait:{3,number,#}", locker,
									key, result.request.getForWrite(), stop
											- now);
						}
						try {
	 ...
	 *            the key identifying the lock
	 * @param downgrade
	 *            whether the lock should only be downgraded
	 * @throws IllegalArgumentException
	 *             if {@code locker} has a different lock manager
	 */
	...
	/**
	 * Notes the start of synchronization on the map associated with {@code key}
	 * . Throws {@link AssertionError} if already synchronized on a key,
	...
	boolean noteKeySync(K key) {
		...
		if (currentKey != null) {
			throw new AssertionError("Attempt to synchronize on map for key "
					+ key + ", but already synchronized on " + currentKey);
		}
	 ...
	 * Throws {@link AssertionError} if not already synchronized on {@code key},
	 * otherwise returns {@code true}.
	 * 
	 * @param key
	 *            the key
	...
	boolean noteKeyUnsync(K key) {
		...
		K currentKey = currentKeySync.get();
		if (currentKey == null) {
			throw new AssertionError("Attempt to unsynchronize on map for key "
					+ key + ", but not currently synchronized on a key");
		} else if (!currentKey.equals(key)) {
			throw new AssertionError("Attempt to unsynchronize on map for key "
					+ key + ", but currently synchronized on " + currentKey);
		}
	...
	/**
	 * Checks that the current thread is synchronized on the map associated with
	 * {@code key}, throwing {@link AssertionError} if it is not, and otherwise
	 * returning {@code true}.
	 * 
	 * @param key
	 *            the key
	...
	boolean checkKeySync(K key) {
		...
		if (currentKey == null) {
			...
			throw new AssertionError("Should be synchronized on " + key
	...
	boolean checkNoKeySync() {
		K currentKey = currentKeySync.get();
		if (currentKey != null) {
			throw new AssertionError("Currently synchronized on key "
					+ currentKey);
		}
		return true;
	}
	...
	boolean noteLockerSync(Locker<K> locker) {
		...
		if (currentLocker != null) {
			throw new AssertionError("Attempt to synchronize on locker " + this
	...
	/**
	 * Notes the end of synchronization on the specified locker. Throws
	 * {@link AssertionError} if not already synchronized on the locker,
	 * otherwise returns {@code true}.
	 * 
	 * @param locker
	 *            the locker
	...
	boolean noteLockerUnsync(Locker<K> locker) {
		...
		Locker<K> currentLocker = currentLockerSync.get();
		if (currentLocker == null) {
			throw new AssertionError("Attempt to unsynchronize on locker "
					+ this + ", but not currently synchronized on a locker");
		} else if (currentLocker != locker) {
			throw new AssertionError("Attempt to unsynchronize on locker "
					+ locker + ", but currently synchronized on "
					+ currentLocker);
		}
	...
	boolean checkAllowLockerSync(Locker<K> locker) {
		...
		if (currentLocker != null && currentLocker != locker) {
			throw new AssertionError("Attempt to synchronize on locker "
					+ locker + ", but already synchronized on " + currentLocker);
		}
		checkNoKeySync();
		return true;
	}

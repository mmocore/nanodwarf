...
/**
 * Provides for making the server available on the network, and removing it from
 * the network during shutdown.
 * 
 * @param <T>
...
public class Exporter<T extends Remote> {
	...
	/**
	 * Makes the server available on the network on the specified port, and
	 * binds the server's proxy in a registry on the same port with the
	 * specified name. If the port is 0, chooses an anonymous port. Returns the
	 * actual port on which the server is available.
	 * 
	 * @param server
	 *            the server
	 * @param name
	 *            the name of the server's proxy in the registry
	 * @param port
	 ...
	 * 
	 * @throws IOException
	 *             if there is a problem exporting the server
	 */
	public synchronized int export(T server, String name, int port)
		...
		proxy = type.cast(UnicastRemoteObject.exportObject(server, port, null,
				ssf));
	...
	/**
	 * Makes the server available on the network on the specified port. If the
	 * port is 0, chooses an anonymous port. Returns the actual port on which
	 ...
	 * {@code export} is invoked successfully. Before the server is exported,

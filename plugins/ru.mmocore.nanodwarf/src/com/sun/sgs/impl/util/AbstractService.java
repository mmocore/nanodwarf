...
/**
 * An abstract implementation of a service. It manages state transitions (i.e.,
 * initialized, ready, shutting down, shutdown), in progress call tracking for
 * services with embedded remote servers, and shutdown support.
 * 
 * <p>
 ...
 * 
 * An {@link #AbstractService} supports the following properties:
 * <p>
 * 
 * Property: com.sun.sgs.io.retries Default: 5 retries Specifies how many times
 * an IO task should be retried before performing failure procedures.
 * 
 ...
 * Specifies how many times an {@link IoRunnable IoRunnable} task should be
 * retried before performing failure procedures. The value must be greater than
 * or equal to {@code 0}.
 * <p>
 * 
 * <dt><i>Property:</i> <code><b>
 ...
 * The value must be greater than or equal to {@code 0}.
 * 
 * </dl>
 * <p>
 * 
...
public abstract class AbstractService implements Service {
	 ...
	 * {@code systemRegistry}, {@code txnProxy}, and {@code logger}. It
	 * initializes the {@code appName} field to the value of the
	 ...
	 *             if the {@code com.sun.sgs.app.name} property is not defined
	 *             in {@code properties}
	 */
	 ...
	 * 
	 * <p>
	 * If this service is in the {@code INITIALIZED} state, this method sets the
	 * state to {@code READY} and invokes the {@link #doReady doReady} method.
	 * If this service is already in the {@code READY} state, this method
	 * performs no actions. If this service is shutting down, or is already shut
	 * down, this method throws {@code IllegalStateException}.
	 * 
	 * @throws Exception
	 *             if a problem occurs
	 * @throws IllegalStateException
	 *             if this service is shutting down or is already shut down
	 */
	...
	/**
	 * Performs ready operations. This method is invoked by the {@link #ready
	 * ready} method only once so that the subclass can perform any operations
	 * necessary during the "ready" phase.
	 * 
	 * @throws Exception
	 *             if a problem occurs
	 */
	 ...
	 * 
	 * <p>
	 * If this service is in the {@code INITIALIZED} state or {@code READY}
	 * state, this method sets the state to {@code SHUTTING_DOWN}, waits for all
	 * calls in progress to complete, starts a thread to invoke the
	 * {@link #doShutdown doShutdown} method, waits for that thread to complete,
	 * and returns. If this service is in the {@code SHUTTING_DOWN} state, this
	 * method will block until the shutdown is complete. If this service is in
	 * the {@code SHUTDOWN} state, then it will return immediately. Any retries
	 * or interruption handling should be done in the service's implementation
	 * of the {@link #doShutdown() doShutdown} method.
	 * 
	 */
	 ...
	 * perform any operations necessary to shutdown the service.
	 */
	protected abstract void doShutdown();

	/**
	 * Checks the service version. If a version is not associated with the given
	 * {@code versionKey}, then a new {@link Version} object (constructed with
	 * the specified {@code majorVersion} and {@code minorVersion}) is bound in
	 * the data service with the specified key.
	 * 
	 * <p>
	 * If an old version is bound to the specified key and that old version is
	 * not equal to the current version (as specified by {@code majorVersion}/
	 * {@code minorVersion}), then the {@link #handleServiceVersionMismatch
	 ...
	 * @throws IllegalStateException
	 *             if {@code handleVersionMismatch} is invoked and throws a
	...
	protected final void checkServiceVersion(String versionKey,
		...
		try {
			Version oldVersion = (Version) dataService
					.getServiceBinding(versionKey);
	...
	/**
	 * Handles conversion from the {@code oldVersion} to the
	 * {@code currentVersion}. This method is invoked by
	 * {@link #checkServiceVersion checkServiceVersion} if a version mismatch is
	 * detected and is invoked from within a transaction.
	 * 
	 * @param oldVersion
	 *            the old version
	 ...
	 *            the current version
	 * @throws IllegalStateException
	 *             if the old version cannot be upgraded to the current version
	 */
	...
	/**
	 * Increments the number of calls in progress. This method should be invoked
	 * by remote methods to both increment in progress call count and to check
	 * the state of this server. When the call has completed processing, the
	 * remote method should invoke {@link #callFinished callFinished} before
	 * returning.
	 * 
	 * @throws IllegalStateException
	 ...
	 * @throws IllegalStateException
	 *             if this method is invoked inside a transactional context
	 */
	 ...
	 * of times. The method will stop retrying if the node with the given
	 * {@code nodeId} is no longer alive. The number of retries and the wait
	 * time between retries are configurable properties.
	 * 
	 * <p>
	 * This method must be called from outside a transaction or
	 * {@code IllegalStateException} will be thrown.
	 ...
	 * @param nodeId
	 *            the node that is the target of the IO operations
	 ...
	 * {@link ExceptionRetryStatus} and invoking its
	 ...
	 * throws {@code IllegalStateException} if the thread is in a transactional
	 * context.
	 */

 ...
 * class does not detect deadlocks, but provides support for
 * {@linkplain #downgradeLock downgrading locks}. All {@link Locker} objects
 * supplied to this class should be instances of {@link MultiLocker}.
 * <p>
 * 
...
public final class MultiLockManager<K> extends LockManager<K> {
	 ...
	 */
	private final ThreadLocal<LockAttemptResult<K>> waitingFor = new ThreadLocal<LockAttemptResult<K>>();
	...
	/**
	 * Downgrades a lock held by a locker from write to read access. This method
	 * does nothing if the lock is not held for write.
	 * 
	 * @param locker
	 ...
	 * @throws IllegalArgumentException
	 *             if {@code locker} has a different lock manager, or if
	 *             {@code locker} is not an instance of {@link MultiLocker}
	 */
	 ...
	 * 
	 * @return the result of the lock request this thread is waiting for or
	 *         {@code null} if it is not waiting
	 */
	...
	/**
	 * Records the lock that this thread is waiting for, or marks that it is not
	 * waiting if the argument is {@code null}.
	 * 
	 * @param waitingFor
	 *            the lock or {@code null}
	 */

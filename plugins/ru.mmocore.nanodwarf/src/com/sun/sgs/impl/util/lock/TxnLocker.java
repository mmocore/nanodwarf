 ...
 * {@link TxnLockManager} as part of a transaction.
 * 
 * @param <K>
 *            the type of key
 */
public class TxnLocker<K> extends BasicLocker<K> {
	...
	/**
	 * A conflict that should cause this transaction's request to be denied, or
	 * {@code null}. This value is cleared after the conflict has been reported
	 * unless the conflict is a deadlock. Synchronize on this locker when
	 ...
	 *            the time in milliseconds that the task associated with the
	 *            transaction was originally requested to start
	 * @throws IllegalArgumentException
	 *             if {@code requestedStartTime} is less than {@code 0}
	 */
	public TxnLocker(TxnLockManager<K> lockManager, Transaction txn,
	...
	protected long getLockTimeoutTime(long now, long lockTimeout) {
		return Math.min(addCheckOverflow(now, lockTimeout),
	 ...
	 * be denied, and notifies waiters. Does nothing if the locker already has a
	 * conflict.
	 * 
	 * @param conflict

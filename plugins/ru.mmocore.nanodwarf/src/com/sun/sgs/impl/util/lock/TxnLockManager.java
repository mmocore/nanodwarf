 ...
 * locker should only be made from a single thread at a time. All {@link Locker}
 * objects supplied to this class should be instances of {@link TxnLocker}.
 * <p>
 * 
 ...
 * <p>
 * 
 * This class and its {@linkplain LockManager superclass} use the {@link Logger}
 * named {@code com.sun.sgs.impl.util.lock} to log information at the following
 * logging levels:
 * <p>
 * 
 * <ul>
 ...
 * requesting locks before waiting, releasing locks, results of attempting to
 * assign locks to waiters
 * </ul>
 * 
 * @param <K>
 *            the type of key
...
public final class TxnLockManager<K> extends LockManager<K> {
	 ...
	 * 
	 * @throws IllegalArgumentException
	 *             {@inheritDoc}, or if {@code locker} is not an instance of
	 *             {@link TxnLocker}
	 */
	...
	LockConflict<K> lockNoWaitInternal(Locker<K> locker, K key, boolean forWrite) {
		checkTxnLocker(locker);
		LockConflict<K> conflict = super.lockNoWaitInternal(locker, key,
				forWrite);
		if (conflict != null) {
			if (logger.isLoggable(FINEST)) {
				logger.log(FINEST, "lock attempt {0}, {1}, forWrite:{2}"
						+ "\n  returns blocked -- checking for deadlocks",
						locker, key, forWrite);
			}
			try {
				LockConflict<K> deadlockConflict = new DeadlockChecker(
						(TxnLocker<K>) locker).check();
				if (deadlockConflict != null) {
					if (logger.isLoggable(FINER)) {
						logger.log(FINER, "lock {0}, {1}, forWrite:{2} found"
								+ " deadlock with {3}", locker, key, forWrite,
								deadlockConflict);
					}
				...
				if (logger.isLoggable(FINER)) {
					logger.logThrow(FINER, e,
							"lock {0}, {1}, forWrite:{2} throws", locker, key,
							forWrite);
				}
				throw e;
			} catch (Error e) {
	...
	private class DeadlockChecker {
		...
		/**
		 * Maps lockers to information about lockers they are waiting for. This
		 * map serves as a cache for information about lock owners, to avoid the
		 * synchronization needed to retrieve it again when checking for
		 * multiple deadlocks.
		 */
		private final Map<TxnLocker<K>, WaiterInfo<K>> waiterMap = new HashMap<TxnLocker<K>, WaiterInfo<K>>();
		 ...
		 * 
		 * @return a lock conflict if a deadlock was found, else {@code null}
		 */
		LockConflict<K> check() {
			...
			for (pass = 1; true; pass++) {
				...
				if (victim == rootLocker) {
					...
					result = new LockConflict<K>(LockConflictType.BLOCKED,
							conflict);
				}
			}
		}
		 ...
		 * information about its waiters. Returns whether a deadlock was found.
		 */
		private boolean checkInternal(TxnLocker<K> locker,
				WaiterInfo<K> waiterInfo) {
			...
			for (LockRequest<K> request : waiterInfo.waitingFor) {
				...
				if (owner == locker) {
					if (logger.isLoggable(FINEST)) {
						logger.log(FINEST, "checking deadlock {0}, pass {1}:"
								+ " locker {2}, waiting for {3}:"
								+ " ignore self-reference", rootLocker, pass,
								locker, request);
					}
				} else {
		 ...
		 * has a newer requested start time than the previously selected victim.
		 */
		private void maybeUpdateVictim(TxnLocker<K> locker) {
			...
			if (locker == cycleBoundary) {
				...
				cycleBoundary = null;
					...
					&& (locker.getRequestedStartTime() > victim
	...
	/**
	 * Provides information about the requests a locker is waiting for. Used in
	 * deadlock detection.
	 */
	private static class WaiterInfo<K> {
		...
		/**
		 * The pass in which the locker was checked. If we encounter an instance
		 * with the current pass number, then we've found a cycle.
		 */
		int pass = 0;
		 ...
		 * 
		 * @param waitingFor
		 *            the requests the locker is waiting for
		 */

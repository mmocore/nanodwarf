 ...
 * 
 * <p>
 * The {@link #ChannelServiceImpl constructor} requires the <a href=
 * "../../../impl/kernel/doc-files/config-properties.html#com.sun.sgs.app.name">
 * <code>com.sun.sgs.app.name</code></a> property and also supports the
 ...
 * {@code ChannelServer} of the local node.
 * <p>
 * 
 * <dt><i>Property:</i> <code><b>
 ...
 * 
 * <dd style="padding-top: .5em">Specifies the timeout, in milliseconds, for
 * client session relocation. This also specifies the amount of time to save
 * reliable channel messages so that relocating client sessions can obtain
 * channel messages that were missed during relocation.
 * <p>
 * 
 * </dl>
 * <p>
 */
public final class ChannelServiceImpl extends AbstractService implements
		ChannelManager {
	...
	private static final String CHANNEL_SERVER_MAP_PREFIX = PKG_NAME
			+ ".server.";
	 ...
	 * transaction.
	 */
	static final String EVENTS_PER_TXN_PROPERTY = PKG_NAME + ".events.per.txn";
	...
	static final String WRITE_BUFFER_SIZE_PROPERTY = PKG_NAME
			+ ".write.buffer.size";
	...
	/**
	 * List of contexts that have been prepared (non-readonly) or committed. The
	 * {@code contextList} is locked when contexts are added (during prepare),
	 * removed (during abort or flushed during commit), and when adding or
	 * removing task queues from the {@code channelTaskQueues} map.
	 */
	...
	private final ConcurrentHashMap<Long, ChannelServer> channelServerCache = new ConcurrentHashMap<Long, ChannelServer>();

	/** The map of channel coordinator information, keyed by channel ID. */
	private final ConcurrentHashMap<BigInteger, Coordinator> coordinatorMap = new ConcurrentHashMap<BigInteger, Coordinator>();

	/**
	 * The cache of channel membership snapshots, keyed by channel ID. The cache
	 * entry timeout is one second.
	 */
	private final CacheMap<BigInteger, Set<BigInteger>> channelMembershipCache = new CacheMap<BigInteger, Set<BigInteger>>(
			1000);

	/**
	 * The local channel membership info, keyed by channel ID. This map is
	 * synchronized, but should only be locked long enough to obtain a value (a
	 * LocalChannelInfo instance) and lock that instance; i.e., map values
	 * should be obtained and locked while synchronized on the map instance. The
	 * locking idiom for this map is:
	 * 
	 * <pre>
	 ...
	 * </pre>
	 */
	private final Map<BigInteger, LocalChannelInfo> localChannelMembersMap = Collections
			.synchronizedMap(new HashMap<BigInteger, LocalChannelInfo>());

	/**
	 * The local per-session channel maps (key: channel ID, value: message
	 * timestamp), keyed by session ID.
	 */
	private final ConcurrentHashMap<BigInteger, Map<BigInteger, LocalMemberInfo>> localPerSessionChannelMap = new ConcurrentHashMap<BigInteger, Map<BigInteger, LocalMemberInfo>>();

	/**
	 * Set of session ids that are currently in the process of being
	 * disconnected
	 */
	private final Set<BigInteger> localSessionDisconnectingSet = Collections
			.synchronizedSet(new HashSet<BigInteger>());

	/** The map of per-session locks, keyed by session ID. */
	private final ConcurrentHashMap<BigInteger, Lock> sessionLocks = new ConcurrentHashMap<BigInteger, Lock>();

	/**
	 * Map of relocation information (new node ID and completion handler) for
	 * client sessions relocating from this node, keyed by the relocating
	 * session's ID.
	 */
	private final Map<BigInteger, RelocationInfo> outgoingSessionRelocationInfo = Collections
			.synchronizedMap(new HashMap<BigInteger, RelocationInfo>());

	/**
	 * Map for storing pending requests for client sessions relocating to this
	 * node, keyed by session ID.
	 */
	private final ConcurrentHashMap<BigInteger, SortedMap<Long, PendingRequests>> incomingSessionPendingRequests = new ConcurrentHashMap<BigInteger, SortedMap<Long, PendingRequests>>();
	 ...
	 * {@code properties}, {@code systemRegistry}, and {@code txnProxy}.
	 * 
	 * @param properties
	 *            service properties
		...
		try {
			synchronized (ChannelServiceImpl.class) {
				...
				if (collectionsFactory == null) {
					collectionsFactory = systemRegistry
							.getComponent(BindingKeyedCollections.class);
				}
				if (channelServerMap == null) {
					channelServerMap = collectionsFactory
							.newMap(CHANNEL_SERVER_MAP_PREFIX);
				}
			}
			contextFactory = new ContextFactory(contextMap);
			WatchdogService watchdogService = txnProxy
					.getService(WatchdogService.class);
			sessionService = txnProxy.getService(ClientSessionService.class);
			localNodeId = txnProxy.getService(DataService.class)
					.getLocalNodeId();

			/*
			 * Get the properties for controlling write buffer size, channel
			 * event processing, and session relocation timeout
			 */
			writeBufferSize = wrappedProps.getIntProperty(
					WRITE_BUFFER_SIZE_PROPERTY, DEFAULT_WRITE_BUFFER_SIZE,
					8192, Integer.MAX_VALUE);
			eventsPerTxn = wrappedProps.getIntProperty(EVENTS_PER_TXN_PROPERTY,
					DEFAULT_EVENTS_PER_TXN, 1, Integer.MAX_VALUE);
			sessionRelocationTimeout = wrappedProps.getLongProperty(
					StandardProperties.SESSION_RELOCATION_TIMEOUT_PROPERTY,
					StandardProperties.DEFAULT_SESSION_RELOCATION_TIMEOUT, 500,
					Long.MAX_VALUE);

			/*
			 * Export the ChannelServer.
			 */
			int serverPort = wrappedProps.getIntProperty(SERVER_PORT_PROPERTY,
			...
			try {
				...
				logger.log(Level.CONFIG,
						"ChannelServer export successful. port:{0,number,#}",
						port);
			} catch (Exception e) {
				try {
					exporter.unexport();
			...
			/*
			 * Create channel server map, keyed by node ID. Then store channel
			 * server in the channel server map.
			 */
			transactionScheduler.runTask(new AbstractKernelRunnable(
					"StoreChannelServerProxy") {
				public void run() {
					getChannelServerMap().put(Long.toString(localNodeId),
							serverProxy);
				}
			}, taskOwner);
			 ...
			 */
			watchdogService
					.addRecoveryListener(new ChannelServiceRecoveryListener());
			...
			ProfileCollector collector = systemRegistry
	...
	public Channel createChannel(String name, ChannelListener listener,
		...
		try {
			Channel channel = ChannelImpl.newInstance(name, listener, delivery,
					writeBufferSize);
			return channel;
	...
	/**
	 * Handles a channel {@code message} that the specified {@code session} is
	 * sending on the channel with the specified {@code channelRefId}. This
	 * method is invoked from the {@code ClientSessionHandler} of the given
	 * session, when it receives a channel message. This method must be called
	...
	private final class ChannelServerImpl implements ChannelServer {
		...
		/** {@inheritDoc} */
		public MembershipStatus isMember(BigInteger channelRefId,
			...
			try {
				if (sessionService.getSessionProtocol(sessionRefId) == null) {
					...
					return isLocalChannelMember(channelRefId, sessionRefId) ? MembershipStatus.MEMBER
		 ...
		 * adds the specified {@code sessionRefId} to the per-channel local
		 * membership set for the given channel, and sends a channel join
		 * message to the session with the corresponding {@code sessionId}.
		 */
		public boolean join(String name, BigInteger channelRefId,
				byte deliveryOrdinal, long timestamp, BigInteger sessionRefId)

		{
			callStarted();
			try {
				if (logger.isLoggable(Level.FINEST)) {
					logger.log(Level.FINEST, "join name:{0} channelId:{1} "
							+ "sessionId:{2} localNodeId:{3}", name,
							channelRefId, sessionRefId, localNodeId);
				}

				return handleNotification(
						sessionRefId,
						timestamp,
						new ChannelJoinTask(name, channelRefId, Delivery
								.values()[deliveryOrdinal]));
		 ...
		 * {@inheritDoc}
		 * 
		 * Removes the specified {@code sessionRefId} from the per-channel local
		 * membership set for the given channel, and sends a channel leave
		 * message to the session with the corresponding {@code sessionRefId}.
		 */
			...
			try {
				if (logger.isLoggable(Level.FINEST)) {
					logger.log(Level.FINEST,
							"leave channelId:{0} sessionId:{1}", channelRefId,
							sessionRefId);
				}

				ChannelLeaveTask task = new ChannelLeaveTask(channelRefId);
				boolean success = handleNotification(sessionRefId,
						msgTimestamp, task);
		...
		public BigInteger[] getSessions(BigInteger channelRefId) {
			...
			try {
				...
				try {
					localMembers = channelInfo.members
							.toArray(new BigInteger[channelInfo.members.size()]);
					return localMembers;
		 ...
		 * {@inheritDoc}
		 * 
		 * Sends the given {@code message} to all local members of the specified
		 * channel.
		 */
		public void send(BigInteger channelRefId, byte[] message, long timestamp) {
			callStarted();
			try {
				if (logger.isLoggable(Level.FINEST)) {
					logger.log(Level.FINEST,
				...
				try {
					...
					ChannelSendTask task = new ChannelSendTask(channelRefId,
							channelInfo.delivery, message);
		 ...
		 * {@inheritDoc}
		 */
		public void relocateChannelMemberships(final BigInteger sessionRefId,
				long oldNodeId, BigInteger[] channelRefIds,
				byte[] deliveryOrdinals, long[] msgTimestamps) {
			callStarted();
			try {
				/*
				 * Schedule task to add the session's new node (the local node)
				 * to each of its channels if not already present.
				 */
				taskScheduler.scheduleTask(
						new AddRelocatingSessionNodeToChannels(sessionRefId,
								oldNodeId, channelRefIds, deliveryOrdinals,
								msgTimestamps), taskOwner);
		...
		public void relocateChannelMembershipsCompleted(
			...
			try {
				...
				// done.
				RelocationInfo relocationInfo = outgoingSessionRelocationInfo
						.get(sessionRefId);
		 ...
		 * {@inheritDoc}
		 * 
		 * Removes the specified channel from the map of local channels, removes
		 * the channel from each of its local member's channel maps and sends a
		 * leave notification to each member.
		 */
		...
		/**
		 * Handles the channel request {@code task} for the specified session
		 * and timestamp.
		 * <p>
		 * 
		 * If the session is connected to this node, then the specified
		 * {@code task} is run with the specified {@code sessionRefId} and
		 * {@code timestamp}, and {@code true} is returned.
		 * <p>
		 * 
		 * If the session is relocating to this node, then the specified
		 * {@code task} is enqueued for execution when the session completes
		 * relocation, and {@code true} is returned.
		 * <p>
		 * 
		 * If the session is not connected to this node or is relocating from
		 * this node, then {@code false} is returned.
		 */
		private boolean handleNotification(BigInteger sessionRefId,
				long timestamp, ChannelRequestTask task) {
			try {
				...
				SessionProtocol protocol = sessionService
						.getSessionProtocol(sessionRefId);
				if (protocol == null) {
					if (!sessionService.isRelocatingToLocalNode(sessionRefId)) {
						...
						if (logger.isLoggable(Level.FINE)) {
							logger.log(
									Level.FINE,
									"Dropping channel request for "
											+ "non-local session:{0} channel:{1} timestamp:{2} "
											+ "localNodeId:{3}", sessionRefId,
									task.channelRefId, timestamp, localNodeId);
						}
						return false;
					}
					...
					SortedMap<Long, PendingRequests> pendingRequestsMap = incomingSessionPendingRequests
							.get(sessionRefId);
					if (pendingRequestsMap == null) {
						SortedMap<Long, PendingRequests> newMap = Collections
								.synchronizedSortedMap(new TreeMap<Long, PendingRequests>());
					...
					synchronized (pendingRequestsMap) {
						PendingRequests pendingRequests = pendingRequestsMap
								.get(timestamp);
					...
					RelocationInfo info = outgoingSessionRelocationInfo
							.get(sessionRefId);
					if (info != null) {
						...
						if (logger.isLoggable(Level.FINE)) {
							logger.log(
									...
									sessionRefId, task.channelRefId, timestamp,
									localNodeId, info.newNodeId);
						}
						return false;
					}
	 ...
	 * {@code sessionRefId} as a local member of the channel with the specified
	 * {@code channelRefId}. ALSO adds the {@code channelRefId} to the the local
	 * per-session channel map for {@code sessionRefId}.
	 * 
	 * @param channelRefId
	 *            a channel ID
	 * @param delivery
	 ...
	 *            a session ID
	 * @param timestamp
	 *            the session's timestamp--if no messages received yet, then
	 *            timestamp of when session joined channel, otherwise the
	 *            timestamp of the last message received
	 * @param isRelocating
	 *            if session is relocating to this node
	 */
	private void addLocalChannelMember(final BigInteger channelRefId,
			Delivery delivery, final BigInteger sessionRefId, long timestamp,
		...
		try {
			if (addedChannelInfo && isRelocating) {
				...
				long currentTimestamp = addLocalNodeToChannel(channelRefId);
				if (currentTimestamp == -1L) {
					// The channel is closed, so send leave message.
					// If invocation returns false, the session
					// probably disconnected.
					serverImpl.handleNotification(sessionRefId, timestamp,
			...
			Map<BigInteger, LocalMemberInfo> channelMap = localPerSessionChannelMap
					.get(sessionRefId);
			if (channelMap == null) {
				Map<BigInteger, LocalMemberInfo> newChannelMap = Collections
						.synchronizedMap(new HashMap<BigInteger, LocalMemberInfo>());
				channelMap = localPerSessionChannelMap.putIfAbsent(
						sessionRefId, newChannelMap);
				if (channelMap == null) {
					channelMap = newChannelMap;
				}
			}
			channelMap.put(channelRefId, new LocalMemberInfo(channelInfo,
					timestamp));
			...
			if (isRelocating && isReliable(delivery)
				...
				if (logger.isLoggable(Level.FINEST)) {
					logger.log(
							...
							"Retrieving missed messages for session:{0} "
									...
									+ "localNodeId:{4}", sessionRefId,
							channelRefId, timestamp + 1,
							channelInfo.msgTimestamp, localNodeId);
				}

				List<ChannelMessageInfo> missingMessages = getChannelMessages(
				...
				if (missingMessages != null) {
					for (ChannelMessageInfo messageInfo : missingMessages) {
						serverImpl.handleNotification(sessionRefId,
								messageInfo.timestamp, new ChannelSendTask(
	 ...
	 * {@code channelRefId} and returns the current message timestamp for the
	 * channel, or returns {@code -1L} if the channel is nonexistent or closed.
	 * This method should be invoked outside of a transaction.
	 */
	private long addLocalNodeToChannel(final BigInteger channelRefId) {
		try {
			return runTransactionalCallable(new KernelCallable<Long>(
					"addLocalNodeIdToChannel") {
				public Long call() {
					ChannelImpl channelImpl = (ChannelImpl) getObjectForId(channelRefId);
	...
	/**
	 * Returns a list containing saved channel messages for the channel with the
	 * specified {@code channelRefId} with timestamps between
	 * {@code fromTimestamp} and {@code toTimestamp} inclusive or {@code null}
	...
	private List<ChannelMessageInfo> getChannelMessages(
		...
		try {
			return runTransactionalCallable(new KernelCallable<List<ChannelMessageInfo>>(
				...
				public List<ChannelMessageInfo> call() {
					...
					if (channelImpl == null || channelImpl.isClosed()) {
						...
						return channelImpl.getChannelMessages(fromTimestamp,
								toTimestamp);
					}
				}
			});
	 ...
	 */
	private void removeLocalChannelMember(BigInteger channelRefId,
			BigInteger sessionRefId) {
		LocalChannelInfo channelInfo = lockChannel(channelRefId);
		if (channelInfo != null) {
			try {
	 ...
	 * set for the channel.
	 */
	private void removeLocalPerSessionChannel(BigInteger channelRefId,
			BigInteger sessionRefId) {
		Map<BigInteger, LocalMemberInfo> channelMap = localPerSessionChannelMap
				.get(sessionRefId);
		if (channelMap != null) {
	...
	/**
	 * Removes the local session from each of its channel's membership sets, and
	 * removes the local session from the local per-session channel map. This
	 * method is invoked when a session is relocated or disconnected to clean up
	 * the session's transient information.
	 */
	 ...
	 * {@code sessionRefId} is a local member of the channel with the specified
	 * {@code channelRefId}.
	 * 
	 * @param channelRefId
	 *            a channel ID
	 ...
	 *         {@code sessionRefId} is a local member of the channel with the
	...
	/**
	 * Collects a snapshot of the channel membership for the channel with the
	 * specified {@code channelRefId} and set of member {@code nodeIds} and
	...
	Set<BigInteger> collectChannelMembership(Transaction txn,
		...
		if (nodeIds.size() == 1 && nodeIds.contains(getLocalNodeId())) {
			...
			synchronized (channelMembershipCache) {
				Set<BigInteger> members = channelMembershipCache
						.get(channelRefId);
				if (members != null) {
					return members;
				}
			...
			CollectChannelMembershipTask task = new CollectChannelMembershipTask(
					channelRefId, nodeIds);
			...
			synchronized (task) {
				...
				if (task.completed) {
					if (logger.isLoggable(Level.FINEST)) {
						logger.log(Level.FINEST, "channelId:{0} nodeIds:{1} "
								+ "members:{2}", channelRefId, nodeIds,
								task.getMembers());
					}
	...
	/**
	 * Iterates through the context list, in order, to flush any committed
	 * changes. During iteration, this method invokes {@code flush} on the
	 * {@code Context} returned by {@code next}. Iteration ceases when either a
	 * context's {@code flush} method returns {@code false} (indicating that the
	 * transaction associated with the context has not yet committed) or when
	...
	/**
	 * Returns the currently active transaction, or throws
	 * {@code TransactionNotActiveException} if no transaction is active.
	 */
	 ...
	 * invoking {@link AbstractService#runIoTask runIoTask} with the
	 * {@code ioTask} and {@code nodeId}) to the task list of the given
	 * {@code channelRefId} (when the current transaction commits).
	 */
	void addChannelTaskOnCommit(BigInteger channelRefId,
			final IoRunnable ioTask, final long nodeId) {
		addChannelTaskOnCommit(channelRefId, new AbstractKernelRunnable(
				"RunIoTask") {
			public void run() {
	...
	/**
	 * Adds the specified non-transactional {@code task} to the task list of the
	 * given {@code channelRefId} (when the current transaction commits).
	 * 
	 * @param channelRefId
	 *            a channel ID
	 * @param task
	 ...
	 * 
	 * @param channelRefId
	 *            a channel ID for a locally-coordinated channel
	 */
	 ...
	 * non-transactional tasks to perform when the transaction commits. A task
	 * is added to the context by a {@code ChannelImpl} via the
	 * {@code addChannelTaskOnCommit} method. Such non-transactional tasks
	 * include sending a notification to a channel server to modify the channel
	 * membership list, or forwarding a send request to a set of channel
	 * servers.
	 */
	final class Context extends TransactionContext {

		private final Map<BigInteger, List<KernelRunnable>> internalTaskLists = new HashMap<BigInteger, List<KernelRunnable>>();

		/**
		 * Locally-coordinated channels that need servicing as a result of
		 * operations during this context's associated transaction. When this
		 * context is flushed, a task to service the event queue will be added
		 * to the channel coordinator's task queue.
		 */
		private final Set<BigInteger> channelsToService = new HashSet<BigInteger>();
		...
		/**
		 * Adds the specified non-transactional {@code task} to the task list of
		 * the given {@code channelRefId}. If the transaction commits, the task
		 * will be added to the channel's tasks queue.
		 */
		 ...
		 * locally-coordinated channels whose event queues need to be serviced
		 * as a result of operations executed during this context's associated
		 * transaction.
		 * <p>
		 * 
		 * When this context is flushed, for each channel that needs to be
		 * serviced, a task to service the channel's event queue will be added
		 * to that channel coordinator's task queue.
		 * 
		 * @param channelRefId
		 *            a locally-coordinated channel that needs to be serviced
		 */
		...
		/**
		 * Marks this transaction as prepared, and if there are pending changes,
		 * adds this context to the context list and returns {@code false}.
		 * Otherwise, if there are no pending changes returns {@code true}
		 * indicating readonly status.
		 */
		public boolean prepare() {
			isPrepared = true;
			boolean readOnly = internalTaskLists.isEmpty()
					&& channelsToService.isEmpty();
			if (!readOnly) {
				synchronized (contextList) {
		...
		/**
		 * Removes the context from the context list containing pending updates,
		 * and flushes all committed contexts preceding prepared ones.
		 */
		...
		/**
		 * Marks this transaction as committed and flushes all committed
		 * contexts preceding prepared ones.
		 */
		public void commit() {
			isCommitted = true;
			flushContexts();
		...
		/**
		 * If the context is committed, flushes channel tasks (enqueued during
		 * this transaction) to their respective coordinator's channel server
		 * task queues, then (for all channels needing servicing) notifies their
		 * respective coordinators that their event queues need servicing, and
		...
		private boolean flush() {
			...
			if (isCommitted) {
				for (BigInteger channelRefId : internalTaskLists.keySet()) {
					getCoordinator(channelRefId).addChannelNotificationTasks(
	...
	/**
	 * Notifies this service that the channel with the specified
	 * {@code channelRefId} is closed so that this service can clean up any
	...
	private final class SessionStatusListener implements
		...
		public void prepareToRelocate(final BigInteger sessionRefId,
				final long newNodeId, SimpleCompletionHandler handler) {
			try {
				...
				lockSession(sessionRefId);
				outgoingSessionRelocationInfo.put(sessionRefId,
			...
			if (channelMap == null) {
				...
				int i = 0;
				synchronized (channelMap) {
					for (Map.Entry<BigInteger, LocalMemberInfo> entry : channelMap
							.entrySet()) {
						channelRefIds[i] = entry.getKey();
						LocalMemberInfo memberInfo = entry.getValue();
						deliveryOrdinals[i] = (byte) memberInfo.channelInfo.delivery
								.ordinal();
							...
							public void run() throws IOException {
								getChannelServer(newNodeId)
										.relocateChannelMemberships(
												...
												channelRefIds,
												deliveryOrdinals, msgTimestamps);
							}
						}, newNodeId);
					}
				}, taskOwner);
		...
		public void relocated(BigInteger sessionRefId) {
			...
			if (pendingRequestsMap != null) {
				synchronized (pendingRequestsMap) {
					for (PendingRequests pendingRequests : pendingRequestsMap
							.values()) {
	...
	/**
	 * Obtains the lock associated with the specified {@code sessionRefId}. If
	 * the lock is not currently available, this method waits until the lock is
	 * freed.
	 * 
	 * @param sessionRefId
	 *            a session ID
	...
	/**
	 * Frees the lock associated with the specified (@code sessionRefId}. This
	 * method must be invoked to free the lock held by invoking the
	 ...
	 * {@code null} if no channel server exists for the given {@code nodeId}. If
	 * the specified {@code nodeId} is the local node's ID, then this method
	 * returns a reference to the server implementation object, rather than the
	 * proxy.
	 * 
	 */
	ChannelServer getChannelServer(long nodeId) {
		if (nodeId == localNodeId) {
			...
			if (channelServer != null) {
				...
				GetChannelServerTask task = new GetChannelServerTask(nodeId);
				try {
	 ...
	 * @throws Exception
	 *             the exception thrown while running {@code task}
	 */
	...
	/**
	 * Runs the specified non-durable, transactional {@code callable} using this
	 * service's task owner, and returns the result.
	 * 
	 * @param callable
	 *            a callable to call
	 * @throws Exception
	 *             the exception thrown while calliing {@code callable}
	 */
	<R> R runTransactionalCallable(KernelCallable<R> callable) throws Exception {
		return KernelCallable.call(callable, transactionScheduler, taskOwner);
	}

	/**
	 * The {@code RecoveryListener} for handling requests to recover for a
	 * failed {@code ChannelService}.
	 */
			...
			try {
				...
				transactionScheduler.runTask(new AbstractKernelRunnable(
					...
					public void run() {
						 ...
						 */
						taskService
								.scheduleTask(new ChannelImpl.ReassignCoordinatorsTask(
										nodeId));
						/*
						 ...
						 */
						taskService
								.scheduleTask(new RemoveChannelServerProxyTask(
										nodeId));
					}
				}, taskOwner);
	...
	/**
	 * The {@code NodeListener} for handling failed node notifications. When a
	 * node's fails, {@code ChannelService} recovery is distributed. One node
	 * recovers for all the failed coordinators (via the
	 * {@code RecoveryListener}), and each node removes the failed server node
	 * IDs from all the channels for which the node coordinates.
	 */
	private class ChannelServiceNodeListener implements NodeListener {
		...
		public void nodeHealthUpdate(Node node) {
			...
			try {
				if (logger.isLoggable(Level.INFO)) {
					logger.log(Level.INFO, "Node:{0} handling nodeFailed:{1}",
							localNodeId, nodeId);
				}
				...
				transactionScheduler.runTask(new AbstractKernelRunnable(
					...
					public void run() {
						taskService
								.scheduleTask(new ChannelImpl.RemoveFailedNodeFromLocalChannelsTask(
										localNodeId, nodeId));
					}
				}, taskOwner);
				...
				logger.logThrow(Level.WARNING, e,
						"Node:{0} handling nodeFailed:{1} throws", localNodeId,
						nodeId);
			}
		}
	}
	 ...
	 * node.
	 */
	private static class RemoveChannelServerProxyTask implements Task,
			Serializable {
		...
		/**
		 * Removes the channel server proxy for the node ID specified during
		 * construction.
		 */
		public void run() {
	...
	/**
	 * Returns the managed object with the specified {@code refId}, or
	 * {@code null} if there is no object with the specified {@code refId}.
	 * 
	 * @param refId
	 *            the object's identifier as obtained by
	 *            {@link ManagedReference#getId ManagedReference.getId}
	 * 
	 * @throws TransactionException
	 *             if the operation failed because of a problem with the current
	 *             transaction
	 */
	static Object getObjectForId(BigInteger refId) {
		try {
			return getInstance().getDataService().createReferenceForId(refId)
					.get();
	 ...
	 * {@code channelRefId}'s array (specified during construction). When this
	 * task is complete, it notifies the old node that relocation preparation is
	 * complete by invoking the {@code relocateChannelMembershipsCompleted}
	 * method.
	 */
	private class AddRelocatingSessionNodeToChannels extends
		...
		AddRelocatingSessionNodeToChannels(BigInteger sessionRefId,
				long oldNodeId, BigInteger[] channelRefIds,
				byte[] deliveryOrdinals, long[] msgTimestamps) {
			super(null);
		...
		public void run() {
			...
			for (int i = 0; i < channelRefIds.length; i++) {
				...
				addLocalChannelMember(channelRefId, delivery, sessionRefId,
						msgTimestamp, true);
			}

			/*
			 * Finished adding relocating session to channels, so notify old
			 * node that we are done.
			 */
			final ChannelServer server = getChannelServer(oldNodeId);
			runIoTask(new IoRunnable() {
				public void run() throws IOException {
					server.relocateChannelMembershipsCompleted(sessionRefId,
							localNodeId);
				}
			}, oldNodeId);
		}
	...
	/**
	 * An abstract class for processing a channel request (sent by the channel's
	 * coordinator) for the channel specified during construction.
	 */
	private abstract static class ChannelRequestTask {
		...
		/**
		 * Processes the channel request for the specified {@code sessionRefId}
		 * and message {@code timestamp} which may update local, transient
		 * structures, and then may deliver the appropriate notification to the
		 * client session for the given {@code sessionRefId}.
		 * 
		 * @param sessionRefId
		 *            a client session ID
		 * @param timestamp
	...
	/**
	 * A task to update local channel membership set and send a join request to
	 * a client session.
	 */
	private class ChannelJoinTask extends ChannelRequestTask {
		...
		ChannelJoinTask(String name, BigInteger channelRefId, Delivery delivery) {
			super(channelRefId);
			this.name = name;
	...
	/**
	 * A task to update local channel membership set and send a leave request to
	 * a client session.
	 */
	private class ChannelLeaveTask extends ChannelRequestTask {
		...
		public void run(BigInteger sessionRefId, long timestamp) {
			...
			if (protocol != null) {
				try {
					...
					logger.logThrow(Level.WARNING, e, "channelLeave throws");
				}
			}
			isCompleted = true;
		}
		...
		/**
		 * If this task removed the last local channel member, then remove the
		 * local node ID from the channel's server list, and remove the channel
		 * from the map of local channel members.
		 */
				...
				synchronized (localChannelMembersMap) {
					...
					try {
						if (channelInfo.members.isEmpty()) {
							try {
								runTransactionalTask(new AbstractKernelRunnable(
										"removeNodeIdFromChannel") {
									public void run() {
										ChannelImpl channel = (ChannelImpl) getObjectForId(channelRefId);
										if (channel != null) {
											channel.removeServerNodeId(localNodeId);
										}
									}
								});
			...
			if (protocol != null) {
				...
				try {
					...
					logger.logThrow(Level.WARNING, e, "channelMessage "
							+ "session:{0} channel:{0} throws", sessionRefId,
							channelRefId);
				}
			}
		}
	...
	/**
	 * A task to update local channel membership set and send a close request to
	 * a client session.
	 */
		...
		public void run(BigInteger sessionRefId, long timestamp) {
			...
			if (protocol != null) {
				try {
					...
					logger.logThrow(Level.WARNING, e, "channelLeave "
							+ "session:{0} channel:{0} throws", sessionRefId,
							channelRefId);
				}
				removeLocalPerSessionChannel(channelRefId, sessionRefId);
	...
	private class CollectChannelMembershipTask extends AbstractKernelRunnable {
		...
		public void run() {
			...
			for (long nodeId : nodeIds) {
				try {
					...
					if (server != null) {
						BigInteger[] nodeMembers = server
								.getSessions(channelRefId);
		...
		/**
		 * Returns an unmodifiable set containing a snapshot of all the members
		 * of the channel specified during construction.
		 * 
		 * @throws IllegalStateException
		 *             if the task to collect the the channel membership has not
		 *             completed
		 */
	...
	static enum MembershipEventType {
		JOIN, LEAVE
	};

	/**
	 * Channel event information for join/leave requests. If a channel is
	 * coordinated locally, channel join/leave requests should to be saved until
	 * the channel's timestamp reaches the specified {@code expirationTimestamp}
	 * so that a sender's channel membership status can be quickly and correctly
	 * determined without having to verify the membership with the session's
	 * (potentially remote) node.
	 */
	private static class MembershipEventInfo {
		...
		MembershipEventInfo(MembershipEventType eventType,
				BigInteger sessionRefId, long eventTimestamp,
	...
	/**
	 * Returns the locked {@code LocalChannelInfo} instance for the specified
	 * {@code channelRefId}, or {@code null} if no such instance exists. If this
	 * method returns a non-null value, the caller is responsible for unlocking
	 * the returned instance by invoking {@code unlockChannel} passing the
	 * instance.
	 */
	private LocalChannelInfo lockChannel(BigInteger channelRefId) {
		synchronized (localChannelMembersMap) {
			LocalChannelInfo channelInfo = localChannelMembersMap
					.get(channelRefId);
			if (channelInfo != null) {
				channelInfo.lock.lock();
			}
	...
	/**
	 * Information for a channel with local members. An instance's 'lock' must
	 * be locked before accessing other fields of the instance.
	 */
	...
	private static class LocalMemberInfo {
		...
		/**
		 * The member's msgTimestamp--if no messages received yet, then the
		 * timestamp of when session joined channel, otherwise the timestamp of
		 * the last message received. Updated upon message send.
		 */
		long msgTimestamp;
	...
	private static class PendingRequests {
		...
		final List<ChannelRequestTask> membershipUpdates = new LinkedList<ChannelRequestTask>();
		/** The queue of send request tasks. */
		final List<ChannelRequestTask> sendRequests = new LinkedList<ChannelRequestTask>();
	...
	/**
	 * Adds a task to service the event queue of the channel with the specified
	 * {@code channelRefId}. This method is only invoked on the channel's
	 * coordinator node.
	 * <p>
	 * 
	 * @param channelRefId
	...
	/**
	 * Caches the channel membership event with the specified {@code eventType},
	 * {@code channelRefId}, {@code sessionRefId}, and {@code eventTimestamp}.
	 * This method is only invoked on the channel's coordinator node.
	 * 
	 * @param eventType
	 *            an membership event type
	 * @param channelRefId
	 *            a channel ID
	 * @param sessionRefId
	 ...
	 *            the event queue's timestamp
	 */
	void cacheMembershipEvent(MembershipEventType eventType,
			BigInteger channelRefId, BigInteger sessionRefId,
			long eventTimestamp, long expirationTimestamp) {
		getCoordinator(channelRefId).cacheMembershipEvent(eventType,
				sessionRefId, eventTimestamp, expirationTimestamp);
	}

	/**
	 * Returns {@code true} if the session with the specified
	 * {@code sessionRefId} is a member of the channel with the specified
	 * {@code channelRefId}, and {@code false} otherwise. This method is only
	 ...
	 *            if {@code true}, the specified session is considered to be a
	 *            member when current event was added to the event queue
	 * @param timestamp
	 *            the timestamp of the currently executing event, beyond which
	 *            join/leave requests should not be considered in determining
	 *            channel membership
	 * @return {@code true} if the session with the specified
	 *         {@code sessionRefId} is a member of the channel with the
	 *         specified {@code channelRefId}, and {@code false} otherwise
	 */
	boolean isChannelMember(BigInteger channelRefId, BigInteger sessionRefId,
		...
		if (coordinator != null) {
			isChannelMember = coordinator.isChannelMember(sessionRefId,
					isChannelMember, timestamp);
		}
		if (logger.isLoggable(Level.FINEST)) {
	...
	/**
	 * Returns the {@code Coordinator} instance for the specified
	 * {@code channelRefId}, constructing a new instance if absent.
	 */
	...
	/**
	 * Channel coordinator transient data and operations. A coordinator instance
	 * manages incoming 'serviceEventQueue' requests for the coordinator,
	 * manages the ordering of outgoing channel server notifications, and
	 * maintains a cache of recent membership events for determining a sending
	 * client session's membership.
	 */
	private class Coordinator {
		 ...
		 * 'serviceEventQueue' requests for this coordinator so the coordinator
		 * is not overwhelmed by concurrent requests to service its event queue.
		 */
		private final TaskQueue coordinatorNotifications;

		/**
		 * A non-transactional task queue for ordering the execution of channel
		 * server notifications (join, leave, send, etc.) send by this
		 * coordinator to one or more channel servers for the channel.
		 */
		...
		Coordinator(BigInteger channelRefId) {
			...
			coordinatorNotifications = transactionScheduler.createTaskQueue();
		}

		/**
		 * Adds the tasks in the specified {@code taskList} to this
		 * coordinator's channel server notification task queue. This method is
		 * invoked when a context is flushed during transaction commit.
		 */
		 ...
		 * serially, rather than concurrently. If tasks to service a channel's
		 * event queue were processed concurrently, there would be many
		 * transaction conflicts because servicing a channel event accesses a
		 * single per-channel data structure (the channel's event queue).
		 */
		void addServiceEventQueueTask() {
			...
			coordinatorNotifications.addTask(new AbstractKernelRunnable(
					"ServiceEventQueue") {
				public void run() {
		...
		/**
		 * Caches the channel membership event with the specified
		 * {@code eventType}, {@code sessionRefId}, and {@code eventTimestamp}.
		 * The event will remain cached until its corresponding event queue
		 * reaches the specified {@code expirationTimestamp}.
		 * <p>
		 * 
		 * Note: this method removes any expired events (those with an
		 * {@code expirationTimestamp} less than the specified
		 * {@code eventTimestamp} from the queue of cached events.
		 * 
		 * @param eventType
		 *            an membership event type
		 * @param sessionRefId
		...
		void cacheMembershipEvent(MembershipEventType eventType,
			...
			if (logger.isLoggable(Level.FINEST)) {
				logger.log(Level.FINEST,
						"CACHE eventType:{0}, channelRefId:{1}, "
								+ "sessionRefId:{2}, eventTimestamp:{3}, "
								+ "expirationTimestamp:{4}", eventType,
						channelRefId, sessionRefId, eventTimestamp,
						expirationTimestamp);
			}
			synchronized (membershipEventsQueue) {
				...
				// cache event.
				membershipEventsQueue.offer(new MembershipEventInfo(eventType,
						sessionRefId, eventTimestamp, expirationTimestamp));
			}
		}
		...
		/**
		 * Returns {@code true} if the session with the specified
		 * {@code sessionRefId} is a member of this coordinator's channel, and
		 * {@code false} otherwise. This method is only invoked on the channel's
		 * coordinator node. Membership is determined as follows:
		 * <p>
		 * 
		 ...
		 * checks the queue of cached events for join/leave requests for the
		 * specified session with timestamps less than or equal to the specified
		 * timestamp.
		 * <p>
		 * 
		 * Note: this method removes any expired events (those with an
		 ...
		 *            a session ID
		 * @param isChannelMember
		 *            if {@code true}, the specified session is considered to be
		 *            a member when current event was added to the event queue
		 * @param timestamp
		 *            the timestamp of the currently executing event, beyond
		 *            which join/leave requests should not be considered in
		 ...
		 * membership event with an {@code expirationTimestamp} less than the
		 * specified {@code timestamp}. This method must be invoked while
		 * synchronized on {@code membershipEventsQueue}.
		 */
			...
			while (!membershipEventsQueue.isEmpty()) {
				...
				if (logger.isLoggable(Level.FINEST)) {
					logger.log(
							...
							info.eventType,
							(info.sessionRefId != null ? info.sessionRefId
									: "null"), info.eventTimestamp,

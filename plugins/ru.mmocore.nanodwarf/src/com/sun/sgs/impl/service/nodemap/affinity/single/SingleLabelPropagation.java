...
/**
 * A single-node implementation of the algorithm presented in "Near linear time
 * algorithm to detect community structures in large-scale networks" Raghavan,
 * Albert and Kumara 2007.
 */
public class SingleLabelPropagation extends AbstractLPA implements
	...
	public SingleLabelPropagation(AffinityGraphBuilder builder,
			ProfileCollector col, Properties properties) throws Exception {
	 ...
	 *            the properties for configuring this service
	 * @param stats
	 *            pre-constructed JMX Mbean or {@code null} if one should be
	 *            constructed
	 * @throws IllegalArgumentException
	 *             if {@code numThreads} is less than {@code 1}
	 * @throws Exception
	 ...
	 */
	public SingleLabelPropagation(AffinityGraphBuilder builder,
			ProfileCollector col, Properties properties,
			AffinityGroupFinderStats stats) throws Exception {
		super(1, properties);
		if (builder == null) {
	 ...
	 * provided at construction time. The communities are found using the label
	 ...
	 * for testing algorithm optimizations. Finding affinity groups on a single
	 * node is, in general, not useful (the affinity groups are used for load
	 * balancing, and no load balancing is required on a single node).
	 * 
	...
	public NavigableSet<RelocatingAffinityGroup> findAffinityGroups()
		...
		while (true) {
			...
			if (numThreads > 1) {
				...
				try {
					...
					changed = true;
					logger.logThrow(Level.INFO, ie, " during iteration " + t);
				}
		...
		if (logger.isLoggable(Level.FINE)) {
			double modularity = AffinityGroupGoodness.calcModularity(graph,
					groups);
			StringBuilder sb = new StringBuilder();
			sb.append(" LPA (" + numThreads + ") took " + runTime
					+ " milliseconds, " + t + " iterations, and found "
					+ groups.size() + " groups ");

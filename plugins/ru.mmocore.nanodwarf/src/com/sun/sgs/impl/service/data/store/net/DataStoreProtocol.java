 ...
 */
/*
 * FIXME: Send or check version information on initial connection. Might need to
 * create separate client and server side classes to do this.
 * 
 * RFE: Modify failure() to send the server-side stack trace, and checkResult()
 * to append it to the thrown exception's stack trace. This would be useful for
 * debugging.
 */
class DataStoreProtocol implements DataStoreServer {
	...
	private void handleCreateObject(DataStoreServer server) throws IOException {
		try {
	...
	private void handleMarkForUpdate(DataStoreServer server) throws IOException {
		try {
			long tid = in.readLong();
	...
	private void handleRemoveObject(DataStoreServer server) throws IOException {
		try {
			long tid = in.readLong();
			long oid = in.readLong();
	...
	private void handleRemoveBinding(DataStoreServer server) throws IOException {
		try {
			long tid = in.readLong();
			String name = readString(in);
	...
	private void handleNextBoundName(DataStoreServer server) throws IOException {
		try {
			long tid = in.readLong();
			String name = readString(in);
	...
	private void handleGetClassId(DataStoreServer server) throws IOException {
		try {
			long tid = in.readLong();
	...
	private void handleGetClassInfo(DataStoreServer server) throws IOException {
		try {
			long tid = in.readLong();
	...
	private void handleNextObjectId(DataStoreServer server) throws IOException {
		try {
			long tid = in.readLong();
			long oid = in.readLong();
	...
	private void checkResult() throws IOException {
		...
		Throwable exception;
		try {
			Class<? extends Throwable> exceptionClass = Class
					.forName(className).asSubclass(Throwable.class);
			Constructor<? extends Throwable> constructor = exceptionClass

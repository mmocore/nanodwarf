 ...
 */
/*
 * XXX: Use thread pools? XXX: Reap idle connections?
 */
class DataStoreServerRemote implements Runnable {
	...
	DataStoreServerRemote(DataStoreServer server, int port) throws IOException {
	...
	public void run() {
		while (true) {
			...
			try {
				new Thread(new Handler(ss.accept()), "Handler").start();
			} catch (Throwable t) {
			}
		}
	}
	...
	private class Handler implements Runnable {
		...
		public void run() {
			try {
				...
				DataStoreProtocol protocol = new DataStoreProtocol(

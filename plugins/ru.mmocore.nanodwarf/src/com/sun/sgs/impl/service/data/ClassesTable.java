 ...
 * objects. This class caches information about class descriptors and class IDs
 * that it obtains from the data store. The cache is filled on demand, and uses
 * soft and weak references, so it can be cleared by the GC as needed.
 */
final class ClassesTable {

	/*
	 * TBD: Maybe use futures to avoid simultaneous requests for the same class?
	 * -tjb@sun.com (05/21/2007)
	 */
	 ...
	 * useful. Because the maps are likely to quickly include all of the entries
	 * needed for steady state operation, supporting fast concurrent reads via a
	 * shared read lock seems like a better approach than permitting concurrent
	 * modifications. -tjb@sun.com (05/18/2007)
	 */

	/**
	 * Maps class IDs to class descriptors and associated information. Use soft
	 * references to the descriptors since they are still useful if
	 * unreferenced, but can be reconstructed if needed.
	 */
	private final Map<Integer, ClassDescInfo> classDescMap = new HashMap<Integer, ClassDescInfo>();

	/** Reference queue for cleared ObjectStreamClass soft references. */
	private final ReferenceQueue<ObjectStreamClass> refQueue = new ReferenceQueue<ObjectStreamClass>();
	 ...
	 */
	private final Map<ObjectStreamClass, ClassDescInfo> classIdMap = new WeakHashMap<ObjectStreamClass, ClassDescInfo>();
	...
	private static class ClassDescInfo extends SoftReference<ObjectStreamClass> {
		...
		/**
		 * Whether the class is a ManagedObject class with a readResolve method.
		 */
		 ...
		 * class with such a superclass, but serialization does not enforce that
		 * restriction.
		 */
		private final String missingConstructorSuperclass;

		/** Creates an instance of this class. */
		ClassDescInfo(int classId, ObjectStreamClass classDesc,
			...
			hasWriteReplace = isManagedObject
					&& hasSerializationMethod(cl, "writeReplace");
			hasReadResolve = isManagedObject
					&& hasSerializationMethod(cl, "readResolve");
			missingConstructorSuperclass = computeMissingConstructorSuperclass(cl);
		}
	 ...
	 *             if a problem occurs serializing the class descriptor
	 * @throws TransactionAbortedException
	 *             if the data store was consulted and the transaction was
	 *             aborted due to a lock conflict or timeout
	 * @throws TransactionNotActiveException
	 *             if the data store was consulted and the transaction is not
	 *             active
	 * @throws IllegalStateException
	 *             if the data store was consulted and the operation failed
	 *             because of a problem with the current transaction
	 */
	 ...
	 *             because of a problem with the current transaction
	 */
	private ClassDescInfo getClassDescInfo(Transaction txn,
		...
		if (classId > Int30.MAX_VALUE) {
			throw new IllegalStateException("Allocating more than "
					+ Int30.MAX_VALUE + " classes is not supported");
		}
		return updateMaps(classId, classDesc).classDescInfo;
	}

	/**
	 * Updates the maps to refer to the specified class ID and class descriptor.
	 * Returns the descriptor, and the associated information, that ends up
	 * mapped to the class ID. Note that the descriptor returned may be
	 * different from the one passed in if another one was obtained
	 * concurrently.
	 */
	private UpdateMapsResult updateMaps(Integer classId,
		...
		try {
			...
			return new UpdateMapsResult((existing != null) ? existing
					: classDesc, info);
		} finally {
	 ...
	 * @throws ObjectIOException
	 *             if a problem occurs deserializing the class descriptor,
	 *             including if the class ID is not found
	 * @throws TransactionAbortedException
	 *             if the data store was consulted and the transaction was
	 *             aborted due to a lock conflict or timeout
	 * @throws TransactionNotActiveException
	...
	private ObjectStreamClass getClassDesc(Transaction txn, int classId) {
		...
		try {
			UpdateMapsResult result = updateMaps(classId,
					getClassDesc(store.getClassInfo(txn, classId)));
	...
	private static ObjectStreamClass getClassDesc(byte[] classInfo) {
		...
		throw new ObjectIOException("Problem obtaining class descriptor: "
	 ...
	 * Serialization.
	 */
	private static boolean hasSerializationMethod(Class<?> forClass,
			String methodName) {
		Method method = null;
		Class<?> cl = forClass;
		while (cl != null) {
	...
	private static String computeMissingConstructorSuperclass(Class<?> cl) {
		...
		try {
			...
			if (Modifier.isPublic(modifiers)
					...
					|| (!Modifier.isPrivate(modifiers) && samePackage(cl,
							instantiatedClass))) {
				return null;
			} else {
				return cl.getName();

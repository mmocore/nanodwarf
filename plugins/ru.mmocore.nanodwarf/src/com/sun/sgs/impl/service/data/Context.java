...
final class Context extends TransactionContext implements TransactionListener {
	...
	/**
	 * Stores information about managed references. This field is logically part
	 * of the ManagedReferenceImpl class.
	 */
	...
	Context(DataServiceImpl service, DataStore store, Transaction txn,
			int debugCheckInterval, boolean detectModifications,
			ClassesTable classesTable, boolean trackStaleObjects) {
		super(txn);
	 ...
	 * returning null if it is not found. Throws ObjectNotFoundException if the
		...
		try {
			...
			if (logger.isLoggable(Level.FINER)) {
				logger.log(Level.FINER,
						"prepareAndCommit tid:{0,number,#} returns", getTxnId());
			}
		} catch (RuntimeException e) {
			if (logger.isLoggable(Level.FINER)) {
				logger.logThrow(Level.FINER, e,
						"prepareAndCommit tid:{0,number,#} throws", getTxnId());
			}
			throw e;
		}
	}

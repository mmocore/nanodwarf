...
/**
 * Provides an implementation of <code>TransactionCoordinator</code>. This class
 * is thread safe, but the {@link TransactionHandle} returned by the
 * {@link #createTransaction createTransaction} method, and the
 * {@link Transaction} associated with the handle, are not thread safe. Callers
 * should provide their own synchronization to insure that those objects are not
 * accessed concurrently from multiple threads.
 * <p>
 * 
 * Transactions created by this class can only support at most a single durable
 * participant &mdash; one that does not implement
 * {@link NonDurableTransactionParticipant}. The {@link Transaction#join join}
 * method on transactions created using this class will throw
 * {@link UnsupportedOperationException} if more than one durable participant
 * attempts to join the transaction.
 * <p>
 * 
 ...
 * <p>
 * 
 * The default timeout for bounded transactions created by this class is 100ms.
 * The default timeout for unbounded transactions created by this class is
 * Long.MAX_VALUE. These defaults may be overridden using the properties
...
public final class TransactionCoordinatorImpl implements TransactionCoordinator {
	...
	private static final class TransactionHandleImpl implements
			TransactionHandle {
		/** The transaction. */
		...
		TransactionHandleImpl(long tid, long timeout,
			...
			txn = new TransactionImpl(tid, timeout, disablePrepareAndCommitOpt,
					collectorHandle);
		}
	 ...
	 *            the {@code ProfileCollectorHandle} used to report participant
	 *            detail
	 * @throws IllegalArgumentException
	 *             if the bounded or unbounded timeout properties are less than
	 *             {@code 1}
	 */
	public TransactionCoordinatorImpl(Properties properties,
		...
		this.boundedTimeout = props.getLongProperty(
				TransactionCoordinator.TXN_TIMEOUT_PROPERTY,
				BOUNDED_TIMEOUT_DEFAULT, 1, Long.MAX_VALUE);
		this.unboundedTimeout = props.getLongProperty(
				TransactionCoordinator.TXN_UNBOUNDED_TIMEOUT_PROPERTY,
				UNBOUNDED_TIMEOUT_DEFAULT, 1, Long.MAX_VALUE);
		this.disablePrepareAndCommitOpt = props
				.getBooleanProperty(
	...
	public TransactionHandle createTransaction(long timeout) {
		if (timeout == ScheduledTask.UNBOUNDED) {
			return new TransactionHandleImpl(nextTid.getAndIncrement(),
					unboundedTimeout, disablePrepareAndCommitOpt,
					collectorHandle);
		} else if (timeout <= 0) {
			throw new IllegalArgumentException(
					"Timeout value must be greater than 0 : " + timeout);
		}
		return new TransactionHandleImpl(nextTid.getAndIncrement(), timeout,
				disablePrepareAndCommitOpt, collectorHandle);
	}

...
import static com.sun.sgs.impl.service.data.store.DataStoreHeader.ALLOCATION_BLOCK_SIZE;
import static com.sun.sgs.impl.service.data.store.DataStoreHeader.FIRST_PLACEHOLDER_ID_KEY;
import static com.sun.sgs.impl.service.data.store.DataStoreHeader.PLACEHOLDER_OBJ_VALUE;
import static com.sun.sgs.impl.service.data.store.DataStoreHeader.QUOTE_OBJ_VALUE;
 ...
 * interface layer defined in the {@link com.sun.sgs.service.store.db} package.
 * <p>
 * 
 * Note that, although this class provides support for the
 * {@link TransactionParticipant#prepare TransactionParticipant.prepare} method,
 * it does not provide facilities for resolving prepared transactions after a
 * crash. Callers can work around this limitation by insuring that the
 * transaction implementation calls
 ...
 * <code>prepareAndCommit</code> on durable participants, such as this class, so
 * the inability to resolve prepared transactions should have no effect at
 * present.
 * <p>
 * 
 ...
 * <p>
 * 
 * The constructor also passes the properties to the constructor of the
 * {@link DbEnvironment} class chosen at runtime with the
 ...
 * <li> {@link Level#CONFIG CONFIG} - Constructor properties, data store headers
 ...
 * <p>
 * 
 * In addition, name and object operations that throw
 * {@link TransactionAbortedException} will log the failure to the
 * {@code Logger} named
 * {@code com.sun.sgs.impl.service.data.store.DataStoreImpl.abort}, to make it
...
public class DataStoreImpl extends AbstractDataStore {
	...
	/**
	 * Whether object allocations should create a placeholder at the end of each
	 * allocation block. These placeholders help to avoid allocation concurrency
	 * conflicts when using BDB Java edition.
	 */
	 ...
	 * 
	 * @param <T>
	 *            the type of information stored for each transaction
	 */
	protected interface TxnInfoTable<T> {
		 ...
		 *             if the implementation determines that the transaction is
		 *             no longer active
		 * @throws IllegalStateException
		 *             if the implementation determines that the specified
		 ...
		 * @throws IllegalStateException
		 *             if the transaction is not active, or if the
		 *             implementation determines that the specified transaction
	...
	private static class ThreadTxnInfoTable<T> implements TxnInfoTable<T> {
		 ...
		 */
		private final ThreadLocal<Entry<T>> threadInfo = new ThreadLocal<Entry<T>>();
		...
		public T get(Transaction txn) {
			...
			if (entry == null) {
				...
				throw new IllegalStateException("Wrong transaction: Got " + txn
						+ ", expected " + entry.txn);
			} else {
				return entry.info;
			}
	...
	private class TxnInfo {
		...
		/**
		 * Object ID blocks whose last IDs were used during this transaction, or
		 * null if there were no such blocks. The empty blocks will be rolled
		 * back on abort, and will have their placeholders removed on commit.
		 */
		...
		private void prepareFreeObjectIds() {
			 ...
			 * may fail, because it operates on the database, so do it first and
			 * without holding the lock on the free object IDs.
			 */
		...
		String nextName(String name, DbDatabase names) {
			...
			if (name == null) {
				lastNamesCursorKey = namesCursor.findFirst() ? DataEncoding
						.decodeString(namesCursor.getKey()) : null;
			} else {
				...
				if (!matchesLast) {
					 ...
					 */
					lastNamesCursorKey = namesCursor.findNext(DataEncoding
							.encodeString(name)) ? DataEncoding
							.decodeString(namesCursor.getKey()) : null;
		...
		long nextObjectId(long oid, DbDatabase oids) {
			...
			if (oid == -1) {
				lastOidsCursorKey = oidsCursor.findFirst() ? DataEncoding
						.decodeLong(oidsCursor.getKey()) : -1;
			} else {
				...
				if (!matchesLast) {
					 ...
					 */
					lastOidsCursorKey = oidsCursor.findNext(DataEncoding
							.encodeLong(oid)) ? DataEncoding
							.decodeLong(oidsCursor.getKey()) : -1;
					/* Record if we found an exact match */
		...
		/**
		 * Returns information about free object IDs available for allocation in
		 * this transaction, or null if no IDs are already available.
		 */
		...
		/**
		 * Creates and stores information about newly available free object IDs.
		 */
		ObjectIdInfo createObjectIdInfo(long firstObjectId, long lastObjectId) {
	...
	private static final class FreeObjectIds {
		 ...
		 */
		private final Queue<ObjectIdInfo> freeObjectIdInfo = new PriorityQueue<ObjectIdInfo>();

		/**
		 * The set of object IDs of placeholders for allocation blocks that are
		 * still in use, or null if placeholders are not being used. Synchronize
		 * on the FreeObjectIds instance when accessing the contents of this
		 * field.
		 */
		 ...
		 * placeholder currently in use, or -1 if none. This method should only
		 * be called if placeholders are in use.
		 */
	...
	private static final class ObjectIdInfo implements Comparable<ObjectIdInfo> {
	 ...
	 * 
	 * @param properties
	 *            the properties for configuring this instance
	 ...
	 *             if any of the properties are invalid, as specified in the
	 *             class documentation
	 */
	public DataStoreImpl(Properties properties,
			ComponentRegistry systemRegistry, TransactionProxy txnProxy) {
		super(systemRegistry, new LoggerWrapper(Logger.getLogger(CLASSNAME)),
				new LoggerWrapper(Logger.getLogger(CLASSNAME + ".abort")));
		logger.log(Level.CONFIG, "Creating DataStoreImpl");

		PropertiesWrapper wrappedProps = new PropertiesWrapper(properties);
		String specifiedDirectory = wrappedProps
				.getProperty(DIRECTORY_PROPERTY);
		if (specifiedDirectory == null) {
			String rootDir = properties
					.getProperty(StandardProperties.APP_ROOT);
			if (rootDir == null) {
		...
		/*
		 * Use an absolute path to avoid problems on Windows. -tjb@sun.com
		 * (02/16/2007)
		 */
		...
		try {
			...
			if (!directoryFile.exists()) {
				...
				if (!directoryFile.mkdirs()) {
					throw new DataStoreException("Unable to create database "
							+ "directory : " + directoryFile.getName());
				}
			}
			env = wrappedProps.getClassInstanceProperty(
					...
					DbEnvironment.class, new Class<?>[] { String.class,
							...
							TransactionProxy.class }, directory, properties,
			...
			useAllocationBlockPlaceholders = env
			...
			logger.log(Level.CONFIG, "Created DataStoreImpl with properties:"
					+ "\n  " + DIRECTORY_PROPERTY + "=" + specifiedDirectory
					+ "\n  " + ENVIRONMENT_CLASS_PROPERTY + "="
					+ env.getClass().getName());

		} catch (RuntimeException e) {
			throw handleException(null, Level.SEVERE, e,
					"DataStore initialization");
		} catch (Error e) {
			logger.logThrow(Level.SEVERE, e, "DataStore initialization failed");
			throw e;
		} finally {
			if (dbTxn != null && !done) {
				try {
	...
	private void removeUnusedAllocationPlaceholders(DbTransaction dbTxn) {
		byte[] firstPlaceholderKey = DataEncoding
				.encodeLong(FIRST_PLACEHOLDER_ID_KEY);
		long placeholderOid = DataEncoding.decodeLong(infoDb.get(dbTxn,
				firstPlaceholderKey, true));
		if (placeholderOid < 0) {
		...
		try {
			...
			infoDb.put(dbTxn, firstPlaceholderKey, DataEncoding.encodeLong(-1));
		} finally {
			cursor.close();
		}
	}
	...
	protected long createObjectInternal(Transaction txn) {
		...
		if (objectIdInfo == null) {
			...
			boolean done = false;
			try {
				newNextObjectId = DbUtilities.getNextObjectId(infoDb, dbTxn,
						ALLOCATION_BLOCK_SIZE);
				newLastObjectId = newNextObjectId + ALLOCATION_BLOCK_SIZE - 1;
				maybeUpdateAllocationBlockPlaceholders(dbTxn, newLastObjectId);
				done = true;
				dbTxn.commit();
			} finally {
				if (!done) {
			...
			objectIdInfo = txnInfo.createObjectIdInfo(newNextObjectId,
					newLastObjectId);
		}
	...
	/** {@inheritDoc} */
	protected byte[] getObjectInternal(Transaction txn, long oid,
			boolean forUpdate) {
		TxnInfo txnInfo = checkTxn(txn);
		byte[] result = oidsDb.get(txnInfo.dbTxn, DataEncoding.encodeLong(oid),
				forUpdate);
	...
	protected void setObjectInternal(Transaction txn, long oid, byte[] data) {
		TxnInfo txnInfo = checkTxn(txn);
		oidsDb.put(txnInfo.dbTxn, DataEncoding.encodeLong(oid),
				encodeValue(data));
		txnInfo.modified = true;
	}

	/** {@inheritDoc} */
	protected void setObjectsInternal(Transaction txn, long[] oids,
			byte[][] dataArray) {
		TxnInfo txnInfo = checkTxn(txn);
	...
	protected BindingValue getBindingInternal(Transaction txn, String name) {
		TxnInfo txnInfo = checkTxn(txn);
		byte[] value = namesDb.get(txnInfo.dbTxn,
				DataEncoding.encodeString(name), false);
		if (value == null) {
	...
	/** {@inheritDoc} */
	protected BindingValue setBindingInternal(Transaction txn, String name,
			long oid) {
		TxnInfo txnInfo = checkTxn(txn);
	...
	protected BindingValue removeBindingInternal(Transaction txn, String name) {
		TxnInfo txnInfo = checkTxn(txn);
		boolean found = namesDb.delete(txnInfo.dbTxn,
				DataEncoding.encodeString(name));
		if (found) {
			txnInfo.modified = true;
			return new BindingValue(1, txnInfo.nextName(name, namesDb));
		} else {
	...
	protected void shutdownInternal() {
		synchronized (txnCountLock) {
			...
			while (txnCount > 0) {
				try {
					logger.log(Level.FINEST,
							"shutdown waiting for {0} transactions", txnCount);
					txnCountLock.wait();
	...
	protected int getClassIdInternal(Transaction txn, byte[] classInfo) {
		checkTxn(txn);
		return DbUtilities.getClassId(env, classesDb, classInfo,
				txn.getTimeout());
	}
	...
	protected byte[] getClassInfoInternal(Transaction txn, int classId)
		...
		checkTxn(txn);
		byte[] result = DbUtilities.getClassInfo(env, classesDb, classId,
				txn.getTimeout());
		if (result != null) {
			return result;
		} else {
	...
	protected boolean prepareInternal(Transaction txn) {
		...
		if (txnInfo.modified) {
			 ...
			 * whether the Berkeley DB commit operation succeeds, since Berkeley
			 * DB doesn't permit operating on its transaction object after
			 * commit is called.
			 */
			try {
	...
	protected void commitInternal(Transaction txn) {
		...
		/*
		 * Make sure to clear the transaction information, regardless of whether
		 * the Berkeley DB commit operation succeeds, since Berkeley DB doesn't
		 * permit operating on its transaction object after commit is called.
		 */
		txnInfoTable.remove(txn);
		try {
			txnInfo.commit();
	 ...
	 * <p>
	 * 
	 * This implementation converts {@link DbDatabaseException} to
	 ...
	 * 
	 * @param <T>
	 *            the type of the information to be stored
	 * @param txnInfoType
	 *            a class representing the type of the information to be stored
	 * @return the table
	 */
	...
	protected long getNextTxnId(int count, long timeout) {
		try {
			...
			throw handleException(null, Level.FINE, e, "getNextTxnId count:"
					+ count);
		}
	}
	...
	protected void joinNewTransaction(Transaction txn) {
		try {
			...
			throw handleException(txn, Level.FINER, e,
	 ...
	 * in progress. The op argument, if non-null, specifies the operation being
	 * performed under the specified transaction.
	 */
	...
	private TxnInfo joinTransaction(Transaction txn) {
		...
		try {
			...
			if (logger.isLoggable(Level.FINER)) {
				logger.log(Level.FINER, "join txn:{0}, thread:{1}", txn, Thread
	...
	/**
	 * Checks that the correct transaction is in progress, throwing an exception
	 * if the transaction has not been joined. Checks if the store is shutting
	 * down, but does not check the prepared state of the transaction.
	 */
	...
	private long getNextId(long key, int blockSize, long timeout) {
		...
		boolean done = false;
		try {
			long id = DataStoreHeader.getNextId(key, infoDb, dbTxn, blockSize);
			done = true;
			dbTxn.commit();
			return id;
		} finally {
	 ...
	 * placeholders.
	 */
	private void maybeUpdateAllocationBlockPlaceholders(DbTransaction dbTxn,
			long placeholderOid) {
	 ...
	 * more objects. If oid is -1, then returns the first object ID. The values
	 * for the object IDs will not be checked, so this method can be used to
	 * obtain object IDs for placeholders. This method is intended for testing.
	 * 
	 * @param txn
	 *            the transaction under which the operation should take place
	 * @param oid
	...
	long nextObjectIdRaw(Transaction txn, long oid) {
		...
		try {
			boolean found = (oid < 0) ? cursor.findFirst() : cursor
					.findNext(DataEncoding.encodeLong(oid + 1));
			if (found) {

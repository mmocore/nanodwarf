...
/**
 * A distributed implementation of the algorithm presented in "Near linear time
 * algorithm to detect community structures in large-scale networks" by U.N.
 * Raghavan, R. Albert and S. Kumara 2007.
 * <p>
 ...
 * {@code LabelPropagationServer}. This value must be no less than {@code 0} and
 * no greater than {@code 65535}.
 * <p>
 * 
 * <dt><i>Property:</i> <code><b>
 ...
 * 
 * <dd style="padding-top: .5em">The network port for this app node's affinity
 * group finder for communicating with affinity group finders on other app nodes
 * and with the {@code LabelPropagationServer}. This value must be no less than
 * {@code 0} and no greater than {@code 65535}.
 * <p>
 * </dl>
 */
public class LabelPropagation extends AbstractLPA implements LPAClient {
	/** Our class name. */
	private static final String CLASS_NAME = LabelPropagation.class.getName();
	...
	private static final String CLIENT_PORT_PROPERTY = PROP_NAME
			+ ".client.port";

	/** The default value of the server port. */
	private static final int DEFAULT_CLIENT_PORT = 0;
	/**
	 * Our local watchdog service, used in case of IO failures. Can be null for
	 * testing.
	 */
	...
	/**
	 * A map of cached nodeId->LPAClient. The contents of this map can change.
	 */
	private final Map<Long, LPAClient> nodeProxies = new ConcurrentHashMap<Long, LPAClient>();

	/** Our exporter. */
	private final Exporter<LPAClient> clientExporter;

	/**
	 * The map of conflicts in the system, nodeId->objId, count. Updates are
	 * multi-threaded. This includes both conflicts detected locally, and
	 * conflicts that other nodes tell us about. This map is updated during the
	 * prepare phase of the algorithm and read during the iterations. TBD:
	 * consider changing this to another data structure not using inner maps.
	 */
	private final ConcurrentMap<Long, Map<Object, Long>> nodeConflictMap = new ConcurrentHashMap<Long, Map<Object, Long>>();

	/**
	 * Map of identity -> label and count This sums all uses of that identity on
	 * other nodes. The count takes weights for object uses into account.
	 * Updates are currently single threaded, node by node. TBD: consider
	 * changing this to another data structure not using inner maps.
	 */
	private final ConcurrentMap<Identity, Map<Integer, Long>> remoteLabelMap = new ConcurrentHashMap<Identity, Map<Integer, Long>>();
	...
	/**
	 * The current algorithm run number, used to ensure we're returning values
	 * for the correct algorithm run.
	 */
	private long runNumber = -1;

	/**
	 * The current iteration being run, used to detect multiple calls for an
	 * iteration.
	 */
	private int iteration = -1;

	/**
	 * Synchronization for state, runNumber, and iteration. Using this lock
	 * ensures that results from each phase of the algorithm runs (prepare and
	 * iterations) are seen by the next phase, no matter which thread actually
	 * runs the next phase.
	 */
	 ...
	 *            the graph producer
	 * @param wdog
	 *            the watchdog service, used for error reporting
	 * @param nodeId
	 *            the local node ID
	 * @param properties
	 ...
	 *             if {@code numThreads} is less than {@code 1}
	 * @throws Exception
	 *             if any other error occurs
	 */
	public LabelPropagation(DLPAGraphBuilder builder, WatchdogService wdog,
			long nodeId, Properties properties) throws Exception {
		...
		server = (LPAServer) registry
				.lookup(LabelPropagationServer.SERVER_EXPORT_NAME);
	...
	public Set<AffinityGroup> getAffinityGroups(long runNumber, boolean done)
		...
		synchronized (stateLock) {
			if (this.runNumber != runNumber) {
				throw new IllegalArgumentException("bad run number "
						+ runNumber + ", expected " + this.runNumber);
			}
			if (done) {
		...
		if (done && logger.isLoggable(Level.FINER)) {
			logger.log(Level.FINER, "{0}: FINAL GRAPH IS {1}", localNodeId,
					graph);
		}
		...
		logger.log(Level.FINEST, "{0}: returning {1} groups", localNodeId,
				groups.size());
		return Collections.unmodifiableSet(groups);
	}
	...
	/**
	 * Prepare for an algorithm run. If we are unable to contact the server to
	 * report we are finished, log the failure.
	 * <p>
	 * Each pair of nodes needs to exchange conflict information to ensure that
	 * both pairs know the complete set for both (e.g. if node 1 has a data
	 * conflict on obj1 with node 2, it lets node 2 know.
	 * <p>
	 * It might be better to just let the server ask each vertex for its
	 * information and merge it there.
	 * 
	 * @param runNumber
	 *            the algorithm run count, used to detect problems between the
	 *            server and clients
	 */
	private void prepareAlgorithmInternal(long runNumber) {
		synchronized (stateLock) {
			...
			if (this.runNumber > runNumber) {
				...
				// the run. Do nothing.
				logger.log(Level.FINE, "{0}: bad run number {1}, "
						+ " we are on run {2}.  Returning.", localNodeId,
		...
		for (Map.Entry<Long, Map<Object, Long>> entry : nodeConflictMap
				.entrySet()) {
			...
			if (proxy != null) {
				...
				logger.log(Level.FINE,
						"{0}: could not exchange edges with {1}", localNodeId,
						nodeId);
				failed = true;
				break;
			}
	 ...
	 * notified if a node starts or stops.
	 */
	...
	/**
	 * Remove a failed node, telling dependent objects to do the same. This
	 * method is not called remotely.
	 * 
	 * @param nodeId
	 ...
	 * <p>
	 * If we are unable to contact the server to report we are finished, log the
	 * failure.
	 * 
	 * @param iteration
	...
	private void startIterationInteral(final int iteration) {
		...
		synchronized (stateLock) {
			...
			if (this.iteration > iteration) {
				...
				logger.log(Level.FINE, "{0}: bad iteration number {1}, "
						+ " we are on iteration {2}.  Returning.", localNodeId,
		...
		if (!failed) {
			...
			if (numThreads > 1) {
				...
				try {
					...
					failed = true;
					logger.logThrow(Level.INFO, ie, " during iteration "
							+ iteration);
				}
				changed = abool.get();
			...
			if (logger.isLoggable(Level.FINEST)) {
				...
				Set<AffinityGroup> intermediateGroups = gatherGroups(vertices,
						false, runNumber);
		...
		LabelPropagationServer.runIoTask(new IoRunnable() {
			public void run() throws IOException {
				server.finishedIteration(localNodeId, converged, runFailed,
						iteration);
			}
		}, wdog, localNodeId, maxIoAttempts, retryWaitTime, CLASS_NAME);
	...
	public Map<Object, Map<Integer, List<Long>>> getRemoteLabels(
		...
		Map<Object, Map<Integer, List<Long>>> retMap = new HashMap<Object, Map<Integer, List<Long>>>();
		if (objIds == null) {
		...
		for (Object obj : objIds) {
			...
			Map<Integer, List<Long>> labelWeightMap = new HashMap<Integer, List<Long>>();
	 ...
	 * {@inheritDoc}
	 * <p>
	 * Initialize our vertex conflicts. This needs to happen before we send our
	 * vertex conflict information to other nodes in response to an
	 * prepareAlgorithm call from the server, and before any
	 * notifyCrossNodeEdges calls.
	 */
	protected void doOtherInitialization() {
		...
		logger.log(Level.FINEST, "{0}: initialized node conflict map",
				localNodeId);
	}

	/** {@inheritDoc} */
	protected long doOtherNeighbors(LabelVertex vertex,
			Map<Integer, Long> labelMap, StringBuilder logSB) {
		...
		if (remoteMap != null) {
			synchronized (remoteMap) {
				for (Map.Entry<Integer, Long> entry : remoteMap.entrySet()) {
					...
					if (logger.isLoggable(Level.FINEST)) {
						logSB.append("RLabel:" + label + "(" + entry.getValue()
								+ ") ");
					}
					Long value = labelMap.containsKey(label) ? labelMap
							.get(label) : 0;
	...
	private boolean updateRemoteLabels() {
		...
		for (Map.Entry<Long, Map<Object, Long>> entry : nodeConflictMap
			...
			GetRemoteLabelsRunnable task = new GetRemoteLabelsRunnable(proxy,
					new HashSet<Object>(map.keySet()));
			boolean ok = LabelPropagationServer.runIoTask(task, wdog, nodeId,
					maxIoAttempts, retryWaitTime, CLASS_NAME);
			if (ok) {
				labels = task.getLabels();
			} else {
				failed = true;
				logger.log(Level.WARNING, "{0}: could not contact node {1}",
						localNodeId, nodeId);
				break;
			}
			...
			for (Map.Entry<Object, Map<Integer, List<Long>>> remoteEntry : labels
					.entrySet()) {
				...
				for (Map.Entry<Identity, Long> objUseId : objUse.entrySet()) {
					...
					if (labelCount == null) {
						...
						// putIfAbsent
						Map<Integer, Long> newMap = new ConcurrentHashMap<Integer, Long>();
					...
					synchronized (labelCount) {
						for (Map.Entry<Integer, List<Long>> rLabelCount : remoteLabels
								.entrySet()) {
							...
							for (Long rc : rcounts) {
								updateCount += Math.min(localCount,
										rc.longValue());
							}
	...
	/**
	 * Runnable which calls another node to get its labels. This is not an
	 * anonymous class because we need to obtain a result.
	 */
	...
	private LPAClient getProxy(long nodeId) {
		...
		if (proxy == null) {
			...
			boolean ok = LabelPropagationServer.runIoTask(task, wdog,
					localNodeId, maxIoAttempts, retryWaitTime, CLASS_NAME);
			if (!ok) {
	...
	/**
	 * Runnable which gets a proxy from the server. This is not an anonymous
	 * class because we need to obtain a result.
	 */

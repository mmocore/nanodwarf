 ...
 * underlying Berkeley DB database requires recovery. In that case, callers need
 * to restart the application or create a new instance of this class.
 * <p>
 * 
 ...
 * provide facilities for resolving prepared transactions after a crash. Callers
 * can work around this limitation by insuring that the transaction
 ...
 * <p>
 * 
 * The {@link #JeEnvironment constructor} supports the following configuration
 * properties:
 * <p>
 * 
 ...
 * transaction commits. If <code>false</code>, the modifications made in some of
 ...
 * <i>Default:</i> {@value #DEFAULT_LOCK_TIMEOUT_PROPORTION} times the value of
 * the <code>com.sun.sgs.txn.timeout</code> property, if specified, otherwise
 * times the value of the default transaction timeout.
 * 
 * <dd style="padding-top: .5em">The maximum amount of time in milliseconds that
 * an attempt to obtain a lock will be allowed to continue before being aborted.
 * Since Berkeley DB Java edition only detects deadlocks on lock timeouts, this
 * value is also the amount of time it will take to detect a deadlock. The value
 * must be greater than {@code 0}, and should be less than the transaction
 * timeout.
 * <p>
 * 
 ...
 * log database statistics, or a negative value to disable logging. The property
 * is set to {@code -1} by default, which disables statistics logging.
 * <p>
 * 
 ...
 * {@link TxnIsolationLevel#SERIALIZABLE SERIALIZABLE}.
 * <p>
 * 
 * </dl>
 * <p>
 ...
 * Berkeley DB {@link Environment} class that start with the {@code je.} prefix.
 * <p>
 * 
 ...
 * 
 * <dd style="padding-top: .5em">Perform checkpoints after 1 MB of changes. This
 * setting improves performance when there are a large number of changes being
 * committed.
 * <p>
 * 
 ...
 * 
 * <dd style="padding-top: .5em">Use shared latches to improve concurrency.
 * <p>
 * 
 * </dl>
 * <p>
 ...
 * <code>com.sun.sgs.impl.service.data.db.je</code> to log information at the
 * following logging levels:
 * <p>
 * 
 * <ul>
 * <li> {@link java.util.logging.Level#SEVERE SEVERE} - Berkeley DB failures that
 * require application restart and recovery
 * <li> {@link java.util.logging.Level#WARNING WARNING} - Berkeley DB exceptions
...
public class JeEnvironment implements DbEnvironment {
	...
	/** The package name. */
	private static final String PACKAGE = "com.sun.sgs.impl.service.data.store.db.je";
	 ...
	 */
	public static final String FLUSH_TO_DISK_PROPERTY = PACKAGE
			+ ".flush.to.disk";
	 ...
	 */
	public static final String LOCK_TIMEOUT_PROPERTY = PACKAGE
			+ ".lock.timeout";
	...
	/**
	 * The default value of the lock timeout property, if no transaction timeout
	 * is specified.
	 */
	public static final long DEFAULT_LOCK_TIMEOUT = computeLockTimeout(TransactionCoordinatorImpl.BOUNDED_TIMEOUT_DEFAULT);

	/**
	 * The property that specifies the interval in milliseconds between calls to
	...
	public static final String TXN_ISOLATION_PROPERTY = PACKAGE
			+ ".txn.isolation";
	...
	/**
	 * Default values for Berkeley DB Java Edition properties that are different
	 * from the BDB defaults.
	 */
	 ...
	 * @param properties
	 *            the properties to configure this instance
	 ...
	 */
	public JeEnvironment(String directory, Properties properties,
		...
		for (Enumeration<?> names = defaultProperties.propertyNames(); names
			...
			if (key instanceof String) {
				...
				if (propertiesWithDefaults.getProperty(property) == null) {
					propertiesWithDefaults.setProperty(property,
		...
		long defaultLockTimeout = (txnTimeout < 1) ? DEFAULT_LOCK_TIMEOUT
				: computeLockTimeout(txnTimeout);
		long lockTimeout = wrappedProps.getLongProperty(LOCK_TIMEOUT_PROPERTY,
				defaultLockTimeout, 1, Long.MAX_VALUE);
		/* Avoid overflow -- BDB treats 0 as unlimited */
		long lockTimeoutMicros = (lockTimeout < (Long.MAX_VALUE / 1000)) ? lockTimeout * 1000
				: 0;
		...
		/*
		 * Note that it seems that the lock timeout value needs to be set on the
		 * BDB JE environment in order to control how quickly deadlocks are
		...
		//FIXME Deprecated config.setTxnWriteNoSync(!flushToDisk);
		config.setDurability(flushToDisk ? Durability.COMMIT_SYNC : Durability.COMMIT_NO_SYNC);
		
		for (Enumeration<?> names = propertiesWithDefaults.propertyNames(); names
				.hasMoreElements();) {
			Object key = names.nextElement();
			if (key instanceof String) {
				String property = (String) key;
				if (property.startsWith("je.")) {
					config.setConfigParam(property,
		...
		try {
			...
			logger.logThrow(Level.SEVERE, e,
					"JeEnvironment initialization failed");
			throw e;
		}
		if (stats >= 0) {
			statsTask = new StatsRunnable();
			TaskScheduler taskScheduler = systemRegistry
					.getComponent(TaskScheduler.class);
			statsTaskHandle = taskScheduler.scheduleRecurringTask(statsTask,
					txnProxy.getCurrentOwner(), System.currentTimeMillis()
							+ stats, stats);
		}

		logger.log(Level.CONFIG, "Created JeEnvironment with properties:"
	...
	static RuntimeException convertException(Exception e,
		...
		if (convertTxnExceptions && e instanceof LockNotAvailableException) {
			return new TransactionTimeoutException("Transaction timed out: "
					+ e.getMessage(), e);
		} else if (convertTxnExceptions && e instanceof LockConflictException) {
			return new TransactionConflictException("Transaction conflict: "
					+ e.getMessage(), e);
			 ...
			 * know when they are no longer in use. It's OK to catch this Error
			 * and create a new environment instance, but this instance is dead.
		...
		throw new DbDatabaseException("Unexpected database exception: " + e, e);
	}
	...
	/** {@inheritDoc} */
	public DbTransaction beginTransaction(long timeout, boolean fullIsolation) {
		return new JeTransaction(env, timeout,
				fullIsolation ? fullIsolationTxnConfig : defaultTxnConfig);
	}

	/** {@inheritDoc} */
	public DbDatabase openDatabase(DbTransaction txn, String fileName,
			boolean create) throws FileNotFoundException {
		return new JeDatabase(env, JeTransaction.getJeTxn(txn), fileName,
				create);
	}
	 ...
	 * <p>
	 * 
	 * This implementation returns {@code true} to specify that this environment
	 * requires the use of allocation block placeholders.
	 */

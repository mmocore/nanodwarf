 ...
 * object.
 */
final class ManagedReferenceImpl<T> implements ManagedReference<T>,
		Serializable {
	 ...
	 */
	private static final LoggerWrapper debugDetectLogger = new LoggerWrapper(
	 ...
	 * 
	 * State object unmodifiedBytes NEW non-null null EMPTY null null
	 * NOT_MODIFIED non-null null MAYBE_MODIFIED non-null non-null MODIFIED
	 * non-null null FLUSHED null null REMOVED_EMPTY null null REMOVED_FETCHED
	 * non-null null
	 */
	private static enum State {
		...
		/**
		 * A reference to an existing object that has not been dereferenced yet.
		 */
		EMPTY,
	 ...
	 * created. This field is logically final, but is not declared final so that
	 * it can be set during deserialization.
	 */
	...
	/**
	 * The associated object or null. Note that managed references cannot refer
	 * to null, so this field will only be null if the object has not been
	 * fetched yet.
	 */
	 ...
	 * that this value could be different from the bytes used to deserialize the
	 * object if the serialized form of the object is not stable. Unfortunately,
	 * the built-in collection types have non-stable serialized forms.
	 */
	 ...
	 * the reference is not found. Throws ObjectNotFoundException if the object
	 * has been removed.
	 */
	static <T> ManagedReferenceImpl<T> findReference(Context context, T object) {
		assert object != null : "Object is null";
		assert object instanceof ManagedObject : "Object is not a managed object";
		ManagedReferenceImpl<T> ref = Objects.uncheckedCast(context.refs
	...
	static <T> ManagedReferenceImpl<T> safeFindReference(Context context,
		...
		if (object instanceof ManagedObject) {
			return Objects.uncheckedCast(context.refs
					.find((ManagedObject) object));
		} else {
			return null;
		}
	 ...
	 */
	static <T> ManagedReferenceImpl<T> getReference(Context context, T object) {
		assert object != null : "Object is null";
		assert object instanceof ManagedObject : "Object is not a managed object";
		ManagedReferenceImpl<T> ref = Objects.uncheckedCast(context.refs
				.find((ManagedObject) object));
		...
		if (logger.isLoggable(Level.FINEST)) {
			logger.log(Level.FINEST, "getReference tid:{0,number,#}, type:{1}"
					+ " returns oid:{2,number,#}", context.getTxnId(),
					DataServiceImpl.typeName(object), ref.getId());
		}
		return ref;
	}
	...
	void markForUpdate() {
		...
		case EMPTY:
			/*
			 * Presumably this object is being marked for update because it will
			 * be modified, so fetch the object now.
			 */
			object = deserialize(context.store.getObject(context.txn, oid,
					!context.optimisticWriteLocks()));
	...
	/**
	 * Like get, but with optional checking of the context. Checking the context
	 * should only be suppressed if the reference was just obtained from the
	 * context.
	 */
	@SuppressWarnings("fallthrough")
	T get(boolean checkContext) {
		...
		try {
			...
			switch (state) {
			case EMPTY:
				ManagedObject tempObject = deserialize(context.store.getObject(
						context.txn, oid, false));
				if (context.detectModifications) {
					unmodifiedBytes = SerialUtil.serialize(tempObject,
							context.classSerial);
		...
		if (exception == null) {
			if (logger.isLoggable(Level.FINEST)) {
				logger.log(Level.FINEST,
						"getForUpdate tid:{0,number,#}, oid:{1,number,#}"
								+ " returns type:{2}", context.getTxnId(), oid,
						DataServiceImpl.typeName(object));
			}
			@SuppressWarnings("unchecked")
			T result = (T) object;
			...
			DataServiceImpl.getExceptionLogger(exception).logThrow(
					...
					"getForUpdate tid:{0,number,#}, type:{1}, oid:{2,number,#}"
							+ " throws", context.getTxnId(),
					DataServiceImpl.typeName(object), oid);
			throw exception;
		}
	}
	...
	public boolean equals(Object object) {
		if (object == this) {
			 ...
			 * associated with the current data service, which is true since the
			 * data service is obtained from the current context rather than
			 * being represented explicitly within the reference. If it were
			 * possible to compare references associated with different data
			 * services, that would produce false equality for references to
			 * objects in the different data services that happened to have the
			 * same object IDs. -tjb@sun.com (11/09/2007)
			 */
	...
	static void flushAll(Context context) {
		...
		if (info != null) {
			context.store.setObjects(context.txn, info.getOids(),
					info.getDataArray());
		}
	}

	/**
	 * Returns the next object ID, or -1 if there are no more objects. Does not
	 * return IDs for removed objects. Specifying -1 requests the first ID.
	 */
	...
	byte[] flush() {
		...
		case MODIFIED:
			...
			break;
		case MAYBE_MODIFIED:
			byte[] modified = SerialUtil.serialize(object, context.classSerial);
			if (!Arrays.equals(modified, unmodifiedBytes)) {
				result = modified;
				if (debugDetectLogger.isLoggable(Level.FINEST)) {
					debugDetectLogger.log(Level.FINEST,

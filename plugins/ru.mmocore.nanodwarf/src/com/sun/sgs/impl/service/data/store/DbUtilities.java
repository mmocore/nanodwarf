...
import static com.sun.sgs.impl.service.data.store.DataStoreHeader.NEXT_NODE_ID_KEY;
import static com.sun.sgs.impl.service.data.store.DataStoreHeader.NEXT_OBJ_ID_KEY;
 ...
 * {@code DataStore} using the {@link com.sun.sgs.service.store.db} package.
...
public final class DbUtilities {
	 ...
	 */
	public static Databases getDatabases(DbEnvironment env,
			DbTransaction dbTxn, LoggerWrapper logger) {
		...
		try {
			...
			try {
				...
				throw new DataStoreException("Problem creating database: "
						+ e2.getMessage(), e2);
			}
		...
		try {
			...
			throw new DataStoreException("Classes database not found: "
					+ e.getMessage(), e);
		}
		try {
	 ...
	 */
	public static long getNextObjectId(DbDatabase infoDb, DbTransaction dbTxn,
			int blockSize) {
		return DataStoreHeader.getNextId(NEXT_OBJ_ID_KEY, infoDb, dbTxn,
				blockSize);
	}

	/**
	 * Returns the next available node ID and increments the stored value by the
	 * specified amount.
	 * 
	 * @param infoDb
	 *            the info database
	 ...
	 *            the transaction
	 * @param blockSize
	 *            the number of node IDs to allocate
	 * @return the next node ID
	 */
	public static long getNextNodeId(DbDatabase infoDb, DbTransaction dbTxn,
			int blockSize) {
		return DataStoreHeader.getNextId(NEXT_NODE_ID_KEY, infoDb, dbTxn,
				blockSize);
	}
	 ...
	 * @param timeout
	 *            the timeout to use when creating a transaction
	 * @return the associated class ID
	 */
	public static int getClassId(DbEnvironment env, DbDatabase classesDb,
			byte[] classInfo, long timeout) {
		byte[] hashKey = getKeyFromClassInfo(classInfo);
		boolean done = false;
		/*
		 * Use a separate transaction when obtaining the class ID so that the ID
		 * will be available for other transactions to use right away. This
		 * approach means that the class info will be registered even if the
		 * main transaction fails. If any transaction wants to register a new
		 * class, though, it's very likely that the class will be needed, even
		 * if that transaction aborts, so it makes sense to commit this
		 * operation separately to improve concurrency. -tjb@sun.com
		 * (05/23/2007)
		 * 
		...
		try {
			...
			if (hashValue != null) {
				...
				try {
					result = cursor.findLast() ? getClassIdFromKey(cursor
							.getKey()) + 1 : 1;
					byte[] idKey = getKeyFromClassId(result);
					boolean success = cursor.putNoOverwrite(idKey, classInfo);
					if (!success) {
				...
				boolean success = classesDb.putNoOverwrite(dbTxn, hashKey,
						DataEncoding.encodeInt(result));
				if (!success) {
	 ...
	 *            the class ID
	 * @param timeout
	 *            the transaction timeout for creating a transaction
	 * @return the associated class information or {@code null} if the ID is not
	 *         found
	 * @throws IllegalArgumentException
	 *             if {@code classId} is not greater than {@code 0}
	 */
	public static byte[] getClassInfo(DbEnvironment env, DbDatabase classesDb,
			int classId, long timeout) {
		if (classId <= 0) {

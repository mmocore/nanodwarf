 ...
 */
public class AffinityGraphBuilderStats extends StandardMBean implements
		AffinityGraphBuilderMXBean {
	/**
	...
	public AffinityGraphBuilderStats(ProfileCollector collector,
		...
		processingTime = (AggregateProfileCounter) consumer.createCounter(
				"processingTime", type, level);
		updateCount = (AggregateProfileCounter) consumer.createCounter(
				"updateCount", type, level);
		pruneCount = (AggregateProfileCounter) consumer.createCounter(
				"pruneCount", type, level);
	}
	 ...
	 * 
	 * @param inc
	 *            the amount to increment the processing time by
	 */

 ...
 * instance of this class are the persistent state of a client session and are
 * only accessed transactionally.
 */
public class ClientSessionImpl implements ClientSession, NodeAssignment,
		Serializable {
	...
	private static final LoggerWrapper logger = new LoggerWrapper(
			Logger.getLogger(PKG_NAME + "impl"));
	...
	/**
	 * If the value is not {@code -1}, indicates the node ID that this session
	 * is relocating to.
	 */
	private long relocatingToNode = -1;

	/*
	 * TBD: Should a managed reference to the ClientSessionListener be cached in
	 * the ClientSessionImpl for efficiency?
	 */

	/**
	 * Constructs an instance of this class with the specified
	 * {@code sessionService}, {@code identity}, and supported
	 * {@code deliveries}, and stores this instance with the following bindings:
	 * <p>
	 * 
	 * <pre>
	 ...
	 *            a client session service
	 * @param identity
	 *            the session's identity
	 * @param deliveries
	 *            the session's supported delivery requirements
	 * @param maxMessageLength
	 *            the maximum session message length
	 * @throws TransactionException
	 *             if there is a problem with the current transaction
	 */
	ClientSessionImpl(ClientSessionServiceImpl sessionService,
			Identity identity, Set<Delivery> deliveries, int maxMessageLength) {
		...
		ManagedReference<ClientSessionImpl> sessionRef = dataService
				.createReference(this);
		id = sessionRef.getId();
		this.wrappedSessionRef = dataService
				.createReference(new ClientSessionWrapper(sessionRef));
	...
	public ClientSession send(ByteBuffer message, final Delivery delivery) {
		try {
			...
			if (delivery.equals(Delivery.UNRELIABLE)) {
				...
				// server node.
				final ClientSessionServer server = sessionService
						.getClientSessionServer(nodeId);
				sessionService.taskService.scheduleNonDurableTask(
						new AbstractKernelRunnable("SendUnreliableMessage") {
							public void run() {
								try {
									server.send(idBytes, msgBytes,
											(byte) delivery.ordinal());
								} catch (IOException e) {
									if (logger.isLoggable(Level.FINE)) {
										logger.logThrow(Level.FINE, e,
												"send message:{0} throws",
												HexDumper
														.format(msgBytes, 0x50));
									}
								}
							}
						}, false);
			...
			if (logger.isLoggable(Level.FINEST)) {
				logger.logThrow(Level.FINEST, e, "send message:{0} throws",
	 ...
	 *             if the specified {@code delivery} guarantee is not supported
	 *             by any of this session's delivery guarantees
	 */
	private void checkDelivery(Delivery delivery) {
		...
		throw new DeliveryNotSupportedException("client session:" + this
				+ " does not support the delivery guarantee", delivery);
	}

	/**
	 * Initiates a relocation of this client session from the current node to
	 * {@code newNodeId}. If the client session is no longer connected or the
	 * session is already relocating then this request is ignored.
	 * 
	 * @param newNodeId
	 *            the ID of the node this session is relocating to
	 */
	...
	/**
	 * Sets the {@code relocatingToNode} field to the specified node ID. This
	 * method is invoked when a move event is processed on the original node to
	 * flag this client session as one that is relocating. When relocation is
	 * complete, the {@link #relocationComplete} method should be invoked on the
	 * client session's new node to mark the client session as having moved.
	 * 
	 * @param newNodeId
	 *            the new node that client session is relocating to
	 * @throws IllegalStateException
	 *             if this method is invoked from a node other than the
	 *             session's local node
	 */
	 ...
	 * services this session's event queue. This method is invoked when the
	 * associated client connects to the new node to re-establish the client
	 * session.
	 * 
	 * @throws IllegalStateException
	 *             if this method is invoked from a node other than the
	...
	/**
	 * Returns {@code true} if the session is relocating, and {@code false}
	 * otherwise.
	 * 
	 * @return {@code true} if the session is relocating, and {@code false}
	 *         otherwise
	 */
	...
	/**
	 * Updates this client session's node ID and bindings to the client session
	 * to reflect its reassignment to {@code newNodeId}.
	 * 
	 * @param newNodeId
	 *            the node this session is relocating to
	 * @throws IllegalArgumentException
	 *             if {@code newNodeId} does not match the local node ID
	 */
	void move(long newNodeId) {
		if (newNodeId != sessionService.getLocalNodeId()) {
			throw new IllegalArgumentException("newNodeId:" + newNodeId
					+ " must match the local node ID:"
	...
	/**
	 * If the session is connected, enqueues a disconnect event to this client
	 * session's event queue, and marks this session as disconnected.
	 */
	void disconnect() {
		if (isConnected()) {
	...
	public boolean equals(Object obj) {
		if (this == obj) {
			...
			return equalsInclNull(identity, session.identity)
	...
	/**
	 * Returns {@code true} if the given objects are either both null, or both
	 * non-null and invoking {@code equals} on the first object passing the
	 * second object returns {@code true}.
	 */
	...
	/**
	 * Returns the {@code ClientSession} instance for the given {@code id},
	 * retrieved from the specified {@code dataService}, or {@code null} if the
	 * client session isn't bound in the data service. This method should only
	...
	static ClientSessionImpl getSession(DataService dataService, BigInteger id) {
		...
		try {
			ManagedReference<?> sessionRef = dataService
	 ...
	 * {@code ClientSessionListener} (if present and {@code notify} is
	 * {@code true}), removes the listener and its binding (if present), and
	 * then removes this session and its bindings from the specified
	 * {@code dataService}. If the bindings have already been removed from the
	 * {@code dataService} this method takes no action. This method should only
	 * be called within a transaction.
	 * 
	 * @param dataService
	 *            a data service
	 * @param graceful
	 *            {@code true} if disconnection is graceful, and {@code false}
	 *            otherwise
	 * @param notify
	 *            {@code true} if the {@code disconnected} callback should be
	 *            invoked
	 * @throws TransactionException
	 *             if there is a problem with the current transaction
	 */
	void notifyListenerAndRemoveSession(final DataService dataService,
			final boolean graceful, boolean notify) {
		...
		/*
		 * Get ClientSessionListener, and remove its binding and wrapper if
		 * applicable. The listener may not be bound in the data service if: the
		 * AppListener.loggedIn callback either threw a non-retryable exception
		 * or returned a null listener, or the application removed the
		 ...
		 */
		try {
			ManagedObject eventQueue = dataService
					.getServiceBinding(eventQueueKey);
			dataService.removeServiceBinding(eventQueueKey);
			dataService.removeObject(eventQueue);
		} catch (NameNotBoundException e) {
			logger.logThrow(Level.FINE, e,
					"removing EventQueue for session:{0} throws", this);
		}

		/*
		 * Invoke listener's 'disconnected' callback if 'notify' is true and a
		 * listener exists for this client session. If the 'disconnected'
		 * callback throws a non-retryable exception, schedule a task to remove
		 * this session and its associated bindings without invoking the
		 * listener, and rethrow the exception so that the currently executing
		 * transaction aborts.
		 */
				...
				if (!isRetryableException(e)) {
					logger.logThrow(Level.WARNING, e,
							"invoking disconnected callback on listener:{0} "
									+ "for session:{1} throws", listener, this);
		...
		try {
			...
			logger.logThrow(Level.WARNING, e,
					"session binding already removed:{0}", sessionKey);
		}
	...
	private String getSessionKey() {
		return PKG_NAME + SESSION_COMPONENT + HexDumper.toHexString(idBytes);
	}

	/**
	 * Returns the key to access from the data service the
	 * {@code ClientSessionListener} instance for this instance. If the
	 * {@code ClientSessionListener} does not implement {@code ManagedObject},
	...
	private String getListenerKey() {
		return PKG_NAME + LISTENER_COMPONENT + HexDumper.toHexString(idBytes);
	}
	...
	/**
	 * Returns the prefix to access from the data service
	 * {@code ClientSessionImpl} instances with the the specified {@code nodeId}
	 * .
	 */
	 ...
	 *            a data service
	 * @param listener
	 *            a client session listener
	 * @throws TransactionException
	 *             if there is a problem with the current transaction
	 */
	void putClientSessionListener(DataService dataService,
			ClientSessionListener listener) {
		ManagedObject managedObject = (listener instanceof ManagedObject) ? (ManagedObject) listener
	...
	/**
	 * Returns the client session listener, obtained from the specified
	 * {@code dataService}, for this session. This method should only be called
	...
	ClientSessionListener getClientSessionListener(DataService dataService) {
		...
		return (obj instanceof ListenerWrapper) ? ((ListenerWrapper) obj).get()
	 ...
	 */
	private static class ListenerWrapper implements ManagedObject, Serializable {
		private static final long serialVersionUID = 1L;
	 ...
	 * {@code sessionId}, or null if the event queue is not bound in the data
	 * service.
	 */
	private static EventQueue getEventQueue(byte[] sessionId) {
		DataService dataService = ClientSessionServiceImpl.getInstance()
				.getDataService();
	...
	/**
	 * Returns this client session's event queue, or null if the event queue is
	 * not bound in the data service.
	 */
	...
	/**
	 * Returns {@code true} if session is on local node and is not relocating,
	...
	/**
	 * Adds the specified session {@code event} to this session's event queue
	 * and notifies the client session service on the session's node that there
	 * is an event to service.
	 */
	private void addEvent(SessionEvent event) {
		...
		/*
		 * If this session is connected to the local node, the event queue is
		 * empty, and the session is not relocating, then service the event
		 * immediately without adding it to the event queue.
		 * 
		 * Otherwise, add the event to the event queue. If the session is not
		 * relocating, then if the session is connected locally service the head
		 * of the event queue, otherwise schedule a task to send a request to
		 * this session's client session server to service this session's event
		 * queue.
		 * 
		 * If the session is relocating, then the servicing of events will
		 * resume when the client connects to the new node to re-establish the
		 * client session.
		 */
		if (isLocalSession && eventQueue.isEmpty() && !isRelocating()) {
			logger.log(Level.FINEST, "immediately processing event:{0}", event);
			event.serviceEvent(eventQueue, sessionService,
			...
			if (isLocalSession) {
				...
				sessionService.getTaskScheduler().scheduleTask(
						new AbstractKernelRunnable("ServiceEventQueue") {
							public void run() {
								sessionService.runIoTask(new IoRunnable() {
									public void run() throws IOException {
										sessionServer
												.serviceEventQueue(idBytes);
									}
								}, nodeId);
							}
						}, identity);
	...
	/**
	 * Services the event queue for the session with the specified
	 * {@code sessionId}.
	 */
	 ...
	 */
	private abstract static class SessionEvent implements ManagedObject,
			Serializable {
		...
		/**
		 * Services this event, taken from the head of the given
		 * {@code eventQueue}.
		 */
		 ...
		 * to reject events when the total cost is too large. The cost of the
		 * event is the size (in bytes) of a message generated as a result of
	...
	/**
	 * A client session 'send' event, enqueued by a {@code ClientSession.send}
	 * invocation. When this event commits, a task is scheduled to deliver the
	 * message, specified during construction, to the client session.
	 */
	static class SendEvent extends SessionEvent {
		...
		void serviceEvent(EventQueue eventQueue,
			...
			sessionService.checkContext().addCommitAction(
					...
					handler.new SendMessageAction(this), false);
		}
	...
	/**
	 * A client session 'move' event. This event is processed on the client
	 * session's old node to mark the session as relocating. Once the session is
	 * marked for relocation, the associated session's event processing is
	 * suspended until the session is relocated to the new node.
	 * <p>
	 * 
	 * When this event commits, a task is scheduled to commence preparation for
	 * the client session to relocate. The first action is to obtain a
	 * relocation key from the new node and notify interested parties to prepare
	 * for relocation. See {@link ClientSessionHandler#MoveAction} for details.
	 */
	private static class MoveEvent extends SessionEvent {
		...
		void serviceEvent(EventQueue eventQueue,
			...
			if (newNode == null) {
				...
				if (logger.isLoggable(Level.FINE)) {
					logger.log(Level.FINE, "Session:{0} to relocate "
							+ "from node:{1} to node:{2}", this,
							sessionService.getLocalNodeId(), newNodeId);
				}
				sessionService.checkContext().addCommitAction(
	 ...
	 * problem during client login, after the client session has been persisted.
	 * <p>
	 * 
	 * When this event commits, a task is scheduled to disconnect the client
	 * session, cleanup its persistent data, and notify the client session's
	 * listener of the disconnection.
	 */
	...
	private static class EventQueue implements ManagedObjectRemoval,
		...
		EventQueue(ClientSessionImpl sessionImpl) {
			DataService dataService = sessionImpl.sessionService
					.getDataService();
			sessionRef = dataService.createReference(sessionImpl);
			queueRef = dataService
					.createReference(new ManagedQueue<SessionEvent>());
		...
		boolean offer(SessionEvent event) {
			...
			if (logger.isLoggable(Level.FINEST)) {
				logger.log(Level.FINEST,
						...
						event, ClientSessionServiceImpl.getInstance()
								.getLocalNodeId());
			}
		...
		/**
		 * Throws a retryable exception if the event queue is not in a state to
		...
		void serviceEvent() {
			...
			ClientSessionServiceImpl sessionService = ClientSessionServiceImpl
					.getInstance();
			ClientSessionHandler handler = sessionService
					.getHandler(getSessionRefId());
			...
			for (int i = 0; i < sessionService.eventsPerTxn; i++) {
				...
				if (cost > 0) {
					...
					writeBufferAvailable += cost;
					if (logger.isLoggable(Level.FINEST)) {
						logger.log(Level.FINEST, "{0} cleared reservation of "
		...
		public void removingObject() {
			try {
				DataService dataService = ClientSessionServiceImpl
						.getInstance().getDataService();
	...
	/**
	 * A persistent task to schedule tasks to notify (in succession) the client
	 * session listener of each disconnected session on a given failed node and
	 * to clean up the persistent data and bindings of those client sessions. In
	 * a single task, one disconnected session is scheduled to be handled, and
	 * then this task is rescheduled to schedule the handling of the next
	 * disconnected client session (if one exists).
	 */
	static class HandleNextDisconnectedSessionTask implements Task,
			Serializable {
		/** The serialVersionUID for this class. */
		private static final long serialVersionUID = 1L;
		...
		public void run() {
			...
			if (key != null && key.startsWith(nodePrefix)) {
				TaskService taskService = ClientSessionServiceImpl
						.getTaskService();
				taskService
						.scheduleTask(new CleanupDisconnectedSessionTask(key));
				lastKey = key;
	...
	/**
	 * A persistent task to clean up a client session bound to a given
	 * {@code key} (specified during construction), by invoking the
	 * {@code notifyListenerAndRemoveSession} method on that client session.
	 */
	private static class CleanupDisconnectedSessionTask implements Task,
			Serializable {
		/** The serialVersionUID for this class. */
		private static final long serialVersionUID = 1L;
		...
		/**
		 * Constructs an instance of this class with the specified {@code key}.
		 */
		...
		public void run() {
			DataService dataService = ClientSessionServiceImpl.getInstance()
					.getDataService();
			ClientSessionImpl sessionImpl = (ClientSessionImpl) dataService
					.getServiceBinding(key);
			sessionImpl
					.notifyListenerAndRemoveSession(dataService, false, true);
		}
	}
}

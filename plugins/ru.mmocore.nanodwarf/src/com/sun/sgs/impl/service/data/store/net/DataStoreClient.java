 ...
 * a transaction that uses the data store will be permitted to run before it is
 * a candidate for being aborted. This value must be greater than {@code 0}.
 * <p>
 * 
 * <dd style="padding-top: .5em">Whether to run the server by creating an
 * instance of {@link DataStoreServerImpl}, using the properties provided to
 ...
 * present, or {@code localhost} if this node is starting the server.
 * 
 * <dd style="padding-top: .5em">The name of the host running the
 * {@code DataStoreServer}.
 * <p>
 * 
 * <dt><i>Property:</i> <code><b>
 ...
 * {@code DataStoreServer}. This value must be no less than {@code 0} and no
 ...
 * {@code com.sun.sgs.impl.service.data.store.net.client} to log information at
...
public final class DataStoreClient extends AbstractDataStore {
	...
	private static final String SERVER_HOST_PROPERTY = PACKAGE + ".server.host";

	/** The property that specifies the server port. */
	private static final String SERVER_PORT_PROPERTY = PACKAGE + ".server.port";
	...
	/**
	 * The number of milliseconds to wait between attempts to obtain the server.
	 */
	...
	private final ThreadLocal<TxnInfo> threadTxnInfo = new ThreadLocal<TxnInfo>();
	 ...
	 *             if the
	 *             {@code com.sun.sgs.impl.service.data.store.net.server.host}
	 *             property is not set, or if the value of the
	 *             {@code com.sun.sgs.impl.service.data.store.net.server.port}
	 *             property is not a valid integer not less than {@code 0} and
	 *             not greater than {@code 65535}
	 * @throws IOException
	 *             if a network problem occurs
	 * @throws NotBoundException
	 *             if the server is not found in the Java RMI registry
	 */
	public DataStoreClient(Properties properties,
			ComponentRegistry systemRegistry, TransactionProxy txnProxy)
			throws IOException, NotBoundException {
		super(systemRegistry, new LoggerWrapper(Logger.getLogger(PACKAGE
				+ ".client")), new LoggerWrapper(Logger.getLogger(PACKAGE
				+ ".client.abort")));
	 ...
	 * {@link NetworkException}s, converts {@link TransactionNotActiveException}
	 * to {@link TransactionTimeoutException} if the transaction timeout has
	 * passed, and notes in the information for the transaction if the
	 * transaction has been aborted on the server side.
	 */
	@Override
	protected RuntimeException handleException(Transaction txn, Level level,
		...
		if (e instanceof NetworkException) {
			...
			e = new NetworkException(operation
			...
			if (duration > txn.getTimeout()) {
				e = new TransactionTimeoutException(operation
						+ " failed: Transaction timed out after " + duration
						+ " ms", e);
			}
		}
		/*
		 * If we're throwing an exception saying that the transaction was
		 * aborted, then note that the server must already know about the abort.
		 */
	...
	public String toString() {
		return "DataStoreClient[" + "nodeId:" + nodeId + ", serverHost:"
	...
	private TxnInfo checkTxn(Transaction txn) throws IOException {
		...
		if (txnInfo == null) {
			...
			throw new IllegalStateException("Wrong transaction: Found "
					+ txnInfo.txn + ", expected " + txn);
	 ...
	 * if the transaction has not been joined. If notAborting is true, then
	 * checks if the store is shutting down.
	 */

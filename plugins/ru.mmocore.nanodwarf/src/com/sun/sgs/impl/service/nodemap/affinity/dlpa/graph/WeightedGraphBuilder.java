 ...
 * vertices and a single weighted edges representing objects used by both
 * identities.
 * <p>
 * The data access information naturally forms a bipartite graph, with vertices
 * being either identities or objects, and an edge connecting each identity
 * which has accessed an object. However, we want a graph with vertices for
 * identities, and edges representing object accesses between identities, so we
 * need the bipartite graph to be folded.
 * <p>
...
public class WeightedGraphBuilder extends AbstractAffinityGraphBuilder
	...
	/**
	 * Map for tracking object-> map of identity-> number accesses (thus we keep
	 * track of the number of accesses each identity has made for an object, to
	 * aid maintaining weighted edges)
	 */
	private final ConcurrentMap<Object, Map<Identity, Long>> objectMap = new ConcurrentHashMap<Object, Map<Identity, Long>>();

	/**
	 * Our graph of object accesses. Changes to the graph must be made with the
	 * graph locked.
	 */
	 ...
	 * vertices. Changes to this map should occur atomically with vertex changes
	 * to the affinity graph, with a lock held on the {@code affinityGraph}.
	 */
	private final Map<Identity, LabelVertex> identMap = new HashMap<Identity, LabelVertex>();

	/**
	 * Our recorded cross-node accesses. We keep track of this through conflicts
	 * detected in data cache kept across nodes; when a local node is evicted
	 * from the cache because of a request from another node for it, we are told
	 * of the eviction. Map of nodes to objects that were evicted to go to that
	 * node, with a count. TBD: consider changing this to another data structure
	 * not using inner maps.
	 */
	private final ConcurrentMap<Long, Map<Object, Long>> conflictMap = new ConcurrentHashMap<Long, Map<Object, Long>>();
	 ...
	 * changed. Groups of changes are chunked into periods, each the length of
	 * the time snapshot (configured at construction time). We periodically
	 * remove the changes made in the earliest snapshot. The pruneTask is
	 * {@code null} on the core server node because no graph information is
	 * accumulated on that node.
	 */
	private final PruneTask pruneTask;

	/**
	 * Our JMX exposed information, or {@code null} if we are on the core server
	 * node. No graph actions occur on the core server node.
	 */
	 ...
	 * 
	 * @param properties
	 *            the properties for configuring this builder
	...
	public WeightedGraphBuilder(Properties properties,
		...
		NodeType type = NodeType.valueOf(properties
				.getProperty(StandardProperties.NODE_TYPE));
		ProfileCollector col = systemRegistry
		...
		if (type == NodeType.coreServerNode) {
			...
			stats = new AffinityGraphBuilderStats(col, affinityGraph,
	 ...
	 * {@inheritDoc}
	 * <p>
	 * This method is called by a single thread but must protect itself from
	 * changes to data structures made by the pruner.
	 */
			...
			synchronized (affinityGraph) {
				...
				for (Map.Entry<Identity, Long> entry : idMap.entrySet()) {
					...
					if (!ident.equals(owner)) {
						...
						WeightedEdge edge = affinityGraph.findEdge(vowner,
								vident);
						if (edge == null) {
	...
	public UndirectedGraph<LabelVertex, WeightedEdge> getAffinityGraph() {
		return Graphs.unmodifiableUndirectedGraph(Graphs
				.synchronizedUndirectedGraph(affinityGraph));
	}
	 ...
	 * 
	 * @param objId
	 *            the object that was evicted
	 * @param nodeId
	 *            the node that caused the eviction
	 * @param forUpdate
	 *            {@code true} if this eviction was for an update, {@code false}
	 *            if it was for read only access
	 */
	 ...
	 *             if this builder does not support graph pruning.
	 */
	...
	/**
	 * The graph pruner. It runs periodically, and is the only code that removes
	 * edges and vertices from the graph.
	 */
	private class PruneTask extends TimerTask {
		...
		private final Deque<Map<Long, Map<Object, Integer>>> periodConflictQueue = new ArrayDeque<Map<Long, Map<Object, Integer>>>();
		 ...
		 * 
		 * @param count
		 *            the number of full snapshots we wish to retain as live
		 *            data
		 */
		...
		public void run() {
			...
			synchronized (affinityGraph) {
				...
				for (Map.Entry<WeightedEdge, Integer> entry : periodEdgeIncrements
						.entrySet()) {
					WeightedEdge edge = entry.getKey();
					int weight = entry.getValue();
					if (edge.getWeight() == weight) {
						Pair<LabelVertex> endpts = affinityGraph
								.getEndpoints(edge);
			...
			for (Map.Entry<Long, Map<Object, Integer>> entry : periodConflicts
					.entrySet()) {
		...
		/**
		 * Note that an edge's weight has been incremented. Called by a single
		 * thread.
		 * 
		 * @param edge
		 *            the edge
		...
		void incrementEdge(WeightedEdge edge) {
			synchronized (currentPeriodLock) {
				int v = currentPeriodEdgeIncrements.containsKey(edge) ? currentPeriodEdgeIncrements
						.get(edge) : 0;
				v++;
		...
		/**
		 * Note that an object has been accessed. Called by a single thread.
		 * 
		 * @param objId
		 *            the object
		 * @param owner
		...
		void updateObjectAccess(Object objId, Identity owner) {
			synchronized (currentPeriodLock) {
				Map<Identity, Integer> periodIdMap = currentPeriodObject
						.get(objId);
				if (periodIdMap == null) {
					periodIdMap = new HashMap<Identity, Integer>();
					currentPeriodObject.put(objId, periodIdMap);
				}
				int periodValue = periodIdMap.containsKey(owner) ? periodIdMap
						.get(owner) : 0;
				periodValue++;
		 ...
		 * 
		 * @param objId
		 *            the objId of the object causing the conflict
		 * @param nodeId
		 *            the node ID of the node we were in conflict with
		 */
		void updateConflict(Object objId, long nodeId) {
			synchronized (currentPeriodLock) {
				Map<Object, Integer> periodObjMap = currentPeriodConflicts
						.get(nodeId);
				if (periodObjMap == null) {
					periodObjMap = new HashMap<Object, Integer>();
					currentPeriodConflicts.put(nodeId, periodObjMap);
				}
				int periodValue = periodObjMap.containsKey(objId) ? periodObjMap
						.get(objId) : 0;
				periodValue++;
		...
		private void addPeriodStructures() {
			...
			currentPeriodConflicts = new HashMap<Long, Map<Object, Integer>>();

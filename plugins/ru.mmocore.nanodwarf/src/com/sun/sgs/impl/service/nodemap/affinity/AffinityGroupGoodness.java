...
public final class AffinityGroupGoodness {
	...
	/**
	 * A private constructor: we do not want instances of this class to be
	 * constructed, as it contains only static utility methods.
	 */
	 ...
	 * <p>
	 * The modularity will be a number between 0.0 and 1.0, with a higher number
	 * being better.
	 * <p>
	 * See "Finding community structure in networks using eigenvectors of
	 * matrices" 2006 Mark Newman and "Finding community structure in very large
	 * networks" 2004 Clauset, Newman, Moore.
	 * <p>
	 * Note that modularity can only be calculated on a complete graph.
	 * 
	 * @param graph
	 *            the graph which was divided into communities
	 * @param groups
	...
	public static double calcModularity(Graph<LabelVertex, WeightedEdge> graph,
		...
		for (AffinityGroup g : groups) {
			...
			int size = ids.size();
			ArrayList<LabelVertex> groupList = new ArrayList<LabelVertex>(size);
			for (Identity id : ids) {
	 ...
	 * collections. The value will be between {@code 0.0} and {@code 1.0}, with
	 * higher values indicating stronger similarity between two samples. See
	 * page 8 of "Near linear time algorithm to detect community structures in
	 * large-scale networks" 2007 Raghavan, Albert, Kumara.
	 * <p>
	 * Because Jaccard's index uses computed groups, rather than a graph, it can
	 * be useful when the graphs are distributed or incomplete.
	 * <p>
	 * 
	 * @param sample1
	 *            the first sample
	...
	public static double calcJaccard(Collection<AffinityGroup> sample1,
		...
		for (AffinityGroup group : sample1) {
			ArrayList<Identity> groupList = new ArrayList<Identity>(
					group.getIdentities());
	 ...
	 *            the second identity
	 * @param sample
	 *            the affinity group collection
	 * @return {@code true} if {@code id1} and {@code id2} are in the same
	 *         affinity group in the {@code sample} collection of affinity
	 *         groups
	 */

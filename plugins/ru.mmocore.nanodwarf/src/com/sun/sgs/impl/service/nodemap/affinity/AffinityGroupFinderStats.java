 ...
 */
public class AffinityGroupFinderStats extends StandardMBean implements
		AffinityGroupFinderMXBean {
	/**
	...
	/**
	 * Configuration info. TBD: what if we allow this be modified?
	 */
	...
	/**
	 * The total number of stopped algorithm runs. Stopped runs are not
	 * considered failures.
	 */
	 ...
	 * @param stopIter
	 *            the maximum iterations a run will perform
	 */
	public AffinityGroupFinderStats(LPAAffinityGroupFinder finder,
		...
		iterations = (AggregateProfileSample) consumer.createSample(
				"iterations", type, level);
		runtime = (AggregateProfileSample) consumer.createSample("runtime",
				type, level);
		runs = (AggregateProfileCounter) consumer.createCounter("runs", type,
				level);
		failed = (AggregateProfileCounter) consumer.createCounter("failed",
				type, level);
		stopped = (AggregateProfileCounter) consumer.createCounter("stopped",
				type, level);
	}
	 ...
	 * 
	 * @param sample
	 *            the amount of runtime, in msecs
	 */
	...
	/**
	 * Increment the count of stopped algorithm runs, due to reaching the
	 * maximum number of iterations (if set).
	 */
	 ...
	 * 
	 * @param value
	 *            the number of groups found in the last run.
	 */

 ...
 * for name bindings, and is a transaction participant. Object and name accesses
 * are logged to {@link AccessReporter}s whose source name includes the name of
 * the concrete class.
 * <p>
 * 
 ...
 * bindings for details of how each operation fits into this scheme.
 * <p>
 * 
 * Note that this class does not perform next key locking for objects, because
 * the way objects are allocated and used makes that unnecessary. Because there
 * is no method at the application level for obtaining objects by object ID,
 * applications can only officially obtain objects that are stored as the value
 * of a name binding, or that are reachable by navigation from a name binding.
 * In addition, object IDs are never reused. The combination of these two
 * factors means that applications cannot ask to see an object that was created
 * simultaneously by another transaction, which is what next key locking is
 * intended to prevent. This lack of consistency enforcement for objects does
 * mean that services, which can access objects by ID, need to make sure not to
 * ask questions about objects that they don't have other reasons to believe
 * exist. In particular, it means that object iteration may return inconsistent
 * results.
 */
public abstract class AbstractDataStore implements DataStore,
	 ...
	 *            the registry of available system components
	 * @param logger
	 *            the main logger for this class
	 * @param abortLogger
	 *            the logger for transaction abort exceptions
	...
	protected AbstractDataStore(ComponentRegistry systemRegistry,
			LoggerWrapper logger, LoggerWrapper abortLogger) {
		checkNull("logger", logger);
		checkNull("abortLogger", abortLogger);
		this.logger = logger;
		this.abortLogger = abortLogger;
		String className = getClass().getName();
		AccessCoordinator accessCoordinator = systemRegistry
				.getComponent(AccessCoordinator.class);
		objectAccesses = accessCoordinator.registerAccessSource(className
				+ ".objects", Long.class);
		nameAccesses = accessCoordinator.registerAccessSource(className
				+ ".names", String.class);
	}
	 ...
	 * <p>
	 * 
	 * This implementation does logging and calls
	 * {@link #getLocalNodeIdInternal} to perform the actual operation.
	 */
	public long getLocalNodeId() {
		...
		try {
			...
			if (logger.isLoggable(FINEST)) {
				logger.log(FINEST, "getLocalNodeId returns nodeId:{0}", result);
			}
			return result;
	...
	/**
	 * Performs the actual operation for {@link #getLocalNodeId getLocalNodeId}.
	 * 
	...
	public long createObject(Transaction txn) {
		...
		try {
			...
			if (logger.isLoggable(FINEST)) {
				logger.log(FINEST,
						"createObject txn:{0} returns oid:{1,number,#}", txn,
						result);
			}
			return result;
		} catch (RuntimeException e) {
	 ...
	 * 
	 * @param txn
	 *            the transaction under which the operation should take place
	 * @return the new object ID
	 * @throws TransactionAbortedException
	 *             if the transaction was aborted due to a lock conflict or
	 *             timeout
	...
	public void markForUpdate(Transaction txn, long oid) {
		if (logger.isLoggable(FINEST)) {
			logger.log(FINEST, "markForUpdate txn:{0}, oid:{1,number,#}", txn,
					oid);
		}
		try {
			...
			if (logger.isLoggable(FINEST)) {
				logger.log(FINEST,
						"markForUpdate txn:{0}, oid:{1,number,#} returns", txn,
						oid);
			}
		} catch (RuntimeException e) {
			throw handleException(txn, FINEST, e, "markForUpdate txn:" + txn
					+ ", oid:" + oid);
		}
	}
	...
	public byte[] getObject(Transaction txn, long oid, boolean forUpdate) {
		if (logger.isLoggable(FINEST)) {
			logger.log(FINEST,
					"getObject txn:{0}, oid:{1,number,#}, forUpdate:{2}", txn,
					oid, forUpdate);
		}
		try {
			...
			if (logger.isLoggable(FINEST)) {
				logger.log(FINEST, "getObject txn:{0}, oid:{1,number,#}"
						+ ", forUpdate:{2} returns", txn, oid, forUpdate);
			}
			return result;
		} catch (RuntimeException e) {
			throw handleException(txn, FINEST, e, "getObject txn:" + txn
	 ...
	 *            the object ID
	 * @param forUpdate
	 *            whether the caller intends to modify the object
	 ...
	 *             transaction
	 */
	protected abstract byte[] getObjectInternal(Transaction txn, long oid,
			boolean forUpdate);
	...
	public void setObject(Transaction txn, long oid, byte[] data) {
		if (logger.isLoggable(FINEST)) {
			logger.log(FINEST, "setObject txn:{0}, oid:{1,number,#}", txn, oid);
		}
		try {
			checkNull("data", data);
			reportObjectAccess(txn, oid, WRITE);
			...
			if (logger.isLoggable(FINEST)) {
				logger.log(FINEST,
						"setObject txn:{0}, oid:{1,number,#} returns", txn, oid);
			}
		} catch (RuntimeException e) {
			throw handleException(txn, FINEST, e, "setObject txn:" + txn
					+ ", oid:" + oid);
		}
	}
	 ...
	 *             transaction
	 */
	protected abstract void setObjectInternal(Transaction txn, long oid,
			byte[] data);
	 ...
	 * <p>
	 * 
	 * This implementation does logging, checks that {@code oids} is not
	 * {@code null} and its elements are valid, that {@code dataArray} and its
	 * elements are not {@code null}, and that {@code oids} and
	 * {@code dataArray} have the same length, reports object accesses, and
	 * calls {@link #setObjectsInternal setObjectsInternal} to perform the
	 * actual operation.
	 */
	public void setObjects(Transaction txn, long[] oids, byte[][] dataArray) {
		if (logger.isLoggable(FINEST)) {
			logger.log(FINEST, "setObjects txn:{0}, oids:[{1}]", txn,
		...
		try {
			...
			throw handleException(txn, FINEST, e, "setObjects txn:" + txn
	 ...
	 *             transaction
	 */
	protected abstract void setObjectsInternal(Transaction txn, long[] oids,
			byte[][] dataArray);
	...
	public void removeObject(Transaction txn, long oid) {
		if (logger.isLoggable(FINEST)) {
			logger.log(FINEST, "removeObject txn:{0}, oid:{1,number,#}", txn,
					oid);
		}
		try {
			reportObjectAccess(txn, oid, WRITE);
			...
			if (logger.isLoggable(FINEST)) {
				logger.log(FINEST,
						"removeObject txn:{0}, oid:{1,number,#} returns", txn,
						oid);
			}
		} catch (RuntimeException e) {
			throw handleException(txn, FINEST, e, "removeObject txn:" + txn
					+ ", oid:" + oid);
		}
	}
	 ...
	 * <p>
	 * 
	 * This implementation does logging, checks that {@code name} is not
	 * {@code null}, reports name accesses, and calls
	 * {@link #getBindingInternal getBindingInternal} to perform the actual
	 * operation.
	 */
	public long getBinding(Transaction txn, String name) {
		...
		try {
			...
			if (!result.isNameBound()) {
				/*
				 * Read lock the next name if the requested name is not bound to
				 * prevent a concurrent transaction from observing a different
				 * next name than this transaction would.
				 */
				reportNextNameAccess(txn, name, result.getNextName(), READ);
				throw new NameNotBoundException("Name not bound: " + name);
			}
			if (logger.isLoggable(FINEST)) {
				logger.log(FINEST, "getBinding txn:{0}, name:{1} returns "
						+ "oid:{2,number,#}", txn, name, result.getObjectId());
			}
			return result.getObjectId();
		} catch (RuntimeException e) {
			throw handleException(txn, FINEST, e, "getBinding txn:" + txn
					+ ", name:" + name);
		}
	}

	/**
	 * Performs the actual operation for {@link #getBinding getBinding}. If the
	 * name is bound, the return value contains the object ID that the name is
	 * bound to and a next name of {@code null}. If the name is not bound, the
	 * return value contains an object ID of {@code -1} and the next name found,
	 ...
	 * 
	 * This implementation does logging, checks that {@code name} is not
	 * {@code null} and that {@code oid} is valid, reports name accesses, and
	 * calls {@link #setBindingInternal setBindingInternal} to perform the
	 * actual operation.
	 */
	public void setBinding(Transaction txn, String name, long oid) {
		if (logger.isLoggable(FINEST)) {
			logger.log(FINEST,
					"setBinding txn:{0}, name:{1}, oid:{2,number,#}", txn,
					name, oid);
		}
		try {
			checkNull("name", name);
			...
			if (!result.isNameBound()) {
				/*
				 * Write lock the next name if the requested name was unbound to
				 * prevent other transactions from binding it, from getting it's
				 * value, or even from determining if it is bound, until this
				 * transaction commits.
				 */
			...
			throw handleException(txn, FINEST, e, "setBinding txn:" + txn
					+ ", name:" + name + ", oid:" + oid);
		}
	}

	/**
	 * Performs the actual operation for {@link #setBinding setBinding}. If the
	 * name is bound, the return value contains an arbitrary non-negative object
	 * ID and a next name of {@code null}. If the name is not bound, the return
	 * value contains an object ID of {@code -1} and the next name found, which
	 * may be {@code null}.
	 * 
	 * @param txn
	 *            the transaction under which the operation should take place
	 ...
	 * This implementation does logging, checks that {@code name} is not
	 * {@code null}, reports name accesses, and calls
	 * {@link #removeBindingInternal removeBindingInternal} to perform the
	 * actual operation.
	 */
	public void removeBinding(Transaction txn, String name) {
		...
		try {
			checkNull("name", name);
			/*
			 * Write lock the name even though we don't know yet if it is bound.
			 * Doing this means that another transaction cannot change or
			 * observe the existence of the binding until this transaction
			...
			if (!result.isNameBound()) {
				 ...
				 * from creating the name. But the server side will only want to
				 * lock existing items, so it depends on locking the next name.
				 */
				reportNextNameAccess(txn, name, result.getNextName(), READ);
				throw new NameNotBoundException("Name not bound: " + name);
			}
			...
			throw handleException(txn, FINEST, e, "removeBinding txn:" + txn
					+ ", name:" + name);
		}
	}
	 ...
	 * If the name is bound, the return value contains an arbitrary non-negative
	...
	public String nextBoundName(Transaction txn, String name) {
		...
		try {
			String result = reportNextNameAccess(txn, name,
					nextBoundNameInternal(txn, name), READ);
			if (logger.isLoggable(FINEST)) {
				logger.log(FINEST,
						"nextBoundName txn:{0}, name:{1} returns {2}", txn,
						name, result);
			}
			return result;
		} catch (RuntimeException e) {
			throw handleException(txn, FINEST, e, "nextBoundName txn:" + txn
					+ ", name:" + name);
		}
	}
	 ...
	 *            the transaction under which the operation should take place
	 * @param name
	 *            the name to search after, or {@code null} to start at the
	 *            beginning
	 ...
	 * {@code null}, and calls {@link #getClassIdInternal getClassIdInternal} to
	...
	public int getClassId(Transaction txn, byte[] classInfo) {
		...
		try {
			...
			if (logger.isLoggable(FINER)) {
				logger.log(FINER, "getClassId txn:{0} returns {1}", txn, result);
			}
			return result;
		} catch (RuntimeException e) {
	...
	public byte[] getClassInfo(Transaction txn, int classId)
		...
		try {
			...
			throw handleException(txn, FINER, e, "getClassInfo txn:" + txn
					+ ",classId:" + classId);
		}
	}
	 ...
	 *             timeout
	 * @throws IllegalStateException
	 *             if the operation failed because of a problem with the
	 *             transaction
	 */
	...
	public long nextObjectId(Transaction txn, long oid) {
		if (logger.isLoggable(FINEST)) {
			logger.log(FINEST, "nextObjectId txn:{0}, oid:{1,number,#}", txn,
					oid);
		}
		try {
			if (oid < -1) {
			...
			if (logger.isLoggable(FINEST)) {
				logger.log(FINEST, "nextObjectId txn:{0}, oid:{1,number,#} "
						+ "returns oid:{2,number,#}", txn, oid, result);
			}
			return result;
		} catch (RuntimeException e) {
			throw handleException(txn, FINEST, e, "nextObjectId txn:" + txn
					+ ", oid:" + oid);
		}
	}
	 ...
	 *            the transaction under which the operation should take place
	 * @param oid
	 *            the identifier of the object to search after, or {@code -1} to
	...
	public void setObjectDescription(Transaction txn, long oid,
		...
		try {
			...
			throw new IllegalStateException("Problem with transaction " + txn
					+ ": " + e.getMessage(), e);
		}
	}
	...
	public void setBindingDescription(Transaction txn, String name,
		...
		checkNull("description", description);
		try {
			nameAccesses.setObjectDescription(txn, getNameForAccess(name),
					description);
		} catch (IllegalArgumentException e) {
			throw new IllegalStateException("Problem with transaction " + txn
					+ ": " + e.getMessage(), e);
	 ...
	 *         {@code false}
	 * @throws IllegalStateException
	 *             if this participant has already been prepared, committed, or
	 ...
	 *            the transaction
	 * @throws IllegalStateException
	 *             if this participant was not previously prepared, or if this
	 *             participant has already committed or aborted, or if this
	 *             participant is not participating in the given transaction
	 */
	 ...
	 * 
	 * This implementation does logging and calls
	 * {@link #prepareAndCommitInternal prepareAndCommitInternal} to perform the
	 * actual operation.
	 */
	public void prepareAndCommit(Transaction txn) {
		...
		try {
			...
			throw handleException(txn, FINER, e, "prepareAndCommit txn:" + txn);
		}
	}
	 ...
	 *            the transaction
	 * @throws IllegalStateException
	 *             if this participant has already been aborted or committed, or
	 *             if this participant is not participating in the given
	 *             transaction
	 */
	 ...
	 * <p>
	 * 
	 * This implementation does logging, and aborts the transaction if it is not
	 * {@code null} and the exception is a {@link TransactionAbortedException}.
	 * 
	 * @param txn
	 *            the transaction or {@code null}
	 * @param level
	 *            the logging level
	 * @param e
	 *            the exception
	 ...
	 */
	protected RuntimeException handleException(Transaction txn, Level level,
			RuntimeException e, String operation) {
	 ...
	 *             if {@code oid} is negative
	 */
	protected void reportObjectAccess(Transaction txn, long oid, AccessType type) {
		checkOid(oid);
		try {
			objectAccesses.reportObjectAccess(txn, oid, type);
		} catch (IllegalArgumentException e) {
			throw new TransactionNotActiveException("Problem with transaction "
					+ txn + ": " + e.getMessage(), e);
		}
	}
	 ...
	 *            the type of access
	 */
	protected void reportNameAccess(Transaction txn, String name,
			AccessType type) {
		try {
	...
	/**
	 * Returns the name to use for reporting access to a name binding. Uses the
	 * value "z.end" instead of {@code null} to represent a name beyond the last
	 * known name. Add the prefix "z" to any name whose first character is "z".
	 * Note that this scheme does not preserve order, but access reporting does
	 * not require that.
	 * 
	 * @param name
	 ...
	 *            the transaction under which the operation should take place
	 * @param name
	 *            the name, which may be {@code null}
	 * @param next
	 *            the next name after {@code name}, which may be {@code null}
	 * @param accessType
	 *            the type of access to the next name to report
	 * @return the actual next name, which may be {@code null}
	 */
	private String reportNextNameAccess(Transaction txn, String name,
			String next, AccessType accessType) {
		while (true) {

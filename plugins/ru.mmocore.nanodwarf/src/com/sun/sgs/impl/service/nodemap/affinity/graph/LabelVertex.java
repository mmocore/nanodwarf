...
/**
 * Vertices for a Label Propagation Algorithm graph. Labels change as we iterate
 * through the algorithm, and once the algorithm has converged after several
 * iterations, vertices with the same label are in the same cluster.
 * <p>
 * We are using the identity's hash code for the label for faster comparisons.
 * This has some risk of us clustering identities that actually are not related,
...
public final class LabelVertex {
	...
	/**
	 * Constructs a new vertex representing the given {@code id} and initializes
	 * the label information to the hashcode of the {@code id}.
	 * 
	 * @param id
	 *            the identity this vertex represents
	 */

...
/**
 * A driver for the Label Propagation Algorithm affinity group finders. The
 * driver is responsible for instantiating the affinity group system, managing
 * its life cycle, and invoking the group finder as necessary.
 * <p>
 ...
 * 	</b></code><br>
 * <i>Default:</i>
 * {@code com.sun.sgs.impl.service.nodemap.affinity.graph.dlpa.WeightedGraphBuilder}
 * <br>
 * 
 * <dd style="padding-top: .5em">The graph builder to use. Set to {@code None}
 * if no affinity group finding is required, which is useful for testing.
 * <p>
 * 
 * <dt><i>Property:</i> <code><b>
 *   com.sun.sgs.impl.service.nodemap.affinity.update.period
 * 	</b></code><br>
 * <i>Default:</i> {@code 60} (one minute)} <br>
 * 
 * <dd style="padding-top: .5em">The frequency that we find affinity groups, in
 * seconds. The value must be between {@code 5} and {@code 65535}.
 * <p>
 * </dl>
 */
public class LPADriver extends BasicState implements AffinityGroupFinder {
	...
	public static final String GRAPH_CLASS_PROPERTY = PROP_NAME
			+ ".graphbuilder.class";

	/** The property name for the update frequency. */
	public static final String UPDATE_FREQ_PROPERTY = PROP_NAME
			+ ".update.freq";
	...
	/**
	 * The value to be given to {@code GRAPH_CLASS_PROPERTY} if no affinity
	 * group finding should be instantiated (useful for testing).
	 */
	...
	/**
	 * A graph listener, used for detecting affinity groups, or {@code null} if
	 * no graph listener is being used.
	 */
	 ...
	 * 
	 * @param properties
	 *            the properties for configuring this subsystem
	 ...
	 */
	public LPADriver(Properties properties, ComponentRegistry systemRegistry,
		...
		if (builderName != null) {
			...
			// TODO: The following code is commented out while our default is
			// none,
			// mostly to keep findbugs quiet. In the future, we expect the
			// WeightedGraphBuilder to be the default if we're not in single
			// node
			// mode.
		...
		logger.log(Level.CONFIG, "Created LPADriver with listener: "
				+ graphListener + ", builder: " + graphBuilder
	...
	/**
	 * Returns the graph builder used by this driver, or {@code null} if we are
	 * not building graphs.
	 * 
	...
	/**
	 * Returns the graph listener used by this driver, or {@code null} if we are
	 * not building graphs.
	 * 

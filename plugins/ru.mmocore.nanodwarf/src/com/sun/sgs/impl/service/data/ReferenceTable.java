...
final class ReferenceTable {
	 ...
	 */
	private static final WeakIdentityMap<Object, Boolean> staleObjects = new WeakIdentityMap<Object, Boolean>();

	/** Maps object IDs to managed references. */
	private final SortedMap<Long, ManagedReferenceImpl<?>> oids = new TreeMap<Long, ManagedReferenceImpl<?>>();

	/**
	 * Maps managed objects to managed references. The objects are compared by
	 * identity, not the equals method.
	 */
	private final Map<ManagedObject, ManagedReferenceImpl<?>> objects = new IdentityHashMap<ManagedObject, ManagedReferenceImpl<?>>();
	...
	ManagedReferenceImpl<?> find(ManagedObject object) {
		...
		if (result == null && trackStaleObjects
	...
	void add(ManagedReferenceImpl<?> ref) {
		assert !oids.containsKey(ref.oid) : "Found existing reference for oid:"
				+ ref.oid;
		oids.put(ref.oid, ref);
		ManagedObject object = ref.getObject();
		if (object != null) {
			assert !objects.containsKey(object) : "Found existing reference for object with oid:"
					+ ref.oid;
	...
	/**
	 * Updates this table for a reference that has been newly associated with an
	 * object.
	 */
	void registerObject(ManagedReferenceImpl<?> ref) {
		assert oids.get(ref.oid) == ref : "Found duplicate references for oid: "
				+ ref.oid;
		assert ref.getObject() != null : "Object is null for oid:" + ref.oid;
		assert !objects.containsKey(ref.getObject()) : "Found existing reference for object with oid: "
				+ ref.oid;
	...
	void remove(ManagedReferenceImpl<?> ref) {
		...
		assert existing == ref : "Found duplicate reference for oid:" + ref.oid;
		ManagedObject object = ref.getObject();
		if (object != null) {
			existing = objects.remove(object);
			assert existing == ref : "Found duplicate reference for oid:"
					+ ref.oid;
		}
	}
	 ...
	 * object, or -1 if none is found. Does not return IDs for removed objects.
	 * Specifying -1 requests the first ID.
	 */
	long nextNewObjectId(long oid) {
		for (Entry<Long, ManagedReferenceImpl<?>> entry : oids.tailMap(oid)
				.entrySet()) {
	...
	/**
	 * Flushes all references. Returns information about any objects found to be
	 * modified, or null if none were modified.
	 */
	...
	void checkAllState() {
		...
		for (Entry<Long, ManagedReferenceImpl<?>> entry : oids.entrySet()) {
			...
			ref.checkState();
			if (oid != ref.oid) {
				throw new AssertionError("Wrong oids entry: oid = " + oid
						+ ", ref.oid = " + ref.oid);
			}
			...
			if (object != null) {
				...
				if (objectsRef == null) {
					throw new AssertionError("Missing objects entry for oid = "
							+ ref.oid);
				} else if (!ref.equals(objectsRef)) {
					throw new AssertionError("Wrong objects entry for oid = "
							+ ref.oid);
				}
				objectCount++;
			}
		...
		if (objectCount != objects.size()) {
			throw new AssertionError("Objects table has wrong size: was "
					+ objects.size() + ", expected " + objectCount);
		}
	}
}

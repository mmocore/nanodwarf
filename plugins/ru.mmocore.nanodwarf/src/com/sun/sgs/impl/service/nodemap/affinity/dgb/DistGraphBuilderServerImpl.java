 ...
 * 
 * It builds a single graph representing all information for the entire system
 * using the single node graph builder, and uses a single node label propagation
 * implementation to process the graph.
 * 
 * This LPA implementation is expected to be useful for simple multi-node
 * testing. It may have scalability problems due to the amount of data being
 * sent from each node, and the size of the affinity graph and related data
 * structures. It has no dependencies on new multi-node parts of the system.
 */
public class DistGraphBuilderServerImpl extends AbstractAffinityGraphBuilder
	...
	public static final String SERVER_PORT_PROPERTY = PROP_BASE
			+ ".server.port";
	...
	/**
	 * Our JMX exposed information. The group finder stats is held in
	 * {@code lpa}.
	 */
	 ...
	 * @param nodeId
	 *            the core server node id
	 * @throws Exception
	 *             if an error occurs
	 */
	DistGraphBuilderServerImpl(ComponentRegistry systemRegistry,
			TransactionProxy txnProxy, Properties properties, long nodeId)
			throws Exception {
		super(properties);
		transactionScheduler = systemRegistry
		...
		builder = new SingleGraphBuilder(properties, systemRegistry, txnProxy,
				false);

		// Create our group finder and graph builder JMX MBeans
		AffinityGroupFinderStats stats = new AffinityGroupFinderStats(this,
				col, -1);
		builderStats = new AffinityGraphBuilderStats(col,
				builder.getAffinityGraph(), periodCount, snapshot);
		...
		// Export ourself.
		exporter = new Exporter<DistGraphBuilderServer>(
				DistGraphBuilderServer.class);
	 ...
	 * this would be some work, it might be worthwhile to implement if we find
	 * this LPA implementation is useful in deployed systems.
	 */
	...
	private class GetNodeIdTask extends AbstractKernelRunnable {
		...
		public void run() throws Exception {
			nodeId = txnProxy.getService(NodeMappingService.class).getNode(id)
					.getId();
		}

 ...
 * 
 * In the info database, the value for key 0 stores a magic number common to all
 * DataStoreImpl databases.
 * 
 ...
 * there are no placeholders. This field is used during initialization to remove
 * any existing placeholders which were created for allocation blocks that are
 * no longer in use. Placeholders always appear at the end of each allocation
 * block, whose size is fixed (as of version 4.0) at 1024 bytes.
 * 
 ...
 * serialized form of the associated class descriptor. Since these entries come
 * at the end, we can find the next class ID by using a cursor to find the last
 * entry.
 * 
 ...
 * 
 * Object values also have two additional, distinguished initial byte values to
 * support placeholders:
 * 
 * - If the first byte is 3, then the entry represents a placeholder, which
 * means that the entry appears within the database but should not be considered
 * to represent an object. Placeholders are used to create a barrier object in
 * the database to improve concurrency for new object allocations when using BDB
 * Java Edition.
 * 
 ...
 * starts with the 3 that marks placeholders, or the 4 used for quoting. Since
 * serialized data will always start with 1 or 2, though, this value should not
 * be used in practice.
 * 
 * Version history:
 * 
 * Version 1.0: Initial version, 11/3/2006 Version 2.0: Add NEXT_TXN_ID,
 * 2/15/2007 Version 3.0: Add classes DB, compress object values, 5/18/2007
 * Version 4.0: Add placeholders, 7/8/2008 Version 5.0: Add node IDs, 7/21/2009
 */
final class DataStoreHeader {
	 ...
	 * consists of the second and following bytes. Because object data is always
	 * serialized data, which uses a specialized encoding that starts with
	 * either 1 or 2, this value should not be used in practice.
	 */
	 ...
	 *             if the format of the header information is incorrect
	 */
	static int verify(DbDatabase db, DbTransaction dbTxn) {
		byte[] value = db.get(dbTxn, DataEncoding.encodeLong(MAGIC_KEY), false);
		if (value == null) {
	 ...
	 */
	private static void upgrade(DbDatabase db, DbTransaction dbTxn,
			int majorVersion) {
		switch (majorVersion) {
		case 1:
		case 2:
		case 3:
		case 4:
			throw new DataStoreException("Database version number "
	...
	static void create(DbDatabase db, DbTransaction dbTxn) {
		boolean success = db.putNoOverwrite(dbTxn,
				DataEncoding.encodeLong(MAGIC_KEY),
				DataEncoding.encodeLong(MAGIC));
		assert success;
		success = db.putNoOverwrite(dbTxn, DataEncoding.encodeLong(MAJOR_KEY),
				DataEncoding.encodeShort(MAJOR_VERSION));
		assert success;
		success = db.putNoOverwrite(dbTxn, DataEncoding.encodeLong(MINOR_KEY),
				DataEncoding.encodeShort(MINOR_VERSION));
		assert success;
		success = db.putNoOverwrite(dbTxn,
				DataEncoding.encodeLong(NEXT_OBJ_ID_KEY),
				DataEncoding.encodeLong(INITIAL_NEXT_OBJ_ID));
		assert success;
		success = db.putNoOverwrite(dbTxn,
				DataEncoding.encodeLong(NEXT_TXN_ID_KEY),
		...
		assert success;
		success = db.putNoOverwrite(dbTxn,
				DataEncoding.encodeLong(NEXT_NODE_ID_KEY),
	 ...
	 *            the database transaction
	 * @param increment
	 *            the amount to increment the stored amount
	 * @return the next available ID
	 * @throws DbDatabaseException
	 *             if a problem occurs accessing the database
	 * @throws IllegalArgumentException
	 *             if increment is not greater than zero
	 */
	static long getNextId(long key, DbDatabase db, DbTransaction dbTxn,
			long increment) {
		if (increment <= 0) {
	...
	static String headerString(int minorVersion) {
		return "DataStoreHeader[magic:" + toHexString(MAGIC) + ", version:"

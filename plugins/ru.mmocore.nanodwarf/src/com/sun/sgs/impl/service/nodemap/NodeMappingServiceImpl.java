 ...
 * <p>
 * The {@link #NodeMappingServiceImpl constructor} supports the following
 * properties:
 * <p>
 ...
 * 	</b></code><br>
 * <i>Default:</i> the value of the {@code com.sun.sgs.server.host} property, if
 * present, or {@code localhost} if this node is starting the server <br>
 * 
 * <dd style="padding-top: .5em">The name of the host running the
 * {@code NodeMappingServer}.
 * <p>
 * 
 * <dt><i>Property:</i> <code><b>
 ...
 * {@code com.sun.sgs.node.type} property is not {@code appNode}, and means that
 * an anonymous port will be chosen for running the server.
 * <p>
 * 
 * <dt><i>Property:</i> <code><b>
 ...
 * {@code NodeMapppingServer}. This value must be no less than {@code 0} and no
 ...
 * <code>com.sun.sgs.impl.service.nodemap</code> to log information at the
 ...
 * <p>
 */
public class NodeMappingServiceImpl extends AbstractService implements
		NodeMappingService {
	// Design notes:
	//
	...
	/** Class name. */
	private static final String CLASSNAME = NodeMappingServiceImpl.class
			.getName();
	...
	private static final String CLIENT_PORT_PROPERTY = PKG_NAME
			+ ".client.port";
	...
	/**
	 * The registered node change listeners. There is no need to persist these:
	 * these are all local services, and if the node goes down, they'll need to
	 * re-register.
	 */
	private final Set<NodeMappingListener> nodeChangeListeners = new CopyOnWriteArraySet<NodeMappingListener>();

	/** The remote backend to this service. */
	private final NodeMappingServer server;

	/**
	 * The implementation of the server, if we're on the special node. Only one
	 * server should be created in a system, but this is not enforced.
	 */
	private final NodeMappingServerImpl serverImpl;

	/**
	 * The object we send to our global server for callbacks when there are map
	 * changes on this node.
	 */
	private final NotifyClient changeNotifier;

	/**
	 * Our instantiated change notifier object. This reference is held so the
	 * object is not garbage collected; it is also used for local calls.
	 */
	...
	/**
	 * The list of notifications which couldn't be sent because we weren't in
	 * ready yet. This list is added to while we're in the initialized state,
	 * and emptied in the ready method. Protected by the lock.
	 */
	private final List<TaskReservation> pendingNotifications = new ArrayList<TaskReservation>();
	...
	private final Set<IdentityRelocationListener> idRelocationListeners = new CopyOnWriteArraySet<IdentityRelocationListener>();

	/** A map of identities to outstanding idRelocation handlers. */
	private final ConcurrentMap<Identity, Queue<SimpleCompletionHandler>> relocationHandlers = new ConcurrentHashMap<Identity, Queue<SimpleCompletionHandler>>();
	 ...
	 * when {@link #ready} is called), because this service does not use
	 * Managers and will not run application code. Managers are not available
	 * until {@code ready} is called.
	 * <p>
	 * 
	 * @param properties
	 *            the properties for configuring this service
	...
	public NodeMappingServiceImpl(Properties properties,
			...
			throws Exception {
		super(properties, systemRegistry, txnProxy, new LoggerWrapper(
		...
		try {
			...
			NodeType nodeType = wrappedProps.getEnumProperty(
					StandardProperties.NODE_TYPE, NodeType.class,
					NodeType.singleNode);
			...
			if (instantiateServer) {
				...
				serverImpl = null;
				host = wrappedProps.getProperty(SERVER_HOST_PROPERTY,
						wrappedProps
			...
			server = (NodeMappingServer) registry
					.lookup(NodeMappingServerImpl.SERVER_EXPORT_NAME);

			// Export our client object for server callbacks.
			int clientPort = wrappedProps.getIntProperty(CLIENT_PORT_PROPERTY,
			...
			fullName = "NodeMappingServiceImpl[host:" + localHost
					+ ", clientPort:" + clientPort + ", fullStack:" + fullStack
					+ "]";
	...
	/**
	 * Throws {@code IllegalStateException} if this service is not running. Code
	 * swiped from the data service.
	 */
	 ...
	 * {@inheritDoc}
	 * <p>
	 * If the identity is not associated with a client (i.e., if it is an AI
	 * object), it will be assigned to the local node. Otherwise, a remote call
	 * will be made to determine a node assignment.
	 */
	public long assignNode(final Class service, final Identity identity) {
		...
		//
		return callServer(new Callable<Long>() {
	...
	/**
	 * Executes the specified {@code serverCall} by invoking its
	 * {@link Callable#call call} method. If the specified call throws an
	 * {@code IOException}, this method will retry the task for a fixed number
	 * of times. The method will stop retrying if this service is shutting down.
	 ...
	 * server, we ask that the local node be shut down. This is because "server"
	 * is the core server, which contains the data store. If it is shutdown, the
	 * entire cluster is shut down. If we have a loss of connectivity with the
	 * server, we assume the problem is with the local node. If the core server
	 * is disconnected from all nodes, the watchdog server will eventually
	 * detect that and declare all nodes dead.
	 * 
	 * This method must be called from outside a transaction or
	 * {@code IllegalStateException} will be thrown.
	 * <p>
	 * 
	 * @param <T>
	 *            return type of the value returned by the server call
	 * @param serverCall
	 *            a callable with the server call
	 * 
	 * @return the value returned by the server call
	 * 
	 * @throws IllegalStateException
	 *             if this method is invoked within a transactional context
	 */
		...
		while (!shuttingDown()) {
			try {
				...
				if (maxAttempts-- == 0) {
					...
					watchdogService.reportFailure(localNodeId, this.getClass()
							.toString());
					break;
	 ...
	 * {@inheritDoc}
	 * <p>
	 * The local node makes the status change, avoiding a remote call where
	 * possible. However, if it appears that an identity might be ready for
	 * garbage collection, it tells the server, which will perform the deletion.
	 */
	public void setStatus(Class service, final Identity identity, boolean active)
		...
		SetStatusTask stask = new SetStatusTask(identity, service.getName(),
				active);
		try {
			transactionScheduler.runTask(stask, taskOwner);
		} catch (Exception e) {
			logger.logThrow(Level.WARNING, e, "Setting status for {0} failed",
					identity);
			throw new UnknownIdentityException("id: " + identity, e);
		}

		if (stask.canRemove()) {
			callServer(new Callable<Void>() {
	...
	/**
	 * Task for setting a status and returning information about whether the
	 * identity is considered dead by this node.
	 */
	...
	private static class IdentityIterator implements Iterator<Identity> {
		...
		IdentityIterator(DataService dataService, long nodeId) {
			...
			iterator = BoundNamesUtil.getServiceBoundNamesIterator(dataService,
		...
		public Identity next() {
			...
			IdentityMO idmo = (IdentityMO) dataService.getServiceBinding(key);
	 ...
	 * mapping. An instance of this class is registered with our global server;
	 * methods of {@link NotifyClient} are called when an identity is added to
	...
	private class MapChangeNotifier implements NotifyClient {
		...
		public void removed(Identity id, Node newNode) {
			...
			synchronized (lock) {
				if (isInInitializedState()) {
					logger.log(Level.FINEST, "Queuing remove notification for "
							+ "identity: {0}, " + "newNode: {1}}", id, newNode);
					for (NodeMappingListener listener : nodeChangeListeners) {
						TaskReservation res = taskScheduler.reserveTask(
			...
			// The normal case.
			for (NodeMappingListener listener : nodeChangeListeners) {
				taskScheduler.scheduleTask(new MapRemoveTask(listener, id,
						newNode), taskOwner);
			}
		}
		...
		public void added(Identity id, Node oldNode) {
			...
			synchronized (lock) {
				if (isInInitializedState()) {
					logger.log(Level.FINEST, "Queuing added notification for "
							+ "identity: {0}, " + "oldNode: {1}}", id, oldNode);
					for (NodeMappingListener listener : nodeChangeListeners) {
						TaskReservation res = taskScheduler.reserveTask(
		...
		public void prepareRelocate(Identity id, long newNodeId) {
			...
			Queue<SimpleCompletionHandler> handlerQueue = new ConcurrentLinkedQueue<SimpleCompletionHandler>();
			...
			synchronized (lock) {
				if (isInInitializedState()) {
					logger.log(Level.FINEST, "Queuing added notification for "
							+ "identity: {0}, " + "newNode: {1}}", id,
							newNodeId);
					for (IdentityRelocationListener listener : idRelocationListeners) {
						final SimpleCompletionHandler handler = new PrepareMoveCompletionHandler(
								id);
			...
			// The normal case.
			for (final IdentityRelocationListener listener : idRelocationListeners) {
				final SimpleCompletionHandler handler = new PrepareMoveCompletionHandler(
						id);
				handlerQueue.add(handler);
				taskScheduler.scheduleTask(new MapRelocateTask(listener, id,
						newNodeId, handler), taskOwner);
			}
		}
	}

	/**
	 * Let a listener know that the mapping for an id to this node has been
	 * removed, and what the new node mapping is (or null if there is no new
	 * mapping).
	 */
	...
	/**
	 * Let a listener know that the mapping for an id to this node has been
	 * added, and what the old node mapping was (or null if this is a brand new
	 * mapping).
	 */
	...
	private static final class MapRelocateTask extends AbstractKernelRunnable {
		...
		MapRelocateTask(IdentityRelocationListener listener, Identity id,
				long newNodeId, SimpleCompletionHandler handler) {
			super(null);
			this.listener = listener;
			this.id = id;
	...
	/**
	 * A {@code SimpleCompletionHandler} implementation for identity relocation
	 * listeners. When {@code completed} is invoked, the handler instance is
	 * removed from the relocation handler queue for the associated identity. If
	 * a given handler is the last one to be removed from an identity's queue,
	 * then relocation preparations are complete for that identity, and the node
	 * mapping service can actually move the identity to its new node.
	 */
	private final class PrepareMoveCompletionHandler implements
		...
		public void completed() {
			...
			Queue<SimpleCompletionHandler> handlerQueue = relocationHandlers
					.get(id);
	...
	/**
	 * Tell the server that it's OK to move identity, all listeners have been
	 * notified and have finished.
	 * 
	 * @param id
	 *            the id to move
	 */
	...
	/**
	 * Check the validity of the data store for a particular identity. Used for
	 * testing.
	 * 
	 * @param identity
	 *            the identity

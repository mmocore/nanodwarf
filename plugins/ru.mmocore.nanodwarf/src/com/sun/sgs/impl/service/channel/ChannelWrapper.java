 ...
 * application when a channel is created. A {@code ChannelWrapper} is handed
 * back to the application instead of a direct reference to a
 ...
 * 
 * <p>
 * When a {@code ChannelWrapper} instance is removed from the data service, the
 * underlying channel is closed.
 */
class ChannelWrapper implements Channel, Serializable, ManagedObjectRemoval {
	 ...
	 * 
	 * @param channelRef
	 *            a reference to a channel to wrap
	 */
	 ...
	 * {@inheritDoc}
	 * 
	 * The application removed the wrapper, so close the channel, indicating
	 * that the the channel name mapping should be removed as well.
	 */
	...
	public String toString() {
		...
		return getClass().getName()
				+ "["
				+ (channelImpl == null ? channelRef.toString() : channelImpl
						.toString()) + "]";
	}
	...
	/**
	 * Set the channel reference for this wrapper to the specified
	 * {@code channelRef}. The underlying channel reference changes when the
	 * application invokes {@code leaveAll} on the channel, which closes and
	 * removes the old channel (except for the channel's name binding) and
	 * reuses the existing wrapper, replacing the previous channel reference
	 * with a reference to the "next generation" channel of the same name.
	 * 
	 * @param channelRef
	...
	/**
	 * Returns the underlying {@code ChannelImpl} instance for this wrapper. If
	 * the underlying channel has been removed, then the channel has been
	 * closed, so {@code IllegalStateException} is thrown.
	 */

 ...
 * have not yet run to completion or been dropped. This class maintains all of
 * the meta-data associated with these pending tasks, and if needed, makes sure
 * that the {@code Task} itself is persisted. This meta-data classs can be
 * flagged as re-usable, so that a single instance can be persisted and then
 * re-used as needed, although a given instance must always be re-used for tasks
 * for the same {@code Identity}.
 */
class PendingTask implements ManagedObject, Serializable {
	 ...
	 * 
	 * @param identity
	 *            the {@code Identity} of the owner of the task
	 */
	...
	/**
	 * Re-sets this {@code PendingTask} with the given values, and marks it as
	 * unavailable. This can now be used to represent a durable task that needs
	 * to be run. As described on {@code TaskManager}, any {@code Task} may
	 * optionally implement {@code ManagedObject} and this affects how the task
	 * is handled by the system. {@code PendingTask} takes care of the task's
	 * durability, and how the task is eventually run, in both situations.
	 * 
	 * @param t
	 *            the {@code Task} that is pending
	 * @param s
	 *            the starting time for the task, in milliseconds since January
	 *            1, 1970, or {@code TaskServiceImpl.START_NOW}
	 * @param p
	 *            the period between recurrences, or
	...
	void resetValues(Task t, long s, long p) {
		...
		/*
		 * If the Task is also a ManagedObject then the assumption is that the
		 * object was already managed by the application so we just keep a
		 * reference...otherwise, we make it part of our state, which has the
		 * effect of persisting the task. In either case we set one of the two
		 * fields to null to disambiguate what the situation is.
		 */
	...
	/**
	 * Returns the node where the associated task is running if the task is
	 * periodic, or -1 if the task is non-periodic or the node hasn't been
	 * assigned.
	 */
	long getRunningNode() {
		return runningNode;
	 ...
	 * periodic. If the task is not periodic, {@code IllegalStateException} is
	 * thrown.
	 */
	...
	/**
	 * Checks that the underlying task is available, which is only at question
	 * if the task was managed by the application and therefore could have been
	 * removed by the application. Must be called in a transaction.
	 */
	...
	/**
	 * This is a convenience method used to correctly resolve the pending task
	 * and run it, throwing any exceptions raised by the task. Note that if
	 * {@code isTaskAvailable} returns {@code false} then this method will
	 * simply return without running anything.
	 */

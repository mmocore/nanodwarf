 ...
 * application in the {@code AppListener.loggedIn} method when a client session
 * logs in. A {@code ClientSessionWrapper} is passed to the application instead
 * of a direct reference to a {@code ClientSessionImpl} instance to avoid the
 * possibility of the application removing the {@code ClientSessionImpl}
 * instance from the data service and interfering with the client session
 * service's persistent data.
 * 
 * <p>
 * When a {@code ClientSessionWrapper} instance is removed from the data
 * service, the underlying client session is disconnected.
 */
public class ClientSessionWrapper implements ClientSession, Serializable,
	 ...
	 * 
	 * @param sessionRef
	 *            a reference to a client session to wrap
	 */
	...
	public boolean equals(Object object) {
		if (object == this) {
			...
			return sessionRef
					.equals(((ClientSessionWrapper) object).sessionRef);
		} else {
			return false;
		}
	}
	...
	public String toString() {
		...
		return getClass().getName()
				+ "["
				+ (sessionImpl == null ? sessionRef.toString() : sessionImpl
						.toString()) + "]";
	}
	 ...
	 * wrapper. If the underlying client session has been removed, then the
	 * client session has been disconnected, so {@code IllegalStateException} is
	 * thrown.
	 * 

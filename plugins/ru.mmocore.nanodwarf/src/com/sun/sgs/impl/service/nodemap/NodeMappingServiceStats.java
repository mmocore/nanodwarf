...
class NodeMappingServiceStats implements NodeMappingServiceMXBean {
	...
	NodeMappingServiceStats(ProfileCollector collector) {
		...
		addNodeMappingListenerOp = (AggregateProfileOperation) consumer
				.createOperation("addNodeMappingListener", type, level);
		addIdentityRelocationListenerOp = (AggregateProfileOperation) consumer
				.createOperation("addIdentityRelocationListener", type, level);
		assignNodeOp = consumer.createOperation("assignNode", type, level);
		getIdentitiesOp = consumer
				.createOperation("getIdentities", type, level);
		getNodeOp = consumer.createOperation("getNode", type, level);
		setStatusOp = consumer.createOperation("setStatus", type, level);
	}

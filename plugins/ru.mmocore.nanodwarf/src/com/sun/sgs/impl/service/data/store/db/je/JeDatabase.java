...
public class JeDatabase implements DbDatabase {
	 ...
	 */
	JeDatabase(Environment env, Transaction txn, String fileName, boolean create)
			throws FileNotFoundException {
		try {
			db = env.openDatabase(txn, fileName, create ? createConfig
					: openConfig);
	...
	/**
	 * Converts a Berkeley DB DatabaseEntry data value, replacing the null that
	 * BDB uses if the data is empty with an empty array.
	 */
	...
	public byte[] get(DbTransaction txn, byte[] key, boolean forUpdate) {
		try {
			...
			OperationStatus status = db.get(JeTransaction.getJeTxn(txn),
					new DatabaseEntry(key), valueEntry,
					forUpdate ? LockMode.RMW : null);
			if (status == SUCCESS) {
	...
	public void markForUpdate(DbTransaction txn, byte[] key) {
		try {
			...
			OperationStatus status = db.get(JeTransaction.getJeTxn(txn),
					new DatabaseEntry(key), valueEntry, LockMode.RMW);
	...
	public void put(DbTransaction txn, byte[] key, byte[] value) {
		try {
			OperationStatus status = db.put(JeTransaction.getJeTxn(txn),
	...
	/** {@inheritDoc} */
	public boolean putNoOverwrite(DbTransaction txn, byte[] key, byte[] value) {
		try {
	...
	public boolean delete(DbTransaction txn, byte[] key) {
		try {
			OperationStatus status = db.delete(JeTransaction.getJeTxn(txn),

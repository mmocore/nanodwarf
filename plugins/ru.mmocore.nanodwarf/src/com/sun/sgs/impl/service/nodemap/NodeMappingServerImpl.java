...
/**
 * The remote server portion of the node mapping service. This portion of the
 * service is used for any global operations, such as selecting a node for an
 * identity. Additionally, all changes to the map are made by the server so it
 ...
 * 
 * <dd style="padding-top: .5em">The network port for the
 * {@code NodeMappingServer}. This value must be no less than {@code 0} and no
 * greater than {@code 65535}. The value {@code 0} can only be specified if the
 * {@code com.sun.sgs.impl.service.nodemap.start.server} property is
 * {@code true}, and means that an anonymous port will be chosen for running the
 * server.
 * <p>
 * 
 * <dt><i>Property:</i> <code><b>
 ...
 * The name of the class that implements {@link NodeAssignPolicy}, used for the
 * node assignment policy. The class should be public, not abstract, and should
 * provide a public constructor with {@link Properties} and
 * {@link NodeMappingServerImpl} parameters.
 * <p>
 * 
 * <dt><i>Property:</i> <code><b>
 ...
 * 
 * <dd style="padding-top: .5em">
 * The minimum time, in milliseconds, that this server will wait before removing
 * a potentially inactive identity from the map. This value must be greater than
 * {@code 0}. Shorter expiration times cause the map to be cleaned up more
 * frequently, potentially causing more
 * {@link NodeMappingService#assignNode(Class, Identity) assignNode} calls;
 * longer expiration times will increase the chance that an identity will become
 * active again before it can be removed.
 * <p>
 * 
 * <dt><i>Property:</i> <code><b>
 ...
 * 
 * <dd style="padding-top: .5em">
 * The time allowed, in milliseconds, for {@code  IdentityRelocationListener}s to
 * call {@link SimpleCompletionHandler#completed completed} on the handler they
 * receive. If this time has elapsed, this server disregards the proposed
 * identity relocation. This value is used to guard against listeners which
 * never respond they are finished. During this time period, the identity is
 * prohibited from moving elsewhere unless the node has failed.
 * <p>
 * 
 * </dl>
 * <p>
 ...
 * <code>com.sun.sgs.impl.service.nodemap.server</code> to log information at
 ...
 */
public final class NodeMappingServerImpl extends AbstractService implements
		NodeMappingServer {
	 ...
	 * DataStore.
	 */
	private static final String ASSIGN_POLICY_CLASS_PROPERTY = PKG_NAME
			+ ".policy.class";
	 ...
	 */
	private static final String REMOVE_EXPIRE_PROPERTY = PKG_NAME
			+ ".remove.expire.time";
	 ...
	 */
	private static final String RELOCATION_EXPIRE_PROPERTY = PKG_NAME
			+ ".relocation.expire.time";

	/**
	 * Default time allowed for IdentityRelocationListeners to respond that they
	 * have completed preparations for an identity move, in milliseconds.
	 */
	...
	private static final LoggerWrapper logger = new LoggerWrapper(
			Logger.getLogger(PKG_NAME + ".server"));
	...
	/**
	 * The policy for assigning new nodes. This will likely morph into the load
	 * balancing policy, as well.
	 */
	...
	/**
	 * Identities waiting to be removed, with the time they were entered in the
	 * map.
	 * 
	 * TODO For failover, will need to persist these identities.
	 */
	private final Queue<RemoveInfo> removeQueue = new ConcurrentLinkedQueue<RemoveInfo>();
	/**
	 * The set of clients of this server who wish to be notified if there's a
	 * change in the map.
	 * 
	 * TODO For failover, these need to be persisted if we have a way to
	 * reconnect an existing service to a new server
	 */
	private final Map<Long, NotifyClient> notifyMap = new ConcurrentHashMap<Long, NotifyClient>();

	/**
	 * The amount of time allowed for id relocation listeners to state they have
	 * completed their work. If this time expires, the move is effectively
	...
	private final Map<Identity, MoveIdTask> moveMap = new ConcurrentHashMap<Identity, MoveIdTask>();

	/**
	 * Creates a new instance of NodeMappingServerImpl, called from the local
	 * NodeMappingService.
	 * <p>
	 * The application context is resolved at construction time (rather than
	 * when {@link NodeMappingServiceImpl#ready} is called), because this server
	 * will never need Managers and will not run application code. Managers are
	 * not available until {@code Service.ready} is called.
	 * <p>
	 * 
	 * @param properties
	 *            service properties
	...
	public NodeMappingServerImpl(Properties properties,
		...
		assignPolicy = wrappedProps.getClassInstanceProperty(
				ASSIGN_POLICY_CLASS_PROPERTY, DEFAULT_ASSIGN_POLICY_CLASS,
				NodeAssignPolicy.class, new Class[] { Properties.class },
				properties);
		...
		transactionScheduler.runTask(new AbstractKernelRunnable(
				"CheckServiceVersion") {
			public void run() {
				checkServiceVersion(NodeMapUtil.VERSION_KEY,
						NodeMapUtil.MAJOR_VERSION, NodeMapUtil.MINOR_VERSION);
			}
		}, taskOwner);
		...
		long removeExpireTime = wrappedProps.getLongProperty(
				REMOVE_EXPIRE_PROPERTY, DEFAULT_REMOVE_EXPIRE_TIME, 1,
				Long.MAX_VALUE);
		...
		relocationExpireTime = wrappedProps.getLongProperty(
				RELOCATION_EXPIRE_PROPERTY, DEFAULT_RELOCATION_EXPIRE_TIME, 1,
				Long.MAX_VALUE);
		...
		fullName = "NodeMappingServiceImpl[host:"
				+ InetAddress.getLocalHost().getHostName() + ", port:" + port
				+ "]";
	...
	/**
	 * {@inheritDoc} Called from the instantiating service.
	 */
	...
	/** {@inheritDoc} */
	public long assignNode(Class service, Identity identity, long requestingNode)
			throws IOException {
		callStarted();
		try {
			if (identity == null) {
			...
			try {
				...
				logger.logThrow(Level.WARNING, ex, "Lookup of {0} failed",
						identity);
			}
			...
			try {
				long newNodeId = mapToNewNode(identity, serviceName, node,
						requestingNode);
				logger.log(Level.FINEST, "assignNode id:{0} to {1}", identity,
						newNodeId);
				return newNodeId;
	...
	/**
	 * Check for an id, and make the node available if it was assigned to a
	 * failed node. Otherwise, update the status information.
	 */
	private class CheckTask extends AbstractKernelRunnable {
		...
		public void run() {
			try {
				IdentityMO idmo = (IdentityMO) dataService
						.getServiceBinding(idkey);
				...
				final String statuskey = NodeMapUtil.getStatusKey(id, nodeId,
						serviceName);
	 ...
	 * periodically wakes up and looks at each entry in the removeQueue. If an
	 * appropriate amount of time has passed since the entry was put in the
	 * removeQueue (which allows the system some settling time, so we don't
	 * thrash removing and adding an identity), the data store is checked to see
	 * if the identity can still be removed. A service could have called
	 * {@link #NodeMappingService setStatus} during our waiting time, marking
	 * the identity as active, during the waiting time. If it is still
	 * appropriate to remove the identity, all traces of it are removed from the
	 * data store.
	 */
	...
	/**
	 * Task which, under a transaction, checks that it's still appropriate to
	 * remove an identity, and, if so, removes the service bindings and object.
	 */
	private class RemoveTask extends AbstractKernelRunnable {
		...
		/**
		 * Returns the node the identity was removed from, which can be null if
	...
	public void registerNodeListener(NotifyClient client, long nodeId)
		...
		try {
			...
			logger.log(Level.FINEST, "Registered node listener for {0} ",
					nodeId);
		} finally {
			callFinished();
		}
	...
	/**
	 * {@inheritDoc} Also called internally when we hear a node has died.
	 */
	public void unregisterNodeListener(long nodeId) throws IOException {
		...
		try {
			...
			logger.log(Level.FINEST, "Unregistered node listener for {0} ",
					nodeId);
		} finally {
			callFinished();
		}
	...
	private void notifyListeners(final Node oldNode, final Node newNode,
		...
		logger.log(Level.FINEST, "In notifyListeners, identity: {0}, "
				+ "oldNode: {1}, newNode: {2}", id, oldNode, newNode);
		if (oldNode != null) {
	...
	/**
	 * Run the given task synchronously, and transactionally, retrying if the
	 * exception is of type <@code ExceptionRetryStatus>.
	 * 
	 * @param task
	 *            the task
	 */
	...
	/**
	 * Move an identity. First, choose a new node for the identity (which can
	 * take a while) and then update the map to reflect the choice, cleaning up
	 * old mappings as appropriate. If given a {@code serviceName}, the status
	 * of the identity is set to active for that service on the new node. The
	 * change in mappings might need to wait for registered
	 * {@code IdentityRelocationListener}s to prepare for the move.
	 * 
	 * @param id
	 *            the identity to map to a new node
	 * @param serviceName
	 *            the name of the requesting service's class, or null
	 * @param oldNode
	 *            the last node the identity was mapped to, or null if there was
	 *            no prior mapping
	 * @param requestingNode
	 *            the node making the mapping request
	 * 
	 * @throws NoNodesAvailableException
	 *             if there are no nodes to map to
	 */
	private long mapToNewNode(final Identity id, String serviceName,
			Node oldNode, long requestingNode) throws NoNodesAvailableException {
		assert (id != null);
		...
		try {
			...
			logger.logThrow(Level.FINEST, ex, "mapToNewNode: id {0} from {1}"
					+ " failed because no live nodes are available", id,
					oldNode);
			throw ex;
		}
	...
	private void moveIdAndNotifyListeners(MoveIdTask moveTask) {
		...
		try {
			...
			logger.logThrow(Level.FINE, e,
					"Move {0} mappings from {1} to {2} failed", id, oldNode,
					newNodeId);
		}
	}
	...
	private class MoveIdTask extends AbstractKernelRunnable {
		...
		MoveIdTask(Identity id, Node oldNode, long newNodeId, String serviceName) {
			super(null);
			this.id = id;
			...
			// The oldNode will be null if this is the first assignment.
			oldNodeKey = (oldNode == null) ? null : NodeMapUtil.getNodeKey(
					oldNode.getId(), id);
			oldStatusKey = (oldNode == null) ? null : NodeMapUtil
					.getPartialStatusKey(id, oldNode.getId());

			newNodekey = NodeMapUtil.getNodeKey(newNodeId, id);
			newStatuskey = (serviceName == null) ? null : NodeMapUtil
					.getStatusKey(id, newNodeId, serviceName);
		...
		public void run() {
			...
			if (oldNode != null) {
				try {
					...
					IdentityMO oldidmo = (IdentityMO) dataService
							.getServiceBinding(idkey);
					...
					Iterator<String> iter = BoundNamesUtil
							.getServiceBoundNamesIterator(dataService,
									oldStatusKey);
					while (iter.hasNext()) {
						iter.next();
						iter.remove();
	...
	private class GetNodeTask extends AbstractKernelRunnable {
		...
		/**
		 * Returns the node found by the watchdog service, or null if this task
		 * has not run or the node has failed and been removed from the data
		 * store.
		 */
		public Node getNode() {
			return node;
	...
	/**
	 * The listener registered with the watchdog service. These methods will be
	 * notified of node health updates.
	 */
		...
		public void nodeHealthUpdate(Node node) {
			...
			if (logger.isLoggable(Level.FINE)) {
				logger.log(Level.FINE,
						"Node {0} health update, health is: {1}", nodeId,
						node.getHealth());
			}
	...
	/**
	 * Task to support node failure, run under a transaction. Finds an identity
	 * that was on the failed node. Code outside the transaction moves the
	 * identity to another node and removes the old id<->failedNode mapping, and
	...
	private static class GetIdOnNodeTask extends AbstractKernelRunnable {
		...
		/**
		 * The identity MO retrieved from the data store, or null if the task
		 * has not yet executed or there was an error while executing.
		 * 
	...
	/**
	 * Task which gets an IdentityMO from a data service. This is a separate
	 * task so we can retrieve the result. An exception will be thrown if the
	 * IdentityMO is not found or the name binding doesn't exist.
	 */
	static class GetIdTask extends AbstractKernelRunnable {
		 ...
		 * 
		 * @param dataService
		 *            the data service to retrieve from
		 * @param idkey
		 *            Identitifier key
		 */
		...
		/**
		 * {@inheritDoc} Get the IdentityMO.
		 * 
		 * @throws NameNotBoundException
		 *             if no object is bound to the id
	...
	/**
	 * Return the data store keys found for a particular identity. Used for
	 * testing.
	 * 
	 * @param identity
	 *            the identity
	...
	/**
	 * Task to assert some invariants about our use of the data store are true.
	 * Assumes that we are in a transaction.
	 */
	private static class AssertTask extends AbstractKernelRunnable {
		...
		public void run() {
			...
			if (idmo != null) {
				...
				// Check status
				Iterator<String> iter = BoundNamesUtil
						.getServiceBoundNamesIterator(dataService, statuskey);

...
/**
 * A graph builder which builds a bipartite graph of identities and object ids,
 * with edges between them. Identities are never linked with other edges, nor
 * are object ids linked to other object ids.
 * <p>
 * This graph builder folds the graph upon request. The folded graph does not
...
public class BipartiteGraphBuilder extends AbstractAffinityGraphBuilder
	...
	private final CopyableGraph<Object, WeightedEdge> bipartiteGraph = new CopyableGraph<Object, WeightedEdge>();

	/**
	 * A map of identity->graph vertex, allowing fast lookups of particular
	 * vertices. This graph is updated when each call to getAffinityGraph is
	 * made, so results may be a little stale compared to the bipartite graph.
	 */
	 ...
	 * of the eviction. Map of object to map of remote nodes it was accessed on,
	 * with a weight for each node.
	 */
	 ...
	 * remove the changes made in the earliest snapshot.
	 */
	...
	public BipartiteGraphBuilder(Properties properties,
		...
		NodeType type = NodeType.valueOf(wrappedProps
		...
		if (type == NodeType.coreServerNode) {
			...
			stats = new AffinityGraphBuilderStats(col, bipartiteGraph,
	...
	public UndirectedGraph<LabelVertex, WeightedEdge> getAffinityGraph() {
		...
		CopyableGraph<Object, WeightedEdge> graphCopy = new CopyableGraph<Object, WeightedEdge>(
				bipartiteGraph);
		...
		UndirectedGraph<LabelVertex, WeightedEdge> foldedGraph = new UndirectedSparseGraph<LabelVertex, WeightedEdge>();
		...
		for (Object objVert : objVerts) {
			...
			List<Object> neighbors = new ArrayList<Object>(
					graphCopy.getNeighbors(objVert));
	...
	public Map<Object, Map<Identity, Long>> getObjectUseMap() {
		Map<Object, Map<Identity, Long>> retMap = new HashMap<Object, Map<Identity, Long>>();
		// Copy our input graph
		CopyableGraph<Object, WeightedEdge> graphCopy = new CopyableGraph<Object, WeightedEdge>(
				bipartiteGraph);
			...
			if (!(vert instanceof Identity)) {
				...
				for (WeightedEdge edge : graphCopy.getIncidentEdges(vert)) {
					...
					if (v1 instanceof Identity) {
						...
						logger.log(Level.FINE, "unexpected vertex type {0}", v1);
					}
				}
	...
	private class PruneTask extends TimerTask {
		...
		public void run() {
			...
			Map<WeightedEdge, Integer> periodEdgeIncrements = periodEdgeIncrementsQueue
					.remove();
			Map<Long, Map<Object, Integer>> periodConflicts = periodConflictQueue
					.remove();
	...
	/**
	 * A version of undirected sparse multigraph which has a copy constructor.
	 * 
	 * @param <V>
	 *            the vertex type
	 * @param <E>

...
/**
 * This is an implementation of {@code TaskService} that works on a single node
 * and across multiple nodes. It handles persisting tasks and keeping track of
 * which tasks have not yet run to completion, so that in the event of a system
 * failure the tasks can be run on re-start.
 * <p>
 * Durable tasks that have not yet run are persisted as instances of
 * {@code PendingTask}, indexed by the owning identity. When a given identity is
 * mapped to the local node, all tasks associated with that identity are started
 * running on the local node. As long as an identity still has pending tasks
 * scheduled to run locally, that identity is marked as active. To help minimize
 * object creation, a cache of {@code PendingTask}s is created for each
 * identity.
 * <p>
 ...
 * already-scheduled, persisted task tries to run on the old node, that task is
 * dropped since it is already scheduled to run on the new node. After an
 * identity has been moved, any subsequent attempts to schedule durable tasks on
 * behalf of that identity on the old node will result in the tasks being
 ...
 * entry in the data store. Each node will periodically query this entry to see
 * if any tasks have been handed off. The time in milliseconds for this period
 * is configurable via the {@value #HANDOFF_PERIOD_PROPERTY} property described
 * below. This checking will be delayed on node startup to give the system a
 * chance to finish initializing. The time in milliseconds for this delay is
 * configurable via the {@value #HANDOFF_START_PROPERTY} property.
 * <p>
 ...
 * however, a delay is taken to see if the status is about to change back to its
 * previous state. This helps avoid voting too frequently. The time in
 ...
 * that the {@code TaskServiceImpl} will regularly query its handoff queue for
 * tasks that have been handed off by other nodes.
 * <p>
 * 
 * <dt><i>Property:</i> <code><b>
 ...
 * {@code TaskServiceImpl} will wait at startup before querying its handoff
 ...
 * before reporting a status change for an identity. The status change is not
 * reported if the identity's status changes back to its original state during
 * this delay period.
 * <p>
 * 
 * <dt><i>Property:</i> <code><b>
 ...
 * 
 * <dd style="padding-top: .5em">Specifies the fully qualified class name of the
 * class which will be used as the {@link ContinuePolicy} for the task service.
 * The given class should be a non-abstract class that implements the
 * {@code ContinuePolicy} interface, and that provides a constructor with the
 * parameters ({@link Properties}, {@link ComponentRegistry},
 * {@link TransactionProxy})
 * <p>
 * 
 * 
 * </dl>
...
public class TaskServiceImpl extends AbstractService implements TaskService,
	...
	/**
	 * The name prefix used to bind all service-level objects associated with
	 * this service.
	 */
	...
	public static final String VOTE_DELAY_PROPERTY = NAME + ".vote.delay";
	...
	public static final String HANDOFF_START_PROPERTY = NAME + ".handoff.start";
	...
	public static final String HANDOFF_PERIOD_PROPERTY = NAME
			+ ".handoff.period";
	 ...
	 */
	public static final String CONTINUE_POLICY_PROPERTY = NAME
			+ ".continue.policy";
	...
	/**
	 * Creates an instance of {@code TaskServiceImpl}. See the class javadoc for
	 ...
	 * @param transactionProxy
	 *            the system's {@code TransactionProxy}
	 * 
	 * @throws Exception
	 *             if the service cannot be created
	 */
	public TaskServiceImpl(Properties properties,
			ComponentRegistry systemRegistry, TransactionProxy transactionProxy)
			throws Exception {
		...
		identityRecurringMap = new HashMap<Identity, Set<RecurringTaskHandle>>();
		availablePendingMap = new ConcurrentHashMap<Identity, Set<BigInteger>>();
		...
		continuePolicy = wrappedProps.getClassInstanceProperty(
				CONTINUE_POLICY_PROPERTY, CONTINUE_POLICY_DEFAULT,
	...
	public void doReady() {
		...
		handoffTaskHandle = transactionScheduler.scheduleRecurringTask(
				new HandoffRunner(), taskOwner, System.currentTimeMillis()
						+ handoffStart, handoffPeriod);
	...
	public PeriodicTaskHandle schedulePeriodicTask(Task task, long delay,
		...
		PendingTask ptask = (PendingTask) (dataService
				.createReferenceForId(objId).getForUpdate());
		ptask.setRunningNode(nodeId);

		RecurringTaskHandle handle = transactionScheduler
				.scheduleRecurringTask(runner, owner,
	...
	/**
	 * Private helper that creates a {@code KernelRunnable} for the task, also
	 * generating a unique name for this task and persisting the associated
	 * {@code PendingTask}.
	 */
	private TaskRunner getRunner(Task task, Identity identity,
		...
		BigInteger objId = allocatePendingTask(task, identity, appStartTime,
				period);

		if (logger.isLoggable(Level.FINEST)) {
			logger.log(Level.FINEST, "created pending task {0} for {1}", objId,
					identity);
		}
	...
	private BigInteger allocatePendingTask(Task task, Identity identity,
		...
		if (set != null) {
			synchronized (set) {
				if (!set.isEmpty()) {
					...
					set.remove(objId);
					ctxFactory.joinTransaction().notePendingIdAllocated(
							identity, objId);
				}
			}
		}
		...
		if (objId == null) {
			...
			ManagedReference<PendingTask> taskRef = dataService
					.createReference(ptask);
			objId = taskRef.getId();
			dataService.setServiceBinding(generateObjName(identity, objId),
					ptask);
		}
	...
	private void scheduleTask(KernelRunnable task, Identity owner,
		...
		if (logger.isLoggable(Level.FINEST)) {
			logger.log(Level.FINEST,
					"reserving a task starting "
							+ (appStartTime == START_NOW ? "now" : "at "
									+ appStartTime));
		}
		...
		try {
			...
			if (logger.isLoggable(Level.FINE)) {
				logger.logThrow(Level.FINE, tre, "could not get a reservation");
			}
			throw tre;
		}
	}
	 ...
	 * managed map of pending tasks. This method is typically used when a task
	 * actually runs. If the Task was managed by the application and has been
	 * removed by the application, or another TaskService task has already
	 * removed the pending task entry, then this method returns null meaning
	 * that there is no task to run.
	 */
	PendingTask fetchPendingTask(BigInteger objId) {
		...
		try {
			...
			if (recurringMap.containsKey(objId)) {
				ctxFactory.joinTransaction().cancelRecurringTask(objId,
						txnProxy.getCurrentOwner());
			} else {
				ctxFactory.joinTransaction().decrementStatusCount(
						txnProxy.getCurrentOwner());
			}
			return null;
		}
	 ...
	 * underlying recurring task, removes the task and name binding, and notes
	 * the cancelled task in the local transaction state.
	 */
	private void cancelPeriodicTask(PendingTask ptask, BigInteger objId) {
		...
		dataService.removeServiceBinding(generateObjName(ptask.getIdentity(),
				objId));
		dataService.removeObject(ptask);
	}

	/**
	 * Private helper that notifies the service about a task that failed and is
	 * not being re-tried. This happens whenever a task is run by the scheduler,
	 * and throws an exception that doesn't request the task be re-tried. In
	 * this case, the transaction gets aborted, so the pending task stays in the
	 * map. This method is called to start a new task with the sole purpose of
	 * creating a new transactional task where the pending task can be removed,
	 * if that task is not periodic. Note that this method is not called within
	...
	private void notifyNonRetry(final BigInteger objId) {
		...
		try {
			transactionScheduler.scheduleTask(
					new NonRetryCleanupRunnable(objId),
					txnProxy.getCurrentOwner());
		} catch (TaskRejectedException tre) {
			...
			if (shuttingDown()) {
				if (logger.isLoggable(Level.WARNING)) {
					logger.log(Level.WARNING, "Service is shutdown, so a "
							+ "non-retried task {0} will not be removed", objId);
				}
				throw new IllegalStateException("Service is shutdown");
			}
	...
	private class TxnState extends TransactionContext {
		...
		private HashMap<Identity, Integer> statusMap = new HashMap<Identity, Integer>();
		...
		public void commit() {
			...
			if (addedRecurringMap != null) {
				for (Entry<BigInteger, RecurringDetail> entry : addedRecurringMap
						.entrySet()) {
		...
		public void abort(boolean retryable) {
			...
			if (allocatedTaskIds != null) {
				for (Entry<Identity, HashSet<BigInteger>> entry : allocatedTaskIds
						.entrySet()) {
					Set<BigInteger> localSet = availablePendingMap.get(entry
							.getKey());
					if (localSet != null) {
		...
		void cancelRecurringTask(BigInteger objId, Identity identity) {
			RecurringDetail detail = (addedRecurringMap != null) ? addedRecurringMap
					.remove(objId) : null;
	...
	private class TransactionContextFactoryImpl extends
			TransactionContextFactory<TxnState> {
	...
	/**
	 * Private implementation of {@code KernelRunnable} that is used to run the
	 * {@code Task}s scheduled by the application.
	 */
	private class TaskRunner implements KernelRunnable {
		...
		/**
		 * This method is used in the case where the associated identity is not
		 * mapped to the local node, but no assignment exists yet. In these
		 * cases, the task is just run locally, so no check should be done to
		 * see if the identity is local.
		 */
		...
		public void run() throws Exception {
			...
			try {
				...
				if (ptask.isPeriodic()) {
					...
					ptask.setLastStartTime(watchdogService
	 ...
	 * when a non-durable task runs the status count for the associated identity
	 * is decremented, and runs the task within a transaction if this was
	 * requested.
	 */
	...
	/**
	 * Private helper that restarts all of the tasks associated with the given
	 * identity. This must be called within a transaction.
	 */
	...
	private void restartTask(String objName) {
		...
		if (ptask.getPeriod() == PERIOD_NONE) {
			...
			RecurringTaskHandle handle = transactionScheduler
					.scheduleRecurringTask(runner, identity,
							watchdogService.getSystemTimeMillis(restartTime),
							ptask.getPeriod());
			ctxFactory.joinTransaction().addRecurringTask(objId, handle,
					identity);
		}
	}
	...
	/**
	 * Private helper to remove a recurring handle from the set for an identity.
	 */
	...
	private void cancelHandlesForIdentity(Identity identity) {
		synchronized (identityRecurringMap) {
			Set<RecurringTaskHandle> set = identityRecurringMap
					.remove(identity);
			if (set != null) {
	...
	/**
	 * Private helper method that hands-off a durable task from the current node
	 * to a new node. The task needs to have already been persisted as a
	 * {@code PendingTask} under the given name binding. This method must be
	 * called in the context of a valid transaction. If this method returns
	 * {@code false} then the task was not handed-off, and so it must be run on
	 * the local node. NOTE: we may want to revisit this final assumption,
	 * perhaps delaying such tasks, or coming up with some other policy
	 */
	private boolean handoffTask(String objName, Identity identity) {
		...
		try {
			StringHashSet set = (StringHashSet) dataService
					.getServiceBinding(handoffName);
			set.add(objName);
		} catch (NameNotBoundException nnbe) {
	...
	private void assignNode(final Identity identity) {
		(new Thread(new Runnable() {
			public void run() {
				nodeMappingService.assignNode(TaskServiceImpl.class, identity);
			}
		})).start();
	}

	/**
	 * Private runnable that periodically checks to see if any tasks have been
	 * handed-off from another node.
	 */
	private class HandoffRunner implements KernelRunnable {
		...
		public void run() throws Exception {
			StringHashSet set = (StringHashSet) dataService
					.getServiceBinding(localHandoffSpace);
			if (!set.isEmpty()) {
	 ...
	 * currently thought to be mapped to the local node. This does not need to
	...
	/**
	 * Private helper that checks if the given {@code Identity} is running any
	 * tasks on the local node. This does not need to be called from within a
	 * transaction.
	 */
	...
	/**
	 * Private helper that accepts status change votes. This method does not
	 * access any transactional context or other services, but may be called in
	 * a transaction. Note that a count of 0 is ignored.
	 */
		...
		if (newNode == null) {
			try {
				transactionScheduler.runTask(new KernelRunnable() {
					...
					public void run() throws Exception {
						String prefix = DS_PENDING_SPACE + id.getName() + ".";
						String objName = dataService
								.nextServiceBoundName(prefix);
						while ((objName != null)
								&& (objName.startsWith(prefix))) {
							Object obj = dataService.getServiceBinding(objName);
	...
	/**
	 * Private implementation of {@code PeriodicTaskHandle} that is provided to
	 * application developers so they can cancel their tasks in the future. This
	 * class uses the internally assigned name to reference the task in the
	 * future, and uses the thread local service reference to find its service.
	 */
	private static class PeriodicTaskHandleImpl implements PeriodicTaskHandle,
		...
		public void cancel() {
			...
			try {
				PendingTask ptask = (PendingTask) (service.dataService
						.getServiceBinding(objName));
				service.cancelPeriodicTask(ptask,
						TaskServiceImpl.getIdFromName(objName));
			} catch (NameNotBoundException nnbe) {

...
/**
 * Static utility methods used by both the server and service for data service
 * name bindings.
 */
final class NodeMapUtil {
	...
	/**
	 * The major and minor version numbers for the layout of keys and data
	 * persisted by the node mapping service.
	 */
	...
	/**
	 * Returns a node key for the given {@code nodeId}; used for iterating
	 * through a node for all identities assigned to it.
	 */
	...
	/**
	 * Private helper method; returns a {@code StringBuilder} for a partial node
	 * key.
	 */
	...
	/**
	 * Returns a status key for the given {@code id}, {@code nodeId}, and
	 * {@code serviceName}. The status mappings are held per identity per node.
	 */
	...
	/**
	 * Returns a status key for the given {@code id} and {@code nodeId}. Used
	 * for finding all status entries for an identity on a particular node.
	 */
	...
	/**
	 * Returns a status key for a given {@code id}. Used for finding all status
	 * entries for an identity (on all nodes).
	 */
	...
	/**
	 * Private helper method; returns a {@code StringBuilder} for a partial
	 * status key.
	 */

 ...
 * Nor is this check done for {@code TransactionListener}s. If two participants
 * or listeners have the same type name then their profiling data will be
 * aggregated and reported as a single result.
 */
final class TransactionImpl implements Transaction {
	...
	/**
	 * The transaction participants. If there is a durable participant, it will
	 * be listed last. Participants whose prepare method returns true
	...
	/**
	 * Collected profiling data on each participant, created only if global
	 * profiling is set to MEDIUM at the start of the transaction.
	 */
	private final HashMap<String, ProfileParticipantDetailImpl> participantDetailMap;

	/**
	 * Collected profiling data on each listener, created only if global
	 * profiling is set to MEDIUM at the start of the transaction.
	 */
	...
	/**
	 * Creates an instance with the specified transaction ID, timeout, prepare
	 * and commit optimization flag, and collectorHandle.
	 */
	TransactionImpl(long tid, long timeout, boolean usePrepareAndCommitOpt,
		...
		state = State.ACTIVE;
		if (collectorHandle.getCollector().getDefaultProfileLevel().ordinal() >= ProfileLevel.MEDIUM
				.ordinal()) {
			participantDetailMap = new HashMap<String, ProfileParticipantDetailImpl>();
			listenerDetailMap = new HashMap<String, TransactionListenerDetailImpl>();
		} else {
	...
	public void checkTimeout() {
		...
		if (runningTime > getTimeout()) {
			TransactionTimeoutException exception = new TransactionTimeoutException(
	...
	public void join(TransactionParticipant participant) {
		...
		if (!participants.contains(participant)) {
			...
			if (participantDetailMap != null) {
				...
				participantDetailMap.put(name,
						new ProfileParticipantDetailImpl(name));
			}
		}
	}
	...
	public void abort(Throwable cause) {
		...
		for (TransactionParticipant participant : participants) {
			if (logger.isLoggable(Level.FINEST)) {
				logger.log(Level.FINEST, "abort {0} participant:{1}", this,
						getParticipantInfo(participant));
			}
			if (participantDetailMap != null) {
			...
			try {
				...
				if (logger.isLoggable(Level.WARNING)) {
					logger.logThrow(Level.WARNING, e,
							"abort {0} participant:{1} failed", this,
							getParticipantInfo(participant));
				}
			}
			if (participantDetailMap != null) {
				long finishTime = System.currentTimeMillis();
				ProfileParticipantDetailImpl detail = participantDetailMap
						.get(participant.getTypeName());
	...
	public void registerListener(TransactionListener listener) {
		...
		if (listenerDetailMap != null) {
			...
			listenerDetailMap
					.put(name, new TransactionListenerDetailImpl(name));
		}
	}
	...
	public String toString() {
		return "TransactionImpl[tid:" + tid + ", creationTime:" + creationTime
	 ...
	 * 
	 * @return <code>true</code> if the argument equals this instance, otherwise
	 *         <code>false</code>
	 */
	 ...
	 *             if a call to {@link TransactionParticipant#prepare prepare}
	 *             on a transaction participant or to
	 *             {@link TransactionListener#beforeCompletion beforeCompletion}
	 *             on a transaction listener aborts the transaction but does not
	 *             throw an exception
	 * @throws IllegalStateException
	 *             if {@code prepare} has been called on any transaction
	 *             participant and {@link Transaction#abort abort} has not been
	 *             called on the transaction, or if called from a thread that is
	 *             not the thread that created this transaction
	 * @throws Exception
	 *             any exception thrown when calling {@code prepare} on a
	 *             participant or {@code beforeCompletion} on a listener
	...
	void commit() throws Exception {
		...
		for (Iterator<TransactionParticipant> iter = participants.iterator(); iter
				.hasNext();) {
				...
				if (iter.hasNext() || disablePrepareAndCommitOpt) {
					...
					if (logger.isLoggable(Level.FINEST)) {
						logger.log(Level.FINEST,
								...
								this, getParticipantInfo(participant), readOnly);
					}
				} else {
					participant.prepareAndCommit(this);
					if (detail != null) {
						detail.setCommittedDirectly(System.currentTimeMillis()
								- startTime);
						collectorHandle.addParticipant(detail);
					}
					iter.remove();
		...
		for (TransactionParticipant participant : participants) {
			if (logger.isLoggable(Level.FINEST)) {
				logger.log(Level.FINEST, "commit {0} participant:{1}", this,
						getParticipantInfo(participant));
			}
			if (participantDetailMap != null) {
				detail = participantDetailMap.get(participant.getTypeName());
			...
			try {
				...
				if (detail != null) {
					detail.setCommitted(System.currentTimeMillis() - startTime);
					collectorHandle.addParticipant(detail);
				}
			} catch (RuntimeException e) {
				if (logger.isLoggable(Level.WARNING)) {
					logger.logThrow(Level.WARNING, e,
							"commit {0} participant:{1} failed", this,
							getParticipantInfo(participant));
				}
			}
		}
	...
	private void checkThread(String methodName) {
		if (Thread.currentThread() != owner) {
			throw new IllegalStateException("The " + methodName
					+ " method must be called from the"
	 ...
	 * participant is null.
	 */
	private static String getParticipantInfo(TransactionParticipant participant) {
		return participant == null ? null : (participant.getTypeName() + " ("
				+ participant + ")");
	}
}

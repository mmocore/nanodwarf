...
/**
 * Provides an implementation of <code>DataService</code> based on
 * {@link DataStoreImpl}.
 * <p>
 * 
 * The {@link #DataServiceImpl constructor} requires the <a href=
 ...
 * properties</a> and the following additional properties:
 * <p>
 * 
 ...
 * 	</b></code> <br>
 * <i>Default:</i>
 * <code>com.sun.sgs.impl.service.data.store.net.DataStoreClient</code> unless
 * the {@code com.sun.sgs.node.type} property is {@code singleNode}, which
 * defaults to <code>com.sun.sgs.impl.service.data.store.DataStoreImpl</code>
 * 
 * <dd style="padding-top: .5em">The name of the class that implements
 * {@link DataStore}. The class should be public, not abstract, and should
 * provide a public constructor with {@link Properties},
 ...
 * to managed objects. If set to something other than <code>true</code>, then
 * applications need to call {@link DataManager#markForUpdate
 * DataManager.markForUpdate} or {@link ManagedReference#getForUpdate
 * ManagedReference.getForUpdate} for any modified objects to make sure that the
 * modifications are recorded by the <code>DataService</code>.
 * <p>
 * 
 ...
 * references table. Note that the number of operations is measured separately
 * for each transaction. This property is intended for use in debugging.
 * <p>
 * 
 ...
 * acquires write locks as soon as it knows that an object is being modified. If
 * <code>true</code>, the service delays obtaining write locks until commit
 * time, which may improve performance in some cases, typically when there is
 * low contention. Note that setting this flag to <code>true</code> does not
 * delay write locks when removing objects.
 * <p>
 * 
 ...
 * objects. If <code>true</code>, the <code>DataService</code> keeps track of
 * persistent or removed objects from completed transactions and throws
 * {@link TransactionNotActiveException} if the application refers to those
 * objects from another transaction.
 * <p>
 * 
 * </dl>
 * <p>
 ...
 * <code>com.sun.sgs.impl.service.data.DataServiceImpl</code> to log information
 ...
 * <p>
 * 
 * It also uses an additional {@code Logger} named
 * {@code com.sun.sgs.impl.service.data.DataServiceImpl.detect.modifications} to
 * log information about managed objects that are found to be modified but were
 * not marked for update. Note that this logging output will only be performed
 * if the
 * {@code com.sun.sgs.impl.service.data.DataServiceImpl.detect.modifications}
 ...
 * <p>
 * 
 * In addition, operations that throw a {@link TransactionAbortedException} will
 * log the failure to the {@code Logger} named
 * {@code com.sun.sgs.impl.service.data.DataServiceImpl.abort}, to make it
 * easier to debug concurrency conflicts by just logging aborts.
 */
public final class DataServiceImpl implements DataService {
	 ...
	 */
	public static final String DEBUG_CHECK_INTERVAL_PROPERTY = CLASSNAME
			+ ".debug.check.interval";

	/**
	 * The property that specifies whether to automatically detect modifications
	 * to objects.
	 */
	public static final String DETECT_MODIFICATIONS_PROPERTY = CLASSNAME
	 ...
	 * {@link DataStore}.
	 */
	public static final String DATA_STORE_CLASS_PROPERTY = CLASSNAME
			+ ".data.store.class";

	/** The property that specifies to use optimistic write locking. */
	public static final String OPTIMISTIC_WRITE_LOCKS = CLASSNAME
			+ ".optimistic.write.locks";

	/** The property that specifies whether to track stale objects. */
	public static final String TRACK_STALE_OBJECTS_PROPERTY = CLASSNAME
			+ ".track.stale.objects";
	...
	private static final LoggerWrapper abortLogger = new LoggerWrapper(
			Logger.getLogger(CLASSNAME + ".abort"));
	...
	/**
	 * The number of operations to skip between checks of the consistency of the
	 * managed reference table.
	 */
	...
	/**
	 * Defines the transaction context map for this class. This class checks the
	 * service state and the reference table whenever the context is accessed.
	 * No check is needed for joinTransaction, though, because the check is
	 * already being made when obtaining the context factory.
	 */
	private static final class ContextMap extends
			TransactionContextMap<Context> {
		...
		@Override
		public Context checkTransaction(Transaction txn) {
	...
	private final class ContextFactory extends
		...
		protected Context createContext(Transaction txn) {
			...
			return new Context(DataServiceImpl.this, store, txn,
					debugCheckInterval, detectModifications, classesTable,
					trackStaleObjects);
		}
	}
	 ...
	 *             if the <code>com.sun.sgs.app.name
	 ...
	 * @throws Exception
	 *             if a problem occurs creating the service
	 */
	public DataServiceImpl(Properties properties,
		...
		try {
			...
			String dataStoreClassName = wrappedProps
					.getProperty(DATA_STORE_CLASS_PROPERTY);
			...
			if (dataStoreClassName != null) {
				baseStore = wrappedProps
						.getClassInstanceProperty(DATA_STORE_CLASS_PROPERTY,
								DataStore.class, new Class[] {
										Properties.class,
										ComponentRegistry.class,
										TransactionProxy.class }, properties,
								systemRegistry, txnProxy);
				logger.log(Level.CONFIG, "Using data store {0}", baseStore);
			} else if (nodeType == NodeType.singleNode) {
				baseStore = new DataStoreImpl(properties, systemRegistry,
						txnProxy);
			} else {
				baseStore = new DataStoreClient(properties, systemRegistry,
						txnProxy);
			}
			...
			systemRegistry.getComponent(TransactionScheduler.class).runTask(
					new AbstractKernelRunnable("BindDataServiceHeader") {
						public void run() {
							...
							try {
								header = (DataServiceHeader) getServiceBinding(CLASSNAME
										+ ".header");
								logger.log(Level.CONFIG,
										"Found existing header {0}", header);
			...
			logger.log(Level.CONFIG, "Created DataServiceImpl with properties:"
					...
					+ dataStoreClassName + "\n  "
					+ DEBUG_CHECK_INTERVAL_PROPERTY + "=" + debugCheckInterval
					+ "\n  " + DETECT_MODIFICATIONS_PROPERTY + "="
					+ detectModifications + "\n  " + OPTIMISTIC_WRITE_LOCKS
					+ "=" + optimisticWriteLocks + "\n  "
					+ TRACK_STALE_OBJECTS_PROPERTY + "=" + trackStaleObjects);

		} catch (RuntimeException e) {
			getExceptionLogger(e).logThrow(Level.SEVERE, e,
					"DataService initialization failed");
			throw e;
		} catch (Error e) {
	...
	public void removeObject(Object object) {
		...
		try {
			...
			if (exceptionLogger.isLoggable(Level.FINEST)) {
				exceptionLogger.logThrow(Level.FINEST, e,
						"removeObject tid:{0,number,#}, type:{1},"
								+ " oid:{2,number,#} throws",
						contextTxnId(context), typeName(object), refId(ref));
			}
	...
	public ManagedObject getServiceBindingForUpdate(String name) {
		...
		return getBindingInternal(name, true, true,
	...
	public BigInteger nextObjectId(BigInteger objectId) {
		...
		try {
			...
			BigInteger result = (nextOid == -1) ? null : BigInteger
					.valueOf(nextOid);
			if (logger.isLoggable(Level.FINEST)) {
				logger.log(Level.FINEST,
						"nextObjectId objectId:{0} returns {1}", objectId,
						result);
			}
			return result;
		} catch (RuntimeException e) {
			getExceptionLogger(e).logThrow(Level.FINEST, e,
					"nextObjectId objectId:{0} throws", objectId);
			throw e;
		}
	}
	...
	private ManagedObject getBindingInternal(String name,
			boolean serviceBinding, boolean forUpdate, String methodName) {
		Context context = null;
		try {
			if (name == null) {
			...
			try {
				...
				throw new NameNotBoundException("Name '" + name
						+ "' is not bound");
			}
			if (logger.isLoggable(Level.FINEST)) {
				logger.log(Level.FINEST,
						"{0} tid:{1,number,#}, name:{2} returns type:{3},"
								+ " oid:{4,number,#}", methodName,
			...
			if (exceptionLogger.isLoggable(Level.FINEST)) {
				exceptionLogger.logThrow(Level.FINEST, e,
						"{0} tid:{1,number,#}, name:{2} throws", methodName,
	...
	private void setBindingInternal(String name, Object object,
		...
		try {
			...
			if (logger.isLoggable(Level.FINEST)) {
				logger.log(Level.FINEST,
						...
						serviceBinding ? "removeServiceBinding"
								: "removeBinding", contextTxnId(context), name);
			}
		} catch (RuntimeException e) {
			LoggerWrapper exceptionLogger = getExceptionLogger(e);
			if (exceptionLogger.isLoggable(Level.FINEST)) {
		...
		try {
			...
			if (logger.isLoggable(Level.FINEST)) {
				logger.log(Level.FINEST,
						...
						serviceBinding ? "nextServiceBoundName"
								: "nextBoundName", contextTxnId(context), name,
						result);
			}
			return result;
		} catch (RuntimeException e) {
	...
	public String toString() {
		return "DataServiceImpl[appName:\"" + appName + ", store:" + store
				+ "\"]";
	}
	 ...
	 * 
	 * @param debugCheckInterval
	 *            the number of operations to skip between checks of the
	 *            consistency of the managed references table
	 */
	 ...
	 * binding. If name is null, then returns a name that will sort earlier than
	 * any non-null name.
	 */
	private static String getInternalName(String name, boolean serviceBinding) {
		if (name == null) {
	 ...
	 */
	private static String getExternalName(String name, boolean serviceBinding) {
		if (name == null) {
			return null;
		}
		 ...
		 * "s." if we've moved past all of the application bindings. Otherwise,
		 * the prefix should be correct. -tjb@sun.com (12/14/2006)
		 */
		assert name.startsWith(prefix)
				|| (!serviceBinding && name.startsWith("s.")) : "Name has wrong prefix";
		return name.startsWith(prefix) ? name.substring(2) : null;
	}

	/**
	 * Converts a BigInteger object ID into a long, throwing an exception if the
	 * argument is invalid.
	 */
	private static long getOid(BigInteger objectId) {
		if (objectId == null) {
			...
			throw new IllegalArgumentException("The object ID is invalid: "
					+ objectId);
		}
	...
	/**
	 * Returns the object ID of the object, or null if the object is null or has
	 * not assigned an ID, or if the context is null. Returns an ID even if the
	 * object is removed.
	 */
	...
	/**
	 * Returns the logger that should be used to log the specified exception. In
	 * particular, use the abortLogger for TransactionAbortedException, and the
	 * class logger for other runtime exceptions.
	 */
	static LoggerWrapper getExceptionLogger(RuntimeException exception) {
		return exception instanceof TransactionAbortedException ? abortLogger
				: logger;
	}
}

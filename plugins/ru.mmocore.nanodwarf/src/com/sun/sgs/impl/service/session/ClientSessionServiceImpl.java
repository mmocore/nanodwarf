...
import com.sun.sgs.impl.service.session.ClientSessionImpl.HandleNextDisconnectedSessionTask;
 ...
 * <p>
 * 
 * The {@link #ClientSessionServiceImpl constructor} requires the <a href=
 * "../../../impl/kernel/doc-files/config-properties.html#com.sun.sgs.app.name">
 * <code>com.sun.sgs.app.name</code></a> configuration property and supports
 * these public configuration <a href=
 * "../../../impl/kernel/doc-files/config-properties.html#ClientSessionService">
 * properties</a>. It also supports the following additional properties:
 * <p>
 * 
 ...
 * <code>ClientSessionService</code>'s internal server.
 * <p>
 * 
 * <dt><i>Property:</i> <code><b>
 ...
 * the same username as an already connected client will not be permitted to
 * login. If {@code true}, the user's existing session will be disconnected and
 * the new login is allowed to proceed.
 * <p>
 * 
 * <dt><i>Property:</i> <code><b>
 ...
 * 
 * <dd style="padding-top: .5em">Specifies the login high water. When the number
 * of logins reaches the high water, the service's health is set to
 * {@link Health#YELLOW}. If the number of logins exceeds 10% above the high
 * water the service's health is set to {@link Health#ORANGE}. Legal values are
 * between 0 and {@code Integer.MAX_VALUE}.
 * <p>
 * 
 * <dt><i>Property:</i> <code><b>
 ...
 * 
 * <dd style="padding-top: .5em">Specifies the name of the class which will be
 * used as the protocol acceptor. The default value uses an acceptor based on
 * the {@link SimpleSgsProtocol}. Other values should specify the fully
 * qualified name of a non-abstract class that implements
 * {@link ProtocolAcceptor}.
 * <p>
 * 
 * <dt><i>Property:</i> <code><b>
 ...
 * 
 * <dd style="padding-top: .5em">Specifies the length, in bytes, of a relocation
 * key.
 * <p>
 * 
 * <dt><i>Property:</i> <code><b>
 ...
 * client session relocation.
 * <p>
 * 
 * </dl>
 * <p>
 */
public final class ClientSessionServiceImpl extends AbstractService implements
		ClientSessionService {
	/** The package name. */
	private static final String PKG_NAME = "com.sun.sgs.impl.service.session";

	/** The name of this class. */
	private static final String CLASSNAME = ClientSessionServiceImpl.class
			.getName();
	...
	private static final String PROTOCOL_DESCRIPTORS_MAP_KEY = PKG_NAME
			+ ".service.protocol.descriptors.map";
	...
	static final String ALLOW_NEW_LOGIN_PROPERTY = PKG_NAME
			+ ".allow.new.login";

	/** The name of the login high water property. */
	static final String LOGIN_HIGH_WATER_PROPERTY = PKG_NAME
			+ ".login.high.water";

	/** The protocol acceptor property name. */
	static final String PROTOCOL_ACCEPTOR_PROPERTY = PKG_NAME
			+ ".protocol.acceptor";
	...
	static final String RELOCATION_KEY_LENGTH_PROPERTY = PKG_NAME
	...
	private final Set<ClientSessionStatusListener> sessionStatusListeners = Collections
			.synchronizedSet(new HashSet<ClientSessionStatusListener>());

	/** A map of local session handlers, keyed by session ID . */
	private final Map<BigInteger, ClientSessionHandler> handlers = Collections
			.synchronizedMap(new HashMap<BigInteger, ClientSessionHandler>());

	/** Queue of contexts that are prepared (non-readonly) or committed. */
	private final Queue<Context> contextQueue = new ConcurrentLinkedQueue<Context>();
	...
	/**
	 * The map of logged in {@code ClientSessionHandler}s, keyed by identity.
	 */
	private final ConcurrentHashMap<Identity, ClientSessionHandler> loggedInIdentityMap = new ConcurrentHashMap<Identity, ClientSessionHandler>();

	/** The map of session task queues, keyed by session ID. */
	private final ConcurrentHashMap<BigInteger, TaskQueue> sessionTaskQueues = new ConcurrentHashMap<BigInteger, TaskQueue>();

	/**
	 * Information for preparing a session to relocate from this node, keyed by
	 * session ID.
	 */
	private final ConcurrentHashMap<BigInteger, PrepareRelocationInfo> prepareRelocationMap = new ConcurrentHashMap<BigInteger, PrepareRelocationInfo>();

	/**
	 * The map of relocation information for sessions relocating to this node,
	 * keyed by relocation key.
	 */
	private final ConcurrentHashMap<BigInteger, RelocationInfo> incomingSessionRelocationKeys = new ConcurrentHashMap<BigInteger, RelocationInfo>();

	/**
	 * The map of relocation information for sessions relocating to this node,
	 * keyed by session ID.
	 */
	private final ConcurrentHashMap<BigInteger, RelocationInfo> incomingSessionRelocationInfo = new ConcurrentHashMap<BigInteger, RelocationInfo>();

	/** The set of identities that are relocating to this node. */
	private final Set<Identity> incomingRelocatingIdentities = Collections
			.synchronizedSet(new HashSet<Identity>());

	/** The maximum number of session events to service per transaction. */
	final int eventsPerTxn;

	/**
	 * The flag that indicates how to handle same user logins. If {@code true},
	 * then if the same user logs in, the existing session will be disconnected,
	 * and the new login is allowed to proceed. If {@code false}, then if the
	 * same user logs in, the new login will be denied.
	 */
	...
	public ClientSessionServiceImpl(Properties properties,
		...
		try {
			/*
			 * Get the property for controlling session event processing and
			...
			loginHighWater = wrappedProps.getIntProperty(
					LOGIN_HIGH_WATER_PROPERTY, Integer.MAX_VALUE / 2, 0,
					Integer.MAX_VALUE / 2);
			relocationKeyLength = wrappedProps.getIntProperty(
					RELOCATION_KEY_LENGTH_PROPERTY,
					DEFAULT_RELOCATION_KEY_LENGTH, 16, Integer.MAX_VALUE);
			...
			exporter = new Exporter<ClientSessionServer>(
					ClientSessionServer.class);
			try {
			...
			transactionScheduler.runTask(new AbstractKernelRunnable(
					"StoreClientSessionServiceProxy") {
				public void run() {
			...
			nodeMapService
					.addIdentityRelocationListener(new IdentityRelocationListenerImpl());
			nodeMapService
			...
			protocolAcceptor = wrappedProps.getClassInstanceProperty(
					PROTOCOL_ACCEPTOR_PROPERTY, DEFAULT_PROTOCOL_ACCEPTOR,
					ProtocolAcceptor.class, new Class[] { Properties.class,
			...
			ConfigManager config = (ConfigManager) collector
					.getRegisteredMBean(ConfigManager.MXBEAN_NAME);
			if (config == null) {
				logger.log(Level.CONFIG, "Could not find ConfigMXBean");
			} else {
				config.setProtocolDescriptor(protocolAcceptor.getDescriptor()
						.toString());
			}
	...
	public void doReady() throws Exception {
		...
		transactionScheduler.runTask(new AbstractKernelRunnable(
				"AddProtocolDescriptorMapping") {
			public void run() {
				getProtocolDescriptorsMap()
						.put(localNodeId,
								Collections.singleton(protocolAcceptor
										.getDescriptor()));
			}
		}, taskOwner);
	}
	...
	ClientSessionServer getClientSessionServer(long nodeId) {
		if (nodeId == localNodeId) {
			...
			try {
				ManagedSerializable wrappedProxy = (ManagedSerializable) dataService
						.getServiceBinding(sessionServerKey);
	...
	/**
	 * This listener receives notifications of identities that are going to be
	 * relocated from this node to give services a chance to prepare for the
	 * relocation.
	 * <p>
	 * 
	 * Before an identity is relocated from this node the
	 * {@link #prepareToRelocate} method is invoked on this listener. If the
	 * identity corresponds to a local client session, then this service starts
	 * to prepare the client session for relocation. First, it adds a
	 * {@code MoveEvent} to the client session's event queue so that the client
	 * session can be marked as relocating.
	 */
	private class IdentityRelocationListenerImpl implements
		...
		public void prepareToRelocate(Identity id, final long newNodeId,
			...
			if (logger.isLoggable(Level.FINEST)) {
				logger.log(Level.FINEST,
						"identity:{0} localNode:{1} newNodeId:{2}", id,
						localNodeId, newNodeId);
			}

			final ClientSessionHandler sessionHandler = loggedInIdentityMap
					.get(id);
			...
			if (sessionHandler != null) {
				...
				PrepareRelocationInfo prepareInfo = new PrepareRelocationInfo(
						sessionRefId, newNodeId, handler);
				PrepareRelocationInfo oldPrepareInfo = prepareRelocationMap
						.putIfAbsent(sessionRefId, prepareInfo);
				if (oldPrepareInfo == null) {
							...
							new AbstractKernelRunnable("AddMoveEvent") {
								public void run() {
									ClientSessionImpl session = ClientSessionImpl
											.getSession(dataService,
													sessionHandler.sessionRefId);
									if (session != null) {
					...
					taskScheduler.scheduleTask(
							new MonitorSessionRelocatingFromLocalNodeTask(
									sessionRefId), id,
							System.currentTimeMillis() + relocationTimeout);
	...
	/**
	 * The client session service's {@code NodeMappingListener} implementation
	 * is interested in mappings that have been removed from the local node that
	 * correspond to client sessions that are relocating from this node. When
	 * the node mapping service removes the identity mapping of a relocating
	 * session from this node, that means relocation preparation is complete and
	 * the client session corresponding to the identity can be notified to
	 * relocate its connection to the new node.
	 */
					...
					if (newNode != null) {
						if (logger.isLoggable(Level.WARNING)) {
							logger.log(Level.WARNING,
									...
									HexDumper.toHexString(sessionRefId), id,
									localNodeId, newNode.getId());
						}
					} else {
						if (logger.isLoggable(Level.WARNING)) {
							logger.log(
	...
	/**
	 * This {@code ProtocolListener} implementation handles new and relocated
	 * client sessions.
	 */
	private class ProtocolListenerImpl implements ProtocolListener {
		...
		public void newLogin(
			...
			new ClientSessionHandler(ClientSessionServiceImpl.this,
					dataService, protocol, identity, completionHandler);
		}
		...
		public void relocatedSession(
				...
				SessionProtocol protocol,
				RequestCompletionHandler<SessionProtocolHandler> completionHandler) {
			RelocationInfo info = incomingSessionRelocationKeys
				...
				if (logger.isLoggable(Level.FINE)) {
					logger.log(Level.FINE,
							"Attempt to relocate to node:{0} with "
									+ "invalid relocation key:{1}",
							localNodeId, HexDumper.toHexString(relocationKey));
				}
				(new SetupCompletionFuture(null, completionHandler))
						.setException(new RelocateFailureException(
								ClientSessionHandler.RELOCATE_REFUSED_REASON,
								RelocateFailureException.FailureReason.OTHER));
				return;
			}
			new ClientSessionHandler(ClientSessionServiceImpl.this,
					dataService, protocol, info.identity, completionHandler,
					info.sessionRefId);
		}
	}
	...
	final class Context extends TransactionContext {
		...
		/**
		 * Map of client session IDs to an object containing a list of actions
		 * to perform upon transaction commit.
		 */
		private final Map<BigInteger, CommitActions> commitActions = new HashMap<BigInteger, CommitActions>();
		...
		/**
		 * Adds an action for the specified session to be performed when the
		 * associated transaction commits. If {@code first} is {@code true},
		 * then the action is added as the first commit action for the specified
		 * session.
		 */
		void addCommitAction(BigInteger sessionRefId, Action action,
				boolean first) {
			try {
				if (logger.isLoggable(Level.FINEST)) {
					logger.log(Level.FINEST,
		...
		/**
		 * Throws a {@code TransactionNotActiveException} if this transaction is
		 * prepared.
		 */
		 ...
		 * adds this context to the context queue and returns {@code false}.
		...
		/**
		 * Removes the context from the context queue containing pending
		 * actions, and checks for flushing committed contexts.
		 */
		...
		/**
		 * Marks this transaction as committed, and checks for flushing
		 * committed contexts.
		 */
		public void commit() {
			isCommitted = true;
		...
		/**
		 * Wakes up the thread to process committed contexts in the context
		 * queue if the queue is non-empty and the first context in the queue is
		 * committed.
		 */
		...
		/**
		 * Sends all message enqueued during this context's transaction (via the
		 * {@code addMessage} and {@code addMessageFirst} methods), and
		 * disconnects any session whose disconnection was requested via the
		 * {@code requestDisconnect} method.
		 */
	...
	interface Action {
		...
		/**
		 * Performs the commit action and returns {@code true} if further
		 * actions should be processed after this one, and returns {@code false}
		 * otherwise. A {@code false} value would be returned, for example, if
		 * an action disconnects the client session or notifies the client of
		 * login failure.
		 */
		boolean flush();
	}
	...
	/**
	 * Thread to process the context queue, in order, to flush any committed
	 * changes.
	 */
	private class FlushContextsThread extends Thread {
		...
		/**
		 * Processes the context queue, in order, to flush any committed
		 * changes. This thread waits to be notified that a committed context is
		 * at the head of the queue, then iterates through the context queue
		 * invoking {@code flush} on the {@code Context} returned by
		 * {@code next}. Iteration ceases when either a context's {@code flush}
		 * method returns {@code false} (indicating that the transaction
		 * associated with the context has not yet committed) or when there are
		...
		public void run() {
			...
			while (true) {
				...
				/*
				 * Wait for a non-empty context queue, returning if the service
				 * is shutting down.
				 */
				synchronized (flushContextsLock) {
				...
				/*
				 * Remove committed contexts from head of context queue, and
				 * enqueue them to be flushed.
				 */
				if (!contextQueue.isEmpty()) {
	...
	/**
	 * Implements the {@code ClientSessionServer} that receives requests from
	 * {@code ClientSessionService}s on other nodes to forward messages local
	 * client sessions or to service a client session's event queue.
	 */
	private class SessionServerImpl implements ClientSessionServer {
		...
		/** {@inheritDoc} */
		public byte[] relocatingSession(Identity identity, byte[] sessionId,
				long oldNodeId) {
			callStarted();
			try {
				if (logger.isLoggable(Level.FINEST)) {
				...
				RelocationInfo info = new RelocationInfo(identity, sessionRefId);
				...
				try {
					transactionScheduler.runTask(new AbstractKernelRunnable(
						...
						public void run() {
							ClientSessionImpl session = ClientSessionImpl
									.getSession(dataService, sessionRefId);
		...
		/** {@inheritDoc} */
		public void send(byte[] sessionId, byte[] message, byte deliveryOrdinal) {
			callStarted();
			try {
				if (logger.isLoggable(Level.FINEST)) {
				...
				SessionProtocol sessionProtocol = getSessionProtocol(new BigInteger(
						1, sessionId));
	...
	/**
	 * Returns the {@code ClientSessionHandler} for the specified client session
	 * ID.
	 * 
	 * @param sessionRefId
	 *            a client session ID
	...
	/**
	 * Returns the key for accessing the {@code ClientSessionServer} instance
	 * (which is wrapped in a {@code ManagedSerializable}) for the specified
	 * {@code nodeId}.
	 */
	 ...
	 * {@code IllegalStateException} if the node is no longer alive. This method
	...
	/**
	 * Checks if the number of logins has passed the high water mark, and if so,
	 * sets this service's health accordingly.
	 */
	...
	private void setHealth(Health newHealth) {
		...
		taskScheduler.scheduleTask(new AbstractKernelRunnable("ReportHealth") {
			public void run() {
	...
	/**
	 * Set the login high water. This call may cause the service's health to
	 * change.
	 * 
	 * @param highWater
	...
	/**
	 * Obtains information associated with the current transaction, throwing
	 * TransactionNotActiveException if there is no current transaction, and
	 * throwing IllegalStateException if there is a problem with the state of
	 * the transaction or if this service has not been initialized with a
	 * transaction proxy.
	 */
	 ...
	 * 
	 * @return the client session service relevant to the current context
	 */
	static synchronized ClientSessionServiceImpl getInstance() {
		if (txnProxy == null) {
			throw new IllegalStateException("Service not initialized");
		} else {
			return (ClientSessionServiceImpl) txnProxy
					.getService(ClientSessionService.class);
		}
	}

	/**
	 * Validates the {@code identity} of the user logging in and returns
	 * {@code true} if the login is allowed to proceed, and {@code false} if the
	 * login is denied.
	 * 
	 * <p>
	 * A user with the specified {@code identity} is allowed to log in if one of
	 * the following conditions holds:
	 * 
	 * <ul>
	 * <li>the {@code identity} is not currently logged in and is not relocating
	 * to the current node, or
	 * <li>the {@code identity} is logged in, and the
	 * {@code com.sun.sgs.impl.service.session.allow.new.login} property is set
	 * to {@code true}.
	 * </ul>
	 ...
	 * 
	 * <p>
	 * If this method returns {@code true}, the {@link #removeUserLogin} method
	 * must be invoked when the user with the specified {@code identity} is
	 * disconnected.
	 * 
	 * @param identity
	 *            the user identity
	 ...
	 * @param loggingIn
	 *            if {@code true} session with specified identity is loggingIn;
	 *            otherwise it is relocating
	 * @return {@code true} if the user is allowed to log in with the specified
	 *         {@code identity}, otherwise returns {@code false}
	 */
	boolean validateUserLogin(Identity identity, ClientSessionHandler handler,
		...
		ClientSessionHandler previousHandler = loggedInIdentityMap.putIfAbsent(
				identity, handler);
	...
	/**
	 * Notifies this service that the specified {@code identity} is no longer
	 * logged in using the specified {@code handler} so that internal
	 * bookkeeping can be adjusted accordingly.
	 * 
	 * @param identity
	 *            the user identity
	 * @param handler
	...
	/**
	 * Adds the handler for the specified session to the internal session
	 * handler map. This method is invoked by the handler once the client has
	 * successfully logged in or has successfully relocated. If the client has
	 * relocated, the {@code identity} should be non-null, otherwise, the
	 * identity should be {@code null}.
	 * 
	 * @param sessionRefId
	 *            the session ID, as a {@code BigInteger}
	 * @param handler
	 *            the client session handler to cache
	 * @param identity
	 *            if the session has been relocated, a non-null identity to be
	 *            removed from the {@code incomingRelocatingIdentities} cache
	 */
	void addHandler(BigInteger sessionRefId, ClientSessionHandler handler,
		...
		if (identity != null) {
			...
			for (ClientSessionStatusListener statusListener : sessionStatusListeners) {
				try {
					statusListener.relocated(sessionRefId);
				} catch (Exception e) {
					if (logger.isLoggable(Level.FINE)) {
						logger.logThrow(Level.FINE, e,
								"Invoking 'relocated' method "
										+ "on listener:{0} throws",
								statusListener);
					}
				}
			}
	 ...
	 * cleans up other session-related transient data, and if
	 * {@code isDisconnecting} is {@code true} notifies all
	 * {@link ClientSessionStatusListener}s of the session's disconnection. This
	 * method is invoked by the handler (in order to clean up the session's
	 * transient data structures) when the session is disconnecting its
	 * connection due to session termination or session relocation. If a session
	 * is disconnecting because the session relocating to another node, then
	 * {@code isDisconnecting} will be {@code false}.
	 */
	...
	/**
	 * Notifies all registered {@code ClientSessionStatusListener}s that the
	 * session with the specified {@code sessionRefId} has disconnected.
	 */
	private void notifyStatusListenersOfDisconnection(BigInteger sessionRefId,
		...
		for (ClientSessionStatusListener statusListener : sessionStatusListeners) {
			try {
				...
				if (logger.isLoggable(Level.WARNING)) {
					logger.logThrow(Level.WARNING, e,
							"notifying listener:{0} of "
									+ "disconnected session:{1} throws",
							statusListener, HexDumper.toHexString(sessionRefId));
				}
			}
		}
	}
	...
	/**
	 * Schedules a transactional task to service the event queue for the session
	 * with the specified {@code sessionId}. If there is no locally-connected
	 * session with the specified {@code sessionId}, then no action is taken.
	 * <p>
	 * 
	...
	void addServiceEventQueueTask(final byte[] sessionId) {
		...
		if (taskQueue == null) {
			TaskQueue newTaskQueue = transactionScheduler.createTaskQueue();
			taskQueue = sessionTaskQueues.putIfAbsent(sessionRefId,
					newTaskQueue);
			if (taskQueue == null) {
				taskQueue = newTaskQueue;
			}
		}
		taskQueue.addTask(new AbstractKernelRunnable("ServiceEventQueue") {
			public void run() {
	...
	/**
	 * Notifies each registered {@link ClientSessionStatusListener} that the
	 * session with the specified {@code sessionRefId} is moving to a new node
	 * (specified by {@code newNodeId}). The specified completion
	 * {@code handler} should be notified (via its {@code completed} method),
	 * when all listeners are finished preparing for relocation.
	 * 
	 * @param sessionRefId
	 *            the ID for the relocating client session
	 * @param newNodeId
	 *            the ID of the new node for the client session
	 */
		...
		if (info != null) {
			...
			logger.log(Level.WARNING,
					"Ignoring request for session:{0} whichi is not "
							+ "relocating from local node:{1}", sessionRefId,
					localNodeId);
		}
	}
	 ...
	 * failed {@code ClientSessionService}.
	 */
	private class ClientSessionServiceRecoveryListener implements
			RecoveryListener {
		/** {@inheritDoc} */
		public void recover(final Node node, SimpleCompletionHandler handler) {
			...
			try {
				...
				transactionScheduler.runTask(new AbstractKernelRunnable(
					...
					public void run() {
						/*
						 * For each session on the failed node, notify the
						 * session's ClientSessionListener and clean up the
						 * session's persistent data and bindings.
						 */
						taskService
								.scheduleTask(new HandleNextDisconnectedSessionTask(
										nodeId));

						/*
						 * Remove client session server proxy and associated
						 * binding for failed node, as well as protocol
						 * descriptors for the failed node.
						 */
						taskService
								.scheduleTask(new RemoveNodeSpecificDataTask(
										nodeId));
					}
				}, taskOwner);
				...
				logger.logThrow(Level.WARNING, e,
						"Node:{0} recovering for node:{1} throws", localNodeId,
						nodeId);
	...
	/**
	 * A persistent task to remove the client session server proxy and protocol
	 * descriptors for a failed node.
	 */
	private static class RemoveNodeSpecificDataTask implements Task,
			Serializable {
		...
		/**
		 * Removes the client session server proxy and binding and also removes
		 * the protocol descriptors for the node specified during construction.
		 */
		public void run() {
			String sessionServerKey = getClientSessionServerKey(nodeId);
	...
	/**
	 * Returns a set of protocol descriptors for the specified {@code nodeId},
	 * or {@code null} if there are no descriptors for the node. This method
	 * must be run outside a transaction.
	 */
	Set<ProtocolDescriptor> getProtocolDescriptors(long nodeId) {
		checkNonTransactionalContext();
		GetProtocolDescriptorsTask protocolDescriptorsTask = new GetProtocolDescriptorsTask(
				nodeId);
		try {
			...
			logger.logThrow(Level.WARNING, e,
					"GetProtocolDescriptorsTask for node:{0} throws", nodeId);
			return null;
		}
	}
	 ...
	 */
	private static class GetProtocolDescriptorsTask extends
	...
	private static Map<Long, Set<ProtocolDescriptor>> getProtocolDescriptorsMap() {
		...
		try {
			protocolDescriptorsMap = Objects.uncheckedCast(dataService
					.getServiceBinding(PROTOCOL_DESCRIPTORS_MAP_KEY));
		} catch (NameNotBoundException e) {
	...
	/**
	 * Contains information about a client session relocating to the local node.
	 */
	...
	/**
	 * A task (run after a delay) that checks to see if a client session with
	 * the {@code sessionRefId} specified during construction has relocated from
	 * the local node. If not, the associated client session is cleaned up.
	 */
	private class MonitorSessionRelocatingFromLocalNodeTask extends
			AbstractKernelRunnable {
		final BigInteger sessionRefId;
		...
		/**
		 * If the associated client hasn't finished preparing to relocate before
		 * this task runs, then clean up the client session. Either the original
		 * server failed to inform the client that it should relocate or the
		 * client session has failed. In either case, the client session needs
		 * to be removed.
		 */
		public void run() {
			...
			if (sessionHandler != null) {
				prepareRelocationMap.remove(sessionRefId);
				logger.log(Level.WARNING,
						"Disconnecting session:{0} that timed out "
								+ "relocating from node:{1}",
						sessionHandler.identity, localNodeId);
	...
	/**
	 * A task (run after a delay) that checks to see if a client session with
	 * the associated relocation key has finished relocating to this node. If
	 * not, the associated client session is cleaned up and all client session
	 * status listeners are notified that the client session has been
	 * disconnected.
	 */
	private class MonitorSessionRelocatingToLocalNodeTask extends
			AbstractKernelRunnable {
		 ...
		 *            a relocation key
		 * @param info
		 *            a client session's relocation info
		 */
		MonitorSessionRelocatingToLocalNodeTask(BigInteger relocationKey,
				RelocationInfo info) {
			super(null);
		...
		/**
		 * If the associated client doesn't attempt reestablish the client
		 * session before this task runs, then clean up the client session.
		 * Either the original server failed to inform the client that it should
		 * relocate or the client session has failed. In either case, the client
		 * session needs to be removed.
		 */
		public void run() {
			if (incomingSessionRelocationKeys.remove(relocationKey) != null) {
				logger.log(Level.FINE,
						"Scheduling clean up of session:{0} that "
								+ "failed to relocate to local node:{1}",
						info.identity, localNodeId);
				transactionScheduler.scheduleTask(new AbstractKernelRunnable(
						"RemoveNonrelocatedSession") {
					public void run() {
						ClientSessionImpl sessionImpl = ClientSessionImpl
								.getSession(dataService, info.sessionRefId);
						if (sessionImpl != null) {
	...
	/**
	 * An abstraction to keep track of the progress of session's relocation
	 * preparation and notify all NodeMappingService's completion handlers when
	 * relocation preparation is complete.
	 * 
	 * 
	 * An instance of this class is constructed when the ClientSessionService's
	 * IdentityRelocationListener is notified to prepare to relocate (via the
	 * {@link IdentityRelocationListener#prepareToRelocate prepareToRelocate}
	 * method.
	 */
	private final class PrepareRelocationInfo {
		...
		private final Set<SimpleCompletionHandler> nmsCompletionHandlers = new HashSet<SimpleCompletionHandler>();

		/**
		 * Completion handlers for {@code ClientSessionStatusListener}s that
		 * need to prepare before the node mapping service completion handler(s)
		 * are notified.
		 */
		private final Set<PrepareCompletionHandler> preparers = new HashSet<PrepareCompletionHandler>();

		/**
		 * Indicates whether the {@code ClientSessionStatusListener}s have been
		 * notified to prepare for relocation.
		 */
		 ...
		 * <p>
		 * 
		 * This constructor takes a snapshot of all registered
		 * {@code ClientSessionStatusListener}s that need to be notified to
		 * prepare to relocate after the client session service has finished
		 * preparing the client session for relocation. Once the client session
		 * has been prepared to relocate, the client session service invokes
		 * this instance's {@link #prepareToRelocate} method so that it can
		 * notify each {@code ClientSessionStatusListener} to prepare to
		 * relocate.
		 * <p>
		 * 
		 * When all {@code ClientSessionStatusListener}s have finished
		 * preparing, this instance notifies all node mapping service completion
		 * handlers that relocation preparation has been completed for the
		 * session. There may be more than one completion handler from the node
		 * mapping service because the node mapping service may notify the
		 * client session service more than once to prepare to relocate a given
		 * client session.
		 */
		...
		/**
		 * Notifies all {@code ClientSessionStatusListener}s to prepare for the
		 * client session (specified at construction) to relocate. If preparing
		 * the {@code ClientSessionStatusListener}s is or has already taken
		 * place, this method takes no action.
		 */
		synchronized void prepareToRelocate() {
			...
			for (final PrepareCompletionHandler handler : preparers) {
				taskScheduler.scheduleTask(new AbstractKernelRunnable(
						"PrepareToRelocateSession") {
					public void run() {
						try {
							handler.listener.prepareToRelocate(sessionRefId,
									newNodeId, handler);
						} catch (Exception e) {
							logger.logThrow(
									Level.WARNING,
		...
		/**
		 * Adds the specified completion {@code handler} from the
		 * {@code NodeMappingService} to the set of completion handlers that
		 * need to be notified when all {@code ClientSessionStatusListener}s are
		 ...
		 * for the specified {@code listenerCompletionhandler} has completed
		 * preparing for relocation. If all listeners have completed
		 * preparation, then all {@code NodeMappingService} completion handlers
		 * are notified that preparation is complete. Once the node mapping
		 * service has removed the mapping (and subsequently invoked this client
		 * session service's {@code NodeMappingListener.mappingRemoved} method),
		...
		synchronized void preparationCompleted(
			...
			if (preparers.isEmpty()) {
				...
				for (SimpleCompletionHandler nmsCompletionHandler : nmsCompletionHandlers) {
					if (logger.isLoggable(Level.FINEST)) {
						logger.log(
								Level.FINEST,
		 ...
		 * preparing for relocation, specified as an argument to the
		 * {@link ClientSessionStatusListener#prepareToRelocate
		 * prepareToRelocate} method.
		 */
		private final class PrepareCompletionHandler implements

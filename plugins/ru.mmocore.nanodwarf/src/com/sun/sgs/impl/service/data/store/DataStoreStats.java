...
class DataStoreStats implements DataStoreStatsMXBean {
	 ...
	 * 
	 * @param collector
	 *            the profile collector used to create profiling objects and
	...
	DataStoreStats(ProfileCollector collector) {
		...
		getObjectForUpdateOp = consumer.createOperation("getObjectForUpdate",
				type, level);
		...
		nextObjectIdOp = consumer
				.createOperation("nextObjectIdOp", type, level);

		// Counters
		readBytesCounter = consumer.createCounter("readBytes", type, level);
		readObjectsCounter = consumer.createCounter("readObjects", type, level);
		writtenBytesCounter = consumer.createCounter("writtenBytes", type,
				level);
		writtenObjectsCounter = consumer.createCounter("writtenObjects", type,
				level);

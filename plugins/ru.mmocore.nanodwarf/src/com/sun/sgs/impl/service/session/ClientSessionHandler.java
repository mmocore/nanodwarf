...
/**
 * Handles sending/receiving messages to/from a client session and disconnecting
 * a client session.
 */
class ClientSessionHandler implements SessionProtocolHandler {
	...
	private static final LoggerWrapper logger = new LoggerWrapper(
			Logger.getLogger("com.sun.sgs.impl.service.session.handler"));
	 ...
	 * {@code disconnectHandled}, {@code relocatePrepareCompletionHandler}, and
	 * {@code shutdown}.
	 */
	...
	/**
	 * If non-null, contains the completion handler for preparing this session
	 * to relocate to a new node.
	 */
	 ...
	 *            the ClientSessionService instance
	 * @param dataService
	 ...
	 *            a completion handler for the associated request
	 */
	ClientSessionHandler(ClientSessionServiceImpl sessionService,
			DataService dataService, SessionProtocol sessionProtocol,
			Identity identity,
	...
	/**
	 * Constructs an handler for a client session. If {@code sessionRefId} is
	 * non-{@code null}, then the associated client session is relocating from
	 * another node, otherwise it is considered a new client session logging in.
	 * 
	 * @param sessionService
	 *            the ClientSessionService instance
	 * @param dataService
	 ...
	 *            a completion handler for the associated request
	 * @param sessionRefId
	 *            the client session ID, or {@code null}
	 */
	ClientSessionHandler(ClientSessionServiceImpl sessionService,
			DataService dataService, SessionProtocol sessionProtocol,
			Identity identity,
	...
	public void sessionMessage(final ByteBuffer message,
		...
		RequestCompletionFuture future = new RequestCompletionFuture(
				completionHandler);
		if (!readyForRequests(future)) {
			return;
		}
		taskQueue.addTask(new AbstractKernelRunnable(
				"NotifyListenerMessageReceived") {
			public void run() {
				ClientSessionImpl sessionImpl = ClientSessionImpl.getSession(
						dataService, sessionRefId);
				if (sessionImpl != null) {
					if (isConnected()) {
						sessionImpl.getClientSessionListener(dataService)
								.receivedMessage(message.asReadOnlyBuffer());
					}
				} else {
				...
				if (sessionImpl != null) {
					if (isConnected()) {
						sessionService.getChannelService()
								.handleChannelMessage(channelId,
	...
	/**
	 * Returns {@code true} if this handler is connected, otherwise returns
	 * {@code false}.
	 * 
	...
	/**
	 * Returns {@code true} if this client session is disconnecting from the
	 * local node AND terminating the associated client session. The client
	 * session is considered to be disconnecting iff the following conditions
	 * are true:
	 * 
	 * 1) this handler has been marked for disconnection 2) the session is not
	 * relocating, OR, the session is relocating and its relocation has not
	 * completed.
	 */
	private boolean isTerminating() {
		synchronized (lock) {
			return !isConnected()
					&& (relocatePrepareCompletionHandler == null || !relocatePrepareCompletionHandler
							.isCompleted());
		}
	}
	 ...
	 * notifies the client that it should suspend messages (before notifying the
	 * client to relocate to another node).
	 */
	 ...
	 * otherwise, sets the appropriate {@code RequestFailureException} on the
	 * specified {@code future} and returns {@code false}.
	 * <p>
	 * 
	 * This method is invoked before proceeding with processing a request, and
	 * if this method returns {@code false}, the request should be dropped.
	 * 
	 * @param future
	 *            a future on which to set an exception if this session is not
	 *            ready to process requests
	 * @return {@code true} if requests can be processed by the associated
	 *         client session and {@code false} otherwise
	 */
	private boolean readyForRequests(RequestCompletionFuture future) {
		if (!loggedIn) {
			logger.log(Level.FINE,
					"request received before login completed:{0}", this);
			future.setException(new RequestFailureException(
	...
	/**
	 * Returns the protocol for the associated client session, or {@code null}
	 * if the session is relocating.
	 * 
	 ...
	 * send the appropriate success indication (either successful login or
	 * relocation) to the client, and sets local state indicating that the
	 * request has been handled and the client is logged in.
	 */
	 ...
	 * {@code exception} so that it can send the appropriate failure indication
	 * to the client, and sets local state indicating that request has been
	 * handled.
	 * 
	 * @param exception
	 *            the login failure exception
	 */
	 ...
	 * handled) by doing the following:
	 * <ol>
	 * 
	 * <li>notifies the client session service to clean up the client session's
	 * handler and login information,
	 * 
	 * <li>notifies the node mapping service to deativate the client's identity
	 * if the identity is no longer active on this node,
	 * 
	 * <li>if {@code closeConnection} is {@code true}, closes this session's
	 * connection,
	 * 
	 * <li>if the session is terminating (not relocating), schedules a
	 * transactional task to invoke, on this session's
	 * {@code ClientSessionListener}, the {@code disconnected} callback with
	 * {@code graceful} as its argument and then clean up the session's
	 * persistent data, and also schedules a task to notify the identity that
	 * its corresponding session has logged out.
	 * </ol>
	 * 
	 * <p>
	 * Note:if {@code graceful} is {@code true}, then {@code closeConnection}
	 * must be {@code false} so that the client's {@code SessionProtocol} can
	 * send a notification of logout success to the client. The client may not
	 * receive such a notification if the connection is disconnected
	 * immediately.
	 * 
	 * <p>
	 * In the cases of login redirection, session relocation, and graceful
	 * logout, it is the responsibility of the client's {@code SessionProtocol}
	 * to close the client's connection in a timely manner after notifying the
	 * client.
	 * 
	 * @param graceful
	 *            if {@code true}, indicates that disconnection is due to a
	 *            (graceful) logout request
	 * @param closeConnection
	 *            if {@code true}, close this session's connection immediately
	 */
		...
		if (sessionRefId != null && isTerminating()) {
			scheduleTask(new AbstractKernelRunnable(
				...
				public void run() {
					...
					sessionImpl.notifyListenerAndRemoveSession(dataService,
							graceful, true);
				}
			});
	...
	/**
	 * Schedules a non-transactional task to handle disconnecting the associated
	 * client session.
	 * 
	 * @param graceful
	 *            if {@code true}, indicates that disconnection is due to a
	...
	/**
	 * Schedules a task to notify the completion handler. Use this method to
	 * delay notification until a task resulting from an earlier request has
	 * been completed.
	 * 
	 * @param completionHandler
	...
	/**
	 * Invokes the {@code setStatus} method on the node mapping service with
	 * {@code false} to mark the identity as inactive. This method is invoked
	 * when a login is redirected and also when this client session is
	 * disconnected.
	 */
	private void deactivateIdentity() {
		try {
			 ...
			 */
			sessionService.nodeMapService.setStatus(ClientSessionHandler.class,
					identity, false);
		} catch (Exception e) {
			logger.logThrow(Level.WARNING, e,
	...
	/**
	 * If {@code loggingIn} is {@code true} handles a login request to establish
	 * a client session); otherwise handles a relocate request to re-establish a
	 * client session. In either case, this method notifies the completion
	 * handler specified during construction when the request is completed.
	 */
	private void setupClientSession(final boolean loggingIn) {
		...
		if (assignedNodeId < 0) {
			logger.log(Level.WARNING,
					"getting node assignment for identity:{0} failed", identity);
			notifySetupFailureAndDisconnect(loggingIn ? new LoginFailureException(
		...
		if (assignedNodeId == sessionService.getLocalNodeId()) {
			 ...
			 */
			if (!sessionService.validateUserLogin(identity,
					ClientSessionHandler.this, loggingIn)) {
				...
				notifySetupFailureAndDisconnect(loggingIn ? new LoginFailureException(
						...
						: new RelocateFailureException(
								RELOCATE_REFUSED_REASON,
								RelocateFailureException.FailureReason.DUPLICATE_LOGIN));
				return;
			}
			...
			if (loggingIn) {
				...
				try {
					...
					logger.logThrow(Level.WARNING, e,
							...
							identity);
					notifySetupFailureAndDisconnect(new LoginFailureException(
							LOGIN_REFUSED_REASON, e));
					return;
				}
			 ...
			 */
			sessionService.addHandler(sessionRefId, ClientSessionHandler.this,
				...
				try {
					...
					logger.logThrow(Level.WARNING, e,
							"Relocating ClientSession for identity:{0} "
									+ "to local node:{1} throws", identity,
							sessionService.getLocalNodeId());
					notifySetupFailureAndDisconnect(new RelocateFailureException(
			 ...
			 */
			if (logger.isLoggable(Level.FINE)) {
				logger.log(Level.FINE, "redirecting login for identity:{0} "
						+ "from nodeId:{1} to node:{2}", identity,
						sessionService.getLocalNodeId(), assignedNodeId);
			}

			final long nodeId = assignedNodeId;
			scheduleNonTransactionalTask(new AbstractKernelRunnable(
					"SendLoginRedirectMessage") {
				public void run() {
					try {
						try {
							loginRedirect(nodeId);
						} catch (Exception ex) {
							setupFailure(new LoginFailureException(
									"Redirect failed", ex));
						}
					} finally {
	 ...
	 * specified {@code nodeId}, and sets local state indicating that the login
	...
	private void loginRedirect(long nodeId) {
		synchronized (lock) {
			checkConnectedState();
			Set<ProtocolDescriptor> descriptors = sessionService
					.getProtocolDescriptors(nodeId);
			setupCompletionFuture.setException(new LoginRedirectException(
					nodeId, descriptors));
			state = State.LOGIN_HANDLED;
		}
	}
	...
	/**
	 * Throws an {@code IllegalStateException} if the associated client session
	 * handler is not in the {@code CONNECTED} state.
	 */
	 ...
	 * 
	 * @param exception
	 *            an exception that occurred while setting up the client
	 *            session, or {@code null}
	 */
	...
	private class CreateClientSessionTask extends AbstractKernelRunnable {
		...
		/** {@inheritDoc} */
		public void run() {
			ClientSessionImpl sessionImpl = new ClientSessionImpl(
					sessionService, identity, protocol.getDeliveries(),
	...
	private class LoginTask extends AbstractKernelRunnable {
		...
		private AppListener getAppListener() {
			ManagedObject obj = dataService
					.getServiceBinding(StandardProperties.APP_LISTENER);
			return (obj instanceof AppListener) ? (AppListener) obj
					: ((ManagedSerializable<AppListener>) obj).get();
		}

		/**
		 * Invokes the {@code AppListener}'s {@code loggedIn} callback, which
		 * returns a client session listener. If the returned listener is
		 * serializable, then this method does the following:
		 * 
		 * a) queues the appropriate acknowledgment to be sent when this
		 * transaction commits, and b) schedules a task (on transaction commit)
		 * to call {@code notifyLoggedIn} on the identity.
		 * 
		 * If the client session needs to be disconnected (if {@code loggedIn}
		 * returns a non-serializable listener (including {@code null}), or
		 * throws a non-retryable {@code RuntimeException}, then this method
		 * submits a non-transactional task to disconnect the client session. If
		 * {@code loggedIn} throws a retryable {@code RuntimeException}, then
		 * that exception is thrown to the caller.
		 */
		public void run() {
			...
			if (returnedListener instanceof Serializable) {
				logger.log(Level.FINEST, "AppListener.loggedIn returned {0}",
						returnedListener);

				sessionImpl.putClientSessionListener(dataService,
						returnedListener);

				sessionService.checkContext().addCommitAction(sessionRefId,
				...
				if (ex == null) {
					...
					logger.logThrow(Level.WARNING, ex,
							"Invoking loggedIn on AppListener:{0} with "
									+ "session: {1} throws", appListener,
							ClientSessionHandler.this);
					loginFailureEx = new LoginFailureException(
							LOGIN_REFUSED_REASON, ex);
				} else {
					throw ex;
				}
				sessionService.checkContext().addCommitAction(sessionRefId,
						new LoginResultAction(false, loginFailureEx), true);
	 ...
	 */
	private static class RequestCompletionFuture extends
			AbstractCompletionFuture<Void> {
		/**
		 ...
		 *            a completionHandler
		 */
		RequestCompletionFuture(RequestCompletionHandler<Void> completionHandler) {
	 ...
	 * passed to one of the {@code ProtocolListener}'s methods: {@code newLogin}
	 * or {@code relocatedSession}.
	 */
	static class SetupCompletionFuture extends
			AbstractCompletionFuture<SessionProtocolHandler> {
	...
	private class LoginResultAction implements Action {
		 ...
		 * client {@code session} can be notified when this context commits. If
		 * {@code success} is {@code false}, the specified {@code exception}
		 * will be used as the cause of the {@code ExecutionException} in the
		 * {@code Future} passed to the {@link RequestCompletionHandler} for the
		 * login request, and no subsequent session messages will be forwarded
		 * to the session, even if they have been enqueued during the current
		 * transaction. If success is {@code true}, then the {@code Future}
		 * passed to the {@code RequestCompletionHandler} for the login request
		 * will contain this {@link SessionProtocolHandler}.
		 * 
		 * <p>
		 * When the transaction commits, the session's associated
		 * {@code ClientSessionHandler} is notified of the login result, and if
		 * {@code success} is {@code true}, all enqueued messages will be
		 * delivered to the client session.
		 * 
		 * @param success
		 *            if {@code true}, login was successful
		 * @param ex
		 *            a login failure exception, or {@code null} (only valid if
	 ...
	 * associated session's {@code ClientSessionImpl} when processing a request
	 * to send a message to the client.
	 * <p>
	 * 
	 * When this action is executed, it notifies the session's
	 * {@link SessionProtocol} to send the message obtained from the
	...
	class SendMessageAction implements Action {
		 ...
		 * 
		 * @param sendEvent
		 *            a send event containing a message and delivery guarantee
		 */
		...
		public boolean flush() {
			...
			try {
				protocol.sessionMessage(ByteBuffer.wrap(sendEvent.message),
						sendEvent.delivery);
			} catch (Exception e) {
	...
	/**
	 * An action to start the process of moving the associated client session
	 * from this node to another node (the {@code newNode} specified during
	 * construction). This commit action is created by the associated session's
	 * {@code ClientSessionImpl} when processing a request to move the client to
	 * a new node.
	 * <p>
	 * 
	 * When this action is executed, it sends a
	 * {@link ClientSessionServer#relocatingSession relocatingSession}
	 * notification to the new node's {@code ClientSessionServer} to obtain a
	 * relocation key for the client session. Once the relocation key is
	 * obtained, it notifies the client session service to notify all registered
	 * {@link ClientSessionStatusListener}s (i.e., the {@code ChannelService})
	 * to prepare to relocate the client session.
	 * <p>
	 * 
	 * When all {@code ClientSessionStatusListener}s have completed preparing
	 * the client session to relocate, this instance's
	 * {@link MoveAction#suspend suspend} method is invoked which notifies the
	 * associated session's {@code SessionProtocol} to suspend sending messages
	 * to the server.
	 * <p>
	 * 
	 * When the suspend operation's {@code SuspendCompletionHandler} is notified
	 * as {@code completed}, that completion handler notifies this action's
	 * {@code completed} method, which, in turn notifies the associated
	 * session's {@link SessionProtocol} to {@code relocate} the client's
	 * connection, supplying the new node's information and the relocation key.
	 */
	class MoveAction implements Action, SimpleCompletionHandler {
		 ...
		 * 
		 * @param newNode
		 *            the new node for the session
		 */
		MoveAction(Node newNode) {
			this.newNode = newNode;
			this.server = sessionService
					.getClientSessionServer(newNode.getId());
		}
		...
		public boolean flush() {
			...
			try {
				 ...
				 */
				key = server.relocatingSession(identity,
			...
			/*
			 * Notify client to relocate its session to the new node specifying
			 * the relocation key.
			 */
			synchronized (lock) {
			...
			sessionService.notifyPrepareToRelocate(sessionRefId,
					newNode.getId());
		 ...
		 * notification. This method is invoked after the session has been
		 * prepared for relocation. When message suspension is complete, this
		 * instance's {@code completed} method is invoked to notify the client
		 * to relocate.
		 */
		public void suspend() {
			try {
				if (!supportsRelocation()) {
					logger.log(
							...
							"Disconnecting a non-relocatable session:{0} "
									+ "that was erroneously prepared to relocate",
							identity);
					handleDisconnect(false, true);
					return;
				}
				((SessionRelocationProtocol) protocol)
						.suspend(new SuspendCompletionHandler());
		 ...
		 * relocation and messages have been suspended from the client (i.e.,
		 * invoked from the {@code SuspendCompletionHandler.completed} method).
		 */
		public void completed() {
			...
			final Set<ProtocolDescriptor> descriptors = sessionService
	...
	/**
	 * An action to disconnect the client session. This commit action is created
	 * by the associated session's {@code ClientSessionImpl} when processing a
	 * request to disconnect the client session.
	 */
	...
	/**
	 * A completion handler for notifying the client to suspend messages. When
	 * messages suspension is completed, this handler notifies
	 * {@code MoveAction} that suspension relocation preparation is complete so
	 * that it can send a 'relocate' notification to the client.
	 */
	private class SuspendCompletionHandler implements
			RequestCompletionHandler<Void> {
		private boolean isCompleted = false;
	 ...
	 */
	private class RelocateCompletionHandler implements
			RequestCompletionHandler<Void> {
		private boolean isCompleted = false;

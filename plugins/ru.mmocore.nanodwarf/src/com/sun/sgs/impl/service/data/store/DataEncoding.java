...
public final class DataEncoding {
	 ...
	 *             if {@code bytes} is less than two bytes in length
	 * @throws IllegalArgumentException
	 *             if {@code bytes} is more than two bytes in length
	 */
	 ...
	 *            the number to encode
	 * @param bytes
	 *            the array in which to store the encoded bytes
	 * @param offset
	 *            the array index at which to store the first byte
	 * @throws IndexOutOfBoundsException
	 *             if {@code bytes} is not long enough to contain the encoded
	 *             bytes
	 */
	 ...
	 * @return the decoded number
	 * @throws IndexOutOfBoundsException
	 *             if {@code bytes} is less than four bytes in length
	 * @throws IllegalArgumentException
	 *             if {@code bytes} is more than four bytes in length
	 */
	 ...
	 *            the byte array to decode
	 * @param offset
	 *            the array index of the first byte to decode
	 * @return the decoded number
	 * @throws IndexOutOfBoundsException
	 *             if {@code bytes} is not long enough to contain the encoded
	 *             bytes
	...
	/**
	 * Encodes a {@code long} into an eight byte array. The higher order bits of
	 * the value are stored in the lower numbered bytes. The high order bit of
	 * the first byte is inverted to insure that negative values sort first.
	 * 
	 * @param n
	...
	/**
	 * Decodes an eight byte array into a {@code long}. The higher order bits of
	 * the value are read from the lower numbered bytes. The high order bit of
	 * the first byte is inverted.
	 * 
	 * @param bytes
	 ...
	 * @return the decoded number
	 * @throws IndexOutOfBoundsException
	 *             if {@code bytes} is less than eight bytes in length
	 * @throws IllegalArgumentException
	 *             if {@code bytes} is more than eight bytes in length
	 */
	...
	/**
	 * Encodes a {@code String} into an array of bytes encoded in modified UTF-8
	 * format, as documented by {@link DataInput}, but with the two byte size
	 * value omitted from the start of the encoding, and with a terminating null
	 * byte. Removing the size from the start of the encoding means that the
	 * strings will sort properly when used as keys, and adding a null
	 * termination removes the need to depend on the length of the array to find
	 * the end of the data, which turns out to be useful for Berkeley DB.
	 * 
	 * @param string
	 *            the string to encode
	 * @return the encoded byte array
	 * @throws IllegalArgumentException
	 *             if the UTF-8 encoding of the string contains more than
	 *             {@code 65535} characters
	 */
	 ...
	 * @throws IllegalArgumentException
	 *             if the format of the bytes is not valid UTF-8 data, if it is
	 *             not null terminated, or if it represents more than
	 *             {@code 65535} bytes
	 */
	public static String decodeString(byte[] bytes) {
		...
		try {
			...
			throw new IllegalArgumentException("Problem decoding string: "
					+ e.getMessage(), e);
		}
	}
}

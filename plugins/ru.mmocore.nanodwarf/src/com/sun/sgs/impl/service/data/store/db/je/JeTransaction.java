...
class JeTransaction implements DbTransaction {
	 ...
	 *            to run
	 * @param txnConfig
	 *            the Berkeley DB transaction configuration, or {@code null} for
	 *            the default
	 ...
	 */
	JeTransaction(XAEnvironment env, long timeout, TransactionConfig txnConfig) {
		this.env = env;
		if (timeout <= 0) {
		...
		try {
			...
			txn.setLockTimeout(timeoutMicros, TimeUnit.MICROSECONDS);

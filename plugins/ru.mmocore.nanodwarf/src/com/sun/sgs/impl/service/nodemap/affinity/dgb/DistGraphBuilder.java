...
/**
 * The portion of the distributed affinity graph builder which resides on a
 * local node. This code forwards graph information to its server, which builds
 * a single large graph for all information in the system.
 * <p>
 ...
 * 
 * <dd style="padding-top: .5em">The amount of time, in milliseconds, for each
 * snapshot of retained data. Older snapshots are discarded as time goes on. A
 * longer snapshot period gives us more history, but also longer compute times
 * to use that history, as more data must be processed.
 * <p>
 * 
 * <dt><i>Property:</i> <code><b>
 ...
 * <i>Default:</i> {@code 1}
 * 
 * <dd style="padding-top: .5em">The number of snapshots to retain. A larger
 * value means more history will be retained. Using a smaller snapshot period
 * with a larger count means more total history will be retained, with a smaller
 * amount discarded at the start of each new snapshot.
 * <p>
 * </dl>
 */
public class DistGraphBuilder extends AbstractAffinityGraphBuilder implements
		AffinityGraphBuilder {
	/** The property name for the server host. */
	private static final String SERVER_HOST_PROPERTY = PROP_BASE
			+ ".server.host";
	...
	public DistGraphBuilder(Properties properties,
		...
		if (nodeType == NodeType.coreServerNode) {
			serverImpl = new DistGraphBuilderServerImpl(systemRegistry,
					txnProxy, properties, localNodeId);
			server = null;
		} else {
			...
			server = (DistGraphBuilderServer) registry
					.lookup(DistGraphBuilderServerImpl.SERVER_EXPORT_NAME);
			serverImpl = null;
		}
	}
	 ...
	 * {@code UnsupportedOperationException}, because the graph is not held on
	 * the local node.
	 */
	 ...
	 * and we use the watchdog's non-transactional call to find out if the local
	 * node is alive. We cannot use the AbstractService version because we are
	 * not an AbstractService. It may be useful to refactor that method into a
		...
		do {
			try {
				...
				if (maxAttempts-- == 0) {
					...
					watchdogService.reportFailure(nodeId, this.getClass()
							.toString());

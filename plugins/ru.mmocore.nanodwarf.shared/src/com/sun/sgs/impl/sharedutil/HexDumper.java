...
public final class HexDumper {
	 ...
	 * converted to hex format. The entire string is enclosed in square
	 * brackets, and the octets are separated by a single space character.
	 * 
	 * @param bytes
	 *            a byte array to format
	 ...
	 *            a byte array to format
	 * @param limit
	 *            the maximum number of bytes to format, or {@code 0} meaning
	 *            unlimited
	 * @return the contents of the byte array as a hex-formatted string
	 */
	...
	public static String format(ByteBuffer buf, int limit) {
		...
		StringBuilder s = new StringBuilder((3 * readBuf.remaining())
				+ (truncate ? 3 : 0) + 1);
		s.append('[');
		// First element
	 ...
	 *             if the {@code String} does not contain a parsable series of
	 *             hex-formatted values {@code int}
	 */

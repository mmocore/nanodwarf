 ...
 * URL and periodically rereads the configuration if it has changed.
 * <p>
 * 
 ...
 * <i>Default:</i> {@code file://localhost/$ java.home}/lib/logging.properties}
 * <dd style="padding-top: .5em">
 * Specifies the URL for the logging configuration. The URL is parsed using the
 * context {@code file://localhost/}, so the value will be parsed correctly as a
 * file on the local host if specified with forward slashes.
 * <p>
 * 
 * <dt><i>Property:</i> <b>{@value #RESAMPLE_INTERVAL_PROPERTY}</b> <br>
 * <i>Default:</i> {@value #RESAMPLE_INTERVAL_DEFAULT}
 * <dd style="padding-top: .5em">
 * Specifies the interval in milliseconds between times that this class checks
 * if the contents of the logging configuration URL have changed.
 * <p>
 * 
 * </dl>
 * <p>
 ...
 * the {@code config} logging property, or as the value of the
 * {@code java.util.logging.config.class} system property, as documented by the
 * {@link LogManager} class. You need to specify the system property if you want
 * to use a URL for the logging configuration file.
 * <p>
 * 
 ...
 * {@code com.sun.sgs.impl.sharedutil.logging.ResamplingUrlLogConfiguration} to
...
public class ResamplingUrlLogConfiguration {
	...
	private static final Logger logger = Logger
			.getLogger(ResamplingUrlLogConfiguration.class.getName());
	...
	private static final String CONFIG_FILE_DEFAULT = System
			.getProperty("java.home") + "/lib/logging.properties";

	/** The property that specifies the resample interval. */
	public static final String RESAMPLE_INTERVAL_PROPERTY = "com.sun.sgs.impl.sharedutil.logging.ResamplingUrlLogConfiguration"
			+ ".interval";
	...
	/**
	 * Runs a thread that checks the configuration and rereads it if it changes.
	 */
	...
	private static void updateResampleInterval() {
		String resampleIntervalProperty = LogManager.getLogManager()
				.getProperty(RESAMPLE_INTERVAL_PROPERTY);
		if (resampleIntervalProperty != null) {
			try {
				resampleInterval = Long.parseLong(resampleIntervalProperty);
			} catch (NumberFormatException e) {
				printException("Problem parsing the "

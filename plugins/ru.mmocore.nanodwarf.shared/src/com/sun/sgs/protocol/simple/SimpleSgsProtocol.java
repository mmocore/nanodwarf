 ...
 * <p>
 * Behavior not specified in this document is left as an implementation decision
 * for the particular client and server. Information on the implementation
 * characteristics of the RedDwarf server can be found in the <a
 * href="../../../../../overview-summary.html"> overview</a> for the
 * implementation.
 */
public class SimpleSgsProtocol {
	...
	/**
	 * Suspend messages notification. Server to client notification. <br>
	 * Opcode: {@code 0x14} <br>
	 * Payload: (none)
	 * <p>
	 * 
	 * This message notifies a client to suspend sending messages to the server
	 * until it receives further instruction (such as
	 * {@link #RELOCATE_NOTIFICATION} or {@link #RESUME_MESSAGES}). The client
	 * should send the acknowledgment {@link #SUSPEND_MESSAGES_COMPLETE} to the
	 * server when it has suspended sending messages. After the server sends a
	 * {@code SUSPEND_MESSAGES} notification to the client, the server may
	 * decide to drop messages from the client if it does not receive the
	 * {@link #SUSPEND_MESSAGES_COMPLETE} acknowledgment in a timely fashion.
	 * <p>
	 * 
	 ...
	 * server after this notification will be dropped, unless the server has
	 * instructed the client to either resume messages or relocate its client
	...
	/**
	 * Resume messages notification. Server to client notification. <br>
	 * Opcode: {@code 0x16} <br>
	 * Payload: (none)
	 * <p>
	 * 
	 * This message notifies the client that it can resume sending messages to
	 * the server.
	 * <p>
	 * 
	 * This opcode was introduced in protocol version {@code 0x05}.
	...
	/**
	 * Relocate session notification. Server to client notification. <br>
	 * Opcode: {@code 0x17} <br>
	 * Payload:
	 * <ul>
	 * <li>(String) hostname
	 ...
	 * </ul>
	 * 
	 * This message notifies a client to relocate its session on the current
	 * node to a new node. The client receiving this request should shut down
	 * the connection to the original node and establish a connection to the
	 * node indicated by the {@code hostname} and {@code port} in the payload.
	 * The client should then attempt to reestablish the client session with the
	 * server (without logging in) using the {@code relocationKey} specified in
	 * the payload.
	 * <p>
	 * 
	 * This opcode was introduced in protocol version {@code 0x05}.
	 ...
	 * relocated to (and re-established with) the server. The
	 * {@code relocationKey} must match the one that the client received in the
	 * previous {@link #RELOCATE_NOTIFICATION} message. If relocation is
	 * successful, the server acknowledges the request with a
	 * {@link #RELOCATE_SUCCESS} message containing a {@code reconnectionKey}
	 * for reconnecting to the server. If relocation is not successful, a
	 * {@link #RELOCATE_FAILURE} message is sent to the client. If the client
	 * receives a {@code RELOCATE_FAILURE} message, the client should disconnect
	 * from the server.
	 * <p>
	 * 
	 * This opcode was introduced in protocol version {@code 0x05}.
	...
	/**
	 * Relocation success. Server response to a client's
	 * {@link #RELOCATE_REQUEST}. <br>
	 * Opcode: {@code 0x19} <br>
	 * Payload:
	 * <ul>
	 * <li>(ByteArray) reconnectionKey
	...
	/**
	 * Relocate failure. Server response to a client's {@link #RELOCATE_REQUEST}
	 * . <br>
	 * Opcode: {@code 0x1a} <br>
	 * Payload:
	 * <ul>
	 ...
	 * {@link #RELOCATE_REQUEST} for some reason, for example
	 * <ul>
	 ...
	 * client has been removed from the channel with the indicated
	 * {@code  channel ID}. The client can no longer send messages on the
	 * channel.
	 */
	 ...
	 * is {@value #MAX_PAYLOAD_LENGTH} bytes minus the sum of the
	 * {@code  channel ID} size and two bytes (the size of the unsigned short
	 * indicating the {@code channel Id} size). Larger messages require
	 * fragmentation and reassembly above this protocol layer. <br>
	 * Opcode: {@code 0x52} <br>
	 * Payload:
	 * <ul>

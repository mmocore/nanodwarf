...
public abstract class AbstractDummyClient extends Assert {
	...
	/**
	 * Connects this client to the given {@code port} and returns this instance.
	 */
	public AbstractDummyClient connect(int port) {
		...
		try {
			SocketEndpoint endpoint = new SocketEndpoint(new InetSocketAddress(
					InetAddress.getLocalHost(), port), TransportType.RELIABLE);
			...
			try {
				...
				if (connected != true) {
					throw new RuntimeException(toString()
							+ " connect timed out to " + port);
				}
			} catch (InterruptedException e) {
				throw new RuntimeException(toString()
						+ " connect timed out to " + port, e);
			}
		}
		return this;
	}
	...
	/**
	 * Returns the port last specified for the {@link #connect connect} method.
	 */
	...
	/**
	 * Throws a {@code RuntimeException} if this session is not logged in.
	 */
	protected void checkLoggedIn() {
		synchronized (lock) {
			if (!(connected && (loginSuccess || relocateSuccess))) {
				throw new RuntimeException(toString()
						+ " not connected or loggedIn");
			}
		}
	}

	/**
	 * Sends a login request and waits for it to be acknowledged, returning
	 * {@code true} if login was successful, and {@code false} if login was
	 * redirected. If the login was not successful or redirected, then a
	 * {@code RuntimeException} is thrown because the login operation timed out
	...
	/**
	 * Sends a login request and if {@code waitForLogin} is {@code true} waits
	 * for the request to be acknowledged, returning {@code true} if login was
	 * successful, and {@code false} if login was redirected, otherwise a
	 * {@code RuntimeException} is thrown because the login operation timed out
	 * before being acknowledged.
	 * 
	 * If {@code waitForLogin} is false, this method returns {@code true} if the
	 * login is known to be successful (the outcome may not yet be known because
	 * the login operation is asynchronous), otherwise it returns false. Invoke
	 * {@code waitForLogin} to wait for an expected successful login.
	 */
	public boolean login(boolean waitForLogin) {
		...
		MessageBuffer buf = new MessageBuffer(2 + MessageBuffer.getSize(name)
				+ MessageBuffer.getSize(password));
		buf.putByte(SimpleSgsProtocol.LOGIN_REQUEST).putByte(protocolVersion)
				.putString(name).putString(password);
		loginAck = false;
		try {
	...
	/**
	 * Waits for a login acknowledgement, and returns {@code true} if login was
	 * successful, {@code false} if login was redirected or failed, otherwise a
	 * {@code RuntimeException} is thrown because the login operation timed out
	 * before being acknowledged.
	 */
	...
	/**
	 * Waits for this client to receive a RELOCATE_NOTIFICATION message. Throws
	 * {@code AssertionFailedError} if the notification is not received before
	 * the timeout period, or if {@code expectedPort} is non-zero and does not
	 * match the relocation port specified in the received
	...
	/**
	 * Relocates this client's connection, if the server has instructed it to do
	 * so via a RELOCATE_NOTIFICATION.
	 * <p>
	 * 
	 ...
	 * first. If a RELOCATE_NOTIFICATION is not received or if the specified
	 * {@code expectedPort} is non-zero and does not match the relocation port,
	 * then {@code AssertionFailedError} is thrown.
	 * <p>
	 * 
	 * If a RELOCATE_NOTIFICATION is correctly received, then this method sends
	 * a RELOCATE_REQUEST message to the local host on the relocation port
	 * received by a RELOCATE_NOTIFICATION. If {@code useValidKey} is
	 * {@code true}, the current valid relocation key is used in the relocate
	 * request, otherwise an invalid relocation key is used.
	 * <p>
	 * 
	 ...
	 * RELOCATE_FAILURE is received, this method throws
	 * {@code AssertionFailedError}; similarly if {@code shouldSucceed} is
	 * {@code false} and a RELOCATE_SUCCESS is received,
	 * {@code AssertionFailedError} will be thrown.
	 */
	public void relocate(int expectedPort, boolean useValidKey,
		...
		buf.put(SimpleSgsProtocol.RELOCATE_REQUEST)
				.put(SimpleSgsProtocol.VERSION).put(key).flip();
		try {
	...
	/**
	 * Sends a SESSION_MESSAGE to the server with the specified content. If
	 * {@code checkSuspend} is {@code true}, then, if the subclass supports
	 * suspending messages, it should check to see if messages are suspended
	 * before forwarding the message to the server. The method default
	 * implementation of {@link #sendRaw} will check if messages are suspended.
	 */
	 ...
	 * If {@code checkSuspended} is {@code true}, and messages sending is
	 * currently suspended, then throw an {@code IllegalStateException}.
	 */
	...
	/**
	 * If this session is not connected, this method returns; otherwise this
	 * method sends a LOGOUT_REQUEST to the server, and waits for this client to
	 * receive a LOGOUT_SUCCESS acknowledgment or the timeout to expire,
	 * whichever comes first. If the LOGOUT_SUCCESS acknowledgment is not
	 * received, then {@code AssertionFailedError} is thrown.
	 */
	public void logout() {
		synchronized (lock) {
			if (connected == false) {
	...
	/**
	 * Disconnects this client, and returns either when the connection is closed
	 * or the timeout expires, which ever comes first.
	 */
	...
	/**
	 * Resets the connection state so that the client can connect and login
	 * again.
	 */
	private void reset() {
	...
	/**
	 * Waits for the connection to close or the timeout to expire, whichever
	 * comes first, and returns {@code true} if the underlying connection
	 * disconnected, and {@code false} otherwise.
	 */
	...
	/**
	 * Sets a flag that indicates this client should wait for a SUSPEND_MESSAGES
	 * notification, instead of replying with a SUSPEND_MESSAGES_COMPLETE ack to
	 * the server. To wait for the SUSPEND_MESSAGES notification, invoke the
	 * {@link #waitForSuspendMessages} method.
	 */
	...
	/**
	 * Waits for a SUSPEND_MESSAGES notification to be sent to this client. A
	 * previous call to {@link #setWaitForSuspendMessages} must be invoked
	 * before the server sends a SUSPEND_MESSAGES notification in order for this
	 * wait to be successful. This method returns when a SUSPEND_MESSAGES
	 * notification has been received; it does NOT allow a
	 * SUSPEND_MESSAGES_COMPLETE ack to be sent. To send the
	 * SUSPEND_MESSAGES_COMPLETE ack, invoke the
	 * {@link #sendSuspendMessagesComplete} method.
	 */
	public void waitForSuspendMessages() {
		...
		synchronized (lock) {
			...
			if (!suspendMessages) {
				fail(toString() + " time out waiting for SUSPEND_MESSAGES");
			}
		}
	}
	 ...
	 * implementation handles login, redirection, relocation, and logout but
	 * does not handle session or channel messages.
	 */
	protected void handleOpCode(byte opcode, MessageBuffer buf) {
		...
		case SimpleSgsProtocol.LOGIN_FAILURE:
			...
			synchronized (lock) {
				loginAck = true;
				loginSuccess = false;
				System.err.println("login failed: " + name + ", reason:"
						+ failureReason);
				lock.notifyAll();
			}
			break;
		...
		case SimpleSgsProtocol.LOGIN_REDIRECT:
			...
			synchronized (lock) {
				loginAck = true;
				loginRedirect = true;
				System.err.println("login redirected: " + name + ", host:"
						+ redirectHost + ", port:" + redirectPort);
				lock.notifyAll();
			}
			break;
		...
		case SimpleSgsProtocol.RELOCATE_NOTIFICATION:
			...
			synchronized (lock) {
				...
				System.err.println("session to relocate: " + name + ", host:"
						+ relocateHost + ", port:" + relocatePort + ", key:"
		...
		case SimpleSgsProtocol.RELOCATE_FAILURE:
			...
			synchronized (lock) {
				relocateAck = true;
				relocateSuccess = false;
				System.err.println("relocate failed: " + name + ", reason:"
						+ relocateFailureReason);
				lock.notifyAll();
			}
			break;
	...
	/**
	 * Throws an {@code AssertionFailedError} if this client's protocol version
	 * does not support suspend or relocate.
	 */
	public void checkRelocateProtocolVersion() {
		if (protocolVersion < RELOCATE_PROTOCOL_VERSION) {
			String badVersion = toString() + " Protocol version: "
					+ protocolVersion + " does not support suspend or relocate";
	...
	private class Listener implements ConnectionListener {
		...
		public void bytesReceived(Connection conn, byte[] buffer) {
			if (connection != conn) {
				System.err.println(toString()
						...
						+ " wrong handle, got:" + conn + ", expected:"
						+ connection);
				return;
			}
		...
		public void connected(Connection conn) {
			System.err.println(AbstractDummyClient.this.toString()
					+ " connected to port:" + connectPort);
			if (connection != null) {
				System.err
						.println("DummyClient.Listener.already connected handle: "
								+ connection);
				return;
			}
			connection = conn;

...
public class IdentityAssigner {
	...
	public IdentityAssigner(SgsTestNode serverNode) throws Exception {
		this.nodemapServer = serverNode.getNodeMappingServer();
		this.watchdogService = serverNode.getWatchdogService();
		this.txnScheduler = serverNode.getSystemRegistry().getComponent(
	...
	/**
	 * Assigns the identity with the specified {@code name} from {@code oldNode}
	 * to {@code newNode}.
	 */
	public void moveIdentity(String name, final long oldNodeId, long newNodeId)
			throws Exception {
		Node oldNode = KernelCallable.call(new KernelCallable<Node>("getNode") {
			public Node call() {
				return watchdogService.getNode(oldNodeId);
			}
		}, txnScheduler, taskOwner);
		mapToNewNode.invoke(nodemapServer, new Object[] {

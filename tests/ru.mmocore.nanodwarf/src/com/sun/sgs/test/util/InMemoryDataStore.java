...
public class InMemoryDataStore extends AbstractDataStore {
	...
	private final NavigableMap<Long, byte[]> oids = new TreeMap<Long, byte[]>();

	/** Maps names to object IDs. */
	private final NavigableMap<String, Long> names = new TreeMap<String, Long>();

	/**
	 * Maps transactions to lists of key/value pairs for undoing the operations
	 * performed by the transaction.
	 */
	private final Map<Transaction, List<Object>> txnEntries = new HashMap<Transaction, List<Object>>();

	/** Stores the next object ID. */
	private long nextOid = 1;

	/** Maps class IDs to class info arrays. */
	private final Map<Integer, byte[]> classIdMap = new HashMap<Integer, byte[]>();

	/** Maps class info arrays to class IDs. */
	private final Map<BigInteger, Integer> classInfoMap = new HashMap<BigInteger, Integer>();
	...
	protected synchronized byte[] getObjectInternal(Transaction txn, long oid,
			boolean forUpdate) {
		txn.join(this);
	...
	protected synchronized void setObjectInternal(Transaction txn, long oid,
			byte[] data) {
		txn.join(this);
	...
	protected synchronized void setObjectsInternal(Transaction txn,
			long[] oids, byte[][] dataArray) {
		txn.join(this);
		List<Object> txnEntry = getTxnEntry(txn);
	...
	protected synchronized void removeObjectInternal(Transaction txn, long oid) {
		txn.join(this);
	...
	protected synchronized BindingValue getBindingInternal(Transaction txn,
			String name) {
		txn.join(this);
	...
	protected synchronized BindingValue setBindingInternal(Transaction txn,
	...
	protected synchronized BindingValue removeBindingInternal(Transaction txn,
			String name) {
		txn.join(this);
	...
	protected synchronized String nextBoundNameInternal(Transaction txn,
			String name) {
		txn.join(this);
		if (name != null) {
	...
	protected synchronized int getClassIdInternal(Transaction txn,
			byte[] classInfo) {
	...
	protected synchronized byte[] getClassInfoInternal(Transaction txn,
			int classId) throws ClassInfoNotFoundException {
	...
	protected synchronized long nextObjectIdInternal(Transaction txn, long oid) {
		txn.join(this);

...
/**
 * A node, used for testing. The node is created using the kernel. Multiple
 * nodes can be created within a single VM.
 * 
 */
public class SgsTestNode {
	...
	static {
		try {
			kernelClass = Class.forName("com.sun.sgs.impl.kernel.Kernel");
	...
	static {
		...
		int port = systemPort == null ? DEFAULT_PORT : systemPort.intValue();
	...
	/**
	 * Creates the first SgsTestNode instance in this VM. This thread's owner
	 * will be set to the owner which created this {@code SgsTestNode}.
	 * 
	 * @param appName
	 *            the application name
	 * @param listenerClass
	 *            the class of the listener object, or null if a simple dummy
	 *            listener should be used
	 * @param properties
	 *            serverProperties to be used, or {@code null} for defaults
	 */
	public SgsTestNode(String appName, Class<?> listenerClass,
	 ...
	 *            serverProperties to be used, or {@code null} for defaults
	 ** @param clean
	 *            if {@code true}, make sure the data store directory is fresh
	 */
	public SgsTestNode(String appName, Class<?> listenerClass,
			Properties properties, boolean clean) throws Exception {
		this(appName, null, listenerClass, properties, clean);
	}

	/**
	 * Creates additional SgsTestNode instances in this VM. This node will be
	 * part of the same cluster as the node specified in the firstNode
	 * parameter.
	 * 
	 * @param firstNode
	 *            the first {@code SgsTestNode} created in this VM
	 * @param listenerClass
	 *            the class of the listener object, or null if a simple dummy
	 *            listener should be used
	 * @param properties
	 *            serverProperties to be used, or {@code null} for defaults
	 *            which will cause an exception to be thrown if the standard
	 *            services have been replaced by custom implementations
	 */
	public SgsTestNode(SgsTestNode firstNode, Class<?> listenerClass,
			Properties properties) throws Exception {
	 ...
	 *            the application name
	 * @param serverNode
	 *            the instance which created the servers, {@code null} if this
	 *            instance should create them. If {@code null}, this thread's
	 *            owner is set to the owner which creates this
	 *            {@code SgsTestNode}
	 * @param listenerClass
	 *            the class of the listener object, or null if a simple dummy
	 *            listener should be used
	 ...
	 *            if {@code true}, make sure the data store directory is fresh
	 */
	public SgsTestNode(String appName, SgsTestNode serverNode,
			Class<?> listenerClass, Properties properties, boolean clean)
			throws Exception {
		...
		String createMBeanServer = props
				.getProperty(ProfileCollectorImpl.CREATE_MBEAN_SERVER_PROPERTY);
		if (createMBeanServer == null) {
			...
			props.setProperty(
					ProfileCollectorImpl.CREATE_MBEAN_SERVER_PROPERTY, "true");
		}
		dbDirectory = props
				.getProperty("com.sun.sgs.impl.service.data.store.DataStoreImpl.directory");
		...
		shutdownCtrl = (KernelShutdownController) kernelShutdownCtrl
				.get(kernel);

		// If an app node, we assume SimpleSgsProtocol and TcpTransport
		// transport
		// for the client IO stack.
		if (sessionService != null) {
			String portProp = props
					.getProperty(com.sun.sgs.impl.transport.tcp.TcpTransport.LISTEN_PORT_PROPERTY);
			appPort = portProp == null ? com.sun.sgs.impl.transport.tcp.TcpTransport.DEFAULT_PORT
					: Integer.parseInt(portProp);
		}
	}

	/**
	 * Returns a service of the given {@code type}, or null, if no service is
	 ...
	 * 
	 * @param clean
	 *            if {@code true}, also delete the data store directory
	 */
	 ...
	 * SgsTestNode is constructed. Note that the node mapping service notes only
	 * "full" stacks as being available for node assignment, so we need to
	 * include an application listener.
	 */
	 ...
	 */
	NodeMappingServerImpl getNodeMappingServer() throws Exception {
		Field serverImplField = NodeMappingServiceImpl.class
	...
	/**
	 * Returns the default properties for a server node, useful for adding
	 * additional properties as required.
	 */
	public static Properties getDefaultProperties(String appName,
			SgsTestNode serverNode, Class<?> listenerClass) throws Exception {
		...
		String nodeType = isServerNode ? NodeType.singleNode.toString()
				: NodeType.appNode.toString();

		int requestedDataPort = isServerNode ? getNextUniquePort()
				: getDataServerPort((DataServiceImpl) serverNode
						.getDataService());

		int requestedWatchdogPort = isServerNode ? getNextUniquePort()
				: ((WatchdogServiceImpl) serverNode.getWatchdogService())
						.getServer().getPort();

		int requestedNodeMapPort = isServerNode ? getNextUniquePort()
		...
		Properties retProps = createProperties(
				...
				"1500", "com.sun.sgs.impl.service.nodemap.server.port",
	...
	/**
	 * Returns a unique port number. Note that the ports are only unique within
	 * the current process.
	 */
	...
	public static int getDataServerPort(DataServiceImpl service)
		...
		DataStoreProfileProducer profileWrapper = (DataStoreProfileProducer) storeField
				.get(service);
		DataStoreClient dsClient = (DataStoreClient) profileWrapper
				.getDataStore();
		Field serverPortField = DataStoreClient.class
				.getDeclaredField("serverPort");
	...
	private static int getNodeMapServerPort(NodeMappingServerImpl nodemapServer)
			throws Exception {
		Method getPortMethod = NodeMappingServerImpl.class

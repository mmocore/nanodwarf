...
public class DummyTransaction implements Transaction {
	...
	/** The default timeout. */
	private static long DEFAULT_TIMEOUT = Long.getLong(
			TransactionCoordinator.TXN_TIMEOUT_PROPERTY, 100);
	...
	public enum UsePrepareAndCommit {
		YES, NO, ARBITRARY
	}
	...
	/**
	 * Whether commit on this transaction should use prepareAndCommit instead of
	...
	public final Set<TransactionParticipant> participants = new HashSet<TransactionParticipant>();

	/** The registered {@code TransactionListener}s. */
	private final Set<TransactionListener> listeners = new HashSet<TransactionListener>();
	...
	/**
	 * Creates an instance of this class that uses prepareAndCommit based on the
	 * argument and uses the specified timeout.
	 */
	public DummyTransaction(UsePrepareAndCommit usePrepareAndCommit,
			long timeout) {
	...
	public byte[] getId() {
		return new byte[] { (byte) (id >>> 56), (byte) (id >>> 48),
	...
	public void checkTimeout() {
		...
		} else if (state == State.ABORTING || state == State.COMMITTING) {
			return;
		}
	...
	public synchronized void abort(Throwable cause) {
		...
		} else if (state != State.ACTIVE && state != State.PREPARING
	...
	public synchronized void commit() throws Exception {
		...
		if (state == State.PREPARED) {
			...
			TransactionParticipant participant = participants.iterator().next();
			try {

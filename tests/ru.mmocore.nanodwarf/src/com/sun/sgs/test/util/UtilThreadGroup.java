 ...
 * that do not mask <code>InterruptedException</code>. If any error does occur
 * while running, then an <code>Exception</code> should be thrown. Any specific
 * detail or other error reporting is left up to the runnable.
 */
public class UtilThreadGroup implements Runnable {
	...
	/**
	 * Creates an instance of <code>UtilThreadGroup</code> that will run all of
	 * the given <code>Runnable</code>s, each in their own thread. Note that the
	 * threads are started here, but activity does not begin until the
	 * <code>run</code> method is called on this instance.
	 * 
	 * @param tasks
	 *            an array of <code>Runnable</code>s
	 */
	...
	/**
	 * Returns the current number of reported errors. An error is recorded when
	 * one of the threads throws an exception. Even after the call to

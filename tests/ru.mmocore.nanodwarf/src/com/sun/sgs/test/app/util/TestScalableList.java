...
public class TestScalableList extends Assert {
	...
	public static void setUpClass() throws Exception {
		...
		randomSeed = Long.getLong("test.seed", System.currentTimeMillis());
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() throws Exception {
				...
				try {
					...
					fail("(test.seed=" + randomSeed
							+ ") Did not expect exception: "
							+ e.getLocalizedMessage());
				}
	...
	public void testEquals() throws Exception {
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() throws Exception {
				...
				ScalableList<String> smallerBranchingFactor = new ScalableList<String>(
						3, 3);
				ScalableList<String> largerBranchingFactor = new ScalableList<String>(
						9, 3);
				...
				assertTrue(smallerBranchingFactor.equals(largerBranchingFactor));

				// Test two differently-sized bucket sizes
				ScalableList<String> smallerBucketSize = new ScalableList<String>(
						3, 3);
				ScalableList<String> largerBucketSize = new ScalableList<String>(
						3, 9);
				...
				AppContext.getDataManager()
						.removeObject(smallerBranchingFactor);
				AppContext.getDataManager().removeObject(largerBranchingFactor);
	...
	public void testListIteratorAdd() throws Exception {
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() throws Exception {
				...
				assertEquals(shadow.contains(valueToAddAndCheck),
						list.contains(valueToAddAndCheck));
				assertEquals(shadow.indexOf(valueToAddAndCheck),
						list.indexOf(valueToAddAndCheck));
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() throws Exception {
				...
				while (shadowIter.hasNext()) {
					iter.next();
					shadowIter.next();
					assertEquals(shadowIter.previousIndex(),
							iter.previousIndex());
	...
	public void testListIteratorSet() throws Exception {
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() throws Exception {
				...
				assertEquals("(test.seed=" + randomSeed + ")", shadow.size(),
						list.size());
				AppContext.getDataManager().removeObject(list);
			}
		}, taskOwner);
			...
			public void run() throws Exception {
				...
				try {
					...
					for (int i = 0; i < 20; i++) {
						...
						String listReturned = performRandomOperation(list,
								operation, randomIndex, value);
						String shadowReturned = performRandomOperation(shadow,
								operation, randomIndex, value);
						assertEquals("Iteration #:" + i + ", Operation #:"
								+ operation, listReturned, shadowReturned);
						assertEquals("The list sizes are different",
								shadow.size(), list.size());

						// check integrity
						for (int j = 0; j < shadow.size(); j++) {
							assertEquals("(test.seed=" + randomSeed
									+ ") iteration #" + i + ": ",
	...
	public void testRetrievingAndModifyingListFromDataService()
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() throws Exception {
				...
				ScalableList<String> d = uncheckedCast((AppContext
	...
	public void testSeriazableWithRemovals() throws Exception {
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() throws Exception {
				ScalableList<String> list = uncheckedCast(AppContext
						.getDataManager().getBinding(name));
				for (int i = 0; i < 5; i++) {
					list.remove(0);
				}
	...
	public void testConcurrentModificationException() throws Exception {
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() throws Exception {
				...
				ManagedSerializable<ListIterator<String>> mgdIterator = new ManagedSerializable<ListIterator<String>>(
						iter);
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() throws Exception {
				...
				ManagedSerializable<ListIterator<String>> mgdIterator = uncheckedCast(AppContext
						.getDataManager().getBinding(iterName));
	...
	/**
	 * Tests whether an iterator only removes an element once, as defined by the
	 * {@code ListIterator.remove()} method.
	 * 
	 * @throws Exception
	 */
	@Test
	...
	public void testMultipleIteratorOperations() throws Exception {
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() throws Exception {
				...
				ManagedSerializable<ListIterator<String>> mgdIterator = new ManagedSerializable<ListIterator<String>>(
						iter);
				AppContext.getDataManager().setBinding(iterName, mgdIterator);
				ManagedSerializable<ListIterator<String>> mgdIterator2 = new ManagedSerializable<ListIterator<String>>(
						iter2);
				AppContext.getDataManager().setBinding(iterName2, mgdIterator2);
			}
		}, taskOwner);
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() throws Exception {
				...
				ManagedSerializable<ListIterator<String>> mgdIterator2 = uncheckedCast(AppContext
						.getDataManager().getBinding(iterName2));
	...
	public void testRandomListIteratorOperations() throws Exception {
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() throws Exception {
				...
				// place iterators in the middle
				ListIterator<String> shadowIter = shadow
						.listIterator(startingPoint);
				...
				for (int i = 0; i < 20; i++) {
					...
					// try the shadow list
					try {
						retObj = performOperation(shadowIter, operation, value);
					...
					assertEquals("(test.seed=" + randomSeed
							+ ") Operations were: " + opList, shadowResult,
							listResult);
				}
				assertEquals(shadow.size(), list.size());
				...
				for (int i = 0; i < shadow.size(); i++) {
					assertEquals("iteration: " + i, shadow.get(i), list.get(i));
				}
	...
	/**
	 * Test removing an object with an iterator that is at the end of the list.
	 * 
	...
	public void testRemoveFromEndOfList() throws Exception {
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() throws Exception {
				...
				ListIterator<String> shadowIter = shadow.listIterator(5);
				ListIterator<String> listIter = list.listIterator(5);
	...
	/**
	 * Test removing an object with an iterator that is at the beginning of the
	 * list.
	 * 
	 * @throws java.lang.Exception
	 */
	...
	public void testRemoveFromEndOfListWithIterator() throws Exception {
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() throws Exception {
				...
				// place iterators at the end
				Iterator<String> shadowIter = shadow.iterator();
	...
	/**
	 * Test setting an object with an iterator after a call to previous returns
	...
	/**
	 * Test setting an object with an iterator after a call to next returns
	 * NoSuchElementException.
	 * 
	 * @throws java.lang.Exception
	 */
	...
	/**
	 * Test setting an object with an iterator after a call to previous returns
	 * NoSuchElementException while a previous call to previous was valid.
	 * 
	 * @throws java.lang.Exception
	 */
	@Test
	...
	/**
	 * Test setting an object with an iterator after a call to next returns
	 * NoSuchElementException while a previous call to next was valid.
	 * 
	 * @throws java.lang.Exception
	 */
	@Test
	public void testSetAfterValidThenInvalidNext() throws Exception {
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() throws Exception {
				...
				ListIterator<String> listIter = list.listIterator(list.size() - 1);
	 ...
	 * @param value
	 *            the value to add, if applicable
	...
	private class GetObjectCountTask extends TestAbstractKernelRunnable {
		...
		public void run() {
			...
			while (true) {
				...
				ManagedReference<?> ref = dataService
						.createReferenceForId(next);
	 ...
	 * @throws Exception
	 */
	private void coreRemovingObjectTest(final int elementsToAdd)
			throws Exception {
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				ScalableList<Integer> d = uncheckedCast(AppContext
						.getDataManager().getBinding(name));
	 ...
	 */
	private void coreClearTest(final int elementsToAdd) throws Exception {
		final String name = "list" + Long.toString(System.currentTimeMillis());

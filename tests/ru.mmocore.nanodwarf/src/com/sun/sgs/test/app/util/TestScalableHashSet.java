...
public class TestScalableHashSet extends Assert {
	...
	public void testConstructorNoArg() throws Exception {
		txnScheduler.runTask(new TestTask(new TestAbstractKernelRunnable() {
			public void run() {
	...
	public void testIterator() throws Exception {
		...
		txnScheduler.runTask(new TestTask(new TestAbstractKernelRunnable() {
			public void run() {
				ManagedSerializable<Iterator<Object>> msIter = uncheckedCast(dataService
						.getBinding("iter"));
	...
	public void testRemoveAfterSerialization() throws Exception {
		...
		txnScheduler.runTask(new TestTask(new TestAbstractKernelRunnable() {
			public void run() {
				...
				ScalableHashSet deserialized = (ScalableHashSet) (dataService
						.getBinding(SET_NAME));
	...
	/**
	 * Updates fields from bindings, setting the fields to null if the objects
	 * are not found.
	 */
	@SuppressWarnings("unchecked")
	...
	private class TestTask extends TestAbstractKernelRunnable {
		...
		TestTask(TestAbstractKernelRunnable r) {
			this.r = r;
		}

...
public class TestScalableHashMapStress extends Assert {
	 ...
	 * previous test run.
	 */
	private static final long seed = Long.getLong("test.seed",
			new Date().getTime());
	...
	private static final int maxEntries = Integer.getInteger("test.entries",
			2000);

	/** The number of operations to perform. */
	private static final int operations = Integer.getInteger("test.operations",
			20000);
	 ...
	 * collisions.
	 */
	private static final int collisions = Integer.getInteger("test.collisions",
			3);
	...
	private static class Value extends Int {
		...
		static Value create(int i, Key key) {
			return random.nextBoolean() ? new Value(i, key) : new ManagedValue(
					i, key);
		}
		...
		void setKey(Key key) {
			this.key = (key instanceof ManagedKey) ? AppContext
					.getDataManager().createReference((ManagedKey) key) : null;
		}
	...
	private class EntrySetNextRemove extends EntrySetNext {
		...
		void run(Entry<Key, Value> entry) {
			if (debug) {
				System.err.println("entrySet next remove " + entry.getKey().i);
			}
			Key key = entry.getKey();
			Value value = entry.getValue();
			assertTrue(control.get(key.i));
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() throws Exception {
				...
				dataService.setBinding("keys",
						new ManagedSerializable<Iterator<Key>>(keys));
				Iterator<Value> values = map.values().iterator();
				dataService.setBinding("values",
						new ManagedSerializable<Iterator<Value>>(values));
				Iterator<Entry<Key, Value>> entries = map.entrySet().iterator();
	...
	public void tearDown() throws Exception {
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			...
			public void run() throws Exception {
				...
				dataService.removeObject(dataService.getBinding("entries"));
				Iterator<Entry<Key, Value>> entries = map.entrySet().iterator();
				dataService.setBinding("entries",
						new ManagedSerializable<Iterator<Entry<Key, Value>>>(
								entries));
	...
	public void testStress() throws Exception {
		...
		System.err.println("ops/sec="
				+ Math.round((1000.0d * operations) / (stop - start)));
	...
	void keyRemoved(int objnum) {
		/*
		 * Clear the items seen by the iterators if they are positioned at a key
		 * with the same hash code. If an entry with the same key hash code is
		 * removed and added again, it might get placed after the current
		 * position of the iterator and get seen again.
		 */
	...
	static abstract class AbstractUndoable implements Undoable {
		...
		private static final List<Undoable> allUndoable = new ArrayList<Undoable>();

...
public class TestClientSessionServiceImplv4 extends Assert {
	...
	private static final String THROW_RUNTIME_EXCEPTION = "throw RuntimeException";

	private static final String DISCONNECT_THROWS_NONRETRYABLE_EXCEPTION = "disconnect throws non-retryable exception";

	private static final String SESSION_PREFIX = "com.sun.sgs.impl.service.session.impl";

	private static final String SESSION_NODE_PREFIX = "com.sun.sgs.impl.service.session.node";

	private static final String LISTENER_PREFIX = "com.sun.sgs.impl.service.session.listener";
	...
	/**
	 * Any additional nodes, keyed by node host name (for tests needing more
	 * than one node).
	 */
	...
	protected void setUp(Properties props, boolean clean, String appName,
			byte protocolVersion) throws Exception {
		if (props == null) {
			props = SgsTestNode.getDefaultProperties(appName, null,
		...
		props.setProperty("com.sun.sgs.impl.protocol.simple.protocol.version",
				Byte.toString(protocolVersion));
		props.setProperty(
				StandardProperties.SESSION_RELOCATION_TIMEOUT_PROPERTY, "5000");

		serverNode = new SgsTestNode(appName, DummyAppListener.class, props,
				clean);
	 ...
	 * 
	 * @param hosts
	 *            contains a host name for each additional node
	 */
	protected void addNodes(String... hosts) throws Exception {
		...
		for (String host : hosts) {
			Properties props = SgsTestNode.getDefaultProperties(appName,
					serverNode, DummyAppListener.class);
			props.setProperty(StandardProperties.AUTHENTICATORS,
					"com.sun.sgs.test.util.SimpleTestIdentityAuthenticator");
			...
			props.setProperty(
					...
					"5000");
			SgsTestNode node = new SgsTestNode(serverNode,
					DummyAppListener.class, props);
	...
	public void testLoginRedirect() throws Exception {
		...
		try {
			...
			if (redirectCount != expectedRedirects) {
				...
				System.err.println("Number of redirected users: "
						+ redirectCount);
			}
			...
			for (DummyClient client : clients) {
				try {
					...
					System.err.println("Exception disconnecting client: "
							+ client);
				}
			}
		}
	...
	public void testLoginSuccessAndNotifyLoggedInCallback() throws Exception {
		...
		try {
			...
			if (SimpleTestIdentityAuthenticator.allIdentities
				...
				System.err.println("notifyLoggedIn invoked for identity: "
						+ name);
			} else {
	...
	public void testLoginTwicePreemptUser() throws Exception {
		...
		Properties props = SgsTestNode.getDefaultProperties(appName, null,
				DummyAppListener.class);
		props.setProperty("com.sun.sgs.impl.service.session.allow.new.login",
				"true");
	...
	public void testHighWater() throws Exception {
		...
		Properties props = SgsTestNode.getDefaultProperties(appName, null,
				DummyAppListener.class);
		props.setProperty("com.sun.sgs.impl.service.session.login.high.water",
				"2");
		setUp(props, false, appName, protocolVersion);
		...
		try {
			...
			assertEquals(Health.GREEN,
					watchdog.getLocalNodeHealthNonTransactional());
	...
	public void testLogoutAndNotifyLoggedOutCallback() throws Exception {
		...
		try {
			...
			if (SimpleTestIdentityAuthenticator.allIdentities
				...
				System.err.println("notifyLoggedOut invoked for identity: "
						+ name);
			} else {
				...
				public void run() {
					...
					for (ClientSession session : appListener.getSessions()) {
						if (session.getName().equals(name)) {
							...
							fail("Expected session name: " + name + ", got: "
									+ session.getName());
						}
					}
	...
	private void testClientSessionRemoveWithManagedListener(
			final boolean removeListener) throws Exception {
		...
		try {
			...
			txnScheduler.runTask(new TestAbstractKernelRunnable() {
				public void run() {
					System.err
							.println("forcing disconnection, session:" + name);
					ClientSession session = (ClientSession) dataService
							.getBinding(name);
					DummyClientSessionListener listener = (DummyClientSessionListener) dataService
							.getBinding(name + ".listener");
					if (removeListener) {
	...
	public void testClientSessionSendUnreliableMessages() throws Exception {
		...
		try {
			...
			sendMessagesFromNodesToClient(client, numAdditionalNodes,
					iterations, Delivery.UNRELIABLE, false);
		} finally {
			client.disconnect();
		}
	}
	...
	public void testClientSessionSendUnreliableMessagesWithFailure()
		...
		try {
			...
			int numAdditionalNodes = 2;
			sendMessagesFromNodesToClient(client, numAdditionalNodes,
					iterations, Delivery.UNRELIABLE, true);
		} finally {
			client.disconnect();
		}
	...
	public void testClientSessionSendSequence() throws Exception {
		...
		try {
			...
			int numAdditionalNodes = 2;
			sendMessagesFromNodesToClient(client, numAdditionalNodes,
					iterations, Delivery.RELIABLE, false);
			int numExpectedMessages = (1 + numAdditionalNodes) * iterations;
			client.validateMessageSequence(client.clientReceivedMessages,
	...
	protected void sendMessagesFromNodesToClient(final DummyClient client,
			int numAdditionalNodes, int iterations, final Delivery delivery,
			final boolean oneUnreliableServer) throws Exception {
		...
		int numExpectedMessages = oneUnreliableServer ? iterations : nodes
				.size() * iterations;
			...
			public void run() {
				...
				for (SgsTestNode node : nodes) {
					String key = "com.sun.sgs.impl.service.session.server."
							+ node.getNodeId();
					ClientSessionServer sessionServer = ((ManagedSerializable<ClientSessionServer>) dataService
							.getServiceBinding(key)).get();
					...
					ClientSessionServer delayingServer = (ClientSessionServer) Proxy
							.newProxyInstance(
									ClientSessionServer.class.getClassLoader(),
									new Class[] { ClientSessionServer.class },
									handler);
					dataService.setServiceBinding(key, new ManagedSerializable(
							delayingServer));
				}
			}
		}, taskOwner);
		...
		for (int i = 0; i < iterations; i++) {
			for (SgsTestNode node : nodes) {
				TransactionScheduler localTxnScheduler = node
						.getSystemRegistry().getComponent(
								TransactionScheduler.class);
				...
				localTxnScheduler.scheduleTask(
						new TestAbstractKernelRunnable() {
							public void run() {
								...
								Counter counter;
								try {
									counter = (Counter) dataManager
											.getBinding(counterName);
								} catch (NameNotBoundException e) {
									throw new MaybeRetryException("retry", true);
								}
								ClientSession session = (ClientSession) dataManager
										.getBinding(client.name);
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				AppContext.getDataManager().setBinding(counterName,
						new Counter());
			}
		}, taskOwner);
	...
	public void testClientSessionSendSameBuffer() throws Exception {
		...
		MessageBuffer msg = new MessageBuffer(MessageBuffer.getSize(msgString));
	...
	@Test
	public void testClientSessionSendSameBufferWithOffset() throws Exception {
		String msgString = "offset buffer";
		MessageBuffer msg = new MessageBuffer(
				MessageBuffer.getSize(msgString) + 1);
		msg.putByte(0);
		msg.putString(msgString);
		ByteBuffer buf = ByteBuffer.wrap(msg.getBuffer());
		buf.position(1);
	...
	private void sendBufferToClient(final ByteBuffer buf,
			final String expectedMsgString) throws Exception {
		sendBufferToClient(buf, expectedMsgString, Delivery.RELIABLE);
	}

	private void sendBufferToClient(final ByteBuffer buf,
			final String expectedMsgString, final Delivery delivery)
			throws Exception {
		final String name = "dummy";
		DummyClient client = createDummyClient(name);
		try {
			...
			for (int i = 0; i < numMessages; i++) {
				txnScheduler.runTask(new TestAbstractKernelRunnable() {
					public void run() {
						ClientSession session = (ClientSession) AppContext
	...
	public void testClientSessionSendAbortRetryable() throws Exception {
		...
		try {
			...
			txnScheduler.runTask(new TestAbstractKernelRunnable() {
				...
				public void run() {
					Set<ClientSession> sessions = getAppListener()
							.getSessions();
	...
	public void testClientSendWithListenerThrowingRetryableException()
		...
		try {
			client.connect(serverNode.getAppPort());
			assertTrue(client.login());
			receivedMessageException = new MaybeRetryException("retryable",
					true);
			client.sendMessagesFromClientInSequence(5, 5);
		} finally {
			client.disconnect();
	...
	public void testClientSendWithListenerThrowingNonRetryableException()
		...
		try {
			client.connect(serverNode.getAppPort());
			assertTrue(client.login());
			receivedMessageException = new MaybeRetryException("non-retryable",
					false);
			int numMessages = 5;
			for (int i = 0; i < numMessages; i++) {
			...
			client.validateMessageSequence(
					client.sessionListenerReceivedMessages, numMessages - 1, 1);
	...
	/**
	 * Creates a new {@code DummyClient} with the specified {@code name} and
	 * this test instance's protocol version.
	 */
	...
	/**
	 * Sends the number of specified messages from the specified node to the
	 * specified client. The content of each message is a consecutive integer
	 * starting at the specified offset.
	 */
	protected void sendMessagesFromNode(SgsTestNode node,
			final DummyClient client, final int numMessages, final int offset)
			throws Exception {
		System.err.println("sending messages to client [" + client.name + "]");
		TransactionScheduler transactionScheduler = node.getSystemRegistry()
		...
		for (int i = 0; i < numMessages; i++) {
			...
			transactionScheduler.runTask(new TestAbstractKernelRunnable() {
				public void run() {
					ClientSession session = (ClientSession) AppContext
							.getDataManager().getBinding(client.name);
					ByteBuffer buf = ByteBuffer.allocate(4);
					buf.putInt(x).flip();
	...
	/**
	 * Sends the number of specified messages from the specified node to the
	 * specified client and waits for the client to receive the expected
	 * messages. The content of each message is a consecutive integer starting
	 * at the specified offset.
	 */
	protected void sendMessagesFromNodeToClient(SgsTestNode node,
			DummyClient client, int numMessages, int offset) throws Exception {
	...
	protected void checkBindings(int numExpected) throws Exception {
		...
		List<String> sessionNodeKeys = getServiceBindingKeys(SESSION_NODE_PREFIX);
	...
	private class GetKeysTask extends TestAbstractKernelRunnable {
		...
		public void run() throws Exception {
			...
			for (;;) {
				...
				if (key == null
						|| !key.regionMatches(0, prefix, 0, prefix.length())) {
					break;
				}
				keys.add(key);
			}
	...
	private DummyAppListener getAppListener() {
		return (DummyAppListener) dataService
				.getServiceBinding(StandardProperties.APP_LISTENER);
	}

	public static class DummyAppListener implements AppListener, ManagedObject,
			Serializable {

		private final static long serialVersionUID = 1L;

		private final Set<ManagedReference<ClientSession>> sessions = Collections
				.synchronizedSet(new HashSet<ManagedReference<ClientSession>>());
		...
		public ClientSessionListener loggedIn(ClientSession session) {
			...
			ManagedReference<ClientSession> sessionRef = dataManager
	...
	private static class NonSerializableClientSessionListener implements
	...
	private static class DummyClientSessionListener implements
			ClientSessionListener, Serializable, ManagedObject {
		private final static long serialVersionUID = 1L;
		private final String name;
		...
		DummyClientSessionListener(String name, ClientSession session,
		...
		public void disconnected(boolean graceful) {
			...
			/*
			 * try { dataManager.removeObject( dataManager.getBinding(name +
			 * ".listener")); } catch (ObjectNotFoundException e) { // listener
			 * already removed }
			 */
			DummyClient client = reconnectKey == null ? null : dummyClients
					.get(reconnectKey);
			if (client != null) {
	...
	private static class MaybeRetryException extends RuntimeException implements
	...
	/**
	 * This invocation handler adds a 100 ms delay before invoking any method on
	 * the underlying instance.
	 */
	private static class DelayingInvocationHandler implements
	 ...
	 */
	private static class HungryInvocationHandler implements InvocationHandler,
			Serializable {
		private final static long serialVersionUID = 1L;
		private Object obj;
		...
		public void run() {
			...
			while (true) {
				...
				if (!name.equals("com.sun.sgs.impl.service.task.PendingTask")
					 ...
					 * + "]:"); try { System.err.println(obj.toString()); }
					 * catch (ObjectNotFoundException e) { System.err.println(
	...
	static class DummyClient extends AbstractDummyClient {
		...
		public Queue<byte[]> sessionListenerReceivedMessages = new ConcurrentLinkedQueue<byte[]>();
		public final Queue<byte[]> clientReceivedMessages = new ConcurrentLinkedQueue<byte[]>();
		...
		/**
		 * Records this client in the global map, keyed by client reconnectKey.
		 */
		...
		protected void handleOpCode(byte opcode, MessageBuffer buf) {
			...
			case SimpleSgsProtocol.SESSION_MESSAGE:
				...
				synchronized (clientReceivedMessages) {
					...
					if (clientReceivedMessages.size() == numClientExpectedMessages) {
		...
		/**
		 * Sends a SESSION_MESSAGE with the specified content, prefixing the
		 * message with the reconnect key. The {@code receivedMessage} method of
		 * this client's associated ClientSessionListener must expect that each
		 * message is prefixed with the reconnect key.
		 */
		@Override
		public void sendMessage(byte[] message, boolean checkSuspend) {
			MessageBuffer buf = new MessageBuffer(5 + reconnectKey.length
					+ message.length);
			buf.putByte(SimpleSgsProtocol.SESSION_MESSAGE)
					.putByteArray(reconnectKey).putByteArray(message);
			sendRaw(buf.getBuffer(), checkSuspend);
		}

		/**
		 * Records that this client's associated ClientSessionListener's
		 * {@code disconnected} method was invoked with the specified value for
		 * {@code graceful}.
		 */
		 ...
		 * associated ClientSessionListener was invoked with the specified value
		 * for {@code graceful}, and if not, this method throws
		 * {@code AssertionError}.
		 */
		...
		/**
		 * Waits for the {@code disconnected} method of this client's associated
		 * ClientSessionListener to be invoked with the specified value for
		 * {@code graceful} (within a timeout period), and if not, this method
		 * throws {@code AssertionError}.
		 */
		 ...
		 * messages to be received by this client's associated
		 * {@code ClientSessionListener}, and validates the sequence of messages
		 * received by the listener.
		 */
		public void sendMessagesFromClientInSequence(int numMessages,
			...
			validateMessageSequence(sessionListenerReceivedMessages,
					numExpectedMessages, 0);
		}

		/**
		 * Waits for this client to receive the number of session messages sent
		...
		public void waitForSessionListenerToReceiveExpectedMessages(
			...
			synchronized (sessionListenerReceivedMessages) {
				if (sessionListenerReceivedMessages.size() != numExpectedMessages) {
					try {
			...
			assertEquals("ClientSessionListener for " + toString()
					+ " did not receive expected number of messages",
					numExpectedMessages, sessionListenerReceivedMessages.size());
			System.err.println("ClientSessionListener for " + toString()
					+ " received expected number of messages: "
					+ numExpectedMessages);
		}
		 ...
		 */
		public void validateMessageSequence(Queue<byte[]> messageQueue,
				int numExpectedMessages, int offset) {
			assertEquals(
			...
			if (numExpectedMessages != 0) {
				...
				for (byte[] message : messageQueue) {
					...
					System.err.println(toString()
							+ " validating message sequence: " + value);
		 ...
		 * 
		 * <p>
		 * This implementation verifies that no messages were received by this
		 * client's associated ClientSessionListener during relocation.
		 */
		 ...
		 * 
		 * <p>
		 * This implementation clears the clientReceivedMessages and

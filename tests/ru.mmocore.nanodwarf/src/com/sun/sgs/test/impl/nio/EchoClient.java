...
public class EchoClient {
	...
	private static final int NUM_CLIENTS = Integer.valueOf(System.getProperty(
			"clients", DEFAULT_NUM_CLIENTS));
	private static final int NUM_MSGS = Integer.valueOf(System.getProperty(
			"messages", DEFAULT_NUM_MSGS));
	private static final int NUM_THREADS = Integer.valueOf(System.getProperty(
			"threads", DEFAULT_NUM_THREADS));
	private static final int MAX_THREADS = Integer.valueOf(System.getProperty(
			"maxthreads", String.valueOf(DEFAULT_MAX_THREADS)));
	private static final boolean DISABLE_NAGLE = Boolean.valueOf(System
			.getProperty("tcp_nodelay", DEFAULT_DISABLE_NAGLE));
	 ...
	 * 
	 * @param group
	 *            the asynchronous channel group for this client
	 */
	...
	final class ConnectHandler implements CompletionHandler<Void, Object> {
		/**
		 * {@inheritDoc}
		 */
	...
	final class ReadHandler implements CompletionHandler<Integer, Integer> {
	...
	final class WriteHandler implements CompletionHandler<Integer, Integer> {
		final ByteBuffer buf = allocateBuffer(BUFFER_SIZE);
	...
	void disconnected() {
		if (log.isLoggable(Level.FINE)) {
			log.log(Level.FINE, "Disconnected {0} read:{1} wrote:{2}",
	...
	public final static void main(String[] args) throws Exception {
		...
		ThreadFactory threadFactory = new VerboseThreadFactory(log,
				Executors.defaultThreadFactory());

		ThreadPoolExecutor executor = (ThreadPoolExecutor) ((MAX_THREADS == Integer.MAX_VALUE) ? Executors
				.newCachedThreadPool(threadFactory) : Executors
				.newFixedThreadPool(MAX_THREADS, threadFactory));

		executor.setKeepAliveTime(10, TimeUnit.SECONDS);
		executor.setCorePoolSize(NUM_THREADS);

		log.log(Level.INFO, "Prestarting {0,number,integer} threads",
				NUM_THREADS);
		...
		AsynchronousChannelGroup group = provider
		...
		log.log(Level.INFO, "Bytes read: {0}  written:{1}", new Object[] {
				totalBytesRead.get(), totalBytesWritten.get() });

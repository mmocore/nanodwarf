...
public class TestDataStoreImplPlaceholders extends Assert {
	...
	private static final BasicDataStoreTestEnv env = new BasicDataStoreTestEnv(
	...
	private static final String dbDirectory = System
			.getProperty("java.io.tmpdir")
			+ File.separator
			+ "TestDataStoreImplPlaceholders.db";

	/** The DataStoreImpl.setObjectRaw(Transaction,long,byte[]) method. */
	private static final Method setObjectRaw = UtilReflection.getMethod(
			DataStoreImpl.class, "setObjectRaw", Transaction.class, long.class,
			byte[].class);

	/** The DataStoreImpl.getObjectRaw(Transaction,long) method. */
	private static final Method getObjectRaw = UtilReflection.getMethod(
			DataStoreImpl.class, "getObjectRaw", Transaction.class, long.class);

	/** The DataStoreImpl.nextObjectIdRaw(Transaction,long) method. */
	private static final Method nextObjectIdRaw = UtilReflection.getMethod(
			DataStoreImpl.class, "nextObjectIdRaw", Transaction.class,
			long.class);
	...
	private static Properties props = createProperties(DataStoreImplClassName
	...
	protected static final AccessCoordinatorHandle accessCoordinator = new NullAccessCoordinator(
			System.getProperties(), txnProxy, new DummyProfileCollectorHandle());
	...
	public void testRemoveAllocationPlaceholders() throws Exception {
		...
		for (int i = 1025; i < 2050; i++) {
			...
			store.setObject(txn, store.createObject(txn),
					new byte[] { (byte) i });
		}
		/* Create objects */
	...
	private static void setObjectRaw(Transaction txn, long oid, byte[] data) {
		try {

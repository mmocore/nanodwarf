...
public class TestLockingAccessCoordinator extends
		BasicAccessCoordinatorTest<LockingAccessCoordinator> {
	...
	protected void init(long lockTimeout, int numKeyMaps) {
		if (lockTimeout > 0
				&& properties
						.getProperty(LockingAccessCoordinator.LOCK_TIMEOUT_PROPERTY) == null) {
		...
		if (numKeyMaps > 0
				&& properties
						.getProperty(LockingAccessCoordinator.NUM_KEY_MAPS_PROPERTY) == null) {
			properties.setProperty(
	...
	protected LockingAccessCoordinator createAccessCoordinator() {
		return new LockingAccessCoordinator(properties, txnProxy,
				profileCollector);
	}
	...
	public void testConstructorIllegalLockTimeout() {
		...
		for (String value : values) {
			...
			try {
				new LockingAccessCoordinator(properties, txnProxy,
						profileCollector);
	...
	public void testAccessedObjectsDetailTimeout() throws Exception {
		...
		assertEquals(ConflictType.ACCESS_NOT_GRANTED, detail.getConflictType());
	...
	public void testAccessedObjectsDetailDeadlock() throws Exception {
		...
		FutureTask<Exception> attempt = new FutureTask<Exception>(
				new Callable<Exception>() {
					public Exception call() throws Exception {
						try {
							reporter.reportObjectAccess(txn, "o2",
									AccessType.WRITE);
							return null;
						} catch (Exception e) {
							e.printStackTrace();
		...
		AccessedObjectsDetail detail = profileCollector
				.getAccessedObjectsDetail();
		assertObjectDetails(detail, "s", "o2", AccessType.READ, null, "s",
				"o1", AccessType.WRITE, null);
					...
					public void run() {
						for (int c = 0; c < count; c++) {
							...
							for (int i = 0; i < locks; i++) {
								reporter.reportObjectAccess(txn, "o" + i,
										AccessType.READ);
							}
							txn.abort(ABORT_EXCEPTION);
						}

...
public class TestDataStoreImpl extends Assert {
	...
	protected static final String dbDirectory = System
	...
	public TestDataStoreImpl() {
		this(defaultEnv == null ? defaultEnv = new BasicDataStoreTestEnv(
				System.getProperties()) : defaultEnv);
	}
	...
	public void testConstructorDirectoryIsFile() throws Exception {
		...
		props.setProperty(DataStoreImplClassName + ".directory", file);
		try {
			createDataStore(props);
	...
	public void testCreateObjectSuccess() throws Exception {
		assertTrue(id >= 0);
		assertTrue(txn.participants.contains(((DataStoreProfileProducer) store)
				.getDataStore()));
	...
	public void testNextBoundNameSuccess() throws Exception {
		for (String name = null; (name = store.nextBoundName(txn, name)) != null;) {
	...
	public void testNextBoundNameTimeout() throws Exception {
		...
		class MyRunnable implements Runnable {
			...
			public void run() {
				...
				try {
					txn2 = createTransaction(UsePrepareAndCommit.ARBITRARY,
							20000);
	...
	public void testAbortNullTxn() throws Exception {
		store.createObject(txn);
		TransactionParticipant participant = txn.participants.iterator().next();
		try {
	...
	@Test
	public void testPrepareAndCommitPrepareAndCommitReadOnly() throws Exception {
		testPreparedReadOnly(prepareAndCommit);
	}

	@Test
	public void testPrepareAndCommitPrepareAndCommitModified() throws Exception {
	...
	@Test
	public void testPrepareAndCommitShuttingDownExistingTxn() throws Exception {
	...
	private final Action getClassId = new Action() {
		void run() {
			store.getClassId(txn, new byte[0]);
		};
	};
	...
	public void testDeadlock() throws Exception {
		for (int i = 0; i < 5; i++) {
			if (i > 0) {
				txn = createTransaction(UsePrepareAndCommit.ARBITRARY, 1000);
			}
			...
			if (myRunnable.exception2 != null
					&& !(myRunnable.exception2 instanceof TransactionAbortedException)) {
	...
	protected DataStore createDataStore(Properties props) throws Exception {
		DataStore store = new DataStoreProfileProducer(new DataStoreImpl(props,
	...
	protected Properties getProperties() throws Exception {
		return createProperties(DataStoreImplClassName + ".directory",
				dbDirectory);
	}
	...
	abstract class Action {
		...
		/**
		 * Perform any setup required for calling the {@link #run} method. This
		 * method needs to put the database in use for the current transaction,
		 * but not in a way that conflicts with the operations performed by
		 * {@code run}.
		 * 
		 * @throws Exception
		 *             if the setup throws an exception
		 */
		void setUp() throws Exception {
		};
	...
	/**
	 * Tests running the action after abort, and expecting to get the specified
	 * exception type.
	 */

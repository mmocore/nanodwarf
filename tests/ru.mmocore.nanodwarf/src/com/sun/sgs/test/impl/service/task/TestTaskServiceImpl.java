...
public class TestTaskServiceImpl extends Assert {
	...
	private static final String PENDING_NS = TaskServiceImpl.DS_PREFIX
			+ "Pending.";
	...
	protected void setUp(Properties props, boolean clean) throws Exception {
		...
		continueThreshold = Long
				.valueOf(serviceProps
						.getProperty("com.sun.sgs.impl.service.task.continue.threshold"));
	...
	public void testConstructorNoScheduler() throws Exception {
		Class<?> criClass = Class
				.forName("com.sun.sgs.impl.kernel.ComponentRegistryImpl");

		Constructor<?> criCtor = criClass
				.getDeclaredConstructor(new Class[] {});
		criCtor.setAccessible(true);
		try {
			new TaskServiceImpl(serviceProps,
					(ComponentRegistry) criCtor.newInstance(new Object[] {}),
					txnProxy);
	...
	public void testScheduleTaskNotManagedObject() throws Exception {
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				...
				try {
					PeriodicTaskHandle handle = taskService
							.schedulePeriodicTask(task, 100L, 100L);
					handle.cancel();
				} catch (Exception e) {
					fail("Did not expect Exception: " + e);
				}
	...
	private void runPeriodicTest(final Identity owner) throws Exception {
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				...
				for (int i = 0; i < 3; i++) {
					PeriodicTaskHandle handle = taskService
							.schedulePeriodicTask(new NonManagedTask(owner),
									20L * i, 500L);
					dataService.setBinding("runHandle." + i, new ManagedHandle(
							handle));
					counter.increment();
					counter.increment();
				}
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				...
				while ((name != null) && (name.startsWith("runHandle."))) {
					ManagedHandle mHandle = (ManagedHandle) dataService
							.getBinding(name);
					mHandle.cancel();
	...
	public void testCancelPeriodicTasksBasic() throws Exception {
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				...
				PeriodicTaskHandle handle = taskService.schedulePeriodicTask(
				...
				handle = taskService.schedulePeriodicTask(new NonManagedTask(
						taskOwner), 500L, 100L);
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				ManagedHandle mHandle = (ManagedHandle) dataService
						.getBinding("TestTaskServiceImpl.handle");
				try {
					mHandle.cancel();
				} catch (Exception e) {
					fail("Did not expect Exception: " + e);
	...
	private class CancelPeriodicTask extends TestAbstractKernelRunnable {
		...
		public void run() {
			Counter counter = getClearedCounter();
			handle = taskService.schedulePeriodicTask(new ManagedTask(), 200L,
					500L);
			counter.increment();
		}
	}
	...
	public void testCancelPeriodicTaskWithItself() throws Exception {
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				PeriodicTaskCanceler canceler = (PeriodicTaskCanceler) dataService
						.getBinding("canceler");
	...
	private static class PeriodicTaskCanceler implements Task, ManagedObject,
			Serializable {
	...
	private class GetManagedHandleTask extends TestAbstractKernelRunnable {
		...
		public void run() {
			mHandle = (ManagedHandle) dataService
					.getBinding("TestTaskServiceImpl.handle");
			mHandle.cancel();
		}
	}
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() throws Exception {
				KernelRunnable r = new TestAbstractKernelRunnable() {
					public void run() throws Exception {
						if (!txnProxy.getCurrentOwner().equals(taskOwner)) {
	...
	public void testRecoveryCleanup() throws Exception {
		...
		final String name = TaskServiceImpl.DS_PREFIX + "Handoff."
				+ node.getNodeId();
		...
		node.shutdown(false);
		String interval = serverNode.getServiceProperties().getProperty(
	...
	public void testRunDelayedWithNewIdentity() throws Exception {
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				Counter counter = getClearedCounter();
				taskService.scheduleTask(new NewIdentityTask(taskOwner), 50L);
				counter.increment();
			}
		}, taskOwner);
		Thread.sleep(200L);
	...
	public void testRunNonDurableWithNewIdentity() throws Exception {
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() throws Exception {
				...
				KernelRunnable r = new NewIdentityKernelRunnable(owner, latch);
				taskService.scheduleNonDurableTask(r, true);
			}
		}, taskOwner);
	...
	public void testRunDelayedAcrossShutdown() throws Exception {
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				AppContext.getDataManager();
				Counter counter = getClearedCounter();
				taskService.scheduleTask(new NonManagedTask(taskOwner), 500L);
				counter.increment();
			}
		}, taskOwner);
		...
		Thread.sleep(100);
		assertCounterNotClearXAction("Delayed task incorrectly ran immediately after restart");

		// verify that the delayed task does run after waiting
		Thread.sleep(500);
		assertCounterClearXAction("Delayed task did not run on time after restart");
	}
	...
	public void testRunPeriodicAcrossShutdown() throws Exception {
		...
		Thread.sleep(1000);
		assertCounterClearXAction("Some periodic tasks did not run on time after restart");
	}
	...
	private void assertCounterValueXAction(final int value, final String message)
			throws Exception {
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
	...
	private static Field getReusableField() throws Exception {
		Class pendingTaskClass = Class
				.forName("com.sun.sgs.impl.service.task.PendingTask");
	...
	public static class ManagedTask extends AbstractTask implements
			ManagedObject {
		private static final long serialVersionUID = 1;
	}
	...
	public static class RetryException extends Exception implements
	...
	@RunWithNewIdentity
	public static class NewIdentityTask extends AbstractTask implements
			Serializable {
		private static final long serialVersionUID = 1;

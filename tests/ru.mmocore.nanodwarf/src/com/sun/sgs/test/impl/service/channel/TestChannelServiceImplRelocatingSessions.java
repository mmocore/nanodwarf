...
public class TestChannelServiceImplRelocatingSessions extends
		AbstractChannelServiceTest {
	...
	public void testChannelJoinAndRelocateWithOldNodeFailure() throws Exception {
		...
		try {
			...
			moveIdentityAndWaitForRelocationNotification(relocatingClient,
					node1, node2);
			// crash node1.
	...
	public void testChannelJoinDuringRelocatePreparation() throws Exception {
		...
		try {
			...
			SimpleCompletionHandler handler = mySessionStatusListener
					.waitForPrepare();
	...
	public void testSessionCleanupIfSessionFailsToPrepare() throws Exception {
		...
		SgsTestNode oldNode = addNode();
		SgsTestNode newNode = addNode();
		MySessionStatusListener oldNodeListener = new MySessionStatusListener(
				true);
		oldNode.getClientSessionService().addSessionStatusListener(
				oldNodeListener);
		MySessionStatusListener newNodeListener = new MySessionStatusListener(
				false);
		newNode.getClientSessionService().addSessionStatusListener(
				newNodeListener);
		String[] users = new String[] { "relocatingClient" };
		DummyClient relocatingClient = createDummyClient(users[0], oldNode);
	 ...
	 */
	private void moveIdentity(DummyClient client, SgsTestNode oldNode,
			SgsTestNode newNode) throws Exception {
		identityAssigner.moveIdentity(client.name, oldNode.getNodeId(),
				newNode.getNodeId());
	}
	 ...
	 * client does not relocated unless instructed to do so via a
	 * {@code relocate} invocation.
	 */
	private void moveIdentityAndWaitForRelocationNotification(
		...
		System.err.println("reassigning identity:" + client.name
				+ " from node: " + oldNode.getNodeId() + " to node: "
				+ newNode.getNodeId());
	...
	private class MySessionStatusListener implements
			ClientSessionStatusListener {
		private SimpleCompletionHandler handler = null;
		private final boolean controlledPreparation;

		private Set<BigInteger> disconnectedSessions = Collections
				.synchronizedSet(new HashSet<BigInteger>());
		private Set<BigInteger> relocatedSessions = Collections
				.synchronizedSet(new HashSet<BigInteger>());

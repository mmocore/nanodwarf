 ...
 * 
 * Results -- best times: Date: 3/6/2007 Hardware: Host freeside, Power Mac G5,
 * 2 2 GHz processors, 2.5 GB memory, HFS+ filesystem with logging enabled
 * Operating System: Mac OS X 10.4.8 Berkeley DB Version: 4.5.20 Java Version:
 * 1.5.0_07 Parameters: test.items=100 test.modify.items=50 test.count=100
 * Testcase: testRead Time: 12 ms per transaction Testcase: testReadNoDetectMods
 * Time: 6.9 ms per transaction Testcase: testWrite Time: 14 ms per transaction
 * Testcase: testWriteNoDetectMods Time: 9.4 ms per transaction
 */
@IntegrationTest
...
public class TestDataServicePerformance extends Assert {
	...
	private static final String DataStoreImplClass = "com.sun.sgs.impl.service.data.store.DataStoreImpl";

	/** The name of the DataServiceImpl class. */
	private static final String DataServiceImplClass = DataServiceImpl.class
			.getName();
	 ...
	 */
	protected final int modifyItems = Integer.getInteger("test.modify.items",
			50);
	...
	private void doTestRead(boolean detectMods) throws Exception {
		...
		for (int r = 0; r < repeat; r++) {
			...
			for (int c = 0; c < count; c++) {
				txnScheduler.runTask(new TestAbstractKernelRunnable() {
					public void run() throws Exception {
						Counters counters = (Counters) service
								.getBinding("counters");
			...
			System.err.println("Time: " + (stop - start) / (float) count
					+ " ms per transaction");
		}
	}
	...
	protected Properties getNodeProps() throws Exception {
		Properties props = SgsTestNode.getDefaultProperties(
				"TestDataServicePerformance", null, null);
	...
	static class Counters implements ManagedObject, Serializable {
		...
		private List<ManagedReference<Counter>> counters = new ArrayList<ManagedReference<Counter>>();

		Counters(int count) {
			for (int i = 0; i < count; i++) {
				counters.add(AppContext.getDataManager().createReference(
						new Counter()));
			}
		}

...
/**
 * An in-memory testing implementation of NodeMappingService. Note that this
 * class is not designed to scale, simply to allow some basic tests which
 * require manipulation or observation of the mappings and votes.
 */
public class DummyNodeMappingService implements NodeMappingService,
	...
	/**
	 * Tries to move the given identity from the local node to the given node,
	 * throwing IllegalArgumentException if the identity is not mapped to the
	 * local node or if nodeId is invalid.
	 */
	public void moveIdentity(Class<?> service, Identity identity, long nodeId) {
		...
		for (Entry<Class<?>, HashSet<Identity>> entry : activeIdentities
				.entrySet()) {

 ...
 * {@code TransactionListener} is reported correctly in the profiling stream.
 * <p>
 * Note that this is really a place-holder to support new profiling
 * functionality. In the future there should be a more robust set of tests for
 * {@code TransactionListener}s and {@code TransactionParticipant}s. This
 * involves implementing both interfaces and using a custom {@code Service} to
 * include them in transactions.
 */
@RunWith(FilteredNameRunner.class)
public class TestReportedTxnMembers {

	final String LISTENER_NAME = "com.sun.sgs.impl.service.data.Context";
	final String TASK_NS = "com.sun.sgs.test.impl.profile.TestReportedTxnMembers";
	...
	public void setUp() throws Exception {
		...
		Properties p = SgsTestNode.getDefaultProperties("TestTxnMembers", null,
				null);
	...
	public void testDataServiceListenerIncluded() throws Exception {
		final AtomicReference<RuntimeException> throwableRef = new AtomicReference<RuntimeException>();
		...
		SimpleTestListener listener = new SimpleTestListener(new Runnable() {
			public void run() {
				...
				for (TransactionListenerDetail detail : SimpleTestListener.report
						.getListenerDetails()) {
					...
					if (detail.getListenerName().equals(LISTENER_NAME)) {
						...
						if (beforeShouldBeCalled.get() != detail
								.calledBeforeCompletion()) {
							throwableRef.set(new RuntimeException(
									"wrong before " + "status"));
						}
						break;
					}
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				AppContext.getDataManager().createReference(new DummyObject());
			}
	...
	private static final class DummyObject implements ManagedObject,
			Serializable {
	}

 ...
 * 
 * Results -- best times: Date: 3/6/2007 Hardware: Power Mac G5, 2 2 GHz
 * processors, 2.5 GB memory, HFS+ filesystem with logging enabled Operating
 * System: Mac OS X 10.4.8 Berkeley DB Version: 4.5.20 Java Version: 1.5.0_07
 * Parameters: test.items=100 test.item.size=100 test.modify.items=50
 * test.count=400 Testcase: testReadIds Time: 1.3 ms per transaction Testcase:
 * testWriteIds Time: 2.2 ms per transaction Testcase: testReadNames Time: 1.2
 * ms per transaction Testcase: testWriteNames Time: 2.0 ms per transaction
 */
@IntegrationTest
...
public class TestDataStorePerformance extends Assert {
	...
	private static final String DataStoreImplClass = DataStoreImpl.class
			.getName();
	...
	public void setUp() throws Exception {
		System.err.println("Parameters:" + "\n  test.items=" + items
				...
				+ modifyItems + "\n  test.count=" + count);
		props = createProperties(DataStoreImplClass + ".directory",
				createDirectory());
	}
	...
	void testWriteIdsInternal(boolean flush) throws Exception {
		props.setProperty(JeEnvironment.FLUSH_TO_DISK_PROPERTY,

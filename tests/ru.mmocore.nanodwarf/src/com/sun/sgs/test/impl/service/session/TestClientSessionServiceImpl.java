...
public class TestClientSessionServiceImpl extends Assert {
	...
	private static final Properties serviceProps = createProperties(
	...
	protected void setUp(Properties props, boolean clean) throws Exception {
		if (props == null) {
			props = SgsTestNode.getDefaultProperties(APP_NAME, null, null);
		}
		...
		props.setProperty(
				...
				"1000");
		serverNode = new SgsTestNode(APP_NAME, null, props, clean);
	...
	public void testConstructorNullProperties() throws Exception {
		try {
			new ClientSessionServiceImpl(null, serverNode.getSystemRegistry(),
					serverNode.getProxy());
	...
	public void testConstructorNoPort() throws Exception {
		Properties props = createProperties(StandardProperties.APP_NAME,
				APP_NAME);
		new ClientSessionServiceImpl(props, serverNode.getSystemRegistry(),
				serverNode.getProxy());
	}
	...
	public void testAddSessionStatusListenerNullArg() {
		try {
			serverNode.getClientSessionService().addSessionStatusListener(null);
			fail("expected NullPointerException");
		} catch (NullPointerException e) {
			System.err.println(e);
		}
	...
	@Test
	public void testAddSessionStatusListenerInTxn() throws Exception {
		try {
			txnScheduler.runTask(new TestAbstractKernelRunnable() {
				public void run() {
					serverNode
	...
	public void testAddSessionStatusListenerNoTxn() {
		serverNode.getClientSessionService().addSessionStatusListener(
	...
	@Test
	public void testGetSessionProtocolInTxn() throws Exception {
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				serverNode.getClientSessionService().getSessionProtocol(
						new BigInteger(1, new byte[] { 0 }));
			}
		}, taskOwner);
	}
	...
	public void testGetSessionProtocolNoTxn() {
		assertNull(serverNode.getClientSessionService().getSessionProtocol(
				new BigInteger(1, new byte[] { 0 })));
	}

	private static class DummyStatusListener implements

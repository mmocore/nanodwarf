...
public class TestBoundNamesUtil extends TestCase {
	...
	/**
	 * Test that an iterator created with the specified prefix on a service with
	 * the specified names, which should result in an iteration that returns no
	 * names, responds properly to the remove method.
	 */
	private void testRemoveEmpty(String prefix, String... names) {
		List<String> list = Arrays.asList(names);
		service.clear();
		service.addAll(list);
		Iterator<String> iter = BoundNamesUtil.getServiceBoundNamesIterator(
				service, prefix);
	...
	/**
	 * Test that an iterator created with the specified prefix on a service with
	 * the specified names, which should result in an iteration that returns one
	 * name, responds properly to the remove method.
	 */
	private void testRemoveOneElement(String prefix, String... names) {
		...
		assertRemoveFails(iter);
		assertNextFails(iter);
		assertContents(BoundNamesUtil.getServiceBoundNamesIterator(service,
				prefix));
	}
	...
	/**
	 * Test that an iterator created with the specified prefix on a service with
	 * the specified names, which should result in an iteration that returns two
	 * names, responds properly to the remove method.
	 */
	private void testRemoveTwoElements(String prefix, String... names) {
		...
		for (String name : BoundNamesUtil.getServiceBoundNamesIterable(service,
				prefix)) {
	...
	private void testMatch(CheckIterator check) {
		service.addAll(Arrays.asList("able", "al", "alan", "alex", "ann",
				"bill", "billy", "bob", "bobby"));
	...
	class CheckIterable extends CheckIterator {
		...
		void checkContents(String prefix, String... names) {
			Iterable<String> ible = BoundNamesUtil
					.getServiceBoundNamesIterable(service, prefix);
	 ...
	 */
	private static class DummyDataService extends TreeSet<String> implements
			DataService {

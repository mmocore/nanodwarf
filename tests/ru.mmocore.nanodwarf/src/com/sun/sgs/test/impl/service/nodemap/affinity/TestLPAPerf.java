...
public class TestLPAPerf {
	...
	public static void before() throws Exception {
		Properties props = SgsTestNode.getDefaultProperties("TestLPA", null,
				null);
	...
	public void warmupZach() throws Exception {
		...
		SingleLabelPropagation lpa = new SingleLabelPropagation(
				new ZachBuilder(), collector, props);
	...
	public void testZachary() throws Exception {
		...
		SingleLabelPropagation lpa = new SingleLabelPropagation(builder,
				collector, props);

		AffinityGroupFinderMXBean bean = (AffinityGroupFinderMXBean) collector
		...
		for (int i = 0; i < RUNS; i++) {
			Set<AffinityGroup> groups = Objects.uncheckedCast(lpa
					.findAffinityGroups());
			double mod = AffinityGroupGoodness.calcModularity(
	...
	public void testDistZach() throws Exception {
		...
		props.put("com.sun.sgs.impl.service.nodemap.affinity.server.port",
				String.valueOf(serverPort));
		LabelPropagationServer server = new LabelPropagationServer(collector,
				wdog, props);
		props.put("com.sun.sgs.impl.service.nodemap.affinity.numThreads",
				String.valueOf(numThreads));
		props.put(LPADriver.GRAPH_CLASS_PROPERTY, builderName);

		LabelPropagation lp1 = new LabelPropagation(new DistributedZachBuilder(
				DistributedZachBuilder.NODE1), wdog,
				DistributedZachBuilder.NODE1, props);
		LabelPropagation lp2 = new LabelPropagation(new DistributedZachBuilder(
				DistributedZachBuilder.NODE2), wdog,
				DistributedZachBuilder.NODE2, props);
		LabelPropagation lp3 = new LabelPropagation(new DistributedZachBuilder(
				DistributedZachBuilder.NODE3), wdog,
		...
		double minMod = 1.0;
		for (int i = 0; i < RUNS; i++) {
			Set<AffinityGroup> groups = Objects.uncheckedCast(server
					.findAffinityGroups());
			double mod = AffinityGroupGoodness.calcModularity(
					new ZachBuilder().getAffinityGraph(), groups);
	...
	private class DistributedZachBuilder implements DLPAGraphBuilder {
		...
		private final HashMap<Identity, LabelVertex> identMap = new HashMap<Identity, LabelVertex>();

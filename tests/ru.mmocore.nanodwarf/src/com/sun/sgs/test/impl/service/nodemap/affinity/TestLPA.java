...
public class TestLPA {
	...
	public void setup() throws Exception {
		...
		collector = serverNode.getSystemRegistry().getComponent(
	...
	public void testLPAAlgorithm() throws Exception {
		...
		LabelPropagation lp1 = new LabelPropagation(new PartialToyBuilder(
				PartialToyBuilder.NODE1), wdog, PartialToyBuilder.NODE1, props);
		LabelPropagation lp2 = new LabelPropagation(new PartialToyBuilder(
				PartialToyBuilder.NODE2), wdog, PartialToyBuilder.NODE2, props);
		LabelPropagation lp3 = new LabelPropagation(new PartialToyBuilder(
				PartialToyBuilder.NODE3), wdog, PartialToyBuilder.NODE3, props);
		Set<RelocatingAffinityGroup> groups = server.findAffinityGroups();
	...
	public void testRegister() throws Exception {
		...
		Exporter<TestLPAClientIface> exporter = new Exporter<TestLPAClientIface>(
				TestLPAClientIface.class);
		exporter.export(client1, 0);
		try {
			server.register(nodeId, exporter.getProxy());

			TestLPAClientIface proxy = (TestLPAClientIface) server
					.getLPAClientProxy(nodeId);
	...
	public void testCrossNodeData() throws Exception {
		...
		Map<Object, Long> map = lp1.getNodeConflictMap().get(
				PartialToyBuilder.NODE2);
	...
	private void printNodeConflictMap(LabelPropagation lp) {
		...
		for (Map.Entry<Long, Map<Object, Long>> entry : lp.getNodeConflictMap()
				.entrySet()) {
			StringBuilder sb1 = new StringBuilder();
			sb1.append(entry.getKey());
			sb1.append(":  ");
			for (Map.Entry<Object, Long> subEntry : entry.getValue().entrySet()) {
	...
	public void testCrossNodeLabels() throws Exception {
		...
		Map<Object, Map<Integer, List<Long>>> labelMap = lp1
				.getRemoteLabels(objIds);
		...
		Method updateRemoteLabelsMethod = UtilReflection.getMethod(
				LabelPropagation.class, "updateRemoteLabels");
		...
		// node 1
		ConcurrentMap<Identity, Map<Integer, Long>> rlm = lp1
				.getRemoteLabelMap();
	...
	/**
	 * A very simple test server that counts the number of client callbacks for
	 * preparation completed and finished iterations.
	 */
	private static class TestLPAServer extends LabelPropagationServer {
		...
		public TestLPAServer(ProfileCollector col, WatchdogService wdog,
		...
		/** {@inheritDoc} */
		public void readyToBegin(long nodeId, boolean failed)
				throws IOException {
	...
	private class PartialToyBuilder extends AbstractTestGraphBuilder implements
			DLPAGraphBuilder {
		private final ConcurrentMap<Long, Map<Object, Long>> conflictMap = new ConcurrentHashMap<Long, Map<Object, Long>>();
		private final ConcurrentMap<Object, Map<Identity, Long>> objUseMap = new ConcurrentHashMap<Object, Map<Identity, Long>>();
	...
	//
	private UndirectedGraph<LabelVertex, WeightedEdge> createPartialToyBuilderGraph(
			long node) {
		UndirectedGraph<LabelVertex, WeightedEdge> graph = new UndirectedSparseGraph<LabelVertex, WeightedEdge>();

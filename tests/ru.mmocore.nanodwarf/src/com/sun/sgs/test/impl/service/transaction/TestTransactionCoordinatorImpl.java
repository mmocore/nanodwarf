...
public class TestTransactionCoordinatorImpl {
	...
	private final TransactionCoordinator coordinator = new TransactionCoordinatorImpl(
			coordinatorProps, collectorHandle);
	...
	public void testConstructorIllegalPropertyValues() {
		Properties[] allProperties = {
				createProperties(TransactionCoordinator.TXN_TIMEOUT_PROPERTY,
						"foo"),
				createProperties(TransactionCoordinator.TXN_TIMEOUT_PROPERTY,
						"0"),
				createProperties(TransactionCoordinator.TXN_TIMEOUT_PROPERTY,
	...
	public void testCommitAborting() throws Exception {
		...
		for (DummyTransactionParticipant participant : participants) {
			if (!participant.prepareReturnedTrue()) {
				assertEquals(participant == participants[2] ? State.ACTIVE
						: State.ABORTED, participant.getState());
			}
		}
	...
	public void testCommitCommitting() throws Exception {
		...
		for (DummyTransactionParticipant participant : participants) {
			if (!participant.prepareReturnedTrue()) {
				assertEquals(participant == participants[2] ? State.PREPARED
						: State.COMMITTED, participant.getState());
			}
		}
		assertCommitted();
	}
		...
		Thread thread = new Thread() {
			public void run() {
				try {
					...
					if (txn != t) {
						throw new RuntimeException("Expected " + txn + ", got "
								+ t);
					}
				} catch (RuntimeException e) {
					e.printStackTrace();
	...
	public void testGetTransactionPreparing() {
		TransactionParticipant participant = new DummyTransactionParticipant() {
			public boolean prepare(Transaction txn) throws Exception {
				handle.getTransaction();
	...
	public void testGetCreationTime() throws Exception {
		...
		Transaction txn2 = coordinator.createTransaction(
				coordinator.getDefaultTimeout()).getTransaction();
		assertTrue(
				"Transaction creation time is too early: "
						+ txn1.getCreationTime(), now < txn1.getCreationTime());
		assertTrue(
				"Transaction creation times out-of-order: "
						+ txn1.getCreationTime() + " vs "
		...
		Thread thread = new Thread() {
			public void run() {
				try {
					...
					if (time != t) {
						throw new RuntimeException("Expected " + time
								+ ", got " + t);
					}
				} catch (RuntimeException e) {
					e.printStackTrace();
	...
	public void testGetTimeout() {
		...
		p.setProperty(TransactionCoordinator.TXN_UNBOUNDED_TIMEOUT_PROPERTY,
				"100000");
		TransactionCoordinator coordinator = new TransactionCoordinatorImpl(p,
				collectorHandle);
		Transaction txn = coordinator.createTransaction(
				coordinator.getDefaultTimeout()).getTransaction();
		assertTrue(
				"Incorrect bounded Transaction timeout: " + txn.getTimeout(),
				txn.getTimeout() == 5000);
		txn = coordinator.createTransaction(ScheduledTask.UNBOUNDED)
				.getTransaction();
		assertTrue(
				"Incorrect unbounded Transaction timeout: " + txn.getTimeout(),
	...
	public void testJoinOtherThread() throws Exception {
		final AtomicReference<RuntimeException> exception = new AtomicReference<RuntimeException>(
				null);
		final TransactionParticipant participant = new DummyTransactionParticipant();
		Thread thread = new Thread() {
			public void run() {
				try {
					txn.join(participant);
	...
	public void testAbortFails() throws Exception {
		...
		for (DummyTransactionParticipant participant : participants) {
			if (!participant.prepareReturnedTrue()) {
				assertEquals((participant == participants[1] ? State.ACTIVE
						: State.ABORTED), participant.getState());
			}
		}
		assertAborted(abortCause);
	...
	public void testRegisterListenerCommit() throws Exception {
		DummyTransactionListener[] listeners = {
				new DummyTransactionListener(), new DummyTransactionListener(),
	...
	public void testRegisterListenerCommitBeforeThrows() throws Exception {
		...
		txn.registerListener(listener);
		DummyTransactionListener failingListener = new DummyTransactionListener(
				exception, null);
		...
		/*
		 * Don't know which listener's beforeCompletion method was called first,
		 * so don't check if this one's was called.
		 */
	...
	public void testRegisterListenerAborting() {
		final DummyTransactionListener listener = new DummyTransactionListener();
	...
	public void testRegisterListenerBeforeCompletion() throws Exception {
		final DummyTransactionListener lateListener = new DummyTransactionListener();
		DummyTransactionListener[] listeners = {
				new DummyTransactionListener(), new DummyTransactionListener() {
					public void beforeCompletion() {
						super.beforeCompletion();
	...
	public void testRegisterListenerAfterCompletion() throws Exception {
		...
		final DummyTransactionListener listener = new DummyTransactionListener() {
			public void afterCompletion(boolean commited) {
				...
				DummyTransactionListener anotherListener = new DummyTransactionListener();
				try {
		...
		Thread thread = new Thread() {
			public void run() {
				try {
					...
					int h = txn.hashCode();
					if (hashCode != h) {
						throw new RuntimeException("Expected hash code of "
								+ hashCode + ", got " + h);
					}
				} catch (RuntimeException e) {
					e.printStackTrace();

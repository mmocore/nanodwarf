...
public class TestTaskSchedulerImpl {
	...
	private static final KernelRunnable testTask = new TestAbstractKernelRunnable() {
	...
	public void startup() throws Exception {
		...
		Properties properties = SgsTestNode.getDefaultProperties(
				"TestTaskSchedulerImpl", null, null);
		...
		taskScheduler = serverNode.getSystemRegistry().getComponent(
				TaskScheduler.class);
	...
	public void useReservedTask() throws Exception {
		TaskReservation reservation = taskScheduler.reserveTask(
				new IncrementRunner(), taskOwner);
		reservation.use();
		Thread.sleep(200L);
	...
	public void reuseReservedTask() throws Exception {
		TaskReservation reservation = taskScheduler.reserveTask(testTask,
				taskOwner);
		reservation.use();
		reservation.use();
	}
	...
	public void reuseReservedTaskDelayed() throws Exception {
		TaskReservation reservation = taskScheduler.reserveTask(testTask,
				taskOwner, System.currentTimeMillis() + 50);
		reservation.use();
		reservation.use();
	}
	...
	public void cancelAfterStartRecurringTask() throws Exception {
		RecurringTaskHandle handle = taskScheduler.scheduleRecurringTask(
				new IncrementRunner(), taskOwner,
				System.currentTimeMillis() + 50, 50);
		handle.start();
		handle.cancel();
		Thread.sleep(200L);
		assertEquals(0, taskCount);
	...
	public void startSleepAndCancelRecurringTask() throws Exception {
		RecurringTaskHandle handle = taskScheduler.scheduleRecurringTask(
				new IncrementRunner(), taskOwner, System.currentTimeMillis(),
				200);
		handle.start();
		Thread.sleep(300L);
	...
	public void restartRecurringTask() throws Exception {
		RecurringTaskHandle handle = taskScheduler.scheduleRecurringTask(
				testTask, taskOwner, System.currentTimeMillis(), 100);
		handle.start();
		try {
			handle.start();
		} finally {

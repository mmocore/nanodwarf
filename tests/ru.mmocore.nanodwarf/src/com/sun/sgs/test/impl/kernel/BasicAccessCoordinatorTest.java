...
public abstract class BasicAccessCoordinatorTest<T extends AccessCoordinatorHandle>
		extends Assert {
	/** The transaction proxy, for creating transactions. */
	protected static final DummyTransactionProxy txnProxy = new DummyTransactionProxy();

	/** A profile collector, for reporting accesses. */
	protected static final DummyProfileCollectorHandle profileCollector = new DummyProfileCollectorHandle();
	...
	public void testRegisterAccessSourceSuccess() {
		AccessReporter<Integer> reporter = coordinator.registerAccessSource(
				"test", Integer.class);
	...
	@Test
	public void testReportObjectAccessWithTxnAndNonNullDesc() throws Exception {
		reporter.reportObjectAccess(txn, "o1", AccessType.READ, "desc");
		txn.commit();
		txn = null;
		assertObjectDetails(profileCollector.getAccessedObjectsDetail(), "s",
				"o1", AccessType.READ, "desc");
	}
	...
	public void testReportObjectAccessDescription() throws Exception {
		...
		txn = null;
		assertObjectDetails(profileCollector.getAccessedObjectsDetail(), "s",
				"o1", AccessType.READ, "object 1 (a)", "s", "o1",
				AccessType.WRITE, "object 1 (a)", "s", "o2", AccessType.WRITE,
				"object 2", "s", "o3", AccessType.READ, null, "s", "o4",
				AccessType.READ, null);
	}
	...
	public void testReportObjectAccessMultipleReporters() {
		...
		AccessReporter<String> reporterPrime = coordinator
				.registerAccessSource("s", String.class);
		AccessReporter<String> reporter2 = coordinator.registerAccessSource(
				"s2", String.class);
		AccessReporter<String> reporter2Prime = coordinator
				.registerAccessSource("s2", String.class);
		reporter.reportObjectAccess("o1", AccessType.READ);
	...
	public void testReportObjectAccessMisc() throws Exception {
		...
		txn = null;
		assertObjectDetails(profileCollector.getAccessedObjectsDetail(), "s",
				"read1", AccessType.READ, null, "s2", "read1", AccessType.READ,
				null, "s", "upgrade", AccessType.READ, null, "s", "write1",
				AccessType.WRITE, null, "s", "upgrade", AccessType.WRITE, null);
	}
	...
	public void testSetObjectDescriptionMultipleAccesses() throws Exception {
		...
		txn = null;
		assertObjectDetails(profileCollector.getAccessedObjectsDetail(), "s",
				"o1", AccessType.READ, "desc1", "s", "o2", AccessType.READ,
				"desc2", "s", "o3", AccessType.READ, "desc3");
	}
	...
	public void testSetObjectDescriptionMisc() throws Exception {
		...
		txn = null;
		assertObjectDetails(profileCollector.getAccessedObjectsDetail(), "s",
				"o1", AccessType.READ, "object 1 (a)", "s", "o2",
				AccessType.READ, "object 2 (a)", "s", "o3", AccessType.READ,
				"object 3 (a)");
	}
	...
	public void testAccessedObjectsDetailNone() throws Exception {
		txn.commit();
		txn = null;
		AccessedObjectsDetail detail = profileCollector
	...
	public void testAccessedObjectsDetailNoConflict() throws Exception {
		...
		AccessedObjectsDetail detail = profileCollector
				.getAccessedObjectsDetail();
		assertObjectDetails(detail, "s", "o1", AccessType.READ, null, "s",
				"o2", AccessType.WRITE, null);
		assertEquals(ConflictType.NONE, detail.getConflictType());
		assertEquals(null, detail.getConflictingId());
	}
	 ...
	 * argument should provide groups of 4 items: the source, the object ID, the
	 * access type, and the description.
	 */
	protected static void assertObjectDetails(AccessedObjectsDetail detail,
			Object... expected) {
		...
		assertTrue(
				"Expected " + numExpected + " accesses, found "
						+ accesses.size(), numExpected == accesses.size());

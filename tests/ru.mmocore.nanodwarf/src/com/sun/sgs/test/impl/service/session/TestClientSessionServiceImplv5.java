...
import com.sun.sgs.test.impl.service.session.TestClientSessionServiceImplv4.DummyClient;
...
public class TestClientSessionServiceImplv5 extends
		TestClientSessionServiceImplv4 {
	...
	@Test
	public void testRelocateWithRedirect() throws Exception {
		String host2 = "host2";
		String host3 = "host3";
		...
		waitForExpectedObjectCount(objectCount - MANAGED_OBJECTS_PER_SESSION);
		checkBindings(0);
		client.assertDisconnectedCallbackInvoked(false);
	}

	@Test
	public void testOldNodeFailsDuringRelocateToNewNode() throws Exception {
		String newNodeHost = "newNode";
		DummyClient client = createClientToRelocate(newNodeHost);
		int objectCount = getObjectCount();
		checkBindings(1);
	...
	@Test
	public void testNewNodeFailsDuringRelocateToNewNode() throws Exception {
		String newNodeHost = "newNode";
		DummyClient client = createClientToRelocate(newNodeHost);
		checkBindings(1);
		try {
	 ...
	 * </ul>
	 */
	private DummyClient createClientAndReassignIdentity(String newNodeHost,
			boolean setWaitForSuspendMessages) throws Exception {
		...
		System.err.println("reassigning identity:" + name
				+ " from server node to host: " + newNodeHost);
		identityAssigner.moveIdentity(name, serverNode.getNodeId(),
				newNode.getNodeId());
	...
	/**
	 * Waits for the object count to match the {@code expectedCount}, and throws
	 * an AssertionFailedException if the object count does not match the
	 * {@code expectedCount} after the wait time has expired.
	 */
	private void waitForExpectedObjectCount(int expectedCount) throws Exception {
		int objectCount = 0;
		for (int i = 0; i < 10; i++) {

...
/**
 * Tests for profile data that will be run with data of type {@code AGGREGATE}
 * and {@code TASK_AND_AGGREGATE}.
 */
...
public class TestProfileDataAggregateImpl {
	...
	public void testAggregateProfileCounter() throws Exception {
		...
		final AggregateProfileCounter counter = (AggregateProfileCounter) cons1
				.createCounter(name, testType, ProfileLevel.MIN);
	...
	public void testAggregateProfileOperation() throws Exception {
		...
		ProfileCollector collector = getCollector(serverNode);
		ProfileConsumer cons1 = collector.getConsumer("c1");
		final AggregateProfileOperation op = (AggregateProfileOperation) cons1
				.createOperation(name, testType, ProfileLevel.MIN);
	...
	public void testAggregateProfileNegSmoothing() throws Exception {
		...
		final AggregateProfileSample samp = (AggregateProfileSample) cons1
				.createSample("foo", testType, ProfileLevel.MIN);
	...
	public void testSetSmoothingFactor() throws Exception {
		...
		ProfileConsumer cons1 = collector.getConsumer("c1");
		final AggregateProfileSample samp = (AggregateProfileSample) cons1
				.createSample("bar", testType, ProfileLevel.MIN);
		double smooth = 0.5;
	...
	public void testAggregateProfileSample() throws Exception {
		...
		ProfileConsumer cons1 = collector.getConsumer("c1");
		final AggregateProfileSample samp = (AggregateProfileSample) cons1
				.createSample(name, testType, ProfileLevel.MIN);
	...
	public void testAggregateProfileSampleCapacity() throws Exception {
		ProfileCollector collector = getCollector(serverNode);
		ProfileConsumer cons1 = collector.getConsumer("c1");
		final AggregateProfileSample samp1 = (AggregateProfileSample) cons1
				.createSample("s1", testType, ProfileLevel.MIN);
		// default capacity is zero
		assertEquals(0, samp1.getCapacity());

		final AggregateProfileSample samp2 = (AggregateProfileSample) cons1
				.createSample("s2", testType, ProfileLevel.MIN);
		samp2.setCapacity(5);
	...
	private void testStatistics(AggregateProfileSample samp, int expectedSize,
			long expectedMin, long expectedMax, long[] testData) {
		...
		System.err.println("Sample data:  " + "size: " + currSize + " avg: "
				+ currAvg + " min: " + currMin + " max: " + currMax);

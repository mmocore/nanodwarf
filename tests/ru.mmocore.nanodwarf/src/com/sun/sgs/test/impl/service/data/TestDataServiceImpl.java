...
import static com.sun.sgs.impl.service.transaction.TransactionCoordinator.TXN_TIMEOUT_PROPERTY;
import static com.sun.sgs.impl.service.transaction.TransactionCoordinatorImpl.BOUNDED_TIMEOUT_DEFAULT;
...
public class TestDataServiceImpl extends Assert {
	...
	private static final String DataStoreImplClassName = DataStoreImpl.class
			.getName();

	/** The name of the DataServiceImpl class. */
	protected static final String DataServiceImplClassName = DataServiceImpl.class
			.getName();
	...
	/**
	 * Boolean to say we'll *always* need a new node for each run; this allows
	 * us to ensure that each test is independent. We don't run in this mode
	 * normally for speed (it's quite slow to always recreate the data store)
	 * and because it's helpful to have random data in the data store from test
	 * to test.
	 * <p>
	 * However, it's useful to be able to sometimes ensure that the tests are
	 * really independent.
	 * <p>
	 * This boolean overrides cleanup.
	 * <p>
	 * NOTE: right now this is final, would be nice to be able to pick it up
	 * from the system properties eventually.
	 */
	...
	/**
	 * Boolean to say if we should run with transaction that disable the
	 * prepareAndCommit optimization (in which the last participant prepared has
	 * prepareAndCommit called, rather than prepare, and at a later point
	 * commit).
	 */
	 ...
	 * 
	 * @param disableTxnCommitOpt
	 *            if {@true}, don't call prepareAndCommit on the last
	 *            transaction participant to be commited. This parameter is set
	 *            by the parameterized test runner, using the {@link data}
	 *            method.
	 */
	...
	/**
	 * Create a new SgsTestNode for use by this test if {@link serverNode} is
	 * {@code null}. Note that most tests use the same data store (it is not
	 * recreated for each run) unless {@link alwaysInitializeServerNode} is set
	 * to true.
	 * 
	 * @param properties
	 *            the properties used to initialize a {@link SgsTestNode}, or
	 *            {@code null} to use the default properties returned by
	 *            {@link getProperties}
	 * @param clean
	 *            {@code true} if the data store should be freshly created
	 * 
	 */
	protected void setUp(Properties properties, boolean clean) throws Exception {
		...
		if (serverNode == null) {
			...
			txnScheduler = componentRegistry
	...
	/**
	 * Utility method for shutting down a server node and restarting it, and
	 * arrange for the node to be restarted for the next test run.
	 */
	...
	public void testConstructorNullArgs() throws Exception {
		Properties props = SgsTestNode.getDefaultProperties(APP_NAME, null,
				null);
		try {
	...
	public void testConstructorBadDebugCheckInterval() throws Exception {
		Properties props = SgsTestNode.getDefaultProperties(APP_NAME, null,
				null);
		props.setProperty(DataServiceImplClassName + ".debug.check.interval",
				"gorp");
		try {
			createDataServiceImpl(props, componentRegistry, txnProxy);
			fail("Expected IllegalArgumentException");
	...
	public void testConstructorNoDirectory() throws Exception {
		...
		DataServiceImpl testSvc = createDataServiceImpl(props,
	...
	private static class DataStoreConstructorException extends RuntimeException {
	...
	private void testGetBindingEmptyName(final boolean app) throws Exception {
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				DummyManagedObject result = (DummyManagedObject) getBinding(
						app, service, "");
	...
	private void testGetBindingObjectNotFound(final boolean app)
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				service.removeObject(getBinding(app, service,
						"testGetBindingRemoved"));
				try {
					getBinding(app, service, "testGetBindingRemoved");
					fail("Expected ObjectNotFoundException");
				} catch (ObjectNotFoundException e) {
	...
	private final Action getServiceBinding = new Action() {
		void setUp() {
			service.setServiceBinding("dummy", dummy);
		}
	...
	@Test
	public void testGetServiceBindingShuttingDownExistingTxn() throws Exception {
	...
	public void testGetBindingsDifferent() throws Exception {
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				DummyManagedObject result = (DummyManagedObject) service
						.getBinding("dummy");
				assertEquals(dummy, result);
				result = (DummyManagedObject) service
						.getServiceBinding("dummy");
	...
	private void testGetBindingTimeout(final boolean app) throws Exception {
		final long timeout = Long.getLong(TXN_TIMEOUT_PROPERTY,
	...
	private void testGetBindingForUpdateEmptyName(final boolean app)
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				DummyManagedObject result = (DummyManagedObject) getBindingForUpdate(
						app, service, "");
				assertEquals(dummy, result);
			}
		}, taskOwner);
	...
	private void testGetBindingForUpdateNotFound(final boolean app)
		...
		txnScheduler.runTask(new InitialTestRunnable() {
			public void run() throws Exception {
				super.run();
				try {
					getBindingForUpdate(app, service,
							"testGetBindingForUpdateNotFound");
					fail("Expected NameNotBoundException");
				} catch (NameNotBoundException e) {
					System.err.println(e);
				}
	...
	private void testGetBindingForUpdateObjectNotFound(final boolean app)
		...
		txnScheduler.runTask(new InitialTestRunnable() {
			public void run() throws Exception {
				...
				try {
					getBindingForUpdate(app, service,
							"testGetBindingForUpdateRemoved");
					fail("Expected ObjectNotFoundException");
				} catch (ObjectNotFoundException e) {
					System.err.println(e);
				}
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				service.removeObject(getBindingForUpdate(app, service,
						"testGetBindingForUpdateRemoved"));
				try {
					getBindingForUpdate(app, service,
							"testGetBindingForUpdateRemoved");
	...
	@Test
	public void testGetBindingForUpdateDeserializationFails() throws Exception {
	...
	public void testGetBindingForUpdatesDifferent() throws Exception {
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				DummyManagedObject result = (DummyManagedObject) service
						.getBindingForUpdate("dummy");
				assertEquals(dummy, result);
				result = (DummyManagedObject) service
						.getServiceBindingForUpdate("dummy");
				assertEquals(serviceDummy, result);
			}
		}, taskOwner);
	}
	...
	public void testGetBindingForUpdateLocking() throws Exception {
		 ...
		 * changing the lock timeout for an existing database. -tjb@sun.com
		 * (07/22/2008)
		 */
	...
	private void testSetBindingStaleObject(final boolean app) throws Exception {
		Properties properties = getProperties();
		properties.setProperty(DataServiceImpl.TRACK_STALE_OBJECTS_PROPERTY,
				"true");
		serverNodeRestart(properties, true);
	...
	@Test
	public void testSetServiceBindingShuttingDownExistingTxn() throws Exception {
	...
	private void testSetBindingManagedObjectNoReference(final boolean app)
		...
		try {
			txnScheduler.runTask(new TestAbstractKernelRunnable() {
				public void run() {
					dummy.setValue(new Object[] {
							null,
							new Integer(3),
							new DummyManagedObject[] { null,
									new DummyManagedObject() } });
					setBinding(app, service, "dummy", dummy);
				}
			}, taskOwner);
	...
	private void testSetBindingManagedObjectNotSerializableCommit(
			final boolean app) throws Exception {
		try {
			txnScheduler.runTask(new InitialTestRunnable() {
				public void run() throws Exception {
					super.run();
		...
		try {
			txnScheduler.runTask(new TestAbstractKernelRunnable() {
				public void run() {
					dummy.setValue(new Object[] { null, new Integer(3),
							new Thread[] { null, Thread.currentThread() } });
					setBinding(app, service, "dummy", dummy);
				}
			}, taskOwner);
			fail("Expected ObjectIOException");
	...
	private void testRemoveBindingNullName(final boolean app) throws Exception {
		txnScheduler.runTask(new InitialTestRunnable() {
			public void run() throws Exception {
				super.run();
				try {
	...
	private void testRemoveBindingEmptyName(final boolean app) throws Exception {
		txnScheduler.runTask(new InitialTestRunnable() {
			public void run() throws Exception {
				super.run();
				setBinding(app, service, "", dummy);
	...
	@Test
	public void testRemoveServiceBindingDeserializationFails() throws Exception {
	...
	public void testRemoveBindingsDifferent() throws Exception {
		...
		try {
			txnScheduler.runTask(new TestAbstractKernelRunnable() {
				public void run() {
					service.removeBinding("dummy");
					DummyManagedObject serviceResult = (DummyManagedObject) service
							.getServiceBinding("dummy");
	...
	public void testNextBoundNameNotFound() throws Exception {
		txnScheduler.runTask(new InitialTestRunnable() {
			public void run() throws Exception {
				super.run();
				for (String name = null; (name = service.nextBoundName(name)) != null;) {
	...
	private final Action nextBoundName = new Action() {
		void run() {
			service.nextBoundName(null);
		}
	};
	private final Action nextServiceBoundName = new Action() {
		void run() {
			service.nextServiceBoundName(null);
		}
	};
	...
	private void testNextBoundNameModify(final boolean app) throws Exception {
		txnScheduler.runTask(new InitialTestRunnable() {
			public void run() throws Exception {
				super.run();
				for (String name = "zzz-1"; (name = service.nextBoundName(name)) != null;) {
					service.removeBinding(name);
				}
				setBinding(app, service, "zzz-1", dummy);
				assertEquals("zzz-1", nextBoundName(app, service, "zzz-"));
	...
	public void testNextBoundNameDifferent() throws Exception {
		txnScheduler.runTask(new InitialTestRunnable() {
			public void run() throws Exception {
				...
				for (String name = null; (name = service
						.nextServiceBoundName(name)) != null;) {
	...
	public void testRemoveObjectRemoval() throws Exception {
		class TestTask extends InitialTestRunnable {
			...
			public void run() throws Exception {
				...
				assertTrue("Should call removingObject for transient objects",
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				ObjectWithRemoval removal = (ObjectWithRemoval) service
						.getBinding("removal");
				service.removeObject(removal);
	...
	public void testRemoveObjectRemovalRecurse() throws Exception {
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				ObjectWithRemoval x = (ObjectWithRemoval) service
						.getBinding("x");
				try {
	...
	public void testMarkForUpdateLocking() throws Exception {
		...
		txnScheduler.scheduleTask(new TestAbstractKernelRunnable() {
			public void run() throws Exception {
				try {
					...
					assertFalse(completedMarkForUpdate.await(100,
		...
		txnScheduler.scheduleTask(new TestAbstractKernelRunnable() {
			public void run() throws Exception {
				try {
					DummyManagedObject dummy2 = (DummyManagedObject) service
							.getBinding("dummy");
	...
	public void testCreateReferenceNew() throws Exception {
		txnScheduler.runTask(new InitialTestRunnable() {
			public void run() throws Exception {
				super.run();
				ManagedReference<DummyManagedObject> ref = service
						.createReference(dummy);
	...
	public void testCreateReferenceExisting() throws Exception {
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				DummyManagedObject dummy = (DummyManagedObject) service
						.getBinding("dummy");
				ManagedReference<DummyManagedObject> ref = service
						.createReference(dummy);
				assertEquals(dummy, ref.get());
	...
	public void testCreateReferenceTwoObjects() throws Exception {
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				...
				assertFalse(service.createReference(x).equals(
	...
	public void testGetObjectIdTwoObjects() throws Exception {
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				DummyManagedObject x = new DummyManagedObject();
				DummyManagedObject y = new DummyManagedObject();
				assertFalse(service.getObjectId(x).equals(
	...
	public void testCreateReferenceForIdTooBigId() throws Exception {
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				BigInteger maxLong = new BigInteger(String
	...
	public void testCreateReferenceForIdSuccess() throws Exception {
		class TestTask extends InitialTestRunnable {
			...
			public void run() throws Exception {
				super.run();
				id = service.createReference(dummy).getId();
				ManagedReference<DummyManagedObject> ref = uncheckedCast(service
						.createReferenceForId(id));
	...
	public void testNextObjectIdIllegalIds() throws Exception {
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				BigInteger id = BigInteger.valueOf(Long.MIN_VALUE).subtract(
						BigInteger.ONE);
				try {
	...
	public void testNextObjectIdBoundaryIds() throws Exception {
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				...
				assertEquals(null, service.nextObjectId(BigInteger
						.valueOf(Long.MAX_VALUE)));
			}
		}, taskOwner);
	}
	...
	public void testNextObjectIdRemoved() throws Exception {
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				...
				while (true) {
					id = service.nextObjectId(id);
					assertNotNull("Didn't find dummy2Id after removed dummyId",
							id);
	...
	public void testGetReferenceStaleObject() throws Exception {
		...
		properties.setProperty(DataServiceImpl.TRACK_STALE_OBJECTS_PROPERTY,
				"true");
		serverNodeRestart(properties, true);
		final AtomicReference<ManagedReference<DummyManagedObject>> ref = new AtomicReference<ManagedReference<DummyManagedObject>>();
		txnScheduler.runTask(new InitialTestRunnable() {
			public void run() throws Exception {
				super.run();
	...
	public void testGetReferenceTimeout() throws Exception {
		...
		try {
			txnScheduler.runTask(new TestAbstractKernelRunnable() {
				public void run() throws Exception {
					try {
						dummy = (DummyManagedObject) service
								.getBinding("dummy");
						Thread.sleep(200);
						dummy.getNext();
						fail("Expected TransactionTimeoutException");
			...
			txnScheduler.runTask(new TestAbstractKernelRunnable() {
				public void run() {
					try {
						...
						DeserializationDelayed dummy = (DeserializationDelayed) service
								.getBinding("dummy");
	...
	/**
	 * Test detecting managed objects with readResolve and writeReplace methods.
	 */
	@Test
	...
	static class MOPublicReadResolve extends PublicReadResolve implements
			ManagedObject, Serializable {
		private static final long serialVersionUID = 0;
	}

	static class MOPublicWriteReplace extends PublicWriteReplace implements
			ManagedObject, Serializable {
		private static final long serialVersionUID = 0;
	}

	static class MOPublicReadResolveHere implements ManagedObject, Serializable {
		private static final long serialVersionUID = 0;
	...
	static class MOPublicWriteReplaceHere implements ManagedObject,
			Serializable {
		private static final long serialVersionUID = 0;
	...
	static class MOProtectedReadResolve extends ProtectedReadResolve implements
			ManagedObject, Serializable {
		private static final long serialVersionUID = 0;
	}
	...
	static class MOProtectedReadResolveHere implements ManagedObject,
			Serializable {
		private static final long serialVersionUID = 0;
	...
	static class MOProtectedWriteReplaceHere implements ManagedObject,
			Serializable {
		private static final long serialVersionUID = 0;
	...
	static class MOPackageReadResolve extends PackageReadResolve implements
			ManagedObject, Serializable {
		private static final long serialVersionUID = 0;
	}

	static class MOPackageWriteReplace extends PackageWriteReplace implements
			ManagedObject, Serializable {
		private static final long serialVersionUID = 0;
	}

	static class MOPackageReadResolveHere implements ManagedObject,
			Serializable {
		private static final long serialVersionUID = 0;
	...
	static class MOPackageWriteReplaceHere implements ManagedObject,
			Serializable {
		private static final long serialVersionUID = 0;
	...
	static class MOPrivateReadResolve extends PrivateReadResolve implements
			ManagedObject, Serializable {
		private static final long serialVersionUID = 0;
	}

	static class MOPrivateWriteReplace extends PrivateWriteReplace implements
			ManagedObject, Serializable {
		private static final long serialVersionUID = 0;
	}

	static class MOPrivateReadResolveHere implements ManagedObject,
			Serializable {
		private static final long serialVersionUID = 0;
	...
	static class MOPrivateWriteReplaceHere implements ManagedObject,
			Serializable {
		private static final long serialVersionUID = 0;
	...
	static class MOStaticWriteReplace implements ManagedObject, Serializable {
		...
		public static Object writeReplace() {
			return null;
		}
	}

	static class MOReadResolveWrongReturn implements ManagedObject,
			Serializable {
		private static final long serialVersionUID = 0;

		public static String readResolve() {
			return "hi";
		}
	}

	static class MOWriteReplaceWrongReturn implements ManagedObject,
			Serializable {
		private static final long serialVersionUID = 0;
	...
	static class AbstractReadResolveField implements ManagedObject,
			Serializable {
		private static final long serialVersionUID = 0;
		Class<?> cl = AbstractReadResolve.class;
	}

	abstract static class AbstractReadResolve implements ManagedObject,
			Serializable {
		private static final long serialVersionUID = 0;
	...
	static class AbstractWriteReplaceField implements ManagedObject,
			Serializable {
		private static final long serialVersionUID = 0;
		Class<?> cl = AbstractWriteReplace.class;
	}

	abstract static class AbstractWriteReplace implements ManagedObject,
			Serializable {
		private static final long serialVersionUID = 0;
		...
		txnScheduler.scheduleTask(new TestAbstractKernelRunnable() {
			public void run() throws Exception {
				try {
					dummy = (DummyManagedObject) service.getBinding("dummy");
					dummy.getNext();
					assertTrue(threadFlag
							.tryAcquire(100, TimeUnit.MILLISECONDS));
					mainFlag.release();
					assertFalse(threadFlag.tryAcquire(100,
							TimeUnit.MILLISECONDS));
					doneFlag.release();
				} catch (Throwable t) {
	...
	public void testReferenceEquals() throws Exception {
		class TestTask extends InitialTestRunnable {
			...
			public void run() throws Exception {
				super.run();
				final ManagedReference<DummyManagedObject> ref = service
						.createReference(dummy);
				reference = ref;
				...
				DummyManagedObject dummy2 = new DummyManagedObject();
				ManagedReference<DummyManagedObject> ref2 = service
						.createReference(dummy2);
				assertFalse(ref.equals(ref2));
				ManagedReference<ManagedObject> ref3 = new ManagedReference<ManagedObject>() {
					public ManagedObject get() {
						return null;
					}

					public ManagedObject getForUpdate() {
						return null;
					}
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				dummy = (DummyManagedObject) service.getBinding("dummy");
				ManagedReference<DummyManagedObject> ref4 = service
						.createReference(dummy);
	...
	static class NonManaged extends FailingMethods implements Serializable {
		...
		static {
			class StaticLocal extends FailingMethods implements ManagedObject,
					Serializable {
				private static final long serialVersionUID = 1;
			}
			staticLocal = new StaticLocal();
		}
		static final ManagedObject staticAnonymous = new DummyManagedObjectFailingMethods() {
		...
		static class Member extends FailingMethods implements ManagedObject,
				Serializable {
			private static final long serialVersionUID = 1;
		}
		...
		class Inner extends FailingMethods implements ManagedObject,
				Serializable {
			private static final long serialVersionUID = 1;
		}
		...
		ManagedObject createLocal() {
			class Local extends FailingMethods implements ManagedObject,
					Serializable {
				private static final long serialVersionUID = 1;
			}
			return new Local();
	...
	static class Managed extends FailingMethods implements ManagedObject,
			Serializable {
		private static final long serialVersionUID = 1;
		static final ManagedObject staticLocal;
		static {
	...
	/**
	 * Test what happens if a we store a managed object in the data service that
	 * has a writeObject method that attempts to dereference a managed
	...
	public void testSerializeWriteObjectMethodCallsDataManager()
		...
		try {
			txnScheduler.runTask(new TestAbstractKernelRunnable() {
				public void run() {
					service.setBinding("a",
							new WriteObjectMethodCallsDataManager());
				}
			}, taskOwner);
			fail("Expected TransactionNotActiveException");
		} catch (TransactionNotActiveException e) {
	 ...
	 * reference.
	 */
	private static class WriteObjectMethodCallsDataManager implements
			ManagedObject, Serializable {
		private static final long serialVersionUID = 1;
	...
	/**
	 * Test what happens if a we store a managed object in the data service that
	 * has a readObject method that attempts to dereference a managed reference.
	 * This should work!
	 */
	@Test
	public void testSerializeReadObjectMethodCallsDataManager()
		...
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				service.setBinding("a", new ReadObjectMethodCallsDataManager());
			}
		}, taskOwner);
		txnScheduler.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				service.getBinding("a");
			}
		}, taskOwner);
	}
	 ...
	 * reference.
	 */
	private static class ReadObjectMethodCallsDataManager implements
			ManagedObject, Serializable {
		private static final long serialVersionUID = 1;
		private final ManagedReference<DummyManagedObject> dummy;
	 ...
	 */
	@Test
	public void testSerializeMissingSuperclassConstructor() throws Exception {
	 ...
	 */
	@Test
	public void testSerializePrivateSuperclassConstructor() throws Exception {
		DummyManagedObject dummy = new DummyManagedObject();
	...
	static class WrongPackageSuperclassConstructor extends
			PackageSuperclassConstructor implements ManagedObject, Serializable {
		private static final long serialVersionUID = 1;
	...
	static class ProtectedSuperclassConstructor extends ProtectedConstructor
			implements ManagedObject, Serializable {
		private static final long serialVersionUID = 1;
	 ...
	 */
	@Test
	public void testPublicWrongPackageSuperclassConstructor() throws Exception {
		okOnCommit(new PublicSuperclassConstructor());
	}

	static class PublicSuperclassConstructor extends PublicConstructor
			implements ManagedObject, Serializable {
		private static final long serialVersionUID = 1;
	...
	public void testNonInstantiatedClassProblems() throws Exception {
		final Object[] objects = { MissingSuperclassConstructor.class,
	...
	public void testDeadlock() throws Exception {
		...
		abstract class TestTask extends TestAbstractKernelRunnable {
			...
			TestTask(int taskNumber, int runNumber,
					CountDownLatch firstStepDone, CountDownLatch taskDone) {
			...
			void print(Object message) {
				System.err.println("pass " + runNumber + ", task " + taskNumber
						+ ",  " + txn + ":\n  " + message);
			}
		...
		class TestTask1 extends TestTask {
			TestTask1(int runNumber, CountDownLatch firstStepDone,
			...
			void runInternal() throws Exception {
				...
				((DummyManagedObject) service.getBinding("dummy2"))
						.setValue(runNumber);
				print("commit");
			}
		}
		...
		class TestTask2 extends TestTask {
			TestTask2(int runNumber, CountDownLatch firstStepDone,
					CountDownLatch taskDone) {
	...
	ManagedObject getBinding(boolean app, DataService service, String name) {
		return app ? service.getBinding(name) : service.getServiceBinding(name);
	}

	ManagedObject getBindingForUpdate(boolean app, DataService service,
			String name) {
		return app ? service.getBindingForUpdate(name) : service
				.getServiceBindingForUpdate(name);
	}

	void setBinding(boolean app, DataService service, String name, Object object) {
		if (app) {
	 ...
	 */
	protected DataServiceImpl createDataServiceImpl(Properties props,
			ComponentRegistry componentRegistry, TransactionProxy txnProxy)
			throws Exception {
		File dir = new File(getDbDirectory());
		if (!dir.exists()) {
			if (!dir.mkdir()) {
				throw new RuntimeException("Problem creating directory: " + dir);
			}
		}
	...
	protected Properties getProperties() throws Exception {
		...
		p.setProperty(DataServiceImplClassName + ".debug.check.interval", "0");
		p.setProperty(
		...
		try {
			txnScheduler.runTask(new InitialTestRunnable() {
				public void run() throws Exception {
					super.run();
					action.setUp();
					class Participant extends
							DummyNonDurableTransactionParticipant {
						boolean ok;
	...
	/**
	 * Tests running the action in a TransactionListener.beforeCompletion method
	 * that is called after running the data service's beforeCompletion method.
	 * This test depends on the fact that the transaction implementation calls
	 * transaction listeners in the order in which they are registered.
	 */
	private void testBeforeCompletion(final Action action) throws Exception {
		...
		txnScheduler.runTask(new InitialTestRunnable() {
			public void run() throws Exception {
				super.run();
				action.setUp();
				txnProxy.getCurrentTransaction().registerListener(listener);
			}
		}, taskOwner);
	...
	/**
	 * The exception thrown when we abort a transaction; this gives us a type to
	 * test against when catching exceptions thrown by runTask. This is NOT a
	 * retryable exception.
	 */
	...
	static class DummyDataStore implements DataStore {
		...
		public void setObjectDescription(Transaction txn, long oid,
				Object description) {
		}

		public void setBindingDescription(Transaction txn, String name,
				Object description) {
		}
	}
	 ...
	 */
	static class ObjectWithRemoval extends DummyManagedObject implements

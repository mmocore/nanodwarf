...
public class TestDataStoreServerImpl extends TestCase {
	...
	private static String dbDirectory = System.getProperty("java.io.tmpdir")
			+ File.separator + "TestDataStoreImpl.db";
	...
	protected void setUp() throws Exception {
		...
		props = createProperties(DataStoreImplClassName + ".directory",
				dbDirectory, DataStoreNetPackage + ".server.port", "0");
	...
	DataStoreServerImpl getDataStoreServer() throws Exception {
		return new DataStoreServerImpl(props, env.systemRegistry, env.txnProxy);
	}
	...
	public void testSetObjectsConcurrent() throws Exception {
		testConcurrent(new Runnable() {
			public void run() {
				server.setObjects(tid, new long[] { oid },
						new byte[][] { { 0 } });
			}
		});
	}
	...
	public void testReaperConcurrency() throws Exception {
		...
		server = getDataStoreServer();
		List<TestReaperConcurrencyThread> threads = new ArrayList<TestReaperConcurrencyThread>();
		for (int i = 0; i < 5; i++) {
	...
	private static class TestReaperConcurrencyThread extends Thread {
		...
		public void run() {
			try {
				...
				System.err.println(getName() + ": succeeds:" + succeeds
						+ ", aborted:" + aborted + ", notActive:" + notActive);
			} catch (Throwable t) {
				result = t;
			} finally {
				synchronized (this) {
	...
	public void testNegativeTxnIds() {
		...
		new AssertThrowsIllegalArgumentException() {
			void run() {
				server.setObjects(-4, new long[] { oid },
						new byte[][] { new byte[0] });
			}
		};
		new AssertThrowsIllegalArgumentException() {
	...
	/**
	 * Test that the maximum transaction timeout overrides the standard timeout.
	 */
	...
	/**
	 * Test that the standard transaction timeout gets applied by the reaper.
	 */
	...
	/**
	 * Test that the action throws IllegalStateException if called while another
	 * thread is blocked calling on the same transaction.
	 */

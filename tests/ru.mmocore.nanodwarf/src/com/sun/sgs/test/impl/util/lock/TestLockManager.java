...
public class TestLockManager extends Assert {
	...
	public void testLockWrongLockManager() {
		LockManager<String> lockManager2 = createLockManager(lockTimeout,
				numKeyMaps);
		try {
	...
	public void testLockGranted() {
		...
		assertGranted(lockManager.lock(locker, "o1", false));
		assertGranted(lockManager.lock(createLocker(lockManager), "o1", false));
	}
	 ...
	 * completes. Note that we don't currently provide an API for doing this in
	 * Darkstar.
	 */
	@Test
	 ...
	 * 
	 * locker2: read o1 => granted locker: write o1 => blocked locker2: commit
	 * locker: => granted
	 */
	@Test
	 ...
	 * 
	 * locker: write o1 => granted locker2: read o1 => blocked locker3: read o1
	 * => blocked locker: commit locker2: => granted locker3: => granted
	 */
	@Test
	 ...
	 * 
	 * locker2: read o1 => granted locker: read o1 => granted locker: write o1
	 * => blocked locker2: commit locker: => granted
	 */
	@Test
	 ...
	 * 
	 * locker: write o1 => granted locker2: write o1 => blocked locker: commit
	 * locker2: => granted
	 */
	@Test
	 ...
	 * 
	 * locker2: read o1 => granted locker3: write o1 => blocked locker: read o1
	 * => blocked locker2: commit locker3: => granted locker3: commit locker: =>
	 * granted
	 */
	@Test
	 ...
	 * granted locker3: => blocked locker2: commit locker3: => granted
	 */
	@Test
	...
	public void testGetOwnersNotLocked() {
		List<LockRequest<String>> owners = lockManager.getOwners("unknownLock");
	...
	public void testGetWaitersNotWaiting() {
		List<LockRequest<String>> waiters = lockManager
				.getOwners("unknownLock");
	...
	/**
	 * Asserts that the request resulted in an interrupt. The conflictingLockers
	...
	void assertInterrupted(LockConflict<String> conflict,
		...
		assertDenied(LockConflictType.INTERRUPTED, conflict, conflictingLockers);
	}
	...
	/**
	 * Asserts that the request was denied, with the specified type of conflict.
	 * The conflictingLockers argument specifies all of the other transactions
	 * that may have been involved in the conflict.
	 */
	void assertDenied(LockConflictType type, LockConflict<String> conflict,
			Locker... conflictingLockers) {
		assertTrue("Expected " + type + ": " + conflict, conflict != null
				&& conflict.getType() == type);
	...
	LockConflict<String> acquireLock(Locker<String> locker, String key,
			boolean forWrite) {
	...
	class AcquireLock implements Callable<LockConflict<String>> {
		...
		/**
		 * Set to true when the initial attempt to acquire the lock is complete,
		 * so we know whether the attempt blocked.
		 */
		...
		public LockConflict<String> call() {
			...
			/*
			 * Don't synchronize on this object while waiting for the lock, to
			 * avoid deadlock.
			 */
			if (wait) {

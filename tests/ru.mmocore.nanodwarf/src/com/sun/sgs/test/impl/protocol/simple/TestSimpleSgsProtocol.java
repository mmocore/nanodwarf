...
public class TestSimpleSgsProtocol {
	...
	public static class DummyTransport implements Transport {
		...
		public DummyTransport(Properties properties) {
			delivery = properties.getProperty("UnreliableDelivery") != null ? Delivery.UNRELIABLE
					: Delivery.RELIABLE;
		...
		private class DummyChannel implements AsynchronousByteChannel {
			...
			DummyChannel() {
				final MessageBuffer msg = new MessageBuffer(4
			...
			public <A> IoFuture<Integer, A> read(ByteBuffer dst, A attachment,
					CompletionHandler<Integer, ? super A> handler) {
				System.err.println("**Read called: buff= " + dst.capacity()
						+ " loggedin= " + loggedIn);
				IoFuture<Integer, A> result;
				if (message != null) {
					dst.put(message);
					result = AttachedFuture.wrap(
							new DummyFuture(message.capacity()), attachment);
					message = null;
					callCompletion(handler, attachment, result);
				} else {
					result = AttachedFuture
							.wrap(new DummyFuture(0), attachment);
			...
			// is what happens in the bowels of the sgs nio implementation,
			// either
			...
			private <R, A> void callCompletion(CompletionHandler<R, A> handler,
				...
				try {
					System.err.println("calling completion size= "
							+ future.get());
				} catch (InterruptedException ex) {
					Logger.getLogger(TestSimpleSgsProtocol.class.getName())

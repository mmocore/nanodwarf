...
public class EchoServer {
	 ...
	 * 
	 * @param group
	 *            the asynchronous channel group for this cserverlient
	 */
	...
	void start() throws IOException {
		...
		int backlog = Integer.valueOf(System.getProperty("accept_backlog",
				DEFAULT_BACKLOG));
		try {
			acceptor = group.provider().openAsynchronousServerSocketChannel(
					group);
	...
	final class AcceptHandler implements
	...
	final class ChannelHandler implements
		...
		void readCompleted(IoFuture<Integer, ByteBuffer> result) {
			try {
				...
				if (rc < 0) {
					log.log(Level.FINE, "{0} read {1} bytes", new Object[] {
							channel, rc });

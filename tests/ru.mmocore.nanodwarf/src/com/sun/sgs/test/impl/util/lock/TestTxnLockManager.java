...
public class TestTxnLockManager extends TestLockManager {
	...
	protected LockManager<String> createLockManager(long lockTimeout,
			int numKeyMaps) {
	...
	TxnLocker<String> createTxnLocker(LockManager<String> lockManager,
		...
		return createTxnLocker(lockManager, new DummyTransaction(),
				requestedStartTime);
	}

	/** Create a TxnLocker. */
	TxnLocker<String> createTxnLocker(LockManager<String> lockManager,
			Transaction txn, long requestedStartTime) {
		return new StringTxnLocker((TxnLockManager<String>) lockManager, txn,
				requestedStartTime);
	}
	...
	private static class StringTxnLocker extends TxnLocker<String> {
		...
		StringTxnLocker(TxnLockManager<String> lockManager, Transaction txn,
				long requestedStartTime) {
	...
	public void testMaxLockTimeout() throws Exception {
		...
		Locker<String> locker2 = createTxnLocker(lockManager,
				new DummyTransaction(40), 0);
	...
	public void testMaxTxnTimeout() throws Exception {
		init(40L, numKeyMaps);
		Locker<String> locker2 = createTxnLocker(lockManager,
				new DummyTransaction(Long.MAX_VALUE), 0);
		assertGranted(acquireLock(locker, "o1", true));
		AcquireLock acquire2 = new AcquireLock(locker2, "o1", true);
		acquire2.assertBlocked();
		Thread.sleep(20);
	 ...
	 * 
	 * locker: read o1 => granted locker2: read o1 => granted locker3: write o1
	 * => blocked locker2: write o1 => blocked locker: commit locker2: =>
	 * blocked locker3: => blocked
	 */
	@Test
	public void testUpgradeWaiterConflictTimesOut() throws Exception {
		...
		Locker<String> locker3 = createTxnLocker(lockManager,
				new DummyTransaction(txn.getTimeout() / 3), 0);
	 ...
	 * 
	 * locker: read o1 => granted locker2: read o2 => granted locker: write o2
	 * => blocked locker2: write o1 => deadlock locker2: abort locker: =>
	 * granted
	 */
	@Test
	 ...
	 * 
	 * locker: read o1 => granted locker2: read o2 => granted locker: write o2
	 * => blocked locker2: write o1 => blocked locker: => deadlock locker: abort
	 * locker2: => granted
	 */
	@Test
	 ...
	 * locker is older than locker2
	 * 
	 * locker: read o1 => granted locker2: read o1 => granted locker: write o1
	 * => blocked locker2: write o1 => deadlock locker2: abort locker: =>
	 * granted
	 */
	@Test
	 ...
	 * 
	 * locker: read o1 => granted locker2: read o2 => granted locker3: read o3
	 * => granted locker: write o2 => blocked locker2: write o3 => blocked
	 * locker3: write o1 => deadlock locker3: abort locker2: => granted locker2:
	 * abort locker: => granted
	 */
	@Test
	 ...
	 * 
	 * locker: read o1 => granted locker2: read o2 => granted locker3: read o3
	 * => granted locker2: write o3 => blocked locker3: write o1 => blocked
	 * locker1: write o2 => blocked locker3: => deadlock locker3: abort locker2:
	 * => granted locker2: abort locker: => granted
	 */
	@Test
	 ...
	 * locker is oldest, locker2 in the middle, locker3 is youngest
	 * 
	 * locker: read o1 => granted locker2: read o2 => granted locker3: read o2
	 * => granted locker2: write o1 => blocked locker3: write o1 => blocked
	 * locker: write o2 => blocked locker3: => deadlock locker3: abort locker2:
	 * => deadlock locker2: abort locker: => granted
	 */
	@Test
	 ...
	 * youngest
	 * 
	 * locker: write o1 => granted locker2: read o2 => granted locker3: read o2
	 * => granted locker4: read o2 => granted locker2: write o1 => blocked
	 * locker3: write o1 => blocked locker4: write o1 => blocked locker: write
	 * o2 => blocked locker2: => deadlock locker2: abort locker3: => deadlock
	 * locker3: abort locker4: => deadlock locker4: abort locker: => granted
	 */
	@Test
	 ...
	 * locker is youngest
	 * 
	 * locker: lockNoWait o1, write => granted locker2: lockNoWait o2, write =>
	 * granted locker: lockNoWait o2, write => blocked locker2: lockNoWait o1,
	 * write => blocked locker: => deadlock victim locker2: waitForLock =>
	 * timeout locker2: abort locker: waitForLock => deadlock reported
	 */
	@Test
	public void testDeadlockAndTimeout() throws Exception {
		Locker<String> locker = createTxnLocker(lockManager, 2000);
		Locker<String> locker2 = createTxnLocker(lockManager, 1000);
		LockConflict<String> conflict = lockManager.lockNoWait(locker, "o1",
				true);
		assertSame(null, conflict);
		LockConflict<String> conflict2 = lockManager.lockNoWait(locker2, "o2",
				true);
		assertSame(null, conflict);
		conflict = lockManager.lockNoWait(locker, "o2", true);
		assertTrue("Should be blocked", conflict != null

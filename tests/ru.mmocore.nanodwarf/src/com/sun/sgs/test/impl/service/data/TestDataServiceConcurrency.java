...
public class TestDataServiceConcurrency extends Assert {
	...
	protected WhichOperations whichOperations = WhichOperations.valueOf(System
			.getProperty("test.which.operations", "MODIFY"));
	...
	protected Properties getNodeProps() throws Exception {
		Properties props = SgsTestNode.getDefaultProperties(
				"TestDataServiceConcurrency", null, null);
	...
	private void runOperations(int threads) throws Throwable {
		...
		System.err.println("Threads: " + threads + ", " + "time: " + ms
				+ " ms, " + "aborts: " + aborts + ", " + "commits: " + commits
				+ ", " + "ops/sec: " + Math.round((threads * operations) / s));
	}

	/**
	 * Notes that a thread has completed successfully, and records the number of
	 * aborts and commits that occurred in the thread.
	 */
	...
	class OperationThread extends Thread {
		...
		private void op(int i) throws Exception {
			...
			case 1:
				/* Set bindings */
				ModifiableObject obj = (ModifiableObject) service
						.getBinding(name);
				String name2 = getObjectName(start + random.nextInt(objects));
				ModifiableObject obj2 = (ModifiableObject) service
						.getBinding(name2);
			...
			case 2:
				/* Modify object */
				((ModifiableObject) service.getBinding(name)).incrementNumber();
				break;
			case 3:
				/* Create object */

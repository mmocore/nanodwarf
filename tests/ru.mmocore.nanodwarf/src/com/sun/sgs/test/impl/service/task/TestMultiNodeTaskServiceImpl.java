 ...
 * nodes. Rather than use the production Node Mapping Service, these tests use a
 * dummy implementation (and therefore a backing dummy Watchdog Service) so that
 * explicit movement and interaction cases can be tested that cannot be done
 * with the production service. This assumes that basic interaction tests are
 * already passing from {code TestTaskServiceImpl}.
 */
...
public class TestMultiNodeTaskServiceImpl extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		serverNode = new SgsTestNode(appName, null, createProps(true, appName,
				dbDirectory));
		addNodes(createProps(false, appName, dbDirectory), 1);

		txnSchedulerZero = serverNode.getSystemRegistry().getComponent(
				TransactionScheduler.class);
		txnSchedulerOne = additionalNodes[0].getSystemRegistry().getComponent(
				TransactionScheduler.class);
		...
		mappingServiceZero = (DummyNodeMappingService) (serverNode
				.getNodeMappingService());
		mappingServiceOne = (DummyNodeMappingService) (additionalNodes[0]
				.getNodeMappingService());
		...
		txnSchedulerZero.runTask(new TestAbstractKernelRunnable() {
	...
	public void testMovePeriodicTask() throws Exception {
		...
		txnSchedulerZero.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				Counter counter = getClearedCounter();
				taskServiceZero.schedulePeriodicTask(new TestTask(), 0L, 500L);
				counter.increment();
				counter.increment();
			}
		}, id);
	...
	public void testMoveAfterScheduledPeriodicTask() throws Exception {
		...
		txnSchedulerZero.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				Counter counter = getClearedCounter();
				taskServiceZero
						.schedulePeriodicTask(new TestTask(), 100L, 500L);
				counter.increment();
			}
		}, id);
	...
	public void testCancelPeriodicHandle() throws Exception {
		...
		txnSchedulerZero.runTask(new TestAbstractKernelRunnable() {
			public void run() {
				Counter counter = getClearedCounter();
				PeriodicTaskHandle h = taskServiceZero.schedulePeriodicTask(
						new TestTask(), 250L, 500L);
				dataServiceZero.setBinding("handle", new ManagedHandle(h));
			}
		}, id);

		try {
			txnSchedulerOne.runTask(new TestAbstractKernelRunnable() {
				public void run() {
					((ManagedHandle) dataServiceOne.getBinding("handle"))
							.cancel();
				}
			}, id);
		} catch (Exception e) {
	...
	private Properties createProps(boolean server, String appName,
		...
		int port = server ? SgsTestNode.getNextUniquePort() : SgsTestNode
				.getDataServerPort((DataServiceImpl) (serverNode
						.getDataService()));
		String portStr = String.valueOf(port);

		String nodeType = server ? NodeType.singleNode.name()
	...
	public static class TestTask implements Task, Serializable {
		...
		public void run() throws Exception {
			TestMultiNodeTaskServiceImpl.lastNodeUsed.set(AppContext
					.getManager(NodeIdManager.class).getNodeId());

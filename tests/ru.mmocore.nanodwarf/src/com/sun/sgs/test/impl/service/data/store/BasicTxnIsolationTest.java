...
public abstract class BasicTxnIsolationTest extends Assert {
	...
	protected static final Properties props = UtilProperties.createProperties();

	/**
	 * The number of milliseconds to wait to see if an operation is successful.
	 */
	protected static final long timeoutSuccess = Long.parseLong(props
			.getProperty("test.timeout.success", "1000"));
	 ...
	 */
	protected static final long timeoutBlock = Long.parseLong(props
			.getProperty("test.timeout.block",
					String.valueOf(timeoutSuccess / 10)));

	/** The test environment, using a locking access coordinator. */
	protected static final BasicDataStoreTestEnv env = new BasicDataStoreTestEnv(
	...
	protected static final AccessCoordinatorHandle accessCoordinator = env.accessCoordinator;
	...
	/**
	 * Create the properties, access coordinator, and data store if needed; then
	 * create a transaction, create an object, and clear existing bindings.
	 */
	@Before
	public void before() {
		if (store == null) {
	 ...
	 * Operations to test:
	 * 
	 * Operation Lock --------- ---- markForUpdate WRITE getObject
	 * forUpdate=false READ getObject forUpdate=true WRITE setObject WRITE
	 * setObjects WRITE removeObject WRITE
	 * 
	 * Use getObject with forUpdate=false or true as the probes for checking for
	 * conflicts.
	 */
	...
	public void testSetObjectsRead() throws Exception {
		newTransaction();
		store.getObject(txn, id, false);
		Runner runner = new Runner(new SetObjects(new long[] { id },
				new byte[][] { secondValue }));
		runner.assertBlocked();
		commitTransaction();
		runner.getResult();
	}
	 ...
	 * 
	 * getBinding notFound READ READ getBinding found READ setBinding create
	 * WRITE WRITE setBinding existing WRITE removeBinding notFound WRITE READ
	 * removeBinding found WRITE WRITE nextBoundName READ
	 * 
	 * Use getBinding and setBinding to check for conflicts, probing keys before
	 * and after as needed to check locks on the key and the next key.
	 */
	 ...
	 * 
	 * tid:1 create a create b commit
	 * 
	 * tid:2 create c write lock c write lock end
	 * 
	 * tid:3 remove b write lock b write lock c (blocks)
	 * 
	 * tid:2 abort release lock c release lock end
	 * 
	 * tid:3 [remove b] write lock c (but it is now missing, so check next
	 * again) write lock end
	 * 
	 * tid:4 next a read lock end (blocks)
	 * 
	 * tid:3 commit release lock b release lock end
	 * 
	 * tid:4 [next a] write lock end return null
	 * 
	 * So, this test will fail if removeBinding does not make sure to check that
	 * the next key exists after it obtains a lock on it. This test only
	 ...
	 * tid:2 create c write lock c write lock end
	 * 
	 * tid:3 create d write lock d write lock end (blocks)
	 * 
	 * tid:4 remove b write lock b write lock c (blocks)
	 * 
	 * tid:2 abort release lock c release lock end
	 * 
	 * tid:3 [create d] write lock end
	 * 
	 * tid:4 [remove b] write lock c (but it is now missing, so check next
	 * again) write lock d (blocks)
	 * 
	 * tid:3 abort release lock d release lock end
	 * 
	 * tid:4 [remove b] write lock d write lock end
	 * 
	 * tid:5 next a read lock end (blocks)
	 * 
	 * tid:4 commit release lock b release lock end
	 * 
	 * tid:5 [next a] write lock end return null
	 */
	@Test
	 ...
	 * exists.
	 * 
	 * tid:1 create a commit
	 * 
	 * tid:2 create b write lock b write lock end
	 * 
	 * tid:3 create c write lock c write lock end (blocks)
	 * 
	 * tid:4 next a read lock b (blocks)
	 * 
	 * tid:2 abort release lock b release lock end
	 * 
	 * tid:3 [create c] write lock end
	 * 
	 * tid:4 [next a] read lock b (but it is now missing, so check next again)
	 * read lock c (blocks)
	 * 
	 * tid:3 abort release lock c release lock end
	 * 
	 * tid:4 [next a] read lock c (but it is now missing, so check next again)
	 * read lock end return null
	 */
	@Test
	 ...
	 * tid:3 create c write lock c write lock end (blocks)
	 * 
	 * tid:4 get a read lock a read lock b (blocks)
	 * 
	 * tid:2 abort release lock b release lock end
	 * 
	 * tid:3 [create c] write lock end
	 * 
	 * tid:4 [get a] read lock b (but it is now missing, so check next again)
	 * read lock c (blocks)
	 * 
	 * tid:3 abort release lock c release lock end
	 * 
	 * tid:4 [get a] read lock c (but it is now missing, so check again) read
	 * lock end
	 */
	@Test
	 ...
	 * tid:3 create c write lock c write lock end (blocks)
	 * 
	 * tid:4 create a write lock a write lock b (blocks)
	 * 
	 * tid:2 abort release lock b release lock end
	 * 
	 * tid:3 [create c] write lock end
	 * 
	 * tid:4 [create a] write lock b (but it is now missing, so check next
	 * again) write lock c (blocks)
	 * 
	 * tid:3 abort release lock c release lock end
	 * 
	 * tid:4 [create a] write lock c (but it is now missing, so check next
	 * again) write lock end
	 */
	@Test
	...
	/**
	 * A utility class for running a sequence of actions under a new transaction
	 * in another thread. A newly created {@code Runner} starts a thread and a
	 * new transaction. It passes the transaction to the initial action, using
	 * {@link TxnCallable#setTransaction}, so that the action can use it as part
	 * of its operations. When the runner is done running the first action, it
	 * then waits for additional actions to be specified with calls to
	 * {@link #setAction}, or until the transaction and thread are ended by
	 * calling {@link #commit} or {@link #abort}.
	 * <p>
	 * 
	 * Callers can use the {@link #blocked}, {@link #assertBlocked}, or
	 * {@link #getResult} methods to interrogate the state of the runner's
	 * activities from another thread.
	 */
	static class Runner {
		...
		/**
		 * Specifies another action to run. This method should only be called if
		...
		/**
		 * Returns the result of the last action, throwing an exception if it is
		 * blocked.
		 * 
		 * @return the result of the last action
		 */
		Object getResult() throws InterruptedException, TimeoutException {
			try {
		...
		static void abortAllOpen() throws InterruptedException,
				TimeoutException {
	...
	/**
	 * A {@code Callable} that can be supplied to a {@link Runner} to perform an
	 * action in the context of a transaction, which the runner will supply
	...
	/**
	 * Calls {@link DataStore#getBinding}, returning {@code null} if the name is
	 * not bound.
	 */
	...
	/**
	 * Calls {@link DataStore#removeBinding}, returning {@code true} if the name
	 * was bound, otherwise {@code false}.
	 */

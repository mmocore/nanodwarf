...
public class FilterTestHarness {
	...
	/**
	 * Constructs a new {@code FilterTestHarness} with the given callback.
	 * 
	 * @param callback
	 *            the callback for this harness
	 */
	...
	/**
	 * Sets a {@code RuntimeException} to be thrown on the next complete message
	 * processed by {@link #send send} or {@link #recv recv}. If the given
	 * exception is {@code null}, clears an exception if one was set.
	 * 
	 * @param e
	 *            the exception to throw on the next complete message processed
	 *            by {@code send} or {@code recv}
	 */
	 ...
	 * 
	 * @param buf
	 *            the data to add and process
	 */
	public void recv(IoBuffer buf) {
		filter.filterReceive(testingCallback, buf);
	}

	/**
	 * Prepends a 4-byte length field to the given bytes, and passes the result
	 * to the {@linkplain FilterListener#sendUnfiltered sendUnfiltered} method
	 * of the callback for this harness.
	 * 
	 * @param bytes
	 *            the data to process
	 */
